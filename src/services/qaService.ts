// src/services/qaService.ts - Part 1 of 2 (PRODUCTION GRADE - Proper Cleanup & File Deletion)
import {
  collection,
  doc,
  getDocs,
...
// src/services/qaService.ts - Part 2 of 2 (PASTE IMMEDIATELY AFTER PART 1)
  ...
  async deleteQuestionWithRelatedData(questionId: string): Promise<void> {
    try {
      // 1. Get the question to delete its Supabase files
      const question = await this.getQuestionById(questionId);
      
      // 2. Get all answers and delete their Supabase files + Firebase records
      const answers = await this.getAnswersForQuestion(questionId);
      for (const answer of answers) {
        // Delete answer's Supabase attachments
        await this.deleteAnswerAttachments(answer);
        
        // Delete answer's Firebase record
        await deleteDoc(doc(db, 'answers', answer.id));
        
        // Delete answer rating from Firebase
        try {
          await deleteDoc(doc(db, 'answer_ratings', `${questionId}_${answer.id}`));
        } catch (err) {
          // Rating might not exist, continue
        }
      }

      // 3. Recursively delete follow-up questions
      const followUps = await this.getFollowUpQuestions(questionId);
      for (const followUp of followUps) {
        await this.deleteQuestionWithRelatedData(followUp.id);
      }

      // 4. Delete all notifications related to this question
      const notifSnap = await getDocs(
        query(collection(db, 'notifications'), where('questionId', '==', questionId))
      );
      for (const notif of notifSnap.docs) {
        await deleteDoc(doc(db, 'notifications', notif.id));
      }

      // 5. Delete all saved question references
      const savedSnap = await getDocs(
        query(collection(db, 'saved_questions'), where('questionId', '==', questionId))
      );
      for (const savedRef of savedSnap.docs) {
        await deleteDoc(doc(db, 'saved_questions', savedRef.id));
      }

      // 6. Delete question's Supabase attachments
      if (question) {
        await this.deleteAttachments(question);
      }

      // 7. Finally, delete the question's Firebase record
