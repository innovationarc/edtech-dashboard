// src/services/qaService.ts - Part 1 of 2 (FIXED - Proper OCR & Similarity)
import {
  collection,
  doc,
  getDocs,
...
export const qaService = {
  ...
  async askQuestion(
    ...
    try {
      ...
      // Extract OCR in background if image exists (don't block)
      if (question.imageUrl) {
        ...
        this.extractTextFromImage(question.imageUrl).then(async (extractedText) => {
          if (extractedText && extractedText.length > 5) {
            console.log('📝 Updating question with OCR text...');
  ...
  async findSimilarQuestionsWithFile(
    ...
    try {
      ...
      // Upload image and extract OCR if file provided
      ...
      if (file) {
        ...
        console.log('📝 Current OCR extracted:', currentOCR.substring(0, 60));
      }
      ...
      console.log('📊 Total answered questions in subject:', allQuestions.length);
      ...
      console.log('🧹 Cleaned current question:', cleanCurrentText.substring(0, 60));
      ...
      const scored = allQuestions.map((q, idx) => {
        ...
        let imageScore = 0;
        let combinedOCRScore = 0;
        
        // If both have images with OCR
        if (currentOCR && q.extractedText) {
          const cleanCurrentOCR = this.aggressiveClean(currentOCR);
          const cleanStoredOCR = this.aggressiveClean(q.extractedText);
          
          if (cleanCurrentOCR.length > 5 && cleanStoredOCR.length > 5) {
            imageScore = this.calculateSimilarity(cleanCurrentOCR, cleanStoredOCR);
            
            // Also compare OCR with opposite question text
            const ocrToTextScore1 = this.calculateSimilarity(cleanCurrentOCR, cleanStoredText);
            const ocrToTextScore2 = this.calculateSimilarity(cleanCurrentText, cleanStoredOCR);
            combinedOCRScore = Math.max(ocrToTextScore1, ocrToTextScore2);
          }
        }
        ...
        if (imageScore > 0 || combinedOCRScore > 0) {
          // If we have image/OCR data, weight it heavily
          const maxImageRelatedScore = Math.max(imageScore, combinedOCRScore);
          finalScore = (maxImageRelatedScore * 0.6) + (textScore * 0.4);
        }
        
        if ((idx + 1) <= 5) {
          console.log(`\n[${idx + 1}] Q: ${q.id.substring(0, 8)}`);
          console.log(`   Text: ${textScore.toFixed(3)} | Image: ${imageScore.toFixed(3)} | Combined: ${combinedOCRScore.toFixed(3)}`);
  ...
  async findSimilarQuestions(
    ...
    try {
      ...
      const scored = questionsToSearch.map((q) => {
        ...
        let imageScore = 0;
        if (imageUrl && q.imageUrl && q.extractedText) {
          const cleanStoredOCR = this.aggressiveClean(q.extractedText);
          if (cleanStoredOCR.length > 5) {
            imageScore = this.calculateSimilarity(cleanCurrentText, cleanStoredOCR);
          }
        }
        
        const finalScore = imageScore > 0 
