// src/services/qaService.ts - Part 1 of 2 (Enhanced with Advanced Text & Image Similarity Detection + Attachment Deletion + OCR)
import {
  collection,
  doc,
  getDocs,
...
export const qaService = {
  ...
  // OCR: Extract text from image using Tesseract.js or API
  async extractTextFromImage(imageUrl: string): Promise<string> {
    try {
      // Using Google Cloud Vision API or similar OCR service
      // For this implementation, we'll use a simple fetch approach
      // You can replace this with Tesseract.js or Google Cloud Vision API
      ...
      // Use Google Cloud Vision API for OCR (you'll need to set up API key)
  ...
  async calculateImageSimilarity(
    ...
    // 1. Filename similarity (weight: 0.2)
    ...
    if (name1 && name2) {
      if (name1 === name2) {
        ...
        totalScore += filenameSimilarity * 0.2;
      }
      weightSum += 0.2;
    }

    // 2. OCR text similarity (weight: 0.8) - Most important for academic content
    if (extractedText1 && extractedText2) {
      const ocrSimilarity = this.calculateAdvancedTextSimilarity(extractedText1, extractedText2);
      totalScore += ocrSimilarity * 0.8;
      weightSum += 0.8;
  ...
  async findSimilarQuestions(
    ...
    try {
      ...
      // Extract text from current image if present and not already extracted
      ...
      const scoredQuestions = await Promise.all(
        questionsToSearch.map(async (q) => {
          ...
          if (currentImageText && q.questionText) {
            ocrTextSimilarity = this.calculateAdvancedTextSimilarity(
              currentImageText,
              q.questionText
            );
          }
          ...
          const finalScore = (textSimilarity * 0.50) + (imageSimilarity * 0.30) + (ocrTextSimilarity * 0.20);
...
// src/services/qaService.ts - Part 2 of 2 (Continuation with Notifications, Knowledge, and Listeners + Enhanced Deletion)
