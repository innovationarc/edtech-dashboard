// src/services/qaService.ts - Part 1 of 2 (FIXED - Combined Text Similarity)
import {
  collection,
  doc,
  getDocs,
...
export interface Question {
  ...
  combinedText?: string; // New field: questionText + extractedText
}
...
export const qaService = {
  ...
  removeStopWords(text: string): string {
    ...
    const stopWords = [
      ...
      'problem', 'question', 'a', 'an', 'and', 'or', 'but'
    ];
  ...
  // Advanced similarity using multiple algorithms
  ...
  async askQuestion(
    ...
    try {
      ...
      const cleanQuestion: any = {
        ...
        combinedText: this.normalizeText(question.questionText), // Initial combined text
      };
      ...
      // Extract OCR and update combinedText in background
  ...
  async findSimilarQuestionsWithFile(
    ...
    try {
      ...
      // Step 1: Upload image and extract OCR if provided
      ...
      // Step 2: Create combined text for current question
      ...
      console.log('🔗 Current combined text length:', currentCombinedText.length);
      console.log('🔗 Preview:', currentCombinedText.substring(0, 80));
      
      // Step 3: Get all answered questions in the subject
      let q = query(
      ...
      console.log('📊 Total answered questions to check:', allQuestions.length);

      if (allQuestions.length === 0) {
        console.log('❌ No answered questions found in this subject');
        return [];
      }

      // Step 4: Calculate similarity for each question
      const scored = allQuestions.map((q, idx) => {
        // Use combinedText if available, otherwise fall back to questionText
        ...
        if ((idx + 1) <= 10) {
          ...
          console.log(`   Stored combined text: ${storedCombinedText.substring(0, 60)}...`);
        ...
        return { 
          ...
          uploadedImageUrl // Return for potential use
        };
      });

      // Step 5: Filter and sort by similarity
      const threshold = 0.40; // 40% similarity threshold
      ...
      if (similar.length > 0) {
        ...
        similar.forEach((s, i) => {
          console.log(`║   ${i + 1}. ${(s.score * 100).toFixed(1)}%`.padEnd(41) + '║');
        });
      }
      console.log('╚════════════════════════════════════════╝\n');
