// src/services/qaService.ts - Enhanced with Ratings, Save Features, Voice Support, and Improved Similarity Matching
import {
  collection,
  doc,
  getDocs,
...
export const qaService = {
  ...
  // Helper function to calculate advanced text similarity using n-grams
  ...
  async checkImageSimilarity(imageUrl1?: string, imageUrl2?: string): Promise<number> {
    // If both have no images, return 0 (neutral)
    if (!imageUrl1 && !imageUrl2) return 0;
    
    // If only one has image, return 0 (no similarity)
    if (!imageUrl1 || !imageUrl2) return 0;
    
    // If exact same URL, return 1 (perfect match)
    if (imageUrl1 === imageUrl2) return 1.0;
    
    // Check if images have similar names/patterns (partial match)
    ...
    if (name1 && name2 && name1 === name2) {
      return 0.8; // High similarity if same filename
    }
  ...
  async findSimilarQuestions(
    ...
    try {
      // Build query with subject filter (required)
      let q = query(
      ...
      // Add courseId filter if provided for more precise matching
      if (courseId) {
      ...
      // Map questions from filtered results
      let questionsToSearch = querySnapshot.docs.map((doc) => ({
        id: doc.id,
        ...doc.data(),
        createdAt: doc.data().createdAt.toDate(),
        updatedAt: doc.data().updatedAt?.toDate(),
      ...
      if (currentQuestionId) {
        questionsToSearch = questionsToSearch.filter(q => q.id !== currentQuestionId);
      }

      // Calculate similarity scores for each question using multiple methods
      const scoredQuestions = await Promise.all(
        questionsToSearch.map(async (q) => {
          // 1. Word-level Jaccard similarity (40% weight)
          const jaccardSimilarity = this.calculateTextSimilarity(questionText, q.questionText);
          
          // 2. Bigram similarity for phrase matching (30% weight)
          const bigramSimilarity = this.calculateNgramSimilarity(questionText, q.questionText, 2);
          
          // 3. Trigram similarity for better context (10% weight)
          const trigramSimilarity = this.calculateNgramSimilarity(questionText, q.questionText, 3);
          
          // 4. Image similarity (20% weight)
          const imageSimilarity = await this.checkImageSimilarity(imageUrl, q.imageUrl);
      ...
      const similarQuestions = scoredQuestions
        .filter(item => item.score >= 0.5) // Only return questions with 50% or more similarity
