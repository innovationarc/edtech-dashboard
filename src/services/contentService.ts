// src/services/contentService.ts - UPDATED VERSION WITH FIXES
...
// CRITICAL HELPER: Remove undefined values from object
const removeUndefinedFields = (obj: any): any => {
  const cleaned: any = {};
  
  for (const key in obj) {
    if (obj[key] !== undefined) {
      // If it's an object (but not null or array), recursively clean it
...
export const contentService = {
  ...
  async createContent(content: Omit<Content, 'id' | 'createdAt'>): Promise<string> {
    try {
      ...
      // CRITICAL FIX: Remove all undefined fields before saving
      const cleanedContent = removeUndefinedFields({
        ...content,
        viewCount: 0,
        uniqueViewers: 0,
        coursesUsing: [],
  ...
  async updateContent(id: string, updates: Partial<Content>): Promise<void> {
    try {
      ...
      // CRITICAL FIX: Remove undefined fields from updates
