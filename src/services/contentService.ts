...
export const contentService = {
  ...
  // ── Notes + exam question/solution images → Supabase (fully unchanged) ──
  ...
  // ── Videos → Bunny.net (browser uploads directly; server never touches bytes) ──
  ...
  async createContent(
    ...
    try {
      ...
      if (content.type === 'exam') {
        ...
        if (content.examVersions) contentData.examVersions = content.examVersions;
      }
  ...
  async updateContent(
    ...
    try {
      ...
      // FIX: Determine if this is an exam update by checking updates.type first,
      // then falling back to the stored document type to handle cases where
      // the caller omits `type` from the partial update object.
      let isExamUpdate = updates.type === 'exam';
      if (!isExamUpdate && updates.type === undefined) {
        // Caller didn't include type — check the existing doc (cached read is fine here)
        const existingDoc = await getDoc(doc(db, 'content', id));
        if (existingDoc.exists() && existingDoc.data().type === 'exam') {
          isExamUpdate = true;
        }
      }
      ...
      if (isExamUpdate) {
        ...
        if (updates.examVersions !== undefined) updateData.examVersions = updates.examVersions;
        // Persist exam-specific settings even when only partial fields are passed
        if (updates.maxAttempts !== undefined) updateData.maxAttempts = updates.maxAttempts;
        if (updates.examTimelineType !== undefined) updateData.examTimelineType = updates.examTimelineType;
        if (updates.examStartDateTime !== undefined) updateData.examStartDateTime = updates.examStartDateTime;
        if (updates.examEndDateTime !== undefined) updateData.examEndDateTime = updates.examEndDateTime;
        if (updates.resultPublishType !== undefined) updateData.resultPublishType = updates.resultPublishType;
        if (updates.resultPublishDateTime !== undefined) updateData.resultPublishDateTime = updates.resultPublishDateTime;
      }
      ...
      // FIX: Force a server-side read to invalidate Firestore's local cache
      // so the next getAllContent/getContentByUser call gets fresh data.
      await getDocFromServer(contentRef);
  ...
  async deleteContent(id: string): Promise<void> {
    try {
      ...
      // Videos → Bunny.net deletion (skips secured:// automatically inside deleteVideoFile)
      if (content.videoUrl) {
        await this.deleteVideoFile(content.videoUrl);
      }
      // Notes → Supabase deletion (unchanged)
      if (content.noteUrl) {
      ...
      if (content.type === 'exam') {
        ...
        if (content.examVersions) {
          for (const version of content.examVersions) {
            for (const question of version.mcqQuestions || []) {
              if (question.questionImage) await this.deleteFile(question.questionImage);
              if (question.solutionImage) await this.deleteFile(question.solutionImage);
            }
            for (const question of version.writtenQuestions || []) {
              if (question.questionImage) await this.deleteFile(question.questionImage);
              if (question.solutionImage) await this.deleteFile(question.solutionImage);
            }
          }
