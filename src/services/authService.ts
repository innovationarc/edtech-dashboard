...
// Get client IP + country via Cloudflare's trace endpoint.
// Free, no API key, no rate limits, CORS-safe from any browser origin.
// Returns real client IP and 2-letter country code (e.g. "BD").
// City is not available from this endpoint — logged as 'Unknown'.
async function getClientIpAndGeo(): Promise<{ ip: string; city: string; country: string }> {
  try {
    const res = await fetch('https://cloudflare.com/cdn-cgi/trace');
    if (!res.ok) return { ip: 'unknown', city: 'Unknown', country: 'Unknown' };
    const text = await res.text();

    // Response is plain text key=value pairs, one per line
    ...
    return {
      ip: data.ip || 'unknown',
      city: 'Unknown',          // Not provided by Cloudflare trace
      country: data.loc || 'Unknown',  // 2-letter country code e.g. "BD"
    };
  } catch {
