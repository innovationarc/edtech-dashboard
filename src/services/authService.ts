...
export const authService = {
  ...
  async signIn(identifier: string, password: string, rememberMe: boolean = false): Promise<UserProfile> {
    try {
      ...
      // Fetch user profile with retry logic for permission issues
      let userDoc;
      let retryCount = 0;
      const maxRetries = 3;
      ...
      while (retryCount < maxRetries) {
        try {
          ...
          throw error; // Re-throw if it's not a permission error or max retries reached
        }
      }
      ...
      // Update with retry logic for permission issues
      ...
      while (updateRetryCount < maxUpdateRetries) {
        try {
          ...
          updateRetryCount++;
          if (error.code === 'permission-denied' && updateRetryCount < maxUpdateRetries) {
            // Wait a bit for auth state to propagate
            await new Promise(resolve => setTimeout(resolve, 500 * updateRetryCount));
            continue;
          }
          // If update fails, just log it but don't block login
