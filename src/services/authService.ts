...
// Helper function to find user by Student ID, Phone Number, or Email (for login only)
const findUserByLoginId = async (loginId: string): Promise<any> => {
  try {
    console.log('🔍 Searching for user with login ID:', loginId.substring(0, 5) + '...');
    
    const usersRef = collection(db, 'users');
    
    // Normalize phone number if it looks like a phone number
    let normalizedLoginId = loginId;
    if (/^\d+$/.test(loginId) || loginId.startsWith('0') || loginId.startsWith('+')) {
      try {
        normalizedLoginId = normalizePhoneNumber(loginId);
        console.log('📱 Normalized phone for search:', normalizedLoginId.substring(0, 5) + '****');
      } catch (e) {
        // If normalization fails, use original
        console.log('⚠️ Phone normalization failed, using original');
      }
    }
    ...
    if (normalizedLoginId !== loginId) {
      q = query(usersRef, where('phoneNumber', '==', normalizedLoginId));
      querySnapshot = await getDocs(q);
      
      if (!querySnapshot.empty) {
        console.log('✅ User found by normalized phone number');
        return { uid: querySnapshot.docs[0].id, ...querySnapshot.docs[0].data() };
      }
    }
    
    // Try original phone format if different
    ...
    console.log('❌ User not found with provided login ID');
    return null;
  } catch (error: any) {
    console.error('❌ Error searching for user:', error.code || error.message);
    ...
    try {
      ...
      if (loginId.includes('@') && !loginId.includes('@student.local')) {
        try {
          ...
          console.log('⚠️ Direct email login failed, trying search...');
        }
      }
