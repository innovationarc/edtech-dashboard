...
// ─── APP BRANCH ───────────────────────────────────────────────────────────────
// Surgical change from original: all /api/* calls now go through the Vercel
// gateway (/api/gateway) instead of hitting routes directly. This means:
//   - No secrets in the APK
//   - APK only needs VITE_GATEWAY_URL + VITE_GATEWAY_APP_KEY
//   - Everything else is 100% identical to the web branch original
// ─────────────────────────────────────────────────────────────────────────────
import { 
...
import { saveLoginLog } from './settingsService';
...
// ─── Gateway helper ───────────────────────────────────────────────────────────
// ONLY change from original: all /api/* calls go through this single function.
// VITE_GATEWAY_URL  = your Vercel deployment URL (where /api/gateway.ts lives)
// VITE_GATEWAY_APP_KEY = the X-App-Key secret (set as GATEWAY_APP_KEY on Vercel)
//
// The APK only needs these 2 env vars + Firebase config. No other secrets.
const GATEWAY_URL = import.meta.env.VITE_GATEWAY_URL ||
                    import.meta.env.VITE_BACKEND_URL ||
                    import.meta.env.VITE_API_URL ||
                    'https://edtech-dashboard-alpha.vercel.app';

const GATEWAY_APP_KEY = import.meta.env.VITE_GATEWAY_APP_KEY || '';

async function gatewayPost(route: string, payload: Record<string, any>): Promise<Response> {
  return fetch(`${GATEWAY_URL}/api/gateway`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      'X-App-Key': GATEWAY_APP_KEY,
    },
...
const findUserByUserId = async (userId: string): Promise<any> => {
  try {
    ...
    // ← SURGICAL CHANGE: was fetch(`${BACKEND_URL}/api/user-search`, ...)
    const response = await gatewayPost('user-search', requestBody);
...
// Get real client IP directly in the browser via api64.ipify.org (CORS-safe, IPv6-aware).
// Proxies (Cloudflare, Vercel) are bypassed because the fetch originates from the browser.
// The IP is then sent to the Render backend for geo lookup (city/country).
async function getClientIpAndGeo(): Promise<{ ip: string; city: string; country: string }> {
  try {
    // Step 1: Get real client IP directly in browser — bypasses Cloudflare/Vercel proxies
    const ipRes = await fetch('https://api64.ipify.org?format=json');
    if (!ipRes.ok) return { ip: 'unknown', city: 'Unknown', country: 'Unknown' };
    const { ip } = await ipRes.json();

    // Step 2: Send IP to Render for geo lookup (server-side avoids CORS on ip-api.com)
    const RENDER_URL = import.meta.env.VITE_RENDER_URL;
    if (!RENDER_URL) return { ip, city: 'Unknown', country: 'Unknown' };

    const geoRes = await fetch(`${RENDER_URL}/api/get-ip?ip=${encodeURIComponent(ip)}`);
    if (!geoRes.ok) return { ip, city: 'Unknown', country: 'Unknown' };
    const geo = await geoRes.json();

    return { ip, city: geo.city, country: geo.country };
  } catch {
...
export const authService = {
  ...
  async signIn(userId: string, password: string, rememberMe: boolean = false): Promise<UserProfile> {
    try {
      ...
      // Single fetch via Cloudflare trace — real client IP, CORS-safe
      const { ip: clientIp, city, country } = await getClientIpAndGeo();
      ...
      // Store session token + reliable firebase session flag
      try {
        localStorage.setItem('auth_session_token', sessionToken);
        // auth_firebase_session: reliable flag tied to actual Firebase sign-in success
        // hasAuthTokens() uses this key — gets cleared on signOut or when Firebase reports null
        localStorage.setItem('auth_firebase_session', 'true');
      } catch {}

      // Remember me: explicitly set OR clear based on current active choice
      // Clears stale data when user actively unchecks "Keep me signed in"
      try {
        if (rememberMe) {
          // Rolling 1-year expiry — renewed each visit so active users never get cut off
          const rmExpiry = String(Date.now() + 365 * 24 * 60 * 60 * 1000);
          localStorage.setItem('auth_remember_me', 'true');
          localStorage.setItem('auth_user_id', normalizedUserId);
          localStorage.setItem('auth_remember_me_expiry', rmExpiry);
        } else {
  ...
  async resetPassword(phoneNumber: string, newPassword: string): Promise<void> {
    try {
      ...
      // ← SURGICAL CHANGE: was fetch(`${BACKEND_URL}/api/password-reset`, ...)
      const response = await gatewayPost('password-reset', requestBody);
   ...
   * preserveRememberMe=true used for force-logout (another device) so user can re-login easily
   */
  async signOut(preserveRememberMe = false): Promise<void> {
    try {
      // Clear device ID and session token on sign out
      ...
      // Always clear session-specific keys
      try {
        localStorage.removeItem('auth_session_token');
        localStorage.removeItem('auth_firebase_session');  // cleared so hasAuthTokens()=false on next load
      } catch {}

      // Only clear remember-me credentials on explicit sign-out (not force-logout)
      // On force-logout (preserveRememberMe=true), keep auth_user_id + auth_remember_me
      // so the user can immediately sign back in with their credentials pre-filled
  ...
  /**
   * Re-assert browserLocalPersistence on page load for remember-me sessions.
   * Called from DashboardContext before the auth listener is set up.
   */
  assertLocalPersistence(): Promise<void> {
