...
// Get client IP + approximate geo location in one call.
// Uses ipapi.co which sets Access-Control-Allow-Origin: * (browser-safe).
// Replaces the old two-step getClientIp() + getGeoFromIp() pattern that
// failed with a CORS error because api.ipify.org does not allow browser origins.
...
export const authService = {
  ...
  async signIn(userId: string, password: string, rememberMe: boolean = false): Promise<UserProfile> {
    try {
      ...
      // Single fetch returns both IP and geo — no CORS issues with ipapi.co
