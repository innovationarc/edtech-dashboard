...
// Get client IP + geo via Render backend (/api/get-ip).
// Render is server-side so no CORS issues, and ipapi.co works freely there.
// Set VITE_RENDER_URL in Vercel env vars to your Render app URL.
async function getClientIpAndGeo(): Promise<{ ip: string; city: string; country: string }> {
  try {
    ...
    if (!RENDER_URL) return { ip: 'unknown', city: 'Unknown', country: 'Unknown' };
    const res = await fetch(`${RENDER_URL}/api/get-ip`);
