...
const generateStudentId = async (): Promise<string> => {
  ...
  try {
    ...
    const fallbackId = `${prefix}${Date.now().toString().slice(-5)}`;
...
export const authService = {
  ...
  async sendPasswordResetOTP(loginId: string): Promise<{ success: boolean; phoneNumber?: string; message: string }> {
    try {
      console.log('🔍 Password reset - searching for account');
      ...
      console.log('✅ Account found for password reset');
  ...
  async resetPassword(phoneNumber: string, newPassword: string): Promise<void> {
    try {
      ...
      // Mark password reset as pending - actual password update happens through Firebase Auth
      await updateDoc(userDoc.ref, {
        passwordResetPending: true,
        passwordResetAt: Timestamp.now()
      });
      
      console.log('✅ Password reset marked for:', userData.userId);
      
      // Note: In production, you should implement actual password reset through Firebase Auth
      // This is a placeholder that marks the reset as pending
    } catch (error: any) {
