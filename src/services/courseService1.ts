...
export interface Course {
  ...
  reviewCount: number;
  studentCount: number;
  duration: string; // Auto-calculated from lessons
  level: 'beginner' | 'intermediate' | 'advanced';
  category: string; // Free text input
  class: string; // NEW: Free text input (e.g., "Class 6", "Grade 10")
  subjects: string[]; // NEW: Array of subjects for Q&A filtering
  tags: string[];
  thumbnail: string;
  thumbnailUrl?: string;
  previewVideo?: string;
...
export const courseService = {
  ...
  async getPublishedCourses(): Promise<Course[]> {
    try {
      ...
      // Try with isPublished filter first
      try {
        const q = query(
          coursesCollection, 
      ...
      // Filter for published courses, or return all if none are explicitly published
      const publishedCourses = allCourses.filter(course => course.isPublished === true);
      
      // If no courses are marked as published, return all courses (for development)
