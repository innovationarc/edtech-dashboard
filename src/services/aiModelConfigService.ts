...
export const aiKeyGroupService = {
   ...
   * Use this for group renames/description edits to avoid overwriting assignments
   * when the local `assignments` state is out of sync with Firestore.
   */
  async updateGroupsOnly(groups: APIKeyGroup[], adminUid: string): Promise<void> {
    await updateDoc(doc(db, GROUPS_DOC), {
      groups,
