...
export const aiModelConfigService = {
  ...
  async testConfig(config: AIModelConfig): Promise<string | null> {
    try {
      const result = await callProviderDirect('Say the word "hello".', config, 30, 0.1);
      return result.trim() ? null : 'Empty response from API — model returned blank text.';
    } catch (e: any) {
...
export const aiKeyGroupService = {
  ...
  /**
   * Patches only the `groups` array in Firestore without touching assignments.
   * Reads the current doc first so assignments (and any other fields) are fully
   * preserved — avoids the stale-closure problem of spreading local state.
   * Uses setDoc (not updateDoc) so it works even if the doc doesn't exist yet.
   */
  async updateGroupsOnly(groups: APIKeyGroup[], adminUid: string): Promise<void> {
    const ref  = doc(db, GROUPS_DOC);
    const snap = await getDoc(ref);
    const existing: Partial<KeyGroupsConfig> = snap.exists() ? snap.data() as KeyGroupsConfig : {};
    const toSave: KeyGroupsConfig = {
      groups:      [],
      assignments: [],
      updatedAt:   '',
      ...existing,
...
export async function callProviderDirect(
  ...
  if (provider === 'gemini') {
    ...
    // Candidates exist but content is blocked/empty — surface the real reason
    ...
    throw new Error(`Gemini returned no content (finishReason: ${reason}). Safety filter or model-tier restriction. Check your key's allowed models.`);
  }
