    ...
    try {
      ...
      [...personalEvSnap.docs, ...teacherEvSnap.docs].forEach(d => {
        ...
        try {
          ...
          // Minimum 1 min so completing an event always marks that day as active,
          // even if startTime === endTime (zero-duration event)
          addMins(ymd(eventDate), Math.max(1, (eh * 60 + em) - (sh * 60 + sm)));
        } catch {
          // If time parsing fails, still mark the day as active (1 min)
