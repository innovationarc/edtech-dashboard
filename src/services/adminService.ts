...
export const adminService = {
  ...
  async resetAdminPassword(
    ...
    try {
      ...
      // Log password reset in security logs (including reason)
      if (resetByAdmin) {
        try {
          await addDoc(collection(db, 'security_logs'), {
            ...
            details: `Password was reset for ${adminData.surname || adminData.name} (${adminData.userId}) by ${resetByAdmin.surname} (${resetByAdmin.userId})${reason ? '. Reason: ' + reason : ''}`,
            changes: JSON.stringify({
              action: 'password_reset',
          ...
          console.log('✅ Password reset logged in security logs with reason');
        } catch (logError) {
