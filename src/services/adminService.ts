...
export const adminService = {
  ...
  async resetAdminPassword(
    ...
    try {
      ...
      let resetPerformer = performedByAdmin;
      if (!resetPerformer) {
        ...
        if (currentUser) {
          ...
          if (currentAdminDoc.exists()) {
            ...
            resetPerformer = {
              uid: currentUser.uid,
              userId: currentAdminData.userId || 'N/A',
              surname: currentAdminData.surname || currentAdminData.name || 'Admin',
              fullName: currentAdminData.fullName || 'Not provided',
              email: currentAdminData.email,
              phoneNumber: currentAdminData.phoneNumber || '',
              role: 'admin',
              status: currentAdminData.status || 'active',
              createdAt: currentAdminData.createdAt?.toDate() || new Date(),
              createdBy: currentAdminData.createdBy || ''
            } as Admin;
            console.log('✅ Current admin retrieved:', resetPerformer.userId);
          }
        }
      }
      ...
      // ENHANCED: ALWAYS log password reset in security logs - even if performer is unknown
      try {
        console.log('📝 Creating comprehensive password reset security log...');
        
        const performerUid = resetPerformer?.uid || 'system';
        const performerUserId = resetPerformer?.userId || 'SYSTEM';
        const performerSurname = resetPerformer?.surname || 'System';
        const performerFullName = resetPerformer?.fullName || 'System';
        ...
        const logData = {
          ...
          changes: JSON.stringify({
            ...
            performedBy: {
              ...
              surname: performerSurname,
              fullName: performerFullName
            },
            reason: resetReason || 'No reason provided',
          ...
          fieldChanges: [
            ...
            { field: 'passwordResetBy', oldValue: adminData.lastPasswordResetBy || 'N/A', newValue: performerUserId }
          ]
        };
