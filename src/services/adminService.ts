...
export const adminService = {
  ...
  async resetAdminPassword(
    ...
    try {
      ...
      // ENHANCED: Log password reset in security logs with COMPREHENSIVE details
      if (performedByAdmin) {
        try {
          ...
          const logData = {
            ...
            details: `Password reset for admin ${adminSurname} (${adminUserId}) by ${performedByAdmin.surname} (${performedByAdmin.userId}). ${resetReason ? 'Reason: ' + resetReason : 'No reason provided.'}`,
            changes: JSON.stringify({
              action: 'password_reset',
              targetAdmin: {
                uid: uid,
              ...
              performedBy: {
                ...
                fullName: performedByAdmin.fullName || 'Not provided'
              },
              reason: resetReason || 'No reason provided',
            ...
            fieldChanges: [
              ...
              { field: 'passwordResetBy', oldValue: adminData.lastPasswordResetBy || 'N/A', newValue: performedByAdmin.userId }
            ]
          };
        ...
        console.warn('⚠️ No performedByAdmin provided - password reset log will not be created');
      }
      ...
      if (updatedByAdmin) {
        try {
          ...
          const logData = {
            ...
            details: `Status changed from ${oldStatus} to ${newStatus} for admin ${adminSurname} (${adminUserId}) by ${updatedByAdmin.surname} (${updatedByAdmin.userId}). ${reason ? 'Reason: ' + reason : 'No reason provided.'}`,
            changes: JSON.stringify({
      ...
      if (fieldChanges.length > 0) {
        ...
        try {
          ...
          const logData = {
            ...
            timestamp: Timestamp.now(),
            details: `Admin profile updated: ${adminSurname} (${adminUserId}). Changed fields: ${changesArray.join(', ')}`,
            changes: JSON.stringify({
              action: 'edit',
              targetAdmin: {
                uid: uid,
