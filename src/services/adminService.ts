...
export const adminService = {
  ...
  async resetAdminPassword(
    ...
    try {
      ...
      // DEBUG: Check what resetReason actually is
      console.log('🔍 DEBUG resetReason type:', typeof resetReason);
      console.log('🔍 DEBUG resetReason value:', resetReason);
      
      // Ensure resetReason is a string, not an object
      let reasonString = '';
      if (resetReason) {
        if (typeof resetReason === 'string') {
          reasonString = resetReason;
        } else if (typeof resetReason === 'object') {
          console.warn('⚠️ WARNING: resetReason is an object, converting to string');
          reasonString = JSON.stringify(resetReason);
        } else {
      ...
      // Send SMS notification
      try {
        await sendPasswordResetSMS(adminPhone, adminUserId, reasonString);
