...
export const adminService = {
  ...
  async updateAdminStatus(
    ...
    try {
      ...
      if (currentAdmin.phoneNumber && updatedByAdmin && previousStatus !== newStatus) {
        ...
        console.log('✅ Status change SMS sent');
      }
  ...
  async updateAdmin(
    ...
    try {
      ...
      const isStatusChanged = updates.status && 
                             updates.status !== currentAdmin.status;
      const previousStatus = currentAdmin.status;
      ...
      if (isPhoneNumberChanged && oldPhoneNumber && updatedByAdmin) {
        ...
        console.log('✅ Phone number change SMS sent');
      }

      // Send SMS if status was changed
      if (isStatusChanged && currentAdmin.phoneNumber && updatedByAdmin) {
        await sendStatusChangeSMS(
          currentAdmin.phoneNumber,
          currentAdmin.userId || uid,
          previousStatus,
      ...
      if (updatedByAdmin) {
        try {
          ...
          if (isStatusChanged) {
            changedFields.status = { from: previousStatus, to: updates.status };
            changesArray.push(`status changed from ${previousStatus} to ${updates.status}`);
            ...
            await addDoc(collection(db, 'security_logs'), {
              ...
              performedBySurname: updatedByAdmin.surname || 'N/A',
              timestamp: Timestamp.now(),
              details: `Status changed from ${previousStatus} to ${updates.status} for ${currentAdmin.surname} (${currentAdmin.userId}) by ${updatedByAdmin.surname} (${updatedByAdmin.userId})`,
              changes: JSON.stringify({
                action: 'status_change',
                previousStatus,
