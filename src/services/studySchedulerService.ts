...
function idealSessionMins(
  ...
  // Spread work evenly: aim to cover (minsLeft/daysLeft) per day.
  // Cap at half the daily capacity so other goals always get a meaningful share
  // of each day — without this cap, one goal consumes full blocks and leaves
  // only unusable 20-min scraps for the other goals.
  const minsPerDay  = minsLeft / Math.max(1, daysLeft);
  const halfDay     = dailyMins / 2;
  const ideal       = Math.min(dynamicMax, Math.min(halfDay, Math.max(MIN_SESSION_MINS, minsPerDay)));
...
export function generateStudySchedule(input: SchedulerInput): ScheduleResult {
  ...
  for (const slot of daySlots) {
    ...
    while (anyPlacedThisRound && slot.freeBlocks.length > 0) {
      ...
      eligible.sort((a, b) => {
        // Rule 1: tighter deadline → higher priority.
      ...
      for (const gw of eligible) {
        ...
        if (bi === -1) {
          // Ideal size doesn't fit. Find ANY block >= MIN_SESSION_MINS and shrink to fit.
          // Only allow sub-60min if this is a final cleanup session (minsLeft < 60).
          const minFallback = gw.minsLeft < MIN_SESSION_MINS ? 30 : MIN_SESSION_MINS;
          bi = slot.freeBlocks.findIndex(b => b.endMins - b.startMins >= minFallback);
          ...
          if (sessionMins < minFallback) sessionMins = Math.min(available, gw.minsLeft);
        }

        // FIX: Block-aware session capping.
        // If our session would consume so much of the block that the remainder is
        // smaller than MIN_SESSION_MINS + GAP, we cap it to leave exactly
        // MIN_SESSION_MINS + GAP of space — so the next goal can still fit a full
        // session in this same block instead of finding only an unusable 20-min scrap.
        //
        // Example: block=180m, sessionMins=150m → remainder=180-150-10=20m (wasted).
        // With cap: max=180-10-60=110m → Physics=105m(snapped), remainder=65m → Math fits 60m ✓
        //
        // Exception: if the block is small enough that only 1 session fits anyway
        // (blockSize < 2*MIN + GAP), don't cap — just take what we need.
        const blockSize = slot.freeBlocks[bi].endMins - slot.freeBlocks[bi].startMins;
        if (blockSize >= MIN_SESSION_MINS * 2 + GAP_MINS) {
          const maxToLeaveRoom = blockSize - GAP_MINS - MIN_SESSION_MINS;
          if (sessionMins > maxToLeaveRoom) {
            sessionMins = snapToGrid(maxToLeaveRoom, SNAP_MINS);
            if (sessionMins < MIN_SESSION_MINS) sessionMins = MIN_SESSION_MINS;
          }
        }

        // Final safety: clamp to block size (guards against rounding edge cases)
