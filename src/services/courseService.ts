...
// Safe timestamp converter — handles Firestore Timestamp, Date, string, number, null
  ...
  async getStudentEnrollments(studentId: string): Promise<Enrollment[]> {
    try {
      ...
      return snapshot.docs.map(doc => ({
        id: doc.id,
        ...doc.data(),
        enrolledAt: safeToDate(doc.data().enrolledAt),
        lastAccessedAt: safeToDate(doc.data().lastAccessedAt),
        paymentDate: doc.data().paymentDate ? safeToDate(doc.data().paymentDate) : undefined
      })) as Enrollment[];
    } catch (error: any) {
