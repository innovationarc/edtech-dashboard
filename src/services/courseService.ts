...
// Updated Course Service with Complete Payment Integration
...
export const courseService = {
  ...
  async getPublishedCourses(): Promise<Course[]> {
    try {
      ...
      try {
        ...
        console.warn('Firestore index missing, filtering locally');
      }
      ...
      return allCourses.filter(c => c.isPublished).sort((a, b) => b.createdAt.getTime() - a.createdAt.getTime());
    } catch (error: any) {
    ...
    try {
      ...
      if (courseData.thumbnail) {
        try {
          ...
          console.warn('Failed to delete thumbnail');
        }
      }
      ...
      if (courseData.routineFiles && Array.isArray(courseData.routineFiles)) {
        for (const file of courseData.routineFiles) {
          try {
            ...
            console.warn('Failed to delete routine file');
          }
        }
      }
      ...
      try {
        ...
        console.warn('Failed to delete enrollments');
      }
      ...
      try {
        ...
        console.warn('Failed to delete student content');
      }
  ...
  // Enrollment Operations with Complete Payment Integration
  ...
  async calculateEnrollmentPrice(
    ...
    try {
      ...
      console.log('CourseService: Previous enrollments:', previousEnrollments.length);
      ...
      // Coupon discount (placeholder for future implementation)
      if (couponCode) {
   ...
   * For paid courses: Initiate payment via paymentService
   */
  async enrollStudent(request: EnrollmentRequest): Promise<EnrollmentResponse> {
    try {
      console.log('CourseService: Enrollment request received', {
  ...
  /**
   * Initiate paid enrollment via payment service
   */
  async initiatePaidEnrollment(
    ...
    try {
      console.log('CourseService: Initiating paid enrollment via payment service');
