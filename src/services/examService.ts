...
export interface WrittenAnswer {
  ...
  evaluatedAt?: Date;
  evaluatedBy?: string;
  reviewRequests?: ReviewRequest[];  // student can request re-evaluation
}
...
const deserializeSession = (id: string, data: any): ExamSession => ({
  ...
  writtenAnswers: (data.writtenAnswers || []).map((wa: any) => ({
    questionId: wa.questionId,
    answerText: wa.answerText,
    attachmentUrls: wa.attachmentUrls || [],
    marksAwarded: wa.marksAwarded,
    evaluatorComment: wa.evaluatorComment,
    evaluatedAt: wa.evaluatedAt ? toDate(wa.evaluatedAt) : undefined,
    evaluatedBy: wa.evaluatedBy,
    reviewRequests: (wa.reviewRequests || []).map((r: any) => ({
      requestedAt: toDate(r.requestedAt),
      message: r.message ?? '',
      status: r.status ?? 'pending',
      resolvedAt: r.resolvedAt ? toDate(r.resolvedAt) : undefined,
      resolvedComment: r.resolvedComment,
    })),
  })),
...
export const examService = {
  ...
  // ── NEW: Request review for a written answer ─────────────────────────────────
  async requestWrittenReview(
    sessionId: string,
    questionId: string,
    message: string
  ): Promise<void> {
    ...
    const updatedAnswers = session.writtenAnswers.map(wa => {
      ...
      return {
        ...wa,
        reviewRequests: [...(wa.reviewRequests || []), newRequest],
      };
    });

    await updateDoc(doc(db, 'examSessions', sessionId), clean({
      writtenAnswers: updatedAnswers.map(wa => ({
        ...wa,
        reviewRequests: (wa.reviewRequests || []).map(r => ({
          ...r,
          requestedAt: Timestamp.fromDate(r.requestedAt instanceof Date ? r.requestedAt : new Date(r.requestedAt)),
          resolvedAt: r.resolvedAt ? Timestamp.fromDate(r.resolvedAt instanceof Date ? r.resolvedAt : new Date(r.resolvedAt)) : undefined,
        })),
      })),
      updatedAt: Timestamp.now(),
    }));
  ...
  // ── Resolve a student review request (teacher responds) ─────────────────────
  async resolveReviewRequest(
    ...
    const updatedAnswers = session.writtenAnswers.map(wa => {
      ...
      const updatedRequests = (wa.reviewRequests || []).map((r, i) =>
        i === requestIndex
          ? { ...r, status: 'resolved' as const, resolvedAt: new Date(), resolvedComment }
          : r
      );
      ...
      writtenAnswers: updatedAnswers.map(wa => ({
        ...
        reviewRequests: (wa.reviewRequests || []).map(r => ({
          ...
          resolvedAt: r.resolvedAt
            ? Timestamp.fromDate(r.resolvedAt instanceof Date ? r.resolvedAt : new Date(r.resolvedAt))
            : undefined,
        })),
      })),
      updatedAt: Timestamp.now(),
