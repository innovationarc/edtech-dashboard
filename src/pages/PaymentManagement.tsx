...
const DEFAULT_AUDIT_FILTERS: AuditFilters = { search: '', action: 'all', dateFrom: '', dateTo: '', role: 'all' };
...
const AuditLogTab = ({ logs, loading, error, onRefresh, auditFilters, onAuditFilterChange, onAuditFilterReset }: {
  ...
  const isAuditFilterActive = auditFilters.search !== '' || auditFilters.action !== 'all' || auditFilters.dateFrom !== '' || auditFilters.dateTo !== '' || auditFilters.role !== 'all';
...
const DEFAULT_TRASH_FILTERS: TrashFilters = { search: '', status: 'all', gateway: 'all', dateFrom: '', dateTo: '' };
...
const TrashTab = ({
  ...
  const filtered = useMemo(() => {
    return trashRecords.filter(r => {
      ...
      return matchSearch && matchStatus && matchGateway && matchDate;
    });
  }, [trashRecords, trashFilters]);

  const isTrashFilterActive = trashFilters.search !== '' || trashFilters.status !== 'all' || trashFilters.gateway !== 'all' || trashFilters.dateFrom !== '' || trashFilters.dateTo !== '';
  const sel = 'bg-white/[0.04] border border-white/10 text-white text-xs rounded-lg px-3 py-2 focus:outline-none focus:ring-1 focus:ring-primary-500 [color-scheme:dark]';
                ...
                <div className="grid grid-cols-2 gap-3 text-xs">
                  <div>
                    ...
                    <p className="text-gray-200 font-medium mt-0.5">
                      {viewRecord.deletedBySurname || viewRecord.deletedByName}
                      {viewRecord.deletedByUserId && (
                        <span className="font-mono text-emerald-400 ml-1">({viewRecord.deletedByUserId})</span>
                      )}
                      <span className="text-gray-500 ml-1">· {viewRecord.deletedByRole}</span>
                    </p>
                  </div>
                  <div>
                ...
                <div className="grid grid-cols-2 gap-2 text-xs">
                  <div>
                    ...
                    <p className="text-gray-300 font-medium mt-0.5">
                      {record.deletedBySurname || record.deletedByName}
                      {record.deletedByUserId && (
                        <span className="font-mono text-emerald-400 ml-1">({record.deletedByUserId})</span>
                      )}
                      <span className="text-gray-600 ml-1">· {record.deletedByRole}</span>
                    </p>
                  </div>
                  <div>
                    <span className="text-gray-600">Deleted at</span>
