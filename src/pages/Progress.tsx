...
function CourseDetailPage({
  ...
  // Scroll ALL possible containers to top when detail opens
  useEffect(() => {
    // Scroll every scrollable ancestor + window — handles any layout
    const scrollAll = () => {
      window.scrollTo(0, 0);
      document.documentElement.scrollTop = 0;
      document.body.scrollTop = 0;
      // Walk up and reset any scrollable ancestor
      const selectors = ['[data-scroll-container]', '[data-main-scroll]', '.main-content', '.page-content', '.layout-content', 'main', '.overflow-y-auto', '.overflow-auto'];
      selectors.forEach(sel => {
        document.querySelectorAll(sel).forEach(el => { (el as HTMLElement).scrollTop = 0; });
      });
    };
    scrollAll();
    // Run again after paint in case layout shifts
  ...
  return (
    // In-flow full-page — no position:fixed so parent transforms can't break it.
    // Progress component returns ONLY this element, so it fills the layout naturally.
    <div style={{
      minHeight: '100vh',
      display: 'flex',
      flexDirection: 'column',
      fontFamily: "'DM Sans', 'Outfit', sans-serif",
      // Solid bg: covers starfield so no bleed-through on any section
      background: T.isLight ? '#f8fafc' : '#0a0a14',
      // Bleed flush to layout edges — var(--page-padding) with 16px fallback
      margin: 'calc(-1 * var(--page-padding, 16px)) calc(-1 * var(--page-padding, 16px)) 0',
      padding: '0 0 60px',
    }}>
      {/* ── Top bar ── */}
      <div style={{
        ...
        top: 0,
        zIndex: 10,
        background: T.isLight ? 'rgba(248,250,252,0.95)' : 'rgba(10,10,20,0.95)',
      ...
      <div style={{
        ...
        background: T.isLight ? `linear-gradient(135deg, ${rc}18 0%, #f8fafc 60%)` : `linear-gradient(135deg, ${rc}20 0%, #0e0e1e 60%)`,
        borderBottom: `1px solid ${T.border}`,
        padding: '20px 16px',
      }}>
        {/* decorative rings */}
        ...
        {/* Stat pills — themed to match dark/light, colored accents, no opaque card bg */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
          {[
            ...
            <div key={label} style={{
              background: T.isLight ? 'rgba(255,255,255,0.85)' : 'rgba(255,255,255,0.06)',
              ...
              <div style={{ minWidth: 0 }}>
                ...
                <p style={{ fontSize: 10, color: T.text2, margin: '3px 0 0', fontWeight: 500 }}>{label}</p>
              </div>
            </div>
          ))}
        </div>
      ...
      {/* ── Tabs ── */}
      <div style={{ display: 'flex', borderBottom: `1px solid ${T.border}`, background: T.isLight ? '#f0f2f5' : '#0d0d1a', position: 'sticky', top: 57, zIndex: 9 }}>
        {([
