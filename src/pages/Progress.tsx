...
// ═══════════════════════════════════════════════════════════════════════════════
// COURSE DETAIL PAGE
// Renders in-flow (no fixed/absolute) so parent transforms don't break it.
// The Progress component returns ONLY this when selected, so it naturally fills
// the page layout container — same as any other page route would.
// ═══════════════════════════════════════════════════════════════════════════════
function CourseDetailPage({
  ...
  // Scroll to top when detail tab opens
  ...
  return (
    // Full-screen tab panel — fixed overlay, independent scroll, app theme bg
    <div data-course-detail-tab style={{
      minHeight: '100vh',
      display: 'flex',
      ...
      {/* ── Top bar ── */}
      <div style={{ padding: '10px 16px', display: 'flex', alignItems: 'center', gap: 10, borderBottom: `1px solid ${T.border}` }}>
        <button onClick={onBack} style={{
          display: 'flex', alignItems: 'center', gap: 4,
          background: T.surface, border: `1px solid ${T.border}`,
          borderRadius: 8, padding: '6px 10px', color: T.text2,
        ...
        <div style={{ flex: 1, minWidth: 0 }}>
          <h2 style={{ color: T.text, fontSize: 13, fontWeight: 700, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
            {course.title}
          </h2>
          <p style={{ color: T.text2, fontSize: 10, margin: 0 }}>{course.category}{course.class ? ` · Class ${course.class}` : ''}</p>
        </div>
        {grade && (
          <div style={{ background: `${grade.color}15`, border: `1px solid ${grade.color}35`, borderRadius: 7, padding: '3px 8px', flexShrink: 0 }}>
            <span style={{ fontSize: 14, fontWeight: 900, color: grade.color }}>{grade.letter}</span>
          </div>
        )}
      </div>

      {/* ── Rank + KPI section ── */}
      <div style={{ padding: '14px 16px 16px', borderBottom: `1px solid ${T.border}` }}>
        {/* Rank row — compact */}
        <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
          <div style={{ width: 44, height: 44, borderRadius: '50%', background: `${rc}15`, border: `1.5px solid ${rc}35`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
            {myRank === 1 ? <Trophy size={18} color={GOLD} /> :
             myRank === 2 ? <Medal size={18} color={SILVER} /> :
             myRank === 3 ? <Award size={18} color={BRONZE} /> :
             myRank !== null ? <Hash size={16} color={rc} /> :
             <BookOpen size={16} color={T.text3} />}
          </div>
          <div style={{ flex: 1, minWidth: 0 }}>
            {myRank !== null ? (
              <>
                <div style={{ display: 'flex', alignItems: 'baseline', gap: 4 }}>
                  <span style={{ fontSize: 26, fontWeight: 900, color: rc, lineHeight: 1 }}>#{myRank}</span>
                  <span style={{ fontSize: 11, color: T.text2 }}>of {totalStudents}</span>
                </div>
                <div style={{ display: 'flex', gap: 5, marginTop: 3, flexWrap: 'wrap' }}>
                  {percentile && <span style={{ background: `${rc}15`, border: `1px solid ${rc}30`, color: rc, fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 99 }}>{percentile}</span>}
                  {grade && <span style={{ background: `${grade.color}15`, border: `1px solid ${grade.color}30`, color: grade.color, fontSize: 10, fontWeight: 700, padding: '1px 7px', borderRadius: 99 }}>Grade {grade.letter}</span>}
                </div>
              </>
            ) : (
              <>
                <p style={{ color: T.text, fontSize: 13, fontWeight: 600, margin: 0 }}>Not yet ranked</p>
                <p style={{ color: T.text3, fontSize: 11, margin: '2px 0 0' }}>Complete a 1-time exam to appear on the leaderboard</p>
              </>
            )}
          </div>
        </div>

        {/* KPI grid — identical structure to main page stat cards */}
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2, 1fr)', gap: 8 }}>
          {[
            { icon: Target,       label: 'My Score',    value: myPercentage !== null ? `${myPercentage.toFixed(1)}%` : '—', color: rc },
            { icon: CheckCircle2, label: 'Exams Taken', value: `${examStats.taken.length}/${courseLeaderboard.totalExams}`, color: '#10b981' },
            { icon: Trophy,       label: 'Top 3 Wins',  value: examStats.top3Exams.length,                                 color: GOLD },
            { icon: Star,         label: 'Avg Score',   value: examStats.avgScore > 0 ? `${examStats.avgScore.toFixed(0)}%` : '—', color: '#8b5cf6' },
            ...
            <NoAnimCard key={label} className="!p-3">
              <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                ...
                <div style={{ minWidth: 0 }}>
                  ...
                  <p style={{ fontSize: 10, color: T.text2, margin: '2px 0 0', whiteSpace: 'nowrap' }}>{label}</p>
                </div>
              </div>
            </NoAnimCard>
          ))}
