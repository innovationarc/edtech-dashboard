...
function CourseDetailPage({
  ...
  // Scroll the fixed tab container to top when detail opens
  useEffect(() => {
    // For the fixed overlay, scroll the element itself to top
    const el = document.querySelector('[data-course-detail-tab]') as HTMLElement | null;
  ...
  return (
    ...
    <div data-course-detail-tab style={{
      ...
      background: 'var(--color-bg, ' + (T.isLight ? '#f8fafc' : '#080810') + ')',
      overflowY: 'auto',
      overflowX: 'hidden',
      ...
      <div style={{
        ...
        top: 0,
        zIndex: 10,
        background: T.isLight ? 'rgba(248,250,252,0.95)' : 'rgba(var(--color-bg-rgb, 8,8,16),0.95)',
      ...
      <div style={{
        ...
        background: T.isLight ? `linear-gradient(135deg, ${rc}18 0%, #f8fafc 60%)` : `linear-gradient(135deg, ${rc}20 0%, var(--color-bg, #080810) 60%)`,
        borderBottom: `1px solid ${T.border}`,
        padding: '20px 16px',
      }}>
        {/* decorative rings */}
      ...
      {/* ── Tabs ── */}
      <div style={{ display: 'flex', borderBottom: `1px solid ${T.border}`, background: T.isLight ? '#f0f2f5' : 'var(--color-surface, #0d0d1a)', position: 'sticky', top: 57, zIndex: 9 }}>
        {([
...
const Progress = () => {
  ...
  useEffect(() => {
    if (selectedSummary) {
      // Lock body scroll when tab is open — the fixed overlay handles its own scroll
  ...
  return (
    ...
    <div style={{ color: 'var(--color-text,#f1f5f9)', minHeight: '100vh', padding: '0 0 40px', fontFamily: "'DM Sans', 'Outfit', sans-serif" }}>
      ...
      {/* Full-screen tab overlay — renders on top when a course is selected */}
      {selectedSummary && (
        <CourseDetailPage
