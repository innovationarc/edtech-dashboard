...
// Part 2 of 2: AskQuestionModal Component
...
const AskQuestionModal = ({ onClose, onSuccess, studentId, studentName, subjects, availableCourses }: AskQuestionModalProps) => {
  ...
  const handleAiSatisfaction = async (status: 'satisfied' | 'confused') => {
    ...
    try {
      ...
      if (status === 'satisfied') {
        ...
        if ((window as any).addNotification) {
          (window as any).addNotification('You can now ask a follow-up question', 'info');
        }
      }
    } catch (err: any) {
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-1">Only courses with Q&A enabled</p>
              </div>
