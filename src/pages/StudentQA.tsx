...
${isFollowUp ? 'Follow-up Question' : 'Question'}: ${questionToSolve}${knowledgeContext}`;
  ...
  return (
    <ModalShell onClose={onClose} wide>
      ...
      {/* ── Mobile tab switcher — only when right panel has content ── */}
      {hasRightPanel && (
        <div
          className="lg:hidden"
          style={{ display: 'flex', gap: 6, padding: '10px 16px 0', flexShrink: 0 }}
        >
      ...
      <div style={{ flex: 1, minHeight: 0, display: 'flex', overflow: 'hidden' }}>
        ...
        {/* ── LEFT COLUMN: Form ── */}
        <div
          className={`${hasRightPanel && activeTab !== 'form' ? 'hidden lg:flex' : 'flex'} lg:w-1/2 w-full`}
          style={{
        ...
        <div
          className={`${hasRightPanel && activeTab !== 'preview' ? 'hidden lg:flex' : 'flex'} lg:w-1/2 w-full`}
          style={{ flexDirection: 'column', overflowY: 'auto', padding: '20px 24px' }}
        >
      ...
      {/* ── Footer — identical pattern to ExamFormModal ── */}
      <div style={{
        display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 10,
        padding: '14px 24px', borderTop: `1px solid ${T.divider}`,
        ...
        >
          ...
          <span>
            {loading
              ? 'Submitting...'
              : hasHumanQnA
              ? 'Submit to Human Teacher'
              : 'Teacher Q&A Not Available'}
          </span>
        </button>
