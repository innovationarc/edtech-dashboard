// src/pages/StudentQA.tsx - COMPLETE FILE
import { useState, useEffect } from 'react';
import { Plus, Search, Send, Loader, AlertCircle, BookOpen, User, MessageSquare, X, Brain, Bell, Filter, FileText } from 'lucide-react';
...
const AskQuestionModal = ({ onClose, onSuccess, studentId, studentName, subjects }: AskQuestionModalProps) => {
  ...
  const detectLanguage = (text: string): 'bengali' | 'english' | 'banglish' => {
    ...
    const banglishPattern = /\b(keno|karon|tumi|ami|kothay|kivabe|kobe|ke|ki|ache|hobe|kora|kore|korte|kora|jai|jay|jabe|jabo|jai|chile|chilo|chilam|thake|thaki|thakbo|thakbe|bole|boli|bolte|bolbo|bolbe)\b/i;
  ...
  const handleSubmitToTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (imageFile) {
        const driveResult = await qaService.uploadToGoogleDrive(imageFile, 'question_images');
        imageUrl = driveResult.webViewLink;
      }

      if (docFile) {
        const driveResult = await qaService.uploadToGoogleDrive(docFile, 'question_documents');
  ...
  const handleAiSolve = async (e: React.FormEvent) => {
    ...
    try {
      // First, get knowledge base for the subject
      ...
      const languageInstruction = language === 'bengali' || language === 'banglish'
        ? 'Answer ONLY in Bengali (Bangla script - বাংলা). If the question uses Banglish (Bengali written in English), convert it to Bengali script first, then answer in Bengali.'
...
LANGUAGE RULE:
${languageInstruction}
- Scientific terms and formulas that must be in English should remain in English.
- Mathematical symbols and equations should use standard notation.

TEACHING METHOD:
${knowledgeContext ? '- Use the Teacher\'s Knowledge Base provided above for solving methods and procedures.' : '- Use traditional Bengali school textbook methods.'}

ANSWER FORMAT:
1. Provide ONLY:
   - Clear, numbered step-by-step solution (each step short and necessary)
   - A final line labeled "চূড়ান্ত উত্তর:" (for Bengali) or "Final answer:" (for English)

2. Show all calculations and reasoning:
   - Math: full steps and intermediate results
   - Science: key formula/principle, then steps applying it
   - Language: grammar rule → apply rule → result

3. No extra explanation, no greeting, no summary, no commentary.

4. For multi-part problems, label parts clearly: (ক), (খ), (গ) etc. (for Bengali) or (a), (b), (c) etc. (for English)
      ...
      <div className="bg-background-900 rounded-xl w-full max-w-6xl my-8 relative">
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-4">
              {showAiSolution ? (
                <div className="bg-background-800 rounded-lg p-4 h-full min-h-[500px]">
                  <div className="flex items-center gap-2 mb-4 pb-3 border-b border-background-700">
                  ...
                  {aiLoading ? (
                    ...
                    <div className="prose prose-invert max-w-none">
                      ...
                      <div className="text-xs text-red-400 mt-4 pt-4 border-t border-background-700">
                ...
                <div className="bg-background-800 rounded-lg p-8 h-full min-h-[500px] flex flex-col items-center justify-center text-center">
                  ...
                  <p className="text-xs text-gray-500 mt-4">
                    Supports Bengali (বাংলা), Banglish, and English
                  </p>
                </div>
              )}
            </div>
