// src/pages/StudentQA.tsx - Part 1 of 3 (Updated with Course-based Filtering and Q&A Permissions)
...
const StudentQA = () => {
  ...
  const loadEnrolledCoursesWithQnA = async () => {
    try {
      ...
      // Filter courses that have either AI Q&A or Human Q&A enabled
  ...
  const loadQuestions = async () => {
    ...
    try {
      ...
      // Filter out follow-up questions from main listing
...
// src/pages/StudentQA.tsx - Part 2 of 3 (AskQuestionModal Component - First Half)
...
${isFollowUp ? 'Follow-up Question' : 'Question'}: ${questionToSolve}${knowledgeContext}`;
  ...
  const handleAiSolve = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Create question with uploaded image URL
      const questionId = await qaService.askQuestion({
        studentId,
        studentName,
        subject,
        questionText: questionText.trim(),
        imageUrl: uploadedImageUrl, // Now the image is uploaded to Supabase
        courseId: selectedCourse,
      });
...
// src/pages/StudentQA.tsx - Part 3 of 3 (AskQuestionModal Component - Second Half with UI)
