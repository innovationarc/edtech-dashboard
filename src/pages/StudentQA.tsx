// src/pages/StudentQA.tsx (UPDATED — AI calls via callWithFailover smart failover)
import { useState, useEffect, useRef } from 'react';
import { Plus, Search, Loader, AlertCircle, BookOpen, User, MessageSquare, X, Brain, Bell, FileText, Filter, ThumbsUp, HelpCircle, Mic, Volume2, Eye, Send, Bookmark } from 'lucide-react';
...
const AskQuestionModal = ({ onClose, onSuccess, studentId, studentName, enrolledCoursesWithQnA, helpSupportSubjects }: AskQuestionModalProps) => {
  ...
  // ── AI Solution via smart failover ────────────────────────────────────────
  // Replaces direct Gemini API call. Routes through admin-configured key groups
  // with automatic failover. Image (base64) passed for vision when present.
...
${isFollowUp ? 'Follow-up Question' : 'Question'}: ${questionToSolve}${knowledgeContext}`;
      ...
      // Extract image data if present (for vision-capable models)
      ...
      // Route through admin-configured keys with smart failover
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4 overflow-y-auto">
      <div className="bg-background-900 rounded-xl w-full max-w-6xl my-8 relative">
        <button
          ...
          className="absolute right-4 top-4 text-gray-400 hover:text-white z-10 disabled:opacity-50"
        >
          <X size={20} />
        </button>
        ...
        <div className="p-6">
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            <div className="space-y-4">
              ...
              {/* File attachment */}
              <div>
                <label className="block text-sm text-gray-400 mb-1">Attach Image or File (optional)</label>
                <input
                  type="file"
                  accept="image/jpeg,image/jpg,image/png,image/heic,application/pdf,.docx"
                  onChange={handleFileChange}
                  disabled={loading || aiLoading}
                  className="w-full text-sm text-gray-400 file:mr-4 file:py-1 file:px-3 file:rounded file:border-0 file:text-xs file:bg-primary-600 file:text-white hover:file:bg-primary-700"
                />
                {attachedFile && (
                  <div className="flex items-center gap-2 mt-1">
                    <span className="text-xs text-gray-400">{attachedFile.name}</span>
                    <button onClick={() => setAttachedFile(null)} className="text-xs text-error-light hover:text-red-400">Remove</button>
                  </div>
                )}
              </div>
                ...
                <div className="flex gap-2">
                  {!isRecording ? (
                    <button
                      ...
                      disabled={loading || aiLoading}
                      className="flex items-center gap-2 bg-background-700 hover:bg-background-600 text-white px-3 py-1.5 rounded-lg text-sm transition-colors disabled:opacity-50"
                    >
                      <Mic size={14} />
                      <span>Record</span>
                    </button>
                    ...
                    <button
                      ...
                      className="flex items-center gap-2 bg-red-600 hover:bg-red-700 text-white px-3 py-1.5 rounded-lg text-sm transition-colors animate-pulse"
                    >
                  ...
                  {audioFile && (
                    <div className="flex items-center gap-2">
                      ...
                      <button onClick={() => setAudioFile(null)} className="text-xs text-error-light">Remove</button>
                    </div>
                  )}
                </div>
              </div>
              ...
              <div className="flex gap-3 pt-2">
                {hasHumanQnA && (
                  <button
                    ...
                    disabled={loading || aiLoading || !questionText.trim() || !subject || !selectedCourse}
                    className="flex-1 flex items-center justify-center gap-2 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-800 disabled:opacity-50 text-white px-4 py-2 rounded-lg transition-colors"
                  >
                    {loading && <Loader size={14} className="animate-spin" />}
                    <span>{loading ? 'Checking...' : 'Ask Teacher'}</span>
                  </button>
                )}
                {!isHelpSupport && hasAiQnA && (
                  <button
                    ...
                    disabled={loading || aiLoading || !questionText.trim() || !subject || !selectedCourse}
                    className="flex-1 flex items-center justify-center gap-2 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-800 disabled:opacity-50 text-white px-4 py-2 rounded-lg transition-colors"
                  >
            ...
            {/* Right panel: similar questions or AI solution */}
                ...
                <div className="bg-background-800 rounded-lg p-4 h-full min-h-[500px] max-h-[600px] overflow-y-auto">
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-background-700 sticky top-0 bg-background-800 z-10">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="flex items-center gap-1 text-sm bg-primary-600 hover:bg-primary-700 disabled:opacity-50 text-white px-3 py-1.5 rounded-lg transition-colors"
                    >
                  ...
                  {similarLoading ? (
                    ...
                    <div className="space-y-3">
                      {similarQuestionsWithAnswers.map(({ question, answers }) => (
                        <div key={question.id} className="border border-background-700 rounded-lg overflow-hidden">
                          <div
                            className="p-3 cursor-pointer hover:bg-background-700 transition-colors"
                            onClick={() => toggleQuestionExpansion(question.id)}
                          >
                            <p className="text-sm text-white font-medium line-clamp-2">{question.questionText}</p>
                          ...
                          {expandedQuestionId === question.id && answers.length > 0 && (
                            <div className="border-t border-background-700 p-3 bg-background-900 space-y-2">
                              {answers.map(answer => (
                                <div key={answer.id} className={`p-3 rounded-lg ${answer.type === 'ai' ? 'bg-purple-900/30' : 'bg-blue-900/30'}`}>
                                  <div className="flex items-center gap-2 mb-1 text-xs">
                                    {answer.type === 'ai' ? (
                                      <><Brain size={12} className="text-purple-400" /><span className="text-purple-300">AI Answer</span></>
                                    ) : (
                                      <><User size={12} className="text-blue-400" /><span className="text-blue-300">{answer.teacherName || 'Teacher'}</span></>
                                    )}
                                  </div>
                        ...
                        <div className="pt-4 border-t border-background-700">
                          ...
                          <div className="flex gap-3">
                            <button
                              ...
                              disabled={aiLoading}
                              className="flex-1 flex items-center justify-center gap-2 bg-green-600 hover:bg-green-700 disabled:bg-green-800 disabled:opacity-50 text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium"
                            >
                              <ThumbsUp size={16} />
                              <span>Satisfied</span>
                            </button>
                            <button
                              ...
                              disabled={aiLoading}
                              className="flex-1 flex items-center justify-center gap-2 bg-orange-600 hover:bg-orange-700 disabled:bg-orange-800 disabled:opacity-50 text-white px-4 py-2 rounded-lg transition-colors text-sm font-medium"
                            >
                              <HelpCircle size={16} />
                        ...
                        <div className="pt-4 border-t border-background-700 space-y-3">
                          ...
                          <div className="flex gap-2">
                            ...
                            <button
                              ...
                              disabled={aiLoading}
                              className="px-4 py-2 bg-background-700 hover:bg-background-600 disabled:opacity-50 text-white rounded-lg transition-colors"
                            >
                              Cancel
                            </button>
                          </div>
