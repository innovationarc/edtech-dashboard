// src/pages/StudentQA.tsx - Part 1 of 3 (FIXED - No Duplicate Submissions)
...
// src/pages/StudentQA.tsx - Part 2 of 3 (AskQuestionModal - FIXED)
...
const AskQuestionModal = ({ onClose, onSuccess, studentId, studentName, enrolledCoursesWithQnA, helpSupportSubjects }: AskQuestionModalProps) => {
  ...
  const handleSubmitToTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Store uploaded image URL for later use
      if (attachedFile && getFileType(attachedFile) === 'image') {
        // Image was already uploaded in findSimilarQuestionsWithFile
        // We need to get it from the similar questions logic or re-upload
      }
      ...
      if (similar.length > 0) {
        ...
        await loadSimilarQuestionsWithAnswers(similar);
...
// src/pages/StudentQA.tsx - Part 3 of 3 (AskQuestionModal UI - FIXED)
            ...
            <div className="space-y-4">
              ...
              <div className="flex flex-col gap-3 pt-4 border-t border-background-800">
                <button
                  ...
                  className="w-full px-6 py-3 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-800 disabled:text-gray-500 disabled:cursor-not-allowed text-white rounded-lg transition-all flex items-center justify-center gap-2 font-medium"
                ...
                <button
                  ...
                  className="w-full px-6 py-3 bg-purple-600 hover:bg-purple-700 disabled:bg-purple-800 disabled:text-gray-500 disabled:cursor-not-allowed text-white rounded-lg transition-all flex items-center justify-center gap-2 font-medium"
                  title={
                    isHelpSupport 
                ...
                <button
                  onClick={onClose}
                  disabled={loading || aiLoading}
                  className="w-full px-6 py-3 bg-background-800 hover:bg-background-700 disabled:opacity-50 text-white rounded-lg transition-colors font-medium"
                >
                  Cancel
                </button>
              </div>
                ...
                <div className="bg-background-800 rounded-lg p-4 h-full min-h-[500px] max-h-[600px] overflow-y-auto">
                  <div className="flex items-center justify-between mb-4 pb-3 border-b border-background-700 sticky top-0 bg-background-800 z-10">
                    ...
                    <button
                      ...
                      disabled={loading}
                      className="flex items-center gap-2 bg-primary-600 hover:bg-primary-700 disabled:bg-primary-800 disabled:cursor-not-allowed text-white px-3 py-1.5 rounded-lg transition-colors text-sm"
                    >
                        ...
                        <div key={q.id} className="bg-background-700 rounded-lg p-3">
                          ...
                          {expandedQuestionId === q.id && (
                            <div className="mt-4 pt-4 border-t border-background-600 space-y-3">
                              <div>
                                <h4 className="text-sm font-semibold text-primary-400 mb-2">Full Question:</h4>
                                <p className="text-sm text-white whitespace-pre-wrap">{q.questionText}</p>
                                {q.imageUrl && (
                                  <img src={q.imageUrl} alt="Question" className="mt-2 max-h-48 object-contain rounded-lg" />
                                )}
                                {q.audioUrl && (
                                  <div className="mt-2">
                                ...
                                {q.fileName && q.fileUrl && (
                                  <div className="mt-2 flex items-center gap-2 text-sm">
                                    ...
                                    <span className="text-gray-400">{q.fileName}</span>
                                  </div>
                                )}
                              </div>
                                      ...
                                      <div key={answer.id} className="bg-background-600 rounded-lg p-3">
                                        ...
                                        {answer.fileName && answer.fileUrl && (
                                          <div className="mt-2 flex items-center gap-2 text-sm">
                                            ...
                                            <span className="text-gray-400">{answer.fileName}</span>
                                          </div>
                                        )}
                                      </div>
                                    ))}
