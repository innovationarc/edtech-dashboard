// src/pages/StudentQA.tsx - Part 1 of 2 (Updated with AI Vision)
...
// src/pages/StudentQA.tsx - Part 2 of 2 (Updated with AI Vision)
...
const AskQuestionModal = ({ onClose, onSuccess, studentId, studentName, enrolledCoursesWithQnA, helpSupportSubjects }: AskQuestionModalProps) => {
  ...
  const handleSubmitToTeacher = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Upload file to Supabase for human teacher
      if (attachedFile) {
...
${isFollowUp ? 'Follow-up Question' : 'Question'}: ${questionToSolve}${knowledgeContext}`;
      ...
      // If there's a file to process (image or PDF), add it to the request
                  ...
                  {!isHelpSupport && (
                    <>
                      ...
                      <p className="text-xs text-purple-400">
                        ✨ Can analyze images (JPG, PNG, HEIC)
                      </p>
                    </>
                  )}
                </div>
