...
const ExamEvaluation: React.FC = () => {
  ...
  // Derive course list directly from loaded exam summaries — avoids mismatch with courseService
  const coursesInExams = useMemo(() => {
    const seen = new Map<string, string>(); // courseId -> title
    examSummaries.forEach(es => {
      const cId = (es.content as any).courseId;
      if (cId && !seen.has(cId)) {
        seen.set(cId, es.course?.title || cId);
      }
    });
