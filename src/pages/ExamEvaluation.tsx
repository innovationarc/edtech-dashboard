...
const StatCard: React.FC<{
  ...
  <div className="rounded-xl bg-white/3 border border-white/7 p-3.5 text-center">
...
const ExamEvaluation: React.FC = () => {
  ...
  const loadExamList = async () => {
    try {
      ...
      const writtenExams = allContent.filter(c => c.type === 'exam' && extractWrittenQs(c).length > 0);
  ...
  const openExam = async (content: Content, cId?: string) => {
    ...
    setSessFilter('all'); setSessSearch(''); setSuccessMsg('');
    try {
    ...
    try {
      ...
      setSessions(prev => prev.map(s =>
        s.id === selectedSession.id
          ? { ...s, writtenEvaluationPending: false, writtenMarks: newWritten, totalMarks: newTotal, percentage: newPct }
          : s
      ));
      await loadExamList();
  ...
  const resolveReview = async () => {
    ...
    try {
      ...
      setSelectedSession(prev => {
        ...
        return {
          ...prev,
          writtenAnswers: prev.writtenAnswers.map(wa => {
  ...
  const publishResults = async () => {
    ...
    if (!window.confirm('Publish results? Students will be able to see their scores immediately.')) return;
    setPublishing(true);
    try {
          ...
          <div className="space-y-5">
            ...
            <div className="flex items-start justify-between gap-4 flex-wrap">
              ...
              <div className="flex gap-2 flex-wrap items-center shrink-0">
                <button onClick={() => setShowStats(v => !v)}
                  className="flex items-center gap-1.5 px-3 py-2 rounded-lg bg-white/5 hover:bg-white/10 text-white/40 hover:text-white text-xs transition">
                  {showStats ? <EyeOff size={12}/> : <Eye size={12}/>} {showStats ? 'Hide' : 'Show'} Stats
                </button>
                ...
                <button onClick={publishResults} disabled={publishing}
                  className="flex items-center gap-2 px-4 py-2 rounded-lg bg-emerald-500 hover:bg-emerald-600 text-white text-sm font-medium transition disabled:opacity-50">
            ...
            {evalLoading ? (
              ...
              <div className="flex gap-4 items-start">
                ...
                {/* Session list */}
                <div className={`flex flex-col min-w-0 ${selectedSession ? 'hidden lg:flex lg:w-72 xl:w-80 shrink-0' : 'flex flex-1'}`}>
                ...
                {selectedSession && (
                  <div className="flex-1 min-w-0">
                    <div className="sticky top-4 rounded-2xl bg-[#0c0e1a] border border-white/8 overflow-hidden shadow-2xl shadow-black/40">
                      ...
                      <div className="mx-4 mt-3 flex items-center gap-2 flex-wrap">
                        ...
                        {[
                          { label: 'Full marks', t: 'full'  as const, cls: 'text-emerald-400 hover:bg-emerald-500/10 border-emerald-500/20' },
                          { label: '½ marks',   t: 'half'  as const, cls: 'text-amber-400 hover:bg-amber-500/10 border-amber-500/20' },
                      ...
                      {/* Answer cards */}
                      <div ref={panelScrollRef} className="overflow-y-auto max-h-[56vh] p-4 mt-3 space-y-3">
                      ...
                      <div className="px-5 py-4 border-t border-white/8 space-y-3 bg-white/[0.015]">
                        ...
                        {submitSuccess && (
                          <p className="text-emerald-400 text-sm flex items-center gap-1.5"><CheckCircle size={13}/> {submitSuccess}</p>
                        )}
                        {hasErrors && (
