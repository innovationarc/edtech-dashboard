// src/pages/ManageAdmin.tsx - PART 1 of 2
...
import { 
  ...
  Image as ImageIcon, Key, History
} from 'lucide-react';
import { useNavigate } from 'react-router-dom';
import { adminService, Admin, PasswordResetLog } from '../services/adminService';
...
const ManageAdmin = () => {
  ...
  const [showPasswordResetLogsModal, setShowPasswordResetLogsModal] = useState(false);
  ...
  useEffect(() => {
    if (!hasAccess) {
      setError('Access denied. Only administrators can manage admin accounts.');
      setLoading(false);
      return;
    }
    loadAdmins();
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      await adminService.deleteAdmin(adminToDelete.uid, adminToDelete.email);
  ...
  const handleShowPasswordResetLogs = (admin: Admin) => {
    setSelectedAdmin(admin);
    setShowPasswordResetLogsModal(true);
  };
          ...
          <div className="flex flex-col items-center text-center space-y-6">
            ...
            <div>
              ...
              <p className="text-gray-400 text-lg">
                Only administrators can access the admin management section.
              </p>
            </div>
            <button
              onClick={() => navigate(-1)}
    ...
    <div className="space-y-6 pb-8">
            ...
            <tbody>
              {filteredAdmins.map((admin, index) => (
                ...
                >
                  ...
                  <td className="p-4 text-gray-400 text-sm hidden xl:table-cell">{formatDate(admin.createdAt)}</td>
                  <td className="p-4">
      ...
      {showEditModal && selectedAdmin && (
        <EditAdminModal
          ...
          onSave={async (updates) => {
            try {
              await adminService.updateAdmin(selectedAdmin.uid, updates);
      ...
      {showAddAdminModal && (
        <AddAdminModal
          ...
          currentAdminId={currentUser?.userId || 'Unknown'}
        />
      )}
      ...
      {/* Password Reset Logs Modal */}
      {showPasswordResetLogsModal && selectedAdmin && (
        <PasswordResetLogsModal
          admin={selectedAdmin}
          onClose={() => {
            setShowPasswordResetLogsModal(false);
            setSelectedAdmin(null);
          }}
          formatDate={formatDate}
        />
...
// src/pages/ManageAdmin.tsx - PART 2 of 2
      ...
      <div className="p-7 text-center">
        ...
        <div className="bg-red-900/20 border border-red-700/30 rounded-xl p-4 mb-6 text-left">
          ...
          <ul className="text-red-300 text-sm space-y-1.5 ml-5">
            ...
            <li className="flex items-start gap-2">
              <span className="text-red-400 mt-0.5">•</span>
              <span>Permanently delete from Firebase Auth</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-red-400 mt-0.5">•</span>
...
const AddAdminModal = ({ onClose, onSuccess, currentAdminId }: AddAdminModalProps) => {
  const [formData, setFormData] = useState({
    surname: '',
    fullName: '',
    email: '',
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  >
                    {showPassword ? <X size={18} /> : <CheckCircle size={18} />}
                  </button>
                </div>
              </div>
              ...
              <div className="group">
                ...
                <div className="relative">
                  ...
                  <button
                    ...
                    className="absolute right-3.5 top-3.5 text-gray-400 hover:text-white"
                  >
                    {showConfirmPassword ? <X size={18} /> : <CheckCircle size={18} />}
                  </button>
                </div>
              </div>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="bg-purple-900/20 border border-purple-700/30 rounded-xl p-4">
            ...
            <p className="text-purple-200 text-sm">
              This action will be logged with your admin ID and timestamp for security auditing.
            </p>
          </div>
...
// Password Reset Logs Modal Component
interface PasswordResetLogsModalProps {
  admin: Admin;
  onClose: () => void;
  formatDate: (date: Date | undefined) => string;
}

const PasswordResetLogsModal = ({ admin, onClose, formatDate }: PasswordResetLogsModalProps) => {
  ...
  useEffect(() => {
    const loadLogs = async () => {
      try {
        setLoading(true);
        const logsData = await adminService.getPasswordResetLogs(admin.uid);
        setLogs(logsData);
      } catch (error) {
        ...
        <div className="p-5 max-h-[60vh] overflow-y-auto">
          {loading ? (
            ...
            <div className="text-center py-12">
              ...
              <h3 className="text-lg font-bold text-white mb-2">No Password Resets</h3>
              <p className="text-gray-400">This admin's password has never been reset.</p>
            </div>
          ) : (
            <div className="space-y-3">
              {logs.map((log, index) => (
                <div key={index} className="bg-background-900/40 rounded-xl p-4 border border-background-700/30">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className="h-10 w-10 rounded-lg bg-cyan-700/60 flex items-center justify-center">
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center justify-between">
                      <span className="text-gray-400">Reset By:</span>
                      <span className="text-white font-medium">{log.resetBySurname} ({log.resetByUserId})</span>
                    </div>
                    {log.reason && (
