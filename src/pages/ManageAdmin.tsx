...
import { 
  ...
  Image as ImageIcon, Key, History, Eye, EyeOff
} from 'lucide-react';
      ...
      <div className="p-7 text-center">
        ...
        <div className="bg-red-900/20 border border-red-700/30 rounded-xl p-4 mb-6 text-left">
          ...
          <ul className="text-red-300 text-sm space-y-1.5 ml-5">
            ...
            <li className="flex items-start gap-2">
              <span className="text-red-400 mt-0.5">•</span>
              <span>Delete profile picture from storage</span>
            </li>
            <li className="flex items-start gap-2">
              <span className="text-red-400 mt-0.5">•</span>
...
const EditAdminModal = ({ admin, currentAdmin, onClose, onSave }: EditAdminModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newAdmin = await adminService.createAdmin(
        ...
        formData.nid,
        currentAdmin.userId || 'Unknown',
        currentAdmin.uid,
        currentAdmin.surname || 'Unknown',
        formData.profilePictureUrl
      );
              ...
              <div className="group">
                ...
                <div className="relative">
                  <input
                    ...
                    className="w-full bg-gray-800/60 backdrop-blur-xl text-white rounded-xl py-3 pl-11 pr-11 border border-gray-700/50 focus:border-primary-500/50 focus:outline-none focus:ring-2 focus:ring-primary-500/20 transition-all duration-200"
...
const SecurityLogsModal = ({ admin, onClose, formatDateTime }: SecurityLogsModalProps) => {
  ...
  const getActionIcon = (action: string) => {
    switch (action) {
      case 'password_reset':
        return <Key size={20} className="text-purple-300" />;
      case 'admin_created':
        return <Plus size={20} className="text-green-300" />;
      case 'admin_edited':
        return <Edit size={20} className="text-blue-300" />;
      case 'admin_deleted':
        return <X size={20} className="text-red-300" />;
      default:
        return <History size={20} className="text-gray-300" />;
    }
  };
                ...
                <div key={index} className="bg-background-900/40 rounded-xl p-4 border border-background-700/30">
                  <div className="flex items-start justify-between mb-3">
                    <div className="flex items-center gap-2">
                      <div className={`h-10 w-10 rounded-lg ${getActionColor(log.action)} flex items-center justify-center`}>
                        {getActionIcon(log.action)}
                      </div>
                      <div>
                        <p className="text-white font-semibold">{getActionLabel(log.action)}</p>
                  ...
                  <div className="space-y-2 text-sm">
                    <div className="flex items-center justify-between">
                      ...
                      <span className="text-white font-medium">{log.performedBySurname} ({log.performedByUserId})</span>
                    </div>
                    {log.reason && (
