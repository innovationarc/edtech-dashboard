      ...
      if (status?.lastAttemptId) {
        ...
        if (existingSession) {
          ...
          if (existingSession.status === 'in_progress') {
            // Single-device enforcement: try to claim this session for this device
            ...
            if (vs.length > 0) {
              ...
              if (existingSession.writtenStartedAt) {
                // Compute remaining written time from DB timestamp
                const elapsedSecs = Math.floor((Date.now() - existingSession.writtenStartedAt.getTime()) / 1000);
                ...
                // Compute remaining MCQ time from DB timestamp
                const mcqStartTs = existingSession.mcqStartedAt || existingSession.startedAt;
                const elapsedSecs = Math.floor((Date.now() - mcqStartTs.getTime()) / 1000);
