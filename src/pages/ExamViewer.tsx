...
const ExamViewer: React.FC = () => {
  ...
  const startExam = async (part: ExamPart) => {
    ...
    try {
      ...
      if (content.examType !== 'written') {
        ...
        const locked = allMcq.filter(q => q.isLocked);
        const free   = shuffleArray(allMcq.filter(q => !q.isLocked));
  ...
  // ── Directions ────────────────────────────────────────────────────────────────
  if (phase === 'directions') {
    return <DirectionsPage content={content!} examStatus={examStatus} onStart={startExam} submitting={submitting} />;
  }
...
const DirectionsPage: React.FC<{
  ...
  submitting: boolean;
}> = ({ content, examStatus, onStart, submitting }) => {
  const navigate = useNavigate();
