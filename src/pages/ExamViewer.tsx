...
const ExamViewer: React.FC = () => {
  ...
  if (phase === 'scheduled_locked') {
    ...
    const [countdown, setCountdown] = useState(() => start ? Math.max(0, Math.floor((start.getTime() - Date.now()) / 1000)) : 0);
    useEffect(() => {
      if (!start) return;
      const id = setInterval(() => {
        const left = Math.max(0, Math.floor((start.getTime() - Date.now()) / 1000));
        setCountdown(left);
        if (left <= 0) { clearInterval(id); loadExam(); }
      }, 1000);
      return () => clearInterval(id);
    }, []);
    const cd_h = Math.floor(countdown / 3600);
    const cd_m = Math.floor((countdown % 3600) / 60);
    const cd_s = countdown % 60;
    return (
      <Shell><BackBtn />
        <div className="max-w-md mx-auto mt-12 rounded-2xl border border-amber-500/20 bg-amber-500/5 p-8 text-center">
          ...
          {countdown > 0 && (
            <div className="flex items-center justify-center gap-2 mt-2">
              {[{ v: cd_h, l: 'hrs' }, { v: cd_m, l: 'min' }, { v: cd_s, l: 'sec' }].map(({ v, l }) => (
  ...
  if (phase === 'blocked') {
    ...
    useEffect(() => {
      ...
      const id = setInterval(() => setPublishCountdown(Math.max(0, Math.floor((publishDate.getTime() - Date.now()) / 1000))), 1000);
      return () => clearInterval(id);
    }, []);
    const pc_h = Math.floor(publishCountdown / 3600);
    const pc_m = Math.floor((publishCountdown % 3600) / 60);
    const pc_s = publishCountdown % 60;
    return (
      <Shell><BackBtn />
        <div className="max-w-md mx-auto mt-12 rounded-2xl border border-indigo-500/20 bg-indigo-500/5 p-8 text-center">
          ...
          {publishDate && publishCountdown > 0 && (
            <div className="flex items-center justify-center gap-2 mb-5">
              {[{ v: pc_h, l: 'hrs' }, { v: pc_m, l: 'min' }, { v: pc_s, l: 'sec' }].map(({ v, l }) => (
      ...
      <Shell noPad>
        ...
        <div className="flex h-[calc(100vh-57px)]">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-8">
            {(() => {
              const goNext = () => setMcqIndex(p => Math.min(mcqQuestions.length - 1, p + 1));
              const goPrev = () => setMcqIndex(p => Math.max(0, p - 1));
              const swipe  = useSwipe(goNext, goPrev);
              return q ? (
              <div className="max-w-2xl mx-auto" {...swipe}>
                <div className="mb-6">
    ...
    return (
      <Shell noPad>
        ...
        <div className="flex h-[calc(100vh-57px)]">
          ...
          <div className="flex-1 overflow-y-auto p-4 sm:p-8">
            {(() => {
              const goNext = () => setWrittenIndex(p => Math.min(writtenQuestions.length - 1, p + 1));
              const goPrev = () => setWrittenIndex(p => Math.max(0, p - 1));
              const swipe  = useSwipe(goNext, goPrev);
              return wq ? (
              <div className="max-w-2xl mx-auto space-y-5" {...swipe}>
                <div>
  ...
  return (
    <div className="min-h-screen bg-[#080a12] text-white">
      ...
      <div className="relative max-w-4xl mx-auto px-4 py-8">
        ...
        {/* Written Review */}
        {tab === 'written' && (() => {
          const [reviewStates, setReviewStates] = React.useState<Record<string,{open:boolean;text:string;sending:boolean;sent:boolean}>>({});
          const getRS = (qid: string) => reviewStates[qid] || { open:false, text:'', sending:false, sent:false };
          const setRS = (qid: string, patch: object) => setReviewStates(p => ({ ...p, [qid]: { ...getRS(qid), ...patch } }));
          return (
          <div className="space-y-4">
