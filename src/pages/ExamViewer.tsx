...
const ExamViewer: React.FC = () => {
  ...
  // Stable per-tab device token for single-device enforcement
  const deviceTokenRef = useRef<string>(
    (() => {
      const existing = sessionStorage.getItem('examDeviceToken');
      if (existing) return existing;
      const t = `${Date.now()}-${Math.random().toString(36).slice(2)}`;
      sessionStorage.setItem('examDeviceToken', t);
      return t;
    })()
  );
