...
type Phase =
  ...
  | 'directions'
  | 'mcq'
  | 'mcq_submitted'   // BUG 2: new phase — MCQ done, waiting to start written
  | 'written'
  | 'results'
  | 'blocked'
  | 'scheduled_locked'
...
const ExamViewer: React.FC = () => {
  ...
  // BUG 2: Track when MCQ was submitted so we can enforce 10-min written start window
  ...
  // ── Timers ───────────────────────────────────────────────────────────────────
  // BUG 2: When MCQ timer expires, go to mcq_submitted instead of auto-jumping to written
  const mcqExpired = useCallback(() => {
    if (writtenQuestions.length > 0) {
      setMcqTimerOn(false);
      // Save MCQ answers then show mcq_submitted phase
      const sid = sessionIdRef.current;
      if (sid) {
        const a = mcqAnswersRef.current;
        const marks = a.reduce((s, x) => s + (x.marksAwarded ?? 0), 0);
        examService.saveMCQAnswers(sid, a, marks).catch(() => {});
        // Persist mcqSubmittedAt via service
  ...
  // BUG 2: Check if 10-minute window has elapsed since MCQ submission
  useEffect(() => {
    if (phase !== 'mcq_submitted' || !mcqSubmittedAt) return;
    const MCQ_WRITTEN_WINDOW_MS = 10 * 60 * 1000; // 10 minutes
    const checkLock = () => {
      const elapsed = Date.now() - mcqSubmittedAt.getTime();
      if (elapsed >= MCQ_WRITTEN_WINDOW_MS) {
        setWrittenLocked(true);
        clearInterval(writtenLockCheckRef.current);
      }
    };
    checkLock();
    writtenLockCheckRef.current = setInterval(checkLock, 5000);
  ...
  const determinePhase = async (
    ...
    // BUG 1 FIX: Check completed BEFORE attempt limit arithmetic.
    // Previously the attempt-limit check ran first, so a student who used their
    // last attempt (attemptCount === maxAttempts) could never reach 'results'.
    // Now we let them see results first; attempt_limit only blocks *new* attempts.

    // 2. Resume in-progress session (mcq_submitted counts as resumable)
    if (status?.lastAttemptId && (status.status === 'in_progress' || status.status === 'mcq_submitted')) {
      const sess = await examService.getExamSession(status.lastAttemptId);
      if (sess && (sess.status === 'in_progress' || sess.status === 'mcq_submitted')) {
        setSession(sess);
    ...
    // 3. Already completed → show results or blocked
    ...
    // 4. Numeric attempt limit — only blocks starting a NEW attempt
  ...
  const restoreSession = (c: Content, sess: ExamSession, versionList: ExamVersion[]) => {
    ...
    // BUG 2: If session status is mcq_submitted, restore that phase
    if (sess.status === 'mcq_submitted') {
      const submittedAt = sess.mcqSubmittedAt ? new Date(sess.mcqSubmittedAt) : new Date();
      setMcqSubmittedAt(submittedAt);
      // Check if 10-min window already elapsed
      const elapsed = Date.now() - submittedAt.getTime();
      setWrittenLocked(elapsed >= 10 * 60 * 1000);
      setPhase('mcq_submitted');
      return;
    }
  ...
  // BUG 2: Submit MCQ Part — saves answers, records mcqSubmittedAt, transitions to mcq_submitted
  ...
  // BUG 2: Start Written Part — called from mcq_submitted phase
  ...
  // BUG 4: Re-attempt — reset state and go back to directions
  const handleReattempt = async () => {
    ...
    // Refresh status then go to directions
    const status = await examService.getStudentExamStatus(contentId!, user.uid, courseId);
    setExamStatus(status);
  ...
  if (phase === 'directions') {
    ...
    // BUG 2: mixed exam = has both MCQ and written
    ...
    return (
      <div className="min-h-screen bg-[#080a12] text-white">
        ...
        <div className="relative max-w-3xl mx-auto px-4 py-8">
          ...
          {/* BUG 2: Show two-step notice for mixed exams */}
          {isMixed && (
            <div className="rounded-2xl bg-blue-500/8 border border-blue-500/15 p-5 mb-8">
              <p className="text-[11px] uppercase tracking-widest text-blue-400 font-semibold mb-2">Two-Part Exam</p>
              <p className="text-white/60 text-sm leading-relaxed">
                This exam has two separate parts. You'll start with <strong className="text-rose-300">MCQ</strong>, submit it,
                then start the <strong className="text-violet-300">Written</strong> section.
                The written section must be started within <strong className="text-amber-300">10 minutes</strong> of submitting MCQ.
              </p>
            </div>
          )}
          ...
          {/* BUG 2: Show separate Start MCQ / Start Written buttons for mixed exams.
               For single-part exams, show the original single Start button. */}
          {isMixed ? (
    ...
    return (
      <Shell noPad>
        ...
        <div className="sticky top-0 z-30 flex items-center gap-3 px-4 py-3 bg-[#080a12]/95 backdrop-blur border-b border-white/6">
          ...
          {/* BUG 2: For mixed exams use "Submit MCQ" not a written shortcut */}
          {hasBothParts ? (
              ...
              <div className="max-w-2xl mx-auto">
                ...
                <div className="flex items-center justify-between pt-2 border-t border-white/6">
                  ...
                  {mcqIndex < mcqQuestions.length - 1 ? (
                    ...
                    // BUG 2: Last MCQ question → "Submit MCQ" not "Written →"
                    <button onClick={submitMCQPart} disabled={submitting}
  ...
  // ── MCQ Submitted Phase (BUG 2) ───────────────────────────────────────────────
  if (phase === 'mcq_submitted') {
    const windowMs = 10 * 60 * 1000;
    const elapsed  = mcqSubmittedAt ? Date.now() - mcqSubmittedAt.getTime() : windowMs;
  ...
  if (phase === 'results' && session) {
    // BUG 4: Calculate if re-attempt is allowed
    const maxAtt      = content?.maxAttempts;
    const attemptCount = examStatus?.attemptCount ?? 0;
...
const ResultsPage: React.FC<{
  ...
  canReattempt: boolean;          // BUG 4
  onReattempt: () => void;        // BUG 4
  onBack: () => void;
