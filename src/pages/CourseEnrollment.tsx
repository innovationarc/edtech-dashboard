...
// Course Enrollment with Complete Error Handling and User Feedback - FIXED HTTP 405
...
const CourseEnrollment = () => {
  ...
  useEffect(() => {
    ...
    const processPaymentCallback = async () => {
      ...
      if (status && transactionId) {
        console.log('Payment callback detected:', { status, transactionId });
        ...
        // Handle the callback
        await handlePaymentCallback(status, transactionId);
      } else if (user?.uid) {
        // Only load courses if this is not a payment callback
        loadCourses();
      }
    };
  ...
  const handlePaymentCallback = async (status: string, transactionId: string) => {
    try {
      ...
      if (status === 'success') {
        console.log('✅ Payment status is SUCCESS, validating...');
        
        // Show validating message
        setWarning('⏳ Validating your payment, please wait...');

        // Add delay to allow IPN to process
        ...
        try {
          ...
          if (validation.success && validation.validated) {
            ...
            // Load courses to show the enrolled course
            await loadCourses();
          ...
          } else if (validation.success && !validation.validated && validation.status === 'pending') {
            console.log('⏳ Payment is still pending validation');
            setWarning('⏳ Your payment is being processed. This may take a few moments. Please check back in a few minutes or contact support if the issue persists.');
            await loadCourses();
          } else {
            console.warn('⚠️ Payment validation failed:', validation.error);
            setWarning('⚠️ Payment validation is pending. Please contact support if you were charged. Transaction ID: ' + transactionId);
            await loadCourses();
          }
        } catch (validationError: any) {
          console.error('❌ Payment validation error:', validationError);
          setError('❌ Unable to verify payment status. Please contact support with Transaction ID: ' + transactionId);
          await loadCourses();
        }
      } else if (status === 'failed') {
        console.log('❌ Payment failed');
        setError('❌ Payment failed. Please try again or contact support if you were charged.');
        sessionStorage.removeItem('pendingEnrollment');
        await loadCourses();
        ...
        setWarning('⚠️ Unknown payment status. Please contact support if you were charged.');
        await loadCourses();
      }
    } catch (error: any) {
      console.error('');
      ...
      setError('❌ Error processing payment. Please contact support if you were charged. Transaction ID: ' + transactionId);
      await loadCourses();
    } finally {
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      console.log('Gateway URL:', enrollmentResponse.gatewayUrl);
      console.log('Transaction ID:', enrollmentResponse.transactionId);
      console.log('Error:', enrollmentResponse.error);
      ...
      if (enrollmentResponse.success && enrollmentResponse.gatewayUrl) {
        ...
        console.error('❌ Payment initiation failed:', enrollmentResponse.error);
  ...
  // UI Components with Error Handling and User Feedback - FIXED HTTP 405
      ...
      <div className="flex items-center justify-center h-64">
        <div className="flex flex-col items-center gap-4">
          ...
          <p className="text-gray-400">
            {validatingPayment ? 'Validating your payment...' : 'Loading courses...'}
          </p>
        </div>
      </div>
    );
      ...
      {loading ? (
        <div className="flex items-center justify-center py-12">
          <div className="flex flex-col items-center gap-4">
            ...
            <p className="text-gray-400">
              {validatingPayment ? 'Validating payment...' : 'Loading...'}
            </p>
          </div>
        </div>
      ) : (
