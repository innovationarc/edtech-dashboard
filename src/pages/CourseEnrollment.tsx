  ...
  useEffect(() => {
    ...
    const handleReturn = async () => {
      ...
      try {
        ...
        // Note: status 'pending' is OK here -- the IPN may not have fired yet.
        ...
        if (enrollmentFound) {
          // Enrollment confirmed in Firestore -- reload courses and show success
          await loadCourses();
          ...
          setPaymentReturn({
            ...
            status: 'success',
            courseTitle,
            message: `Payment successful! You are now enrolled in "${courseTitle || 'the course'}".`
          });
        } else {
          // The IPN will still fire and create the enrollment -- tell user to refresh
          console.warn('Enrollment not found after polling -- IPN may be delayed');
          try { await loadCourses(); setActiveTab('enrolled'); } catch (_) {}
          setPaymentReturn({
            active: true,
            status: 'success',
            courseTitle,
