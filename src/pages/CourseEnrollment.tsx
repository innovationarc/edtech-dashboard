...
// BUG FIX: Reads tran_id from SSLCommerz return URL, validates payment,
//           then calls courseService.completeEnrollmentAfterPayment() to
//           write the enrollment document — which was the missing step.
...
const CourseEnrollment = () => {
  ...
  // Prevents double-processing the payment return (React StrictMode mounts twice)
  ...
  // Every success path verifies ALL of these in Firestore:
  //   1. transaction exists by tran_id
  //   2. transaction.userId === user.uid  <- ownership check (blocks URL replay attacks)
  //   3. transaction.status checked
  //   4. enrollment doc found with retry/backoff
  //
  // An attacker replaying another user's payment URL fails at step 2.
  ...
  useEffect(() => {
    ...
    // Nothing payment-related -- normal page load, do nothing
    if (!tranId && !statusParam && !errorParam) return;

    paymentHandledRef.current = true;

    // Strip ALL payment params from the URL immediately:
    //   - Prevents re-triggering on page refresh
    //   - Prevents URL being bookmarked/shared as fake proof of payment
    ...
    const handleReturn = async () => {
      ...
      try {
        ...
        // Step 1: Fetch transaction from Firestore by tran_id
        ...
        // Step 2: OWNERSHIP CHECK -- this is the replay-attack security gate.
        // transaction.userId MUST equal the currently logged-in user's uid.
        // If someone copies another user's payment URL, this check blocks them.
        ...
        if (txn.status === 'failed' || txn.status === 'cancelled') {
          setPaymentReturn({ active: true, status: 'failed', courseTitle: '',
            message: `Payment was ${txn.status}. Please try again or contact support.` });
          return;
        }
        if (txn.status === 'validating') {
          setPaymentReturn({ active: true, status: 'processing', courseTitle: '',
            message: 'Your payment is under manual review. You will be enrolled once approved.' });
          return;
        }
        // If transaction is already 'success', this URL was already processed.
        // We still proceed to show the enrollment, but flag it as a replay
        // so we can show "already enrolled" rather than "payment successful".
        const isReplay = txn.status === 'success';
        // Note: status 'pending' is OK -- IPN may not have fired yet.
        // We proceed to poll for the enrollment document.
        ...
        // Step 4: Poll Firestore for the enrollment document.
        // The backend creates it in the callback handler, but Firestore
        // propagation can take a few seconds. We retry with backoff.
        let enrollmentFound = false;
        const delays = [600, 1000, 1500, 1500, 2000, 2000, 2500]; // ~11s total
        ...
        for (let i = 0; i <= delays.length; i++) {
          ...
          try {
            // Primary check: by transactionId (most specific -- ties enrollment to payment)
            const snap1 = await getDocs(query(
              collection(db, 'enrollments'),
              where('transactionId', '==', tranId)
            ));
            if (!snap1.empty) { enrollmentFound = true; break; }

            // Secondary check: by studentId + courseId
            if (courseId) {
              ...
              if (!snap2.empty) { enrollmentFound = true; break; }
            }

            console.log(`Enrollment not yet propagated to Firestore, retry ${i + 1}/${delays.length + 1}`);
          } catch (pollErr: any) {
        ...
        if (enrollmentFound) {
          // Enrollment confirmed in Firestore.
          // Load courses first, then use the returned data to filter immediately
          // so the enrolled tab is populated before we switch to it.
          const freshCourses = await loadCourses();
          // Apply filtering directly with the fresh data and target tab
          // to avoid the stale-closure bug where filterCourses sees old state.
          filterCourses(freshCourses, 'enrolled');
          ...
          setPaymentReturn({
            ...
            courseTitle,
            message: isReplay
              ? `You are already enrolled in "${courseTitle || 'this course'}". Welcome back!`
              : `Payment successful! You are now enrolled in "${courseTitle || 'the course'}".`
          });
        } else {
          // IPN will still fire and create the enrollment shortly
          console.warn('Enrollment not found after polling -- IPN may be delayed');
          try {
            const freshCourses = await loadCourses();
            filterCourses(freshCourses, 'enrolled');
            setActiveTab('enrolled');
          ...
          setPaymentReturn({
            ...
            status: 'success',
            courseTitle,
            message: 'Payment confirmed! Your enrollment is being activated. If the course does not appear below, please refresh the page in a moment.'
          });
        }
        ...
        setPaymentReturn({ active: true, status: 'failed', courseTitle: '',
          message: `An unexpected error occurred. Your payment may still have been processed -- please contact support with ref: ${tranId}` });
      }
    };
  ...
  // Returns the enriched courses so callers can use them synchronously
  // without waiting for React state to propagate.
  const loadCourses = async (): Promise<EnrichedCourse[]> => {
    try {
      setLoading(true);
      setError('');
  ...
  const filterCourses = (
    overrideCourses?: EnrichedCourse[],
    overrideTab?: 'available' | 'enrolled'
  ) => {
    const courses = overrideCourses ?? allCourses;
  ...
  const handleProceedToPayment = async () => {
    ...
    try {
      ...
      if (enrollmentResponse.success && enrollmentResponse.gatewayUrl) {
        ...
        // Redirect to gateway. SSLCommerz will redirect back to this page
        // with ?tran_id=TXN_xxx appended, which the useEffect above will handle.
