...
const CourseEnrollment = () => {
  ...
  const loadCourses = useCallback(async (
    ...
    try {
      ...
      // Sync live class attendance → content progress → enrollment.progress for each course
      // Run in background but await before reading progress values below
      if (user?.uid) {
        await Promise.all(
          enrollments
      ...
      // Re-fetch enrollments to get updated progress after sync
      const freshEnrollments = await courseEnrollmentService.getStudentEnrollments(user?.uid || '');

      const enrolledCourseIds = new Set(freshEnrollments.map(e => e.courseId));
      const enrollmentMap = new Map<string, { progress: number; enrollmentId: string }>();
      freshEnrollments.forEach(e => {
...
const CEStyles = () => (
  <style>{`
    ...
    .ce-card-fallback{position:absolute;inset:0;width:100%;height:100%;display:flex;align-items:center;justify-content:center;background:linear-gradient(135deg,#181f2e,#0e1319);color:var(--ce-tx3)}
