...
const CourseEnrollment = () => {
  ...
  // SSLCommerz posts a form to your backend success/fail/cancel URLs.
  // Your backend (api/payment.ts) updates the transaction status to 'success'
  // and then redirects the browser to e.g.:
  //   https://yourapp.com/course-enrollment?tran_id=TXN_xxx
  //
  // This effect detects that tran_id query param, calls completeEnrollmentAfterPayment,
  // and creates the enrollment document — the step that was missing before.
  ...
  useEffect(() => {
    ...
    // SSLCommerz appends tran_id after the callback handler processes payment.
    // Support the different param names used in various gateway configs.
    const tranId =
      params.get('tran_id') ||
      params.get('tranId') ||
    ...
    // The backend callback sets enrolled=true on the success redirect so we
    // know immediately that the enrollment was already created server-side.
    const enrolledParam = params.get('enrolled');
    // Status set by backend on cancel/fail/validation_error
    const statusParam = params.get('status');
    const errorParam = params.get('error');

    if (!tranId && !errorParam) return; // Normal page load — no payment return

    paymentHandledRef.current = true;

    // Strip the query string from the URL immediately so a browser refresh
    // doesn't re-trigger this handler
    window.history.replaceState({}, '', window.location.pathname);

    const handleReturn = async () => {
      console.log('💳 Payment return detected.', { tranId, enrolledParam, statusParam });

      // ── Error / no tranId ────────────────────────────────────────────────
      if (errorParam || !tranId) {
        setPaymentReturn({
          active: true,
          status: 'failed',
      ...
      // ── Failed / validation_error / unknown ──────────────────────────────
      if (statusParam === 'failed' || statusParam === 'validation_error' || statusParam === 'unknown') {
        setPaymentReturn({
          active: true,
          status: 'failed',
          message: `Payment failed. Please try again or contact support. Ref: ${tranId}`,
          courseTitle: '',
        });
        return;
      }

      // ── Validating (high-risk, pending manual review) ────────────────────
      if (statusParam === 'validating') {
        setPaymentReturn({
          active: true,
          status: 'processing',
          message: 'Your payment is under review. You will be enrolled once it is approved.',
          courseTitle: '',
        });
        return;
      }
      ...
      // ── Success path ─────────────────────────────────────────────────────
      // enrolled=true means the backend callback ALREADY created the enrollment
      // in Firestore AND validated with SSLCommerz. We do NOT need to call the
      // validate API again — that's what was causing the 404 error.
      // Just reload the course list from Firestore directly.

      if (enrolledParam === 'true') {
        setPaymentReturn({
          active: true,
          status: 'processing',
          message: 'Loading your enrolled course…',
          courseTitle: '',
        });

        try {
          // Reload courses — the enrollment doc is already in Firestore
          await loadCourses();
          setActiveTab('enrolled');
          setPaymentReturn({
            active: true,
            status: 'success',
            message: '🎉 Payment successful! Your course is now active.',
            courseTitle: '',
          });
        } catch (err: any) {
          console.error('Failed to reload courses after payment:', err);
          // Even if reload fails, payment succeeded — show success
          setPaymentReturn({
            active: true,
            status: 'success',
            message: '🎉 Payment successful! Please refresh the page to see your enrolled course.',
            courseTitle: '',
          });
        }
        return;
      ...
      // ── Fallback: tran_id present but enrolled param not set ─────────────
      // This shouldn't happen with the current backend but handles edge cases
      // (e.g. user bookmarked the return URL, or IPN fired before callback).
      setPaymentReturn({
        active: true,
        status: 'processing',
      ...
      try {
        const result = await courseService.completeEnrollmentAfterPayment(tranId);

        if (result.success) {
          const title = result.courseTitle || 'the course';
          setPaymentReturn({
            active: true,
            status: 'success',
            message: `🎉 Payment successful! You are now enrolled in "${title}".`,
            courseTitle: title,
          });
          ...
          setPaymentReturn({
            ...
            message: result.error ||
              `Payment verification failed. Please contact support. Ref: ${tranId}`,
            courseTitle: '',
          });
        }
      } catch (err: any) {
        console.error('Payment return handler threw:', err);
        setPaymentReturn({
          active: true,
          status: 'failed',
          message: `An unexpected error occurred. Please contact support. Ref: ${tranId}`,
          courseTitle: '',
        });
      }
    };
    ...
    handleReturn();
    // loadCourses is defined below; we only want this to run once after user loads
