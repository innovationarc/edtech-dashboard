...
// Course Enrollment - UPDATED with payment callback handling
...
const CourseEnrollment = () => {
  ...
  useEffect(() => {
    // CRITICAL: Check if SSLCOMMERZ redirected here with payment status
    const status = searchParams.get('status');
    const tranId = searchParams.get('tran_id');
    
    if (status && tranId) {
      console.log('⚠️ SSLCOMMERZ redirected to wrong page!');
      console.log('Redirecting to correct payment-success page...');
      console.log('Status:', status, 'Transaction:', tranId);
      
      // Immediately redirect to the correct page
      navigate(`/payment-success?status=${status}&tran_id=${tranId}`, { replace: true });
      return; // Stop further processing
    }

    // Check for enrollment success flag (from payment-success page)
