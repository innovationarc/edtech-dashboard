...
const CourseEnrollment = () => {
  ...
  const loadCourses = useCallback(async (
    opts?: {
      ...
      // If provided, this courseId is force-marked as enrolled in the result
      // even if Firestore hasn't propagated the write yet (read-after-write lag).
      // This is the definitive fix for "just-paid course shows as unenrolled".
  ...
  courseEnrollmentService.getStudentEnrollments(user?.uid || ''),
      ...
      // Compute filtered lists for BOTH tabs atomically from this single snapshot.
      ...
      // All state updates in one batch — eliminates React re-render races.
      ...
      // Switch tab AFTER data is ready
      if (targetTab) {
  ...
  useEffect(() => {
    ...
    const handleReturn = async () => {
      ...
      await loadCourses({
        targetTab: 'enrolled',
        guaranteedEnrolledCourseId: result.courseId || '',
      });
  ...
  calculation,
      ...
      if (enrollmentResponse.success) {
        ...
        resetCouponInput();
        // Load and switch atomically. Inject courseId as guaranteed enrolled
        // in case Firestore propagation hasn't caught up with the write.
