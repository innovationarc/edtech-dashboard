      ...
      <div className="w-full max-w-xl bg-gray-900 border border-gray-700 rounded-2xl shadow-2xl flex flex-col max-h-[95vh]">
        ...
        <div className="overflow-y-auto flex-1 p-5 space-y-5">
          ...
          <div>
            ...
            {/* Read-only content info: exam timeline + attempt limit */}
            {selectedContent && (
              <div className="mt-2 grid grid-cols-2 gap-2">
                <div className="flex flex-col gap-1 p-3 bg-gray-800/80 border border-gray-700/60 rounded-xl">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wide">Exam Timeline</span>
                  {selectedContent.examTimelineType === 'anytime' ? (
                    ...
                    <div className="space-y-0.5">
                      ...
                      {selectedContent.examStartDateTime && (
                        <p className="text-xs text-gray-400">{fmtDate(selectedContent.examStartDateTime)}</p>
                      )}
                      {selectedContent.examEndDateTime && (
                        <p className="text-xs text-gray-500">→ {fmtDate(selectedContent.examEndDateTime)}</p>
                      )}
                    </div>
                  )}
                </div>
                <div className="flex flex-col gap-1 p-3 bg-gray-800/80 border border-gray-700/60 rounded-xl">
                  <span className="text-xs text-gray-500 font-medium uppercase tracking-wide">Attempt Limit</span>
                  <span className="flex items-center gap-1.5 text-sm font-semibold text-indigo-300">
                    <Target size={13} />
                    {selectedContent.maxAttempts === 'unlimited' ? '∞ Unlimited' : `${selectedContent.maxAttempts ?? 1}×`}
                  </span>
                </div>
              </div>
            )}
          ...
          {/* Live Window — when this exam is visible on the student live exam page */}
          <div>
            <label className="block text-sm font-medium text-gray-300 mb-1">
              Live Window
            </label>
            <p className="text-xs text-gray-500 mb-2.5">
              Controls when this exam appears on the student's live exam page. Independent of the exam's own timeline.
            </p>
            <div className="grid grid-cols-2 gap-2 mb-3">
              {(['anytime', 'scheduled'] as const).map((opt) => (
                <button
                  key={opt}
                  type="button"
                  onClick={() => setForm((p) => ({ ...p, liveWindowType: opt }))}
                  className={`flex items-center gap-2.5 px-4 py-3 rounded-xl border text-sm font-medium transition-all ${
                    form.liveWindowType === opt
            ...
            {form.liveWindowType === 'scheduled' && (
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Show from</label>
                  <input
                    type="datetime-local"
                    value={form.liveWindowStart}
                    onChange={(e) => setForm((p) => ({ ...p, liveWindowStart: e.target.value }))}
                ...
                <div>
                  <label className="block text-xs text-gray-400 mb-1">Hide after</label>
                  <input
                    type="datetime-local"
                    value={form.liveWindowEnd}
                    onChange={(e) => setForm((p) => ({ ...p, liveWindowEnd: e.target.value }))}
                    className="w-full px-3 py-2 bg-gray-800 border border-gray-700 rounded-xl text-sm text-white focus:outline-none focus:border-indigo-500 transition-colors"
                  />
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
            {filtered.map((exam) =>
              isStaff ? (
                ...
                <div
                  key={exam.id}
                  ref={exam.id === highlightId ? highlightRef : undefined}
                  className={exam.id === highlightId ? 'ring-2 ring-amber-400/70 ring-offset-2 ring-offset-gray-950 rounded-xl' : ''}
                >
                  <StudentExamCard
                    exam={exam}
