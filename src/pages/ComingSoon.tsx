  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      notificationService.createNotification({
        ...
        message: "Your feature request has been received. We'll review it and get back to you.",
        type: 'system',
        priority: 'low',
        isPermanent: false,
...
const ComingSoon = () => {
  ...
  const handleRequestAccess = async (feature: ComingSoonFeature) => {
    ...
    try {
      ...
      notificationService.createNotification({
        ...
        message: `Your request for early access to "${feature.title}" has been submitted and is under review.`,
        type: 'system',
        priority: 'low',
        isPermanent: false,
        relatedId: feature.id,
        relatedType: 'earlyAccess',
        metadata: { featureTitle: feature.title },
      });
  ...
  const handleReRequestAccess = async (feature: ComingSoonFeature, oldRequestId: string) => {
    ...
    try {
      ...
      notificationService.createNotification({
        userId: user.uid,
        title: 'Early Access Re-Requested',
        message: `Your re-request for early access to "${feature.title}" has been submitted.`,
        type: 'system',
        priority: 'low',
        isPermanent: false,
        relatedId: feature.id,
  ...
  const handleCancelEarlyAccess = async (featureId: string, requestId: string) => {
    ...
    try {
      ...
      if (user) {
        notificationService.createNotification({
          userId: user.uid,
          title: 'Early Access Request Cancelled',
          message: 'Your early access request has been cancelled.',
          type: 'system',
          priority: 'low',
        isPermanent: false,
          relatedType: 'earlyAccess',
