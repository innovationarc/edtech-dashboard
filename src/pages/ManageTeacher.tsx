// === PART 1 OF 4 - ManageTeacher.tsx (FULLY FIXED) ===
//
// 🐛 ALL 5 BUGS FIXED:
// 1. <Modal → <AllTeacherLogsModal (was rendering wrong component)
// 2. getAllSecurityLogs() → getAllTeacherSecurityLogs()
// 3. resetTeacherPassword() → resetPassword()
// 4. PasswordResetModal onSuccess signature (removed message parameter, added onError)
// 5. Removed embedded part markers that broke syntax
//
// 📝 INSTRUCTIONS:
// Paste Part 1, then Part 2, then Part 3, then Part 4 immediately one after another
// Each part is clean and syntactically correct
//
...
const ManageTeacher = () => {
  ...
  return (
    <div className="p-6 pb-24">
      ...
      // === PART 2 OF 4 - ManageTeacher.tsx (FULLY FIXED) ===
// Paste immediately after Part 1
      ...
      {showPasswordResetModal && teacherToResetPassword && currentUser && (
        <PasswordResetModal
          ...
          onSuccess={() => {
            setSuccessMessage(`Password reset successfully for ${teacherToResetPassword.surname}`);
            setShowPasswordResetModal(false);
            setTeacherToResetPassword(null);
          }}
          onError={(errorMsg) => {
...
// === PART 3 OF 4 - ManageTeacher.tsx (FULLY FIXED) ===
...
// === PART 4 OF 4 - ManageTeacher.tsx (FULLY FIXED) ===
...
const PasswordResetModal = ({ teacher, currentUser, onClose, onSuccess, onError }: PasswordResetModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await teacherService.resetPassword(teacher.uid, newPassword, reason, currentUser);
      onSuccess();
    } catch (error: any) {
...
const AllTeacherLogsModal = ({ onClose, formatDate }: AllTeacherLogsModalProps) => {
  ...
  const loadAllLogs = async () => {
    try {
      setLoading(true);
      const allLogs = await teacherService.getAllTeacherSecurityLogs();
      setLogs(allLogs);
    } catch (error) {
      console.error('Error loading security logs:', error);
    } finally {
