    ...
    <>
      ...
      <div style={{ minHeight: '100vh', background: baseBg, color: T.text, fontFamily: "'Outfit',sans-serif", userSelect: 'none' }} onContextMenu={e => e.preventDefault()}>
        ...
        <div style={{ position: 'relative', maxWidth: 1024, margin: '0 auto', padding: '0 16px 56px', zIndex: 1 }}>
                  ...
                  <div className={`ctrl-wrap absolute inset-x-0 bottom-0 ${ctrlsHidden ? 'hide' : ''}`}>
                    ...
                    <div className="relative px-3 pb-3 pt-10">
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        {/* Secure badge */}
                        <div className="hidden sm:flex" style={{ alignItems: 'center', gap: 4, color: 'rgba(74,222,128,0.35)', marginRight: 6, flexShrink: 0 }}>
                          <Shield size={10} /><span style={{ fontSize: 10 }}>Secure</span>
                        </div>
          ...
          {/* ── Content metadata ── */}
          <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16 }} className="au2 lg:grid-cols-3-meta">
            <style>{`
              @media(min-width:1024px){
                .lg-meta-grid { grid-template-columns: 1fr 1fr 1fr; }
                .lg-meta-main { grid-column: span 2; }
              }
              @media(min-width:640px){
                .sm-stat-grid { grid-template-columns: repeat(3,1fr); }
              }
              .lv-note-btn { display:flex; align-items:center; justify-content:space-between; gap:8px; padding:12px 16px; border-radius:14px; font-size:13px; font-weight:500; text-decoration:none; transition:background .15s, color .15s; }
              .lv-note-btn-primary { background:rgba(52,211,153,0.08); border:1px solid rgba(52,211,153,0.18); color:rgba(110,231,183,0.85); }
              .lv-note-btn-primary:hover { background:rgba(52,211,153,0.14); color:rgb(110,231,183); }
              .lv-note-btn-secondary { background:${T.surface}; border:1px solid ${T.border}; color:${T.text2}; }
              .lv-note-btn-secondary:hover { background:${isLight ? 'rgba(0,0,0,0.07)' : 'rgba(255,255,255,0.09)'}; color:${T.text}; }
            `}</style>

            <div style={{ display: 'contents' }}>
              {/* Left col spanning 2 — title card + stat chips */}
              <div className="lg-meta-main" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>

                {/* Title card */}
                <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 20, padding: '20px 24px', boxShadow: isLight ? '0 2px 16px rgba(0,0,0,0.06)' : 'none' }}>
                  {/* Badges row */}
                  <div style={{ display: 'flex', alignItems: 'center', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
                    <span style={{
                      display: 'inline-flex', alignItems: 'center', gap: 5,
                      padding: '4px 10px', borderRadius: 99, fontSize: 11, fontWeight: 600,
                      background: content?.type === 'lesson' ? `rgba(${pRgb},0.15)` : 'rgba(245,158,11,0.15)',
                      color: content?.type === 'lesson' ? primaryColor : '#fbbf24',
                      border: `1px solid ${content?.type === 'lesson' ? `rgba(${pRgb},0.25)` : 'rgba(245,158,11,0.25)'}`,
                    }}>
                      {content?.type === 'lesson' ? <Play size={10} /> : <BookOpen size={10} />}
                      {content?.type === 'lesson' ? 'Lesson' : 'Trick'}
                    </span>
                    {content?.videoUrl?.startsWith('secured://') && (
                      <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '3px 9px', borderRadius: 99, fontSize: 10, fontWeight: 600, background: 'rgba(34,197,94,0.1)', color: 'rgba(74,222,128,0.85)', border: '1px solid rgba(34,197,94,0.18)' }}>
                        <Shield size={9} /> Protected
                      </span>
                    )}
                  </div>

                  <h1 style={{ color: T.text, fontWeight: 800, fontSize: 'clamp(18px,3vw,26px)', lineHeight: 1.25, margin: '0 0 6px', letterSpacing: '-0.02em' }}>
                    {content?.title || 'Untitled'}
                  </h1>
                  {content?.subject && (
                    <p style={{ color: T.text3, fontSize: 13, margin: 0 }}>{content.subject}</p>
                  )}
                  {content?.description && (
                ...
                {/* Stat chips */}
                <div className="sm-stat-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 10 }}>
                  {content?.duration && (
                    <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div>
                        <p style={{ fontSize: 10, color: T.text3, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700, margin: '0 0 2px' }}>Duration</p>
                        <p style={{ fontSize: 14, color: T.text, fontWeight: 600, margin: 0 }}>{fmtMinutes(content.duration)}</p>
                      </div>
                    </div>
                  )}
                  {content?.subject && (
                    <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ minWidth: 0 }}>
                        <p style={{ fontSize: 10, color: T.text3, textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700, margin: '0 0 2px' }}>Subject</p>
                        <p style={{ fontSize: 14, color: T.text, fontWeight: 600, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{content.subject}</p>
                      </div>
                    </div>
                  )}
                  {content?.videoUrl?.startsWith('secured://') && (
                    <div style={{ background: 'rgba(34,197,94,0.06)', border: '1px solid rgba(34,197,94,0.16)', borderRadius: 14, padding: '12px 16px', display: 'flex', alignItems: 'center', gap: 12 }}>
                      <div style={{ width: 34, height: 34, borderRadius: 10, background: 'rgba(34,197,94,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Shield size={15} style={{ color: '#4ade80' }} />
                      </div>
                      <div>
                        <p style={{ fontSize: 10, color: 'rgba(74,222,128,0.5)', textTransform: 'uppercase', letterSpacing: '0.08em', fontWeight: 700, margin: '0 0 2px' }}>Security</p>
                        <p style={{ fontSize: 14, color: 'rgba(74,222,128,0.8)', fontWeight: 600, margin: 0 }}>Protected</p>
                      </div>
                    </div>
                  )}
                </div>
              ...
              {/* Right col — Notes + protection */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>

                {/* Notes card */}
                {(content?.noteUrl || content?.noteGDrivePreviewUrl) ? (
                  <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 20, padding: '18px 20px' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 14 }}>
                      <div style={{ width: 30, height: 30, borderRadius: 8, background: 'rgba(52,211,153,0.12)', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {getNotePreview() && (
                        <a href={getNotePreview()!} target="_blank" rel="noopener noreferrer" className="lv-note-btn lv-note-btn-primary">
                          <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}><ExternalLink size={13} /> Preview Notes</span>
                          <span style={{ fontSize: 12, opacity: 0.7 }}>→</span>
                        </a>
                      )}
                      {getNoteHref() && (
                        <a href={getNoteHref()!} target="_blank" rel="noopener noreferrer" className="lv-note-btn lv-note-btn-secondary">
                          <span style={{ display: 'flex', alignItems: 'center', gap: 7 }}><Download size={13} /> Download PDF</span>
                          <span style={{ fontSize: 12, opacity: 0.5 }}>→</span>
                        </a>
                      )}
                      {content?.noteSource === 'gdrive' && (
                        <p style={{ fontSize: 10, color: T.text3, textAlign: 'center', paddingTop: 4, margin: 0 }}>via Google Drive</p>
                      )}
                    </div>
                  </div>
                ) : (
                  <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 20, padding: '20px', textAlign: 'center' }}>
                    <FileText size={22} style={{ color: T.text3, margin: '0 auto 8px', opacity: 0.4 }} />
                    <p style={{ fontSize: 12, color: T.text3, margin: 0 }}>No notes attached</p>
                  </div>
                )}

                {/* Content Protection card */}
                <div style={{ background: T.cardBg, border: `1px solid ${T.border}`, borderRadius: 20, padding: '16px 18px' }}>
                  <p style={{ fontSize: 10, textTransform: 'uppercase', letterSpacing: '0.08em', color: T.text3, fontWeight: 700, margin: '0 0 12px' }}>Content Protection</p>
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                    {['SW-signature anti-IDM', 'Signed stream token', 'Source URL never exposed', 'Screen capture blocked'].map(f => (
                      <div key={f} style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                        <div style={{ width: 20, height: 20, borderRadius: 6, background: 'rgba(34,197,94,0.1)', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          <Shield size={10} style={{ color: 'rgba(74,222,128,0.6)' }} />
                        </div>
                        <span style={{ fontSize: 12, color: T.text3 }}>{f}</span>
                      </div>
                    ))}
                  </div>
                </div>
          ...
          {/* Responsive layout style for metadata grid */}
          <style>{`
            @media(min-width:1024px){
              .au2 > div { display:grid !important; grid-template-columns:1fr 1fr 1fr; gap:16px; }
              .lg-meta-main { grid-column: span 2; }
            }
          `}</style>
