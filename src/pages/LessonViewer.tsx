// src/pages/LessonViewer.tsx — v12: Zero-lag progress bar
//
// FIXES vs v10:
...
//
//   2. Position-preserving manual retry
//      handleRetry now saves the current playhead position and restores it
//      after the new token is issued. No more restarting from 0:00.
//
...
const LessonViewer: React.FC = () => {
  ...
  // isDragging & dragVisualPct replaced by refs for zero-lag DOM updates
  const [isDragging,    setIsDragging]    = useState(false);  // only for CSS class & ctrlsHidden
  ...
  // Direct DOM refs for zero-lag drag updates (bypass React render cycle)
  const prgFillRef    = useRef<HTMLDivElement>(null);
  const prgDotRef     = useRef<HTMLDivElement>(null);
  const prgTipRef     = useRef<HTMLDivElement>(null);
  const prgWrapRef    = useRef<HTMLDivElement>(null); // the .prg element
  const isDraggingRef = useRef(false);               // track drag without setState
  ...
  const onTimeUpdate = () => {
    ...
    // Keep DOM fill in sync during normal playback (bypasses React render for perf)
    if (!isDraggingRef.current && v.duration && prgFillRef.current && prgDotRef.current) {
      const pctPx = `${(ct / v.duration) * 100}%`;
      prgFillRef.current.style.width = pctPx;
      prgDotRef.current.style.left   = pctPx;
    }
  };
  ...
  // FIX: save position so retry resumes where you left off
  const handleRetry = () => {
  ...
  // ── Progress bar — zero-lag DOM-direct drag ─────────────────────────────────
  //
  // During drag: write directly to DOM via refs (no React state, no re-render).
  //   prgFillRef.current.style.width  — the purple fill bar
  //   prgDotRef.current.style.left    — the thumb dot
  //   prgTipRef.current.style.left    — the hover time tooltip
  // On mouseup/touchend: one commitSeek() call, then one setCurrentTime().
  // This gives 60fps drag with zero React overhead — same architecture as YouTube.

  const pctFromClient = (clientX: number): number => {
    const el = progressRef.current; if (!el) return 0;
    const r = el.getBoundingClientRect();
    return clamp((clientX - r.left) / r.width, 0, 1);
  };

  // Write pct to DOM fill + dot directly — no setState
  const applyDragPct = (pct: number) => {
    const pctPx = `${pct * 100}%`;
    if (prgFillRef.current) prgFillRef.current.style.width = pctPx;
    if (prgDotRef.current)  prgDotRef.current.style.left  = pctPx;
    if (prgTipRef.current) {
      prgTipRef.current.style.left = pctPx;
      prgTipRef.current.textContent = fmtTime(pct * duration);
      prgTipRef.current.style.display = 'block';
    }
  };

  // Commit seek on release — one network request, one state update
  ...
  const startDrag = (initPct: number) => {
    isDraggingRef.current = true;
    setIsDragging(true);  // triggers CSS class only
    // Pause immediately so the current Range request cancels — seek feels instant
    videoRef.current?.pause();
    // Remove smooth transition during drag
    if (prgFillRef.current) prgFillRef.current.classList.remove('smooth');
    if (prgWrapRef.current)  prgWrapRef.current.classList.add('drag');
    applyDragPct(initPct);
  };

  const endDrag = (finalPct: number) => {
    isDraggingRef.current = false;
    setIsDragging(false);
    if (prgWrapRef.current)  prgWrapRef.current.classList.remove('drag');
    if (prgTipRef.current)   prgTipRef.current.style.display = 'none';
    if (prgFillRef.current)  prgFillRef.current.classList.add('smooth');
    commitSeek(finalPct);
  ...
  const onProgressMouseDown = (e: React.MouseEvent) => {
    e.preventDefault();
    startDrag(pctFromClient(e.clientX));

    const onMove = (ev: MouseEvent) => {
      applyDragPct(pctFromClient(ev.clientX));
    };
    const onUp = (ev: MouseEvent) => {
      endDrag(pctFromClient(ev.clientX));
  ...
  // Touch drag is attached via useEffect below (needs passive:false, which React can't set)
  ...
  // Attach touchmove with passive:false so preventDefault() works (stops page scroll during drag)
  useEffect(() => {
    ...
    const onTouchStart = (e: TouchEvent) => {
      ...
      const onMove = (ev: TouchEvent) => {
        ev.preventDefault();
        applyDragPct(getPct(ev.touches));
      };
  ...
  }); // no deps — re-attach on every render so duration closure is fresh
  ...
  // ── Derived values ────────────────────────────────────────────────────────────
  const playPct         = duration > 0 ? (currentTime / duration) * 100 : 0; // drag updates DOM directly, not via state
                ...
                {showPlayerCtrls && (
                  <div className={`ctrl-wrap absolute inset-x-0 bottom-0 ${ctrlsHidden ? 'hide' : ''}`}>
                    ...
                    <div className="relative px-3 pb-3 pt-10">
                      ...
                      {/* ── Progress bar ── */}
                      <div
                        ref={(el) => { (progressRef as any).current = el; (prgWrapRef as any).current = el; }}
                        className="prg mb-3"
                        onMouseDown={onProgressMouseDown}
                        onMouseMove={onProgressHover}
                        onMouseLeave={() => { if (!isDraggingRef.current) setHoverInfo(null); }}
                      >
                        <div className="prg-track">
                          <div className="prg-buf" style={{ width: `${bufferedPct}%` }} />
                          {/* fill and dot are updated via DOM ref during drag — no React re-render */}
                          <div ref={prgFillRef} className="prg-fill smooth" style={{ width: `${playPct}%` }} />
                          <div ref={prgDotRef}  className="prg-dot"        style={{ left: `${playPct}%` }} />
                        </div>
                        {/* Static hover tip — updated via ref during drag, via state on hover */}
                        <div ref={prgTipRef} className="prg-tip" style={{ display: 'none', left: `${hoverInfo?.pct ?? 0}%` }}>
                          {fmtTime(hoverInfo?.time ?? 0)}
                        </div>
                        {hoverInfo !== null && !isDraggingRef.current && (
