  ...
  return (
    <>
      ...
      <style>{`
        ...
        .wm{position:absolute;top:12px;z-index:30;font-family:'Outfit',sans-serif;
        ...
        /* ── PAGE ── */
        .lv-page{font-family:'Outfit',sans-serif;user-select:none;}

        /* ── PLAYER BREAKOUT — bulletproof full-bleed regardless of shell padding ──
           Technique: position:relative ancestor + left:50% + translateX(-50%) + width:100vw
           Works even when the app shell has unknown/uncontrollable side padding.        ── */

        /* The spacer reserves vertical space in normal flow */
        .lv-player-spacer{
          position:relative;
          width:100%;
          /* height is set inline via aspectRatio on the actual player */
        }
        /* The actual player breaks out to full viewport width */
        .lv-player{
          position:absolute;
        ...
        /* desktop (≥600px): constrain width back, rounded, shadow */
        ...
        /* info + back button get side padding */
        .lv-info-wrap{
          padding-left:16px;
          padding-right:16px;
          padding-bottom:80px;
        ...
        /* ── INFO CARDS wrapper ── */
        .lv-info{display:flex;flex-direction:column;gap:12px;padding-top:14px;}
        ...
        .lv-note-a{
          ...
          gap:8px;padding:12px 14px;border-radius:10px;font-size:13px;
      ...
      {/* ══════════════════════════════════════════════════════════════════════ */}
      <div className="lv-page" style={{ color:T.text }}
        onContextMenu={e => e.preventDefault()}>

        {/* ── PLAYER: breakout technique = true edge-to-edge on any shell ── */}
        {/* Spacer reserves 16:9 height in flow; player absolutely breaks to 100vw */}
        <div className="lv-player-spacer" style={{ paddingBottom:'56.25%', marginBottom:0 }}>
          <div className="au1 lv-player">
              ...
              <div ref={playerWrapRef} className="sv relative bg-black"
                ...
                {showLoadOverlay && (
                  <div style={{ position:'absolute',inset:0,display:'flex',flexDirection:'column',
                    ...
                    <p style={{ color:'rgba(255,255,255,.35)',fontSize:12,fontWeight:500,letterSpacing:'.04em' }}>
                ...
                {skipFlash&&(
                  <div className="skip-flash">
                    {skipFlash==='fwd'?<><RotateCw size={15}/><span>+10s</span></>:<><RotateCcw size={15}/><span>–10s</span></>}
                  </div>
                )}
                  ...
                  <div className={`ctrl-wrap absolute inset-x-0 bottom-0 ${ctrlsHidden?'hide':''}`}>
                    ...
                    <div className="relative px-3 pb-3 pt-10">
                      <div ref={progressRef}
                        ...
                        onMouseMove={onProgressHover} onMouseLeave={()=>setHoverInfo(null)}>
                      ...
                      <div className="flex items-center gap-0.5">
                        ...
                        <button className="cb" onClick={()=>skipBy(-10)}><SkipBack size={17}/></button>
                        <button className="cb" onClick={()=>skipBy(10)}><SkipForward size={17}/></button>

                        <div className="relative flex-shrink-0" ref={volumeWrapRef}>
                          <button className={`cb ${showVolPanel?'on':''}`}
                            onClick={()=>setShowVolPanel(v=>!v)}><VolumeIcon size={18}/></button>
                          {showVolPanel&&(()=>{
                            ...
                            return(
                              <div className="vol-panel">
                                ...
                                <button className="cb" style={{padding:4}} onClick={toggleMute}>
                                  <VolumeIcon size={16}/>
                                </button>
                              </div>
                            );
                        ...
                        <button className="cb ml-0.5" onClick={toggleFullscreen}>
                ...
                {showPlayerCtrls&&!showLoadOverlay&&(
                  <>
                    <div className="absolute inset-y-0 left-0 w-1/3"
                      style={{bottom:60,pointerEvents:'auto'}} onDoubleClick={()=>skipBy(-10)}/>
        ...
        {/* ══════════════════ INFO CARDS ════════════════════════════════════════ */}
        <div className="lv-info-wrap">
        <div className="lv-info au2">

          {/* ── LESSON DETAILS CARD — production grade ── */}
          <Card className="p-0">
            {/* Top accent strip */}
            <div style={{ height:3, background:T.gradient, borderRadius:'inherit' }}/>
            ...
            <div style={{ padding:'18px 20px 20px' }}>
              ...
              {/* Title */}
              <h1 style={{ color:T.text, fontWeight:800, margin:'0 0 6px',
                fontSize:'clamp(16px,3.5vw,22px)', lineHeight:1.2, letterSpacing:'-.02em' }}>
              ...
              {content?.subject && (
                <div style={{ display:'flex', alignItems:'center', gap:7, marginBottom:2 }}>
                  <BookOpen size={13} style={{ color:T.text3, flexShrink:0 }}/>
              ...
              {/* Description */}
              {content?.description && (
                <p style={{ color:T.text2, fontSize:13, lineHeight:1.65,
          ...
          {(content?.noteUrl || content?.noteGDrivePreviewUrl) ? (
            <Card className="p-0">
              <div style={{ height:3, background:'linear-gradient(90deg,#34d399,#059669)' }}/>
              <div style={{ padding:'18px 20px 20px' }}>
                ...
                <div style={{ display:'flex', alignItems:'center', gap:10, marginBottom:16 }}>
                  ...
                  <div>
                    <p style={{ fontSize:11,fontWeight:700,color:T.text3,
                      textTransform:'uppercase',letterSpacing:'.06em',margin:'0 0 1px' }}>
                      Attached Resource
                    </p>
                    <p style={{ fontSize:14,fontWeight:700,color:T.text,margin:0 }}>Class Notes</p>
                  </div>
                </div>
                ...
                <div style={{ display:'flex',flexDirection:'column',gap:8 }}>
                  ...
                  {content?.noteSource==='gdrive'&&(
                    <p style={{ fontSize:10,color:T.text3,textAlign:'center',
                      margin:'2px 0 0',letterSpacing:'.03em' }}>via Google Drive</p>
                  )}
                </div>
              </div>
            </Card>
          ...
          {/* ── BACK BUTTON — bottom of all cards ── */}
          <button className="lv-back-btn" onClick={()=>navigate(-1)}>
