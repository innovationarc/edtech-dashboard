  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* Player wrapper — edge-to-edge on mobile like YouTube, contained on desktop */
        .lv-player-outer {
          margin: 0 -16px;          /* bleed out of the 16px page padding on mobile */
          background: #000;
        }
        ...
        /* Below-video info layout: stacked on mobile, side-by-side on desktop */
        .lv-meta {
          display: flex;
        ...
        @media(min-width:900px){
          ...
          .lv-meta-left  { flex: 1; min-width: 0; }
          .lv-meta-right { width: 300px; flex-shrink: 0; }
        }

        /* Chip row inside title card */
        .lv-chips {
          display: flex;
          flex-wrap: wrap;
          gap: 8px;
      ...
      {/* ══════════════════════════════════════════════════════════════════════ */}
      {/* PAGE WRAPPER — no background, inherits from the app's layout shell    */}
      {/* ══════════════════════════════════════════════════════════════════════ */}
      <div
        style={{
          color: T.text,
      ...
      >
        {/* Outer page shell — max-width + side padding */}
        <div style={{ position: 'relative', zIndex: 1, maxWidth: 1040, margin: '0 auto', padding: '0 16px 72px' }}>
          ...
          <div style={{ paddingTop: 20, paddingBottom: 12 }}>
            <button
              onClick={() => navigate(-1)}
              className="au inline-flex items-center gap-2 text-sm font-medium text-gray-400 hover:text-white bg-background-700 hover:bg-background-600 border border-white/10 rounded-xl px-4 py-2 transition-colors cursor-pointer"
          ...
          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* PLAYER — edge-to-edge mobile, rounded+shadow desktop               */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          <div className="au1 lv-player-outer">
            {/* DevTools overlay */}
          ...
          {/* ══════════════════════════════════════════════════════════════════ */}
          {/* METADATA — YouTube-style: title+chips LEFT, notes RIGHT on desktop */}
          {/* ══════════════════════════════════════════════════════════════════ */}
          <div className="lv-meta au2">

            {/* ── LEFT: title card with inline chips ── */}
            <div className="lv-meta-left" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
              <Card className="p-5">
                ...
                {content?.description && (
                  <p style={{
                    color: T.text2, fontSize: 13, lineHeight: 1.6,
                    margin: '8px 0 0',
                  }}>
                    {content.description}
                  </p>
                ...
                {/* Inline chips — duration + subject side by side */}
                {((content?.duration && content.duration > 0) || content?.subject) && (
                  <div className="lv-chips" style={{ borderTop: `1px solid ${T.divider}` }}>
                    {content?.duration && content.duration > 0 && (
                      <span className="lv-chip" style={{ color: T.text2 }}>
                        <Clock size={13} style={{ color: primaryColor, flexShrink: 0 }} />
                  ...
                  <div className="flex flex-col gap-2">
                    {getNotePreview() && (
                      <a href={getNotePreview()!} target="_blank" rel="noopener noreferrer"
                        className="lv-note-a"
                        style={{ background: 'rgba(52,211,153,0.08)', border: '1px solid rgba(52,211,153,0.20)', color: 'rgba(110,231,183,0.9)' }}>
                        <span className="flex items-center gap-2"><ExternalLink size={14} /> Preview Notes</span>
                    ...
                    {content?.noteSource === 'gdrive' && (
                      <p style={{ fontSize: 10, color: T.text3, textAlign: 'center', margin: 0, letterSpacing: '0.03em' }}>via Google Drive</p>
                    )}
                  </div>
                </Card>
              ) : (
                <Card className="p-8 flex flex-col items-center justify-center text-center gap-3" style={{ minHeight: 116 }}>
                  <div style={{
                    width: 44, height: 44, borderRadius: 12,
                    background: T.surface, border: `1px solid ${T.border}`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                  }}>
                    <FileText size={18} style={{ color: T.text3, opacity: 0.4 }} />
                  </div>
                  <p className="text-sm font-medium" style={{ color: T.text3, margin: 0 }}>No notes attached</p>
                </Card>
              )}
            </div>
