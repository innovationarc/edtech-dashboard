...
const LessonViewer: React.FC = () => {
  ...
  const [bleed, setBleed] = useState({ ml: 0, mr: 0 });  // negative margins for full-bleed

  // ── Init ───────────────────────────────────────────────────────────────────
  useEffect(() => { injectAntiPiracy(); }, []);

  // Measure page element offset to compute exact full-bleed negative margins.
  // Runs on mount and on resize so it works at every screen size.
  useEffect(() => {
    const measure = () => {
      const el = pageRef.current;
      if (!el) return;
      const rect = el.getBoundingClientRect();
      const ml = rect.left;                        // distance from left viewport edge
      const mr = window.innerWidth - rect.right;   // distance from right viewport edge
      setBleed({ ml, mr });
    };
    measure();
  ...
  return (
    <>
      ...
      <style>{`
        ...
        /* ── PLAYER: negative margins set by JS measurement (see bleed state) ── */
        .lv-player{
          background:#000;
          overflow:hidden;
          position:relative;
        }
        /* desktop (≥600px): normal flow, rounded, shadow */
      ...
      {/* ══════════════════════════════════════════════════════════════════════ */}
      <div ref={pageRef} className="lv-page" style={{ color:T.text }}
        onContextMenu={e => e.preventDefault()}>

        {/* ── PLAYER: JS-measured negative margins = bulletproof full-bleed ── */}
        <div className="au1 lv-player" style={{
          marginLeft: bleed.ml > 4 ? -bleed.ml : 0,
          marginRight: bleed.mr > 4 ? -bleed.mr : 0,
        }}>
