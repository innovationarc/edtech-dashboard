// src/pages/ContentUpload.tsx - PART 1
...
const ContentUpload = () => {
  ...
  const [formData, setFormData] = useState({
    ...
    description: '',
    course: '',
    courseId: '', // NEW: Store selected course ID
    category: '',
    difficulty: 'medium' as 'easy' | 'medium' | 'hard',
    tags: [] as string[],
    topics: [] as string[], // NEW: Topics field
    file: null as File | null,
    duration: '', // NEW: Duration for lessons
    // MCQ specific fields
    question: '',
    choices: [
      { id: 1, text: '' },
  ...
  const [currentTopic, setCurrentTopic] = useState(''); // NEW: For topic input
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (uploadType === 'mcq') {
        ...
        // Handle content upload (lesson/note/trick)
        if (!formData.title.trim()) {
          throw new Error('Title is required');
        }

        // If course is selected, add as lesson to that course
        if (formData.courseId) {
          ...
          // Determine lesson type from file
          ...
          // Regular content upload (not linked to a course)
          if (!formData.file) {
            throw new Error('Please select a file to upload');
          }
...
// src/pages/ContentUpload.tsx - PART 2
          ...
          <div className="lg:col-span-2">
            <Card title="Upload New Content">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* Course Selection - Only for lessons */}
                {uploadType !== 'mcq' && (
                  <div>
                    ...
                    {formData.courseId && (
                      <p className="text-xs text-primary-400 mt-1">
                        This content will be added as a lesson to the selected course
                      </p>
                    )}
                  </div>
                )}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  {/* Duration field - Required for course lessons */}
                  ...
                  {/* Course name for non-course content */}
                  {!formData.courseId && (
                    <div>
                ...
                {/* Topics - NEW FIELD for course lessons */}
                  ...
                  <div>
                    ...
                    <div className="border-2 border-dashed border-background-600 rounded-lg p-6 text-center">
                      ...
                      <p className="text-sm text-gray-500 mt-2">
                        {formData.courseId 
                          ? 'PDF, DOC, PPT, TXT, MP4, WEBM (Max 500MB)' 
                  ...
                  {loading ? (
                    ...
                    <>
                      ...
                      <span>
                        {formData.courseId 
                          ? `Add Lesson to Course` 
                          : `Upload ${uploadType === 'mcq' ? 'MCQ' : 'Content'}`}
                      </span>
                    </>
                  )}
                </button>
                ...
                {formData.courseId && (
                  <div className="bg-primary-900/20 border border-primary-700 p-3 rounded">
                    ...
                    <p className="text-xs text-gray-400">
                      This content will be added as a lesson to the selected course. 
                      Duration and topics are required for course lessons.
                    </p>
                  </div>
                )}
              </div>
              ...
              {filteredMcqs.map(mcq => (
                <div key={mcq.id} className="bg-background-800 rounded-lg p-4 hover:bg-background-700 transition-colors">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <button 
                        onClick={() => alert(`Question: ${mcq.question}\n\nCorrect Answer: ${mcq.choices.find(c => c.id === mcq.correctAnswer)?.text}`)}
                        className="p-1.5 bg-background-700 hover:bg-background-600 text-gray-400 hover:text-white rounded transition-colors"
                      >
                        <Eye size={14} />
                      </button>
