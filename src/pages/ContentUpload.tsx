...
import { Upload, FileText, BookOpen, PenTool, BrainCircuit, Plus, Edit, Trash2, Eye, Download, Search, Filter } from 'lucide-react';
import Card from '../components/ui/Card';
import { contentService, Content } from '../services/contentService'; // Keep Content interface
import { courseService } from '../services/courseService';
import { useDashboard } from '../contexts/DashboardContext';
import { mcqService, MCQQuestion } from '../services/mcqService'; // Import mcqService and MCQQuestion
...
const ContentUpload = () => {
  ...
  const [mcqs, setMcqs] = useState<MCQQuestion[]>([]); // Use MCQQuestion interface
  ...
  const loadContent = async () => {
    try {
      // Fetch content and MCQs from their respective services
      const [contentData, mcqData] = await Promise.all([
        contentService.getAllContent(),
        mcqService.getAllMCQQuestions() // Fetch MCQs from mcqService
      ]);
    ...
    try {
      if (uploadType === 'mcq') {
        ...
        const mcqData: Omit<MCQQuestion, 'id' | 'createdAt'> = {
          ...
          subject: formData.category, // Use category as subject for MCQs
          ...
          tags: formData.tags,
          createdBy: user?.uid || '',
          title: formData.title, // Add title for MCQ
          description: formData.description, // Add description for MCQ
          type: 'mcq' // Explicitly set type for MCQ
        };

        await mcqService.createMCQQuestion(mcqData); // Use mcqService
  ...
  const handleDelete = async (id: string, type: 'content' | 'mcq') => {
    ...
    try {
      if (type === 'content') {
        ...
        await mcqService.deleteMCQQuestion(id); // Use mcqService
  ...
  const filteredMcqs = mcqs.filter(mcq => {
    ...
    const matchesCategory = selectedCategory === 'all' || mcq.subject === selectedCategory; // Use mcq.subject for category
    return matchesSearch && matchesCategory;
  });
  ...
  return (
    <div className="space-y-6">
      ...
      {success && (
        <div className="bg-success-dark text-success-light px-4 py-2 rounded">
          {success}
        </div>
      )}
          ...
          <div className="lg:col-span-2">
            <Card title="Upload New Content">
              <form onSubmit={handleSubmit} className="space-y-6">
                ...
                {/* File Upload (for non-MCQ types) */}
                {uploadType !== 'mcq' && (
                  <div>
                    <label className="block text-sm font-medium text-gray-400 mb-2">
                      Upload File *
                    </label>
                    <div className="border-2 border-dashed border-background-600 rounded-lg p-6 text-center">
                      ...
                      <p className="text-sm text-gray-500 mt-2">
                        PDF, DOC, PPT, TXT, MP4, AVI, MOV (Max 100MB)
                      </p>
                      {formData.file && (
                        <p className="text-sm text-success-DEFAULT mt-2">
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-primary-600 hover:bg-primary-700 disabled:bg-primary-800 disabled:cursor-not-allowed text-white py-3 rounded-lg transition-colors flex items-center justify-center gap-2"
                >
                  {loading ? (
                    <>
                    ...
                    <>
                      <Upload size={16} />
                      <span>Upload {uploadType === 'mcq' ? 'MCQ' : 'Content'}</span>
                    </>
                  )}
                </button>
              </form>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                ...
                <div key={content.id} className="bg-background-800 rounded-lg p-4 hover:bg-background-700 transition-colors">
                  ...
                  <div className="flex justify-between items-center">
                    ...
                    <div className="flex gap-2">
                      <button className="p-1.5 bg-background-700 hover:bg-background-600 text-gray-400 hover:text-white rounded transition-colors">
                        <Eye size={14} />
                      </button>
                      <button className="p-1.5 bg-background-700 hover:bg-background-600 text-gray-400 hover:text-white rounded transition-colors">
                        <Download size={14} />
              ...
              {filteredMcqs.map(mcq => (
                <div key={mcq.id} className="bg-background-800 rounded-lg p-4 hover:bg-background-700 transition-colors">
                  ...
                  <div className="space-y-2 mb-4">
                    <div className="flex items-center gap-2 text-xs text-gray-400">
                      <span>Category: {mcq.subject}</span> {/* Use mcq.subject */}
                    </div>
                    <div className="flex items-center gap-2 text-xs text-gray-400">
              ...
              {courses.map(course => (
                <div key={course.id} className="bg-background-800 rounded-lg overflow-hidden hover:bg-background-700 transition-colors">
                  ...
                  <div className="p-4">
                    ...
                    <div className="flex gap-2">
                      <button className="flex-1 bg-primary-600 hover:bg-primary-700 text-white py-2 px-3 rounded text-sm transition-colors">
                        Edit Course
                      </button>
                      <button className="p-2 bg-background-700 hover:bg-background-600 text-gray-400 hover:text-white rounded transition-colors">
                        <Eye size={14} />
                      </button>
                    </div>
                  </div>
