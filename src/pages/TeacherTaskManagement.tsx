...
const CLASS_GRADES: { value: string; label: string }[] = [
  { value: 'class6', label: 'Class 6' }, { value: 'class7', label: 'Class 7' },
  { value: 'class8', label: 'Class 8' }, { value: 'class9', label: 'Class 9' },
  { value: 'class10', label: 'Class 10' }, { value: 'ssc', label: 'SSC' },
  { value: 'class11', label: 'Class 11' }, { value: 'class12', label: 'Class 12' },
  { value: 'hsc', label: 'HSC' }, { value: 'diploma', label: 'Diploma' },
  { value: 'undergraduate', label: 'Undergraduate' }, { value: 'graduated', label: 'Graduated' },
];
...
const TaskGroupModal = ({ courses, students, editGroup, teacherId, teacherName, onClose, onSaved }: {
  courses: Course[]; students: AppUser[]; editGroup?: TaskGroup | null;
            ...
            {f.assignType === 'class' && (
              <div style={secBg} className="rounded-xl p-3">
                ...
                <select className={inputCls} style={inputStyle} value={f.classGrade}
                  ...
                  {CLASS_GRADES.map(c => <option key={c.value} value={c.value}>{c.label}</option>)}
                </select>
              </div>
            )}
        ...
        <div className="flex-1 overflow-y-auto p-5 space-y-5">
          ...
          {f.type === 'exam' && (
            <div style={secBg} className="rounded-xl p-4 space-y-3">
              ...
              <div><SectionLabel>Content ID (from Content Library)</SectionLabel>
                <input className={inputCls} style={inputStyle} placeholder="Enter exam content ID..."
                  value={f.contentId} onChange={e => set('contentId', e.target.value)} />
...
const TeacherTaskManagement: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [g, c, u] = await Promise.all([
        ...
        // ✅ Correct method: getCoursesByInstructor
        courseService.getCoursesByInstructor(user.uid).catch(() => [] as Course[]),
        userService.getAllUsers().catch(() => [] as AppUser[]),
      ]);
      setGroups(g);
      setCourses(c);
      // Filter only students and active users
  ...
  const handlePublish = async (g: TaskGroup) => {
    try { await taskService.publishTaskGroup(g.id); await load(); if (selectedGroup?.id === g.id) { const updated = await taskService.getTaskGroupById(g.id); if (updated) setSelectedGroup(updated); } }
    catch (e: any) { alert(e.message); }
  };
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                  {filtered.map(group => {
                    ...
                    const assignLabel = at.type === 'all' ? 'All Students'
                      ...
                      : at.type === 'class' ? `Class: ${CLASS_GRADES.find(c => c.value === (at as any).classGrade)?.label || (at as any).classGrade}`
