...
const TeacherTaskManagement: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [g, allPublishedCourses, u] = await Promise.all([
        taskService.getTaskGroupsByTeacher(user.uid),
        // ✅ Fetch ALL published courses (same as enrollment page) for the course dropdown
      ...
      // ✅ Derive available classes dynamically from courses (same as enrollment page)
      const clsSet = new Set<string>();
      allPublishedCourses.forEach(c => { if ((c as any).class) clsSet.add((c as any).class); });
