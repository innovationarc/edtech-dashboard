...
const TeacherTaskManagement: React.FC = () => {
  ...
  const hasGlobalTaskCreation = teacherTaskPerms.some((p: any) => p.hasGlobalTaskCreation);
  const canCreateTasks = isAdminMgr || hasGlobalTaskCreation || teacherTaskPerms.some((p: any) => p.canCreate);
  ...
  useEffect(() => {
    ...
    if (user.role === 'teacher') {
      courseAssignmentService.getTeacherAssignments(user.uid)
        .then(assignments => {
          // Global task creation: teacher has task_creation_global in any assignment's globalPermissions
