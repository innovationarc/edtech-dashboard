...
const TeacherTaskManagement: React.FC = () => {
  ...
  const canCreateTasks = isAdminMgr || teacherTaskPerms.length > 0;
  ...
  const load = useCallback(async (
    taskPerms?: Array<{ courseId: string; allowedSubjects: string[] }>
  ) => {
    if (!user) return;
    ...
    try {
      ...
      const [allPublishedCourses, u] = await Promise.all([
      ...
      if (isAdminMgr) {
        ...
        if (activePerms.length === 0) {
          ...
          setGroups([]); setCourses([]); setAvailableClasses([]);
      ...
      // For teachers: restrict modal course dropdown to permitted courses only
      const visibleCourses = isTeacher
        ? allPublishedCourses.filter(c => permMap.has(c.id))
        : allPublishedCourses;
      ...
      courseAssignmentService.getTeacherAssignments(user.uid)
        .then(assignments => {
          const taskPerms = assignments
            .filter(a => a.isActive && a.permissions.includes('tasks'))
