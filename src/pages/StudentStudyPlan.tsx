...
const StudentStudyPlan = () => {
  ...
  // ── Feature #13: Load streak freeze count from localStorage ─────────────────
  useEffect(() => {
    if (!user) return;
    const key = `studyPlan_streakFreeze_${user.uid}`;
    try {
      const stored = JSON.parse(localStorage.getItem(key) || '{}');
      // Reset to 1 freeze every 7 days
      const lastReset = stored.lastReset ? new Date(stored.lastReset) : new Date(0);
      const daysSinceReset = (Date.now() - lastReset.getTime()) / (1000 * 60 * 60 * 24);
      if (daysSinceReset >= 7) {
        const refreshed = { count: 1, lastReset: new Date().toISOString() };
        localStorage.setItem(key, JSON.stringify(refreshed));
        setStreakFreezeCount(1);
      } else {
        setStreakFreezeCount(stored.count ?? 0);
      }
    } catch { setStreakFreezeCount(0); }
  }, [user]);
  ...
  const handleUseStreakFreeze = () => {
    ...
    try {
      const stored = JSON.parse(localStorage.getItem(key) || '{}');
      localStorage.setItem(key, JSON.stringify({ ...stored, count: newCount }));
    } catch {}
  };
          ...
          <div className="bg-background-900 border border-background-700 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">You have {streakFreezeCount} freeze{streakFreezeCount !== 1 ? 's' : ''} remaining (resets weekly)</p>
              </div>
            </div>
