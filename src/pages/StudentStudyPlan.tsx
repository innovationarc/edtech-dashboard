...
import {
  ...
  Download, Snowflake, History, Search, Star, Settings,
} from 'lucide-react';
...
const NoAnimCard = ({ children, className, style, onClick }: {
  ...
  return (
    <div data-no-anim onClick={onClick} style={{ width: '100%', minWidth: 0, overflow: 'hidden', ...(onClick ? { cursor: 'pointer' } : {}) }}>
      <style>{`
...
const POMODORO_DURATIONS = { focus: 25 * 60, short: 5 * 60, long: 15 * 60, custom: 25 * 60 };
const DAYS = ['Sun', 'Mon', 'Tue', 'Wed', 'Thu', 'Fri', 'Sat'];

// ── Module-level state — survives React unmount/remount (in-app navigation) ───
// _ambientAudio: the <audio> element keeps playing when navigating within app.
// _inAppNavigating: set to true just before React Router unmounts, so the
//   visibilitychange handler knows "hidden" was caused by in-app nav, not by
//   the user leaving the browser — and therefore should NOT pause the timer.
let _ambientAudio: HTMLAudioElement | null = null;
let _inAppNavigating = false;
...
const StudentStudyPlan = () => {
  ...
  const [detailEvent, setDetailEvent] = useState<StudyPlanEvent | null>(null);
  ...
  // FIX: initialise from localStorage so "Set ▸" label doesn't flash on every
  // in-app navigation. The flag is written to localStorage in handleSaveFreeTime.
  const [freeTimeConfigured, setFreeTimeConfigured] = useState<boolean>(() => {
    try { return localStorage.getItem('studyPlan_freeTimeConfigured') === '1'; } catch { return false; }
  });
  ...
  // ── Free time range helpers ──────────────────────────────────────────────
  const _toMin = (t: string) => { const [h, m] = t.split(':').map(Number); return h * 60 + m; };
  const _toStr = (m: number) => `${String(Math.floor(m / 60)).padStart(2, '0')}:${String(m % 60).padStart(2, '0')}`;

  /** True if two time ranges overlap (exclusive end boundary) */
  const rangesOverlap = (s1: string, e1: string, s2: string, e2: string) =>
    _toMin(s1) < _toMin(e2) && _toMin(s2) < _toMin(e1);

  /**
   * Auto-fix ranges: sort by start, then clamp each range's start to be
   * >= the previous range's end (no merging — keeps separate ranges).
   * Ranges that can't be fit (start >= 24:00) are dropped.
   * Zero-length ranges get a minimum 30-min duration if possible.
   */
  const autoFixRanges = (ranges: { start: string; end: string }[]) => {
    const MIN_DURATION = 30; // minutes
    const sorted = [...ranges]
      .filter(r => _toMin(r.end) > _toMin(r.start))
      .sort((a, b) => _toMin(a.start) - _toMin(b.start));
    const result: { start: string; end: string }[] = [];
    let cursor = 0; // minutes, earliest a new range can start
    for (const r of sorted) {
      const rStart = Math.max(_toMin(r.start), cursor);
      const rEnd   = Math.max(_toMin(r.end), rStart + MIN_DURATION);
      if (rStart >= 24 * 60) continue; // no room in the day
      const clampedEnd = Math.min(rEnd, 24 * 60);
      result.push({ start: _toStr(rStart), end: _toStr(clampedEnd) });
      cursor = clampedEnd;
    }
    return result.length > 0 ? result : ranges.slice(0, 1); // always keep at least 1
  };

  /** True if any range in the array overlaps another */
  const hasAnyOverlap = (ranges: { start: string; end: string }[]) =>
    ranges.some((r, i) => ranges.some((o, oi) => oi !== i && rangesOverlap(r.start, r.end, o.start, o.end)));

  /** Find next free 2-hour slot that doesn't overlap existing ranges */
  const nextFreeSlot = (ranges: { start: string; end: string }[]) => {
    for (let start = 6 * 60; start <= 22 * 60; start += 30) {
      const end = start + 120;
      if (end > 24 * 60) break;
      const overlaps = ranges.some(r => _toMin(r.start) < end && start < _toMin(r.end));
      if (!overlaps) return { start: _toStr(start), end: _toStr(end) };
    }
    return null;
  };

  /**
   * Called on every start/end change. Tries to silently fix overlaps by
   * clamping. If it can't (range is impossible), leaves flag for UI warning.
   */
  const updateRangeAndFix = (idx: number, field: 'start' | 'end', value: string) => {
    setFreeTimeRanges(prev => {
      const updated = prev.map((x, i) => i === idx ? { ...x, [field]: value } : x);
      // Attempt silent auto-fix
      const fixed = autoFixRanges(updated);
      // Only apply auto-fix if it actually resolved overlaps AND kept the same count
      if (!hasAnyOverlap(fixed) && fixed.length === updated.length) return fixed;
      // Otherwise just return the raw edit — UI will show overlap warning
      return updated;
    });
  ...
  useEffect(() => {
    ...
    studyPlanService.getFreeTimePrefs(user.uid).then(prefs => {
      if (prefs) {
        ...
        setFreeTimeConfigured(true);
        // FIX: persist to localStorage so "Set ▸" never flashes on remount
        try { localStorage.setItem('studyPlan_freeTimeConfigured', '1'); } catch { /* ignore */ }
  ...
  const handleSaveFreeTime = async () => {
    ...
    try {
      // Silently remove zero-length ranges before saving
      const cleanRanges = freeTimeRanges.filter(r => _toMin(r.end) > _toMin(r.start));
      if (cleanRanges.length !== freeTimeRanges.length) setFreeTimeRanges(cleanRanges);
      const rangesToSave = cleanRanges.length > 0 ? cleanRanges : freeTimeRanges;
      await studyPlanService.saveFreeTimePrefs(user.uid, { mode: freeTimeMode, hours: freeHoursPerDay, ranges: rangesToSave });
      setFreeTimeConfigured(true);
      ...
      // FIX: bust schedule cache so the next Schedule/Reschedule call uses the
      // newly saved ranges — without this, the old cached schedule is returned
      // even after the user changes and saves their free time.
      setScheduleHash('');
      setCachedResult(null);
      // Persist configured flag so it survives navigation (no more "Set ▸" flash)
      try { localStorage.setItem('studyPlan_freeTimeConfigured', '1'); } catch { /* ignore */ }
  ...
  // ── Ambient sound state (loaded from settingsService) ──────────────────────
  ...
  const [pomodoroSoundVolume,   setPomodoroSoundVolume]   = useState(0.4); // default 40%
  // pomodoroSoundEnabled = master switch (card icon) — if off: everything silent
  // pomodoroAmbientEnabled = ambient-only switch (settings modal) — if off: music silent, chime still plays
  const [pomodoroAmbientEnabled, setPomodoroAmbientEnabled] = useState(true);
  // ambientAudioRef wraps the module-level _ambientAudio so the rest of the
  // code uses the same .current API — but the actual audio survives unmount.
  const ambientAudioRef = {
    get current() { return _ambientAudio; },
    set current(v: HTMLAudioElement | null) { _ambientAudio = v; },
  };
  const volumeSaveTimerRef = useRef<ReturnType<typeof setTimeout> | null>(null); // debounce Firestore volume saves
  ...
  // ── Visibility / persistence state ────────────────────────────────────────
  const pomodoroHiddenAtRef    = useRef<number | null>(null);
  const POMODORO_RESET_MS      = 10 * 60 * 1000; // 10 min idle → auto-reset
  const pomodoroLastDbWriteRef = useRef<number>(0);
  const pomodoroLastCacheMinRef= useRef<number>(0);
  const POMO_ALLTIME_KEY = user ? `pomo_alltime_${user.uid}` : 'pomo_alltime';
  // Tracks whether we've already restored state on this mount — prevents double restore
  ...
  // FIX: targetDate from Firestore may be a Timestamp, JS Date, or string.
  // Safely coerce so hash never crashes regardless of service layer output.
  const _safeDate = (d: any): Date =>
    d?.toDate ? d.toDate() : d instanceof Date ? d : new Date(d);

  const computeScheduleHash = () => JSON.stringify({
    // FIX: use local date string so cache busts at local midnight, not UTC midnight.
    today: (() => { const n = new Date(); return n.getFullYear() + "-" + n.getMonth() + "-" + n.getDate(); })(),
    freeHours: effectiveFreeHours,
    freeMode: freeTimeMode,
    freeRanges: freeTimeMode === "range" ? freeTimeRanges : [],
    goals: goals.map(g => ({ s: g.subject, t: _safeDate(g.targetDate).toISOString().slice(0, 10), h: g.hoursNeeded, d: g.difficulty, p: g.currentProgress })),
  ...
  const handleGenerateSchedule = () => {
    ...
    // FIX: guard against range mode with no valid ranges — fall back to hours mode silently
    const effectiveMode = (freeTimeMode === 'range' && freeTimeRanges.filter(r => _toMin(r.end) > _toMin(r.start)).length === 0)
      ? 'hours'
      : freeTimeMode;
    ...
    // FIX: wrap in setTimeout so React renders the loading spinner BEFORE the
    // synchronous scheduler runs. Without this the spinner shows for 0ms.
    ...
    setTimeout(() => {
      try {
        const result = generateStudySchedule({
          ...
          customActivities,
          freeTimeMode: effectiveMode,
          freeHoursPerDay: effectiveFreeHours,
          freeTimeRanges: effectiveMode === 'range' ? freeTimeRanges.filter(r => _toMin(r.end) > _toMin(r.start)) : freeTimeRanges,
          now: new Date(),
          studentId: user?.uid ?? '',
        });
        ...
        setShowScheduleModal(true);
      } catch (e: any) { setError('Schedule error: ' + (e as any).message); }
  ...
  const handleUpdateGoalProgress = async (id: string, progress: number) => {
    // Optimistic update — reflect immediately so UI doesn't wait for round-trip
    setGoals(prev => prev.map(g => g.id === id ? { ...g, currentProgress: progress } : g));
    try {
  ...
  const handleGenerateReschedule = () => {
    ...
    // FIX: wrap in setTimeout so the "Calculating…" spinner actually renders
    // before the synchronous reschedule computation blocks the JS thread.
    ...
    setTimeout(() => {
      try {
        ...
        // FIX: filter invalid ranges before passing to scheduler
        const validRanges = freeTimeRanges.filter(r => _toMin(r.end) > _toMin(r.start));
        const effectiveMode = (freeTimeMode === 'range' && validRanges.length === 0) ? 'hours' : freeTimeMode;
        const result = rescheduleStudyPlan({
          ...
          freeTimeMode: effectiveMode,
          freeHoursPerDay: effectiveFreeHours,
          freeTimeRanges: effectiveMode === 'range' ? validRanges : freeTimeRanges,
          now: new Date(),
          studentId: user?.uid ?? '',
        });
        ...
        setError('Reschedule error: ' + (e as any).message);
      } finally {
  ...
  const handleOpenRecovery = () => {
    ...
    // FIX: filter out zero-length ranges and fall back to hours mode if none left
    const validRanges = freeTimeRanges.filter(r => _toMin(r.end) > _toMin(r.start));
    const effectiveMode = (freeTimeMode === 'range' && validRanges.length === 0) ? 'hours' : freeTimeMode;
  ...
  const playPomodoroSound = (type: 'focus' | 'break') => {
    // Master switch (card icon) off → total silence including chime
    if (!pomodoroSoundEnabled) return;
    // Note: pomodoroAmbientEnabled only controls the looping music, NOT the end-chime.
    // The chime always plays as long as the master switch is on.
    try {
   ...
   * isFocus: explicitly passed so we don't rely on stale closure over pomodoroMode.
   */
  const startAmbientSound = (isFocus = true) => {
    // Never play ambient during breaks
    if (!isFocus) return;
    // Master switch off → total silence
    if (!pomodoroSoundEnabled) return;
    // Ambient toggle off → no music (chime still plays)
    if (!pomodoroAmbientEnabled) return;
    if (!pomodoroSoundUrl) return;
    try {
      ...
      const isCorsUrl = /raw\.githubusercontent\.com|cdn\.|cdnjs\.|r2\.dev|storage\.googleapis\.com/.test(pomodoroSoundUrl);
      if (isCorsUrl) audio.crossOrigin = 'anonymous';
      audio.src = pomodoroSoundUrl;
      audio.play().catch(() => {});
      ambientAudioRef.current = audio;
      // Suppress OS media-session / lock-screen notification.
      // playbackState='none' tells the OS there is no active media playback.
      // We also null the metadata and override all action handlers.
      try {
        if ('mediaSession' in navigator) {
          navigator.mediaSession.metadata = null;
          navigator.mediaSession.playbackState = 'none';
          (['play','pause','stop','seekbackward','seekforward','previoustrack','nexttrack'] as MediaSessionAction[])
            .forEach(a => { try { navigator.mediaSession.setActionHandler(a, null); } catch { /* ok */ } });
        }
      } catch { /* not supported */ }
    } catch { /* silent */ }
  };
  ...
  // Load today's focused minutes — localStorage first (no flicker on navigation), then DB
  useEffect(() => {
    if (!user) return;
    const todayStr = new Date().toISOString().slice(0, 10);
    // Immediate cache restore
    try {
      const cached = JSON.parse(localStorage.getItem(`pomo_today_${user.uid}`) || 'null');
      if (cached && cached.date === todayStr) setPomodoroTodayMinutes(cached.mins);
    } catch { /* silent */ }
    // Authoritative DB load
    studyPlanService.getPomodoroSessions(user.uid, 50).then(sessions => {
      ...
      localStorage.setItem(`pomo_today_${user.uid}`, JSON.stringify({ mins: todayMins, date: todayStr }));
      // Seed all-time from DB
      const totalMins = sessions.filter(s => s.completed).reduce((sum, s) => sum + s.duration, 0);
      // Only update if DB value is bigger (don't overwrite live-session additions)
      const localAllTime = parseInt(localStorage.getItem(POMO_ALLTIME_KEY) || '0', 10);
      if (totalMins > localAllTime) localStorage.setItem(POMO_ALLTIME_KEY, String(totalMins));
    }).catch(() => {});
  }, [user]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Load ambient sound settings from Firestore ─────────────────────────────
  useEffect(() => {
    if (!user) return;
    Promise.all([getPomodoroSounds(), getUserPomodoroSettings(user.uid)])
      .then(([sounds, pref]) => {
        setPomodoroSounds(sounds);
        let selectedId: string | null = null;
        let volume = 0.5;
        if (pref) {
          selectedId = pref.selectedSoundId;
          volume = pref.volume ?? 0.5;
        } else {
          const def = sounds.find(s => s.isDefault);
        ...
        if (selectedId && selectedId !== 'none') {
          ...
          setPomodoroSoundUrl(null);
        }
      })
      .catch(() => {});
  }, [user]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Always-current ref so save callbacks always read the latest state ─────────
  ...
  // ── Always-current ref for values read inside the setInterval tick ────────────
  // This prevents stale-closure bugs where the interval reads an old pomodoroMode,
  // pomodoroSessionCount, etc. from when it was first created — which caused the
  // timer to behave incorrectly (e.g. running faster than real time, wrong mode
  // on session complete) whenever the mode changed while the interval was alive.
  const pomodoroTickRef = useRef({
    mode: pomodoroMode, sessionCount: pomodoroSessionCount,
    subject: pomodoroSubject, linkedEvent: pomodoroLinkedEvent,
    startTime: pomodoroStartTime, user: user,
  });
  pomodoroTickRef.current = {
    mode: pomodoroMode, sessionCount: pomodoroSessionCount,
    subject: pomodoroSubject, linkedEvent: pomodoroLinkedEvent,
    startTime: pomodoroStartTime, user: user,
  };
  ...
  // active=true  → timer will auto-resume on restore (used for in-app nav)
  // active=false → timer restores paused, student resumes manually
  const savePomodoroToStorage = useCallback((saveAsActive: boolean) => {
  ...
  // ── Screen-lock vs navigate-away: the correct solution ──────────────────────
  //
  // KEY INSIGHT: pagehide fires when the page is actually being DESTROYED
  // (tab closed, browser quit, navigate to other site). It does NOT fire for
  // screen lock — the page stays alive in memory on screen lock.
  // visibilitychange fires for ALL of: screen lock, tab switch, navigate away.
  //
  // So the correct two-signal design:
  //
  //   _pageIsBeingDestroyed flag: set to true in pagehide handler.
  //
  //   visibilitychange → hidden:
  //     • If _inAppNavigating → React Router nav, keep timer + audio running.
  //     • Else → screen lock OR navigate-away. Keep timer + audio running,
  //              record hiddenAt, save state with active:true.
  //              (We do NOT know yet if page will be destroyed.)
  //
  //   pagehide (fires when page is actually leaving):
  //     • Set _pageIsBeingDestroyed = true.
  //     • Pause timer + audio + save as active:false (paused).
  //
  //   visibilitychange → visible (screen unlock, coming back):
  //     • _pageIsBeingDestroyed is false (page survived) → screen lock case.
  //     • Fast-forward elapsed seconds. Timer was ticking the whole time.
  //     • Audio was running, just fast-forward.
  //
  //   On fresh page load (other site / browser quit scenario):
  //     • pagehide fired before destroy → saved active:false → restores paused.
  //
  // Result:
  //   Case 1 (screen lock)          → timer + music keep running ✅
  //   Case 2 (in-app nav)           → timer + music keep running ✅
  //   Case 3 (other site / tab close) → pauses ✅
  //   Case 4 (browser quit / kill)  → pauses ✅

  useEffect(() => {
    // pagehide fires reliably on mobile (Chrome, Kiwi, Safari) when page is
    // actually being destroyed. This is our pause trigger — NOT visibilitychange.
    const handlePageHide = () => {
      if (_inAppNavigating) return; // React Router nav, not a real page destroy
      // Page is being destroyed → pause + save as paused
      stopAmbientSound();
      savePomodoroToStorage(false); // active:false = paused
    };

    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Going hidden — could be screen lock OR navigate-away.
        // We do NOT pause here. Let the timer keep running.
        // pagehide will fire separately if the page is actually destroyed.
        pomodoroHiddenAtRef.current = Date.now();

        if (_inAppNavigating) return; // React Router nav handled separately

        // Save state as active so if we come back (screen unlock) it restores running.
        // If page is destroyed, pagehide will overwrite this with active:false.
        ...
        if (_inAppNavigating) {
          // Returned from in-app navigation — audio still playing, timer still ticking.
          // Same fix as screen-unlock: clear + restart the interval so the OS burst
          // ticks don't double-count with our elapsed correction.
          _inAppNavigating = false;
          if (hiddenAt !== null) {
            const elapsedSecs = Math.floor((Date.now() - hiddenAt) / 1000);
            if (elapsedSecs > 0) {
              if (pomodoroRef.current) clearInterval(pomodoroRef.current);
              setPomodoroTime(t => Math.max(0, t - elapsedSecs));
              if (pomodoroStateForSave.current.active) {
                pomodoroRef.current = setInterval(() => {
                  const tick = pomodoroTickRef.current;
                  setPomodoroTime(t => {
                    if (t <= 1) {
                      setPomodoroActive(false);
                      stopAmbientSound();
                      if (tick.mode === 'focus' && tick.user) {
                        playPomodoroSound('focus');
                        const sessionMins = Math.round((POMODORO_DURATIONS[tick.mode] - 1) / 60) || 25;
                        const sessionStart = tick.startTime || new Date(Date.now() - sessionMins * 60 * 1000);
                        studyPlanService.savePomodoroSession({ studentId: tick.user.uid, subject: tick.subject || 'General', startTime: sessionStart, duration: sessionMins, completed: true, notes: tick.linkedEvent ? `Linked to event: ${tick.linkedEvent}` : '' })
                          .then(() => studyPlanService.getStreak(tick.user!.uid))
                          .then(s => { if (s) setStreak({ current: s.currentStreak, longest: s.longestStreak, totalSessions: s.totalSessions }); })
                          .catch(() => {});
                        setPomodoroTodayMinutes(prev => { const next = prev + sessionMins; localStorage.setItem(`pomo_today_${tick.user!.uid}`, JSON.stringify({ mins: next, date: new Date().toISOString().slice(0, 10) })); return next; });
                        const prevAllTime = parseInt(localStorage.getItem(POMO_ALLTIME_KEY) || '0', 10);
                        localStorage.setItem(POMO_ALLTIME_KEY, String(prevAllTime + sessionMins));
                        const newCount = tick.sessionCount + 1;
                        setPomodoroSessionCount(newCount);
                        setPomodoroPhase('break');
                        const nextMode = newCount % 4 === 0 ? 'long' : 'short';
                        setPomodoroMode(nextMode);
                        pomodoroLastDbWriteRef.current = 0;
                        pomodoroLastCacheMinRef.current = 0;
        ...
        // Screen unlock (page survived — pagehide never fired).
        //
        // IMPORTANT: On mobile, the OS FREEZES setInterval while the screen is
        // locked. When the screen unlocks, the interval resumes and may fire its
        // missed ticks in a burst. The OLD code also subtracted elapsed seconds
        // here — so the elapsed time was counted TWICE (interval burst + manual
        // subtract) → timer ran at 2x speed (17.5 min session finished in ~8.5 min).
        //
        // Fix: compute the precise remaining time from wall-clock elapsed, SET it
        // directly (no subtraction), then CLEAR + RESTART the interval so it begins
        // a clean 1-second tick with zero accumulated drift or burst catch-up.
        if (hiddenAt !== null) {
          const elapsedMs = Date.now() - hiddenAt;
          if (elapsedMs >= POMODORO_RESET_MS) {
            // Been hidden too long → auto-reset
            if (pomodoroRef.current) clearInterval(pomodoroRef.current);
            setPomodoroActive(false);
            setPomodoroMode('focus');
            ...
            // Clear the old interval BEFORE touching state so the burst ticks never fire
            if (pomodoroRef.current) clearInterval(pomodoroRef.current);
            // Set exact corrected time from wall clock
            setPomodoroTime(t => Math.max(0, t - elapsedSecs));
            // Restart a clean interval from the corrected value
            pomodoroRef.current = setInterval(() => {
              const tick = pomodoroTickRef.current;
              setPomodoroTime(t => {
                if (t <= 1) {
  ...
  }, [pomodoroActive, savePomodoroToStorage]); // eslint-disable-line react-hooks/exhaustive-deps

  // ── In-app navigation (React Router unmounts this component) ─────────────────
  // Set flag BEFORE unmount fires so visibilitychange knows it's in-app nav.
  useEffect(() => {
    return () => {
      _inAppNavigating = true;
      savePomodoroToStorage(true); // save active:true → auto-resumes on remount
    };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Restore timer state on mount ──────────────────────────────────────────
  // saved.active=true  → timer was running when saved (in-app nav OR screen lock
  //                      while page survived) → auto-resume
  // saved.active=false → pagehide fired (page was destroyed: other site / quit)
  //                      → restore paused, student resumes manually
  useEffect(() => {
    if (pomodoroRestoredRef.current) return;
    pomodoroRestoredRef.current = true;
    _inAppNavigating = false; // always clear on fresh mount
    ...
    try {
      ...
      // Too old → reset
      if (elapsed >= POMODORO_RESET_MS / 1000) {
        localStorage.removeItem('pomo_state');
        return;
      }

      // Fast-forward elapsed time if it was running when saved
      const adjustedTime = saved.active
        ? Math.max(0, saved.time - elapsed)
        : saved.time;

      if (saved.mode === 'custom') POMODORO_DURATIONS.custom = adjustedTime || saved.time;
      ...
      if (saved.active && adjustedTime > 0 && saved.phase !== 'idle') {
        // Was running → auto-resume (covers in-app nav + screen-lock-then-reload)
        setPomodoroActive(true);
        const isFocusSession = saved.phase === 'focus' ||
          saved.mode === 'focus' ||
          (saved.mode === 'custom' && saved.customType === 'focus');
        if (isFocusSession && saved.soundUrl) {
          setTimeout(() => {
            // Don't restart if module-level audio is already playing (in-app nav)
            if (ambientAudioRef.current && !ambientAudioRef.current.paused) return;
            const audio = new Audio();
            audio.loop = true;
            audio.volume = saved.soundVolume ?? 0.4;
            const isCorsUrl = /raw\.githubusercontent\.com|cdn\.|cdnjs\.|r2\.dev|storage\.googleapis\.com/.test(saved.soundUrl);
            if (isCorsUrl) audio.crossOrigin = 'anonymous';
            audio.src = saved.soundUrl;
            audio.play().catch(() => {});
            ambientAudioRef.current = audio;
            try {
              if ('mediaSession' in navigator) {
      ...
      // saved.active=false → page was destroyed → restore paused, student resumes manually
    } catch { localStorage.removeItem('pomo_state'); }
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  useEffect(() => {
    if (pomodoroActive) {
      pomodoroRef.current = setInterval(() => {
        // Read latest values from ref — avoids stale-closure bug where the interval
        // captures old pomodoroMode/sessionCount from the render it was created in.
        // Previously pomodoroMode was in the dep array, causing the interval to be
        // recreated on every mode change which could stack ticks and run faster than
        // real time.
        const tick = pomodoroTickRef.current;
        ...
        setPomodoroTime(t => {
          if (t <= 1) {
            ...
            if (tick.mode === 'focus' && tick.user) {
              ...
              studyPlanService.savePomodoroSession({
                studentId: tick.user.uid,
                subject: tick.subject || 'General',
                startTime: sessionStart,
                duration: sessionMins,
                completed: true,
                notes: tick.linkedEvent ? `Linked to event: ${tick.linkedEvent}` : '',
              }).then(() => studyPlanService.getStreak(tick.user!.uid))
                .then(s => {
              ...
              setPomodoroTodayMinutes(prev => {
                const next = prev + sessionMins;
                localStorage.setItem(`pomo_today_${tick.user!.uid}`, JSON.stringify({ mins: next, date: new Date().toISOString().slice(0, 10) }));
                return next;
              });

              // 3. Update all-time minutes in localStorage
              const prevAllTime = parseInt(localStorage.getItem(POMO_ALLTIME_KEY) || '0', 10);
              localStorage.setItem(POMO_ALLTIME_KEY, String(prevAllTime + sessionMins));

              // 4. Increment sitting session count
              const newCount = tick.sessionCount + 1;
              setPomodoroSessionCount(newCount);

              // 5. Auto-progress linked event: every focus session = 25% → mark full after 4 sessions
              if (tick.linkedEvent) {
                const linkedEv = events.find(e => e.id === tick.linkedEvent);
              ...
              // 6. After every 4 focus sessions, auto-suggest long break
          ...
          // ── Incremental writes every minute (focus sessions only) ───────────
          if (tick.mode === 'focus' && tick.user) {
            const totalSecs    = POMODORO_DURATIONS[tick.mode];
            const elapsedSecs  = totalSecs - t + 1;
            const elapsedMins  = Math.floor(elapsedSecs / 60);

            // Every 5-minute boundary → write a partial session to Firestore
            if (elapsedMins > 0 && elapsedMins % 5 === 0 && elapsedMins !== pomodoroLastDbWriteRef.current) {
              pomodoroLastDbWriteRef.current = elapsedMins;
              const sessionStart = tick.startTime || new Date(Date.now() - elapsedMins * 60 * 1000);
              studyPlanService.savePomodoroSession({
                studentId: tick.user.uid,
                subject: tick.subject || 'General',
                startTime: sessionStart,
                duration: 5, // write 5 min increment
                completed: false,
                notes: `[partial] ${elapsedMins}min elapsed`,
              }).catch(() => {});
            }

            // Every 60-second boundary → update today's cache
            if (elapsedMins > 0 && elapsedMins !== pomodoroLastCacheMinRef.current) {
              pomodoroLastCacheMinRef.current = elapsedMins;
              setPomodoroTodayMinutes(prev => {
                // Only add delta since last cache write
                const delta = 1;
              ...
              const prevAllTime = parseInt(localStorage.getItem(POMO_ALLTIME_KEY) || '0', 10);
              localStorage.setItem(POMO_ALLTIME_KEY, String(prevAllTime + 1));
            }
          }
  ...
  // FIX: removed pomodoroMode from dep array. Previously, every mode change (focus→break→focus)
  // triggered a new interval while the old one may still have been mid-tick, causing stacked
  // ticks and a timer that ran faster than real time. All mutable values (mode, sessionCount,
  // subject, etc.) are now read via pomodoroTickRef.current inside the callback so the single
  // interval always sees the latest state without needing to be recreated.

  const startPomodoro = () => {
    if (!pomodoroActive) {
      setPomodoroStartTime(new Date());
      const isCurrentlyFocus = (pomodoroMode as string) === 'focus' ||
        ((pomodoroMode as string) === 'custom' && pomodoroCustomType === 'focus');
      setPomodoroPhase(isCurrentlyFocus ? 'focus' : 'break');
      pomodoroLastDbWriteRef.current = 0;
      pomodoroLastCacheMinRef.current = 0;
      // Only start ambient for focus sessions
      startAmbientSound(isCurrentlyFocus);
    } else {
      stopAmbientSound();
    }
  ...
  const resetPomodoro = (mode: 'focus' | 'short' | 'long' | 'custom') => {
    setPomodoroActive(false);
    stopAmbientSound();
    pomodoroLastDbWriteRef.current = 0;
    pomodoroLastCacheMinRef.current = 0;
    if (mode === 'custom') {
      const secs = pomodoroCustomMinutes * 60;
      POMODORO_DURATIONS.custom = secs;
      setPomodoroMode('custom' as any);
  ...
  const pomodoroDurationForMode = POMODORO_DURATIONS[(pomodoroMode as keyof typeof POMODORO_DURATIONS)] ?? POMODORO_DURATIONS.focus;
  ...
  // All-time minutes: read from localStorage (kept up-to-date by timer effect)
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, fontFamily: "'Outfit',sans-serif" }}>
      ...
      <style>{`
        ...
        /* View tabs — always full width pill row */
        .ssp-view-tabs {
          display: flex;
          width: 100%;
        ...
        .ssp-range-row {
          display: grid;
          grid-template-columns: 28px 1fr 20px 1fr auto auto;
          align-items: center;
          gap: 6px;
          width: 100%;
        }
        @media (max-width: 400px) {
          .ssp-view-tabs button { font-size: 11px !important; padding: 7px 4px !important; gap: 3px !important; }
          .ssp-view-tabs button svg { width: 11px; height: 11px; }
        }
      `}</style>
              ...
              {showFreeTimePanel && createPortal(
                <>
                  ...
                  <div style={{
                    ...
                    {freeTimeMode === 'range' && (() => {
                      const anyOverlap = hasAnyOverlap(freeTimeRanges);
                      return (
                      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '10px 16px 4px', display: 'flex', flexDirection: 'column', gap: 6 }}>
                        {/* Auto-fix banner */}
                        {anyOverlap && (
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, background: 'rgba(239,68,68,0.08)', border: '1px solid rgba(239,68,68,0.25)', borderRadius: 8, padding: '8px 10px' }}>
                            <span style={{ fontSize: 11, color: '#f87171', display: 'flex', alignItems: 'center', gap: 4 }}>
                              ⚠ Some ranges overlap
                            </span>
                            <button
                              onClick={() => setFreeTimeRanges(prev => autoFixRanges(prev))}
                              style={{ fontSize: 11, fontWeight: 700, color: '#fff', background: '#ef4444', border: 'none', borderRadius: 6, padding: '4px 10px', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", whiteSpace: 'nowrap' as const }}>
                              Auto-fix
                            </button>
                          </div>
                        )}
                        {freeTimeRanges.map((r, idx) => {
                          const rowOverlap = freeTimeRanges.some((other, oi) =>
                            oi !== idx && rangesOverlap(r.start, r.end, other.start, other.end)
                          );
                          const mins = Math.max(0, Math.round(rangeMinutes(r.start, r.end) / 60 * 10) / 10);
                          return (
                            <div key={idx} style={{ display: 'grid', gridTemplateColumns: '28px 1fr 20px 1fr 36px 24px', alignItems: 'center', gap: 6, padding: rowOverlap ? '6px 8px' : '4px 0', borderRadius: 8, background: rowOverlap ? 'rgba(239,68,68,0.08)' : 'transparent', border: `1px solid ${rowOverlap ? 'rgba(239,68,68,0.25)' : 'transparent'}`, transition: 'all 0.2s', width: '100%', boxSizing: 'border-box' as const }}>
                              <label style={{ fontSize: 11, color: T.text3, textAlign: 'right' as const }}>From</label>
                              <input type="time" value={r.start}
                                onChange={e => updateRangeAndFix(idx, 'start', e.target.value)}
                                className={inputCls} style={{ ...inputStyle, width: '100%', minWidth: 0, padding: '6px 8px', fontSize: 12 }} />
                              <label style={{ fontSize: 11, color: T.text3, textAlign: 'center' as const }}>To</label>
                              <input type="time" value={r.end}
                                onChange={e => updateRangeAndFix(idx, 'end', e.target.value)}
                                className={inputCls} style={{ ...inputStyle, width: '100%', minWidth: 0, padding: '6px 8px', fontSize: 12 }} />
                              <span style={{ fontSize: 11, color: rowOverlap ? '#f87171' : T.text3, fontWeight: rowOverlap ? 700 : 400, textAlign: 'right' as const, whiteSpace: 'nowrap' as const }}>
                                {rowOverlap ? '⚠' : `${mins}h`}
                              </span>
                              <div style={{ display: 'flex', justifyContent: 'center' }}>
                                {freeTimeRanges.length > 1
                                  ? <button onClick={() => setFreeTimeRanges(prev => prev.filter((_, i) => i !== idx))} style={{ color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 2 }}><X size={13} /></button>
                                  : null}
                              </div>
                            </div>
                          );
                      ...
                      <div style={{ flexShrink: 0, padding: '8px 16px', borderTop: `1px solid ${T.border}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                        <button
                          onClick={() => {
                            ...
                            const slot = nextFreeSlot(freeTimeRanges);
                            if (!slot) return; // no room
                            setFreeTimeRanges(prev => [...prev, slot]);
                            setFreeTimeConfigured(true);
                          }}
                          disabled={effectiveFreeHours >= 24 || !nextFreeSlot(freeTimeRanges)}
                    ...
                    <div style={{ flexShrink: 0, padding: freeTimeMode === 'range' ? '8px 16px 16px' : 16 }}>
                      {(() => {
                        const overlapBlocked = hasAnyOverlap(freeTimeRanges);
                        const exceedsDay = effectiveFreeHours > 24;
                        const blocked = freeTimeSaving || exceedsDay || overlapBlocked;
                        const label = freeTimeSaving ? 'Saving…'
                          : freeTimeSaved ? 'Saved!'
                          : exceedsDay ? 'Total exceeds 24h'
                          : overlapBlocked ? 'Fix overlapping ranges first'
                          : 'Save Free Time';
                        const bg = freeTimeSaved ? '#16a34a'
                          : (exceedsDay || overlapBlocked) ? '#6b7280'
                          : T.primaryColor;
                        return (
                          <button onClick={handleSaveFreeTime} disabled={blocked}
                            style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', background: bg, color: '#fff', border: 'none', borderRadius: 14, fontSize: 14, fontWeight: 600, cursor: blocked ? 'not-allowed' : 'pointer', opacity: blocked ? 0.6 : 1, fontFamily: "'Outfit',sans-serif" }}>
      ...
      {/* ── View Tabs ── */}
      <div className="ssp-view-tabs" style={{ gap: 4, background: T.surface, border: `1px solid ${T.border}`, padding: 4, borderRadius: 16 }}>
        {([
          ...
          <button key={t.id} onClick={() => setView(t.id)} style={{ flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6, padding: '8px 6px', borderRadius: 12, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", background: 'transparent', color: view === t.id ? T.primaryColor : T.text2, transition: 'color 0.15s', whiteSpace: 'nowrap' as const, position: 'relative' as const }}>
            <t.Icon size={13} />{t.label}
            {view === t.id && <span style={{ position: 'absolute', bottom: 2, left: '50%', transform: 'translateX(-50%)', width: 20, height: 2, borderRadius: 2, background: T.gradient }} />}
          </button>
        ))}
      </div>
      ...
      {/* ══════════════════════════════ CALENDAR VIEW ═══════════════════════════ */}
      {view === 'calendar' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, width: '100%', minWidth: 0, overflow: 'hidden' }}>
          <style>{`
          ...
          `}</style>
          <div className="sp-cal-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, width: '100%', minWidth: 0, overflow: 'hidden' }}>
            {/* Left column */}
              ...
              <NoAnimCard>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                  ...
                  <div style={{ display: 'flex', gap: 6 }}>
                    ...
                    <button onClick={() => setShowPomodoroSettings(true)} title="Pomodoro settings" style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: T.surface, color: T.text2, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                      <Settings size={13} />
                    </button>
                  </div>
                </div>
                ...
                <div style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', marginBottom: 14, height: 130, position: 'relative' }}>
                  <svg width="128" height="128" style={{ position: 'absolute', transform: 'rotate(-90deg)' }}>
                    ...
                    <circle cx="64" cy="64" r="54" strokeWidth="8"
                      stroke={(pomodoroMode as string) === 'custom'
                        ? (pomodoroCustomType === 'focus' ? T.primaryColor : '#f59e0b')
                        : pomodoroMode === 'focus' ? T.primaryColor : pomodoroMode === 'short' ? '#10b981' : '#f59e0b'}
                  ...
                  <div style={{ position: 'relative', textAlign: 'center', zIndex: 1 }}>
                    ...
                    <p style={{ fontSize: 11, color: T.text2, margin: '2px 0 6px' }}>
                      {(pomodoroMode as string) === 'custom'
                        ? (pomodoroCustomType === 'focus' ? 'Custom Focus' : 'Custom Break')
                ...
                <div style={{ display: 'flex', marginBottom: 12, borderBottom: `1px solid ${T.divider}` }}>
                  {([
                    ...
                    // Active logic:
                    // - Custom tab highlights when showCustomTimer panel is open
                    // - Other tabs highlight by pomodoroMode ONLY when Custom panel is closed
                    //   (prevents two tabs lighting up simultaneously)
                    const showActive = key === 'custom'
                      ? showCustomTimer
                      : !showCustomTimer && (pomodoroMode as string) === key;
                    return (
                      <button
                        key={key}
                        onClick={() => {
                          if (key === 'custom') {
                            // Toggle custom panel — never resets the timer
                            setShowCustomTimer(p => !p);
                          } else {
                            setShowCustomTimer(false);
                            // Only reset if timer is NOT currently running
                            if (!pomodoroActive) resetPomodoro(key);
                            // If active: do nothing — don't touch mode or time
                          }
                        }}
                        style={{
                          flex: 1, padding: '8px 0', border: 'none', cursor: pomodoroActive && key !== 'custom' ? 'default' : 'pointer',
                          fontFamily: "'Outfit',sans-serif", fontSize: 12, fontWeight: 600,
                          background: 'transparent',
                          color: showActive ? T.primaryColor : T.text3,
                          transition: 'color 0.15s',
                          position: 'relative' as const,
                          whiteSpace: 'nowrap' as const,
                          opacity: pomodoroActive && key !== 'custom' && !showActive ? 0.45 : 1,
                        }}
                      >
                        {label}
                        {showActive && (
                          <span style={{
                            ...
                            borderRadius: 2, background: T.gradient,
                          }} />
                        )}
                      </button>
                    );
                ...
                {/* Custom timer panel — only visible when Custom tab active */}
                {showCustomTimer && (
                  <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '12px', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        <label style={{ fontSize: 11, color: T.text2, fontWeight: 600, display: 'block', marginBottom: 4 }}>Duration (min)</label>
                        <input
                      ...
                      <div style={{ flex: 1 }}>
                        <label style={{ fontSize: 11, color: T.text2, fontWeight: 600, display: 'block', marginBottom: 4 }}>Type</label>
                        <select
                          value={pomodoroCustomType}
                          onChange={e => setPomodoroCustomType(e.target.value as 'focus' | 'break')}
                          className={inputCls}
                          style={{ ...inputStyle, marginBottom: 0 }}
                        >
                    ...
                    <button
                      onClick={() => { resetPomodoro('custom'); setShowCustomTimer(true); }}
                      style={{ padding: '8px 0', borderRadius: 10, fontSize: 13, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", background: T.gradient, color: '#fff' }}
                    >
                ...
                <div style={{ display: 'flex', gap: 8, marginTop: 4 }}>
                  ...
                  <button onClick={() => resetPomodoro(pomodoroMode as any)} style={{ width: 44, height: 44, padding: 0, background: T.surface, border: `1px solid ${T.border}`, color: T.text2, borderRadius: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><RotateCcw size={14} /></button>
                  {pomodoroSessionCount > 0 && <button onClick={resetPomodoroSitting} style={{ width: 44, height: 44, padding: 0, background: T.surface, border: `1px solid ${T.border}`, color: T.text2, borderRadius: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}><Trash size={14} /></button>}
                </div>
                ...
                <div style={{ marginTop: 12, paddingTop: 12, borderTop: `1px solid ${T.divider}`, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, textAlign: 'center' }}>
                  {[
                    { v: streak.totalSessions, l: 'Total' },
                    { v: `${pomodoroTodayMinutes}m`, l: 'Today' },
                    { v: allTimeMinutes >= 60 ? `${Math.round(allTimeMinutes / 60)}h` : `${allTimeMinutes}m`, l: 'All time' },
                  ].map(s => (
                ...
                {pomodoroSessionCount > 0 && pomodoroSessionCount % 4 === 0 && (pomodoroMode as string) !== 'long' && (pomodoroMode as string) !== 'custom' && !pomodoroActive && (
                      ...
                      {dayEvents.map(ev => {
                        ...
                        return (
                          <div key={ev.id} style={{
                            ...
                            <div onClick={() => setDetailEvent(ev)} style={{ cursor: 'pointer' }}>
                            {/* ── Row 1: icon + title + badges ── */}
                            <div style={{ padding: '12px 14px 0', display: 'flex', alignItems: 'flex-start', gap: 8 }}>
                              <div style={{ width: 28, height: 28, borderRadius: 8, background: `${borderColor}1a`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                                <Icon size={13} style={{ color: borderColor }} />
                              </div>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <p style={{ fontSize: 13, fontWeight: 700, color: ev.completed ? T.text3 : T.text, margin: 0, lineHeight: 1.35, textDecoration: ev.completed ? 'line-through' : 'none', wordBreak: 'break-word' }}>{ev.title}</p>
                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 5, marginTop: 5 }}>
                                  <span style={{ fontSize: 10, background: `${borderColor}1a`, color: borderColor, padding: '2px 7px', borderRadius: 20, fontWeight: 600, textTransform: 'capitalize' as const }}>{ev.eventType.replace('_',' ')}</span>
                                  {ev.isAIGenerated && <span style={{ fontSize: 10, background: 'rgba(139,92,246,0.15)', color: '#c4b5fd', padding: '2px 7px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 3, fontWeight: 600 }}><Sparkles size={8} />AI</span>}
                                  {isExpired && <span style={{ fontSize: 10, background: 'rgba(239,68,68,0.12)', color: '#fca5a5', padding: '2px 7px', borderRadius: 20, border: '1px solid rgba(239,68,68,0.3)', fontWeight: 600 }}>Expired</span>}
                                </div>
                              </div>
                            </div>
                            {/* ── Row 2: date / time / course ── */}
                            <div style={{ padding: '8px 14px 0', display: 'flex', flexWrap: 'wrap', gap: 10 }}>
                              <span style={{ fontSize: 11, color: T.text2, display: 'flex', alignItems: 'center', gap: 3 }}><CalendarIcon size={10} style={{ color: T.text3 }} />{format(ev.date, 'MMM d, yyyy')}</span>
                              <span style={{ fontSize: 11, color: T.text2, display: 'flex', alignItems: 'center', gap: 3 }}><Clock size={10} style={{ color: T.text3 }} />{ev.startTime}–{ev.endTime}</span>
                              {ev.course && <span style={{ fontSize: 11, color: T.text2, display: 'flex', alignItems: 'center', gap: 3 }}><BookOpen size={10} style={{ color: T.text3 }} />{ev.course}</span>}
                            </div>
                            {/* ── Row 3: description / ai tip ── */}
                            {(ev.description || ev.aiTips?.[0]) && (
                              <div style={{ padding: '6px 14px 0' }}>
                                {ev.description && <p style={{ fontSize: 11, color: T.text2, margin: 0, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 1, WebkitBoxOrient: 'vertical' as any }}>{ev.description}</p>}
                                {ev.aiTips?.[0] && <p style={{ fontSize: 11, color: '#c4b5fd', margin: '3px 0 0' }}>{ev.aiTips[0]}</p>}
                              </div>
                            )}
                            {/* ── Row 4: topics ── */}
                            {renderSessionTopics(ev) && (
                              <div style={{ padding: '4px 14px 0' }}>{renderSessionTopics(ev)}</div>
                            )}
                            {/* ── Row 5: progress bar ── */}
                            {ev.isAIGenerated && ev.isPersonal && !ev.completed && (
                              <div style={{ padding: '10px 14px 0' }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                                  <span style={{ fontSize: 10, color: T.text3, fontWeight: 600 }}>PROGRESS</span>
                                  <span style={{ fontSize: 10, fontWeight: 700, color: (ev.completionPercent||0) > 0 ? '#fbbf24' : T.text3 }}>{(ev.completionPercent||0) > 0 ? `${ev.completionPercent}%` : 'Not started'}</span>
                                </div>
                                <div style={{ display: 'flex', gap: 3, height: 6, borderRadius: 6, overflow: 'hidden', cursor: 'pointer' }}>
                                  {[25,50,75,100].map(pct => (
                                    <div key={pct} onClick={() => handleUpdateCompletionPercent(ev, pct)} title={pct === 100 ? 'Mark complete' : `Mark ${pct}% done`} style={{ flex: 1, borderRadius: 3, transition: 'background 0.15s', background: (ev.completionPercent||0) >= pct ? (pct===100?'#10b981':'#f59e0b') : T.divider }} />
                                  ))}
                                </div>
                                <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 10, color: T.text3, marginTop: 3, padding: '0 2px' }}>
                            ...
                            <div style={{ padding: '10px 14px 12px', display: 'flex', alignItems: 'center', gap: 8, borderTop: `1px solid ${T.divider}`, marginTop: 10 }}>
                              <button
                                onClick={() => handleToggleComplete(ev)}
                                title={ev.completed ? 'Mark incomplete' : 'Mark complete'}
                                style={{
                                  flex: (ev.isPersonal && ev.studentId === user?.uid) ? 'unset' : 1,
                                  display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                                  padding: '7px 14px', borderRadius: 10, border: 'none', cursor: 'pointer',
                                  fontFamily: "'Outfit',sans-serif", fontSize: 12, fontWeight: 600,
                                  background: ev.completed ? 'rgba(16,185,129,0.15)' : T.btnSecBg,
                                  color: ev.completed ? '#4ade80' : T.text2,
                                }}
                              >
                                <CheckCircle2 size={13} />{ev.completed ? 'Done' : 'Complete'}
                              </button>
                              {ev.isPersonal && ev.studentId === user?.uid && (<>
                                <button
                                  onClick={() => { setEditingEvent(ev); setShowModal(true); }}
                                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '7px 14px', borderRadius: 10, border: `1px solid ${T.btnSecBorder}`, cursor: 'pointer', background: T.btnSecBg, color: T.text2, fontFamily: "'Outfit',sans-serif", fontSize: 12, fontWeight: 600 }}
                                >
                                  <Edit size={12} />Edit
                                </button>
                                <button
                                  onClick={() => handleDelete(ev.id)}
                                  style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '7px 14px', borderRadius: 10, border: '1px solid rgba(239,68,68,0.20)', cursor: 'pointer', background: 'rgba(239,68,68,0.08)', color: '#f87171', fontFamily: "'Outfit',sans-serif", fontSize: 12, fontWeight: 600 }}
                                >
                              ...
                              <div style={{ marginBottom: 10 }}>
                                <div style={{ display: 'flex', height: 28, borderRadius: 8, overflow: 'hidden', gap: 2 }}>
                                  {[25,50,75,100].map((p, i) => {
                                    const active = g.currentProgress >= p;
                                    const isLast = p === 100;
                                    return (
                                      <div key={p} onClick={() => handleUpdateGoalProgress(g.id, p)}
                                        title={`Set progress to ${p}%`}
                                        style={{
                                          flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center',
                                          cursor: 'pointer', fontSize: 11, fontWeight: 700, userSelect: 'none' as const,
                                          transition: 'background 0.2s, color 0.2s',
                                          background: active
                                            ? isLast ? '#10b981' : `rgba(${T.pRgb},${0.55 + i * 0.12})`
                                            : T.isLight ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.06)',
      ...
      {view === 'list' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, width: '100%', minWidth: 0, overflow: 'hidden' }}>
          ...
          <div className="sp-list-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, width: '100%', minWidth: 0, overflow: 'hidden' }}>
            <NoAnimCard>
              {/* Sub-tabs row */}
              <div style={{ display: 'flex', width: '100%', borderBottom: `1px solid ${T.border}`, marginBottom: 0, alignItems: 'center', gap: 0 }}>
                {[{id:'upcoming',label:'Upcoming',Icon:ListTodo},{id:'history',label:'History',Icon:History}].map(t => (
                  <button key={t.id} onClick={() => setListSubView(t.id as any)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '10px 18px', fontSize: 13, fontWeight: 600, border: 'none', borderBottom: listSubView === t.id ? `2px solid ${T.primaryColor}` : '2px solid transparent', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", background: 'transparent', color: listSubView === t.id ? T.primaryColor : T.text2, transition: 'color 0.15s, border-color 0.15s', marginBottom: -1, whiteSpace: 'nowrap' as const }}>
                    <t.Icon size={12} />{t.label}
                  </button>
                ))}
              </div>
              {/* History search — full width below tab bar */}
              {listSubView === 'history' && (
                <div style={{ position: 'relative', paddingTop: 14 }}>
                  <Search size={13} style={{ position: 'absolute', left: 12, top: '50%', transform: 'translateY(5px)', color: T.text3, pointerEvents: 'none' }} />
                  <input
                    value={historySearch}
                    onChange={e => setHistorySearch(e.target.value)}
                    placeholder="Search history…"
                    className={inputCls}
                    style={{ ...inputStyle, paddingLeft: 34, width: '100%', fontSize: 13, boxSizing: 'border-box' as const }}
                  />
                </div>
              )}
                  ...
                  {upcomingEvents.length === 0 ? (
                    ...
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 10, width: '100%', overflow: 'hidden' }}>
                      {upcomingEvents.map(ev => {
                        ...
                        const Icon = TYPE_ICON[ev.eventType] || CalendarIcon;
                        const pct = ev.completionPercent || 0;
                        const hasProgressBar = ev.isAIGenerated && ev.isPersonal && !ev.completed;
                        const canEdit = ev.isPersonal && ev.studentId === user?.uid;
                        const urgencyColor = d === 0 ? '#f87171' : d <= 3 ? '#fbbf24' : T.text3;
                        const urgencyLabel = d === 0 ? 'Today' : d === 1 ? 'Tomorrow' : `In ${d}d`;
                        return (
                          <div key={ev.id} style={{
                            background: T.cardBg,
                            border: `1px solid ${T.border}`,
                            ...
                            <div style={{ padding: '12px 14px 0', display: 'flex', alignItems: 'flex-start', gap: 8, flexWrap: 'wrap' }}>
                              <div style={{ width: 28, height: 28, borderRadius: 8, background: `${bc}1a`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 1 }}>
                                <Icon size={13} style={{ color: bc }} />
                              </div>
                              <div style={{ flex: 1, minWidth: 0 }}>
                                <p style={{ fontSize: 13, fontWeight: 700, color: T.text, margin: 0, lineHeight: 1.35, wordBreak: 'break-word' }}>{ev.title}</p>
                                <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 5, marginTop: 5 }}>
                                  {ev.isAIGenerated && (
                                    <span style={{ fontSize: 10, background: 'rgba(139,92,246,0.15)', color: '#c4b5fd', padding: '2px 7px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 3, fontWeight: 600 }}>
                                      <Sparkles size={8} />AI
                                    </span>
                                  )}
                                  <span style={{ fontSize: 10, background: `${bc}1a`, color: bc, padding: '2px 7px', borderRadius: 20, fontWeight: 600, textTransform: 'capitalize' as const }}>
                                    {ev.eventType.replace('_', ' ')}
                                  </span>
                                  <span style={{ fontSize: 10, fontWeight: 700, color: urgencyColor, background: `${urgencyColor}18`, padding: '2px 7px', borderRadius: 20 }}>
                                    {urgencyLabel}
                                  </span>
                                </div>
                              </div>
                            ...
                            {/* ── Row 2: date / time / course ── */}
                            <div style={{ padding: '8px 14px 0', display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center' }}>
                              <span style={{ fontSize: 11, color: T.text2, display: 'flex', alignItems: 'center', gap: 3 }}>
                                <CalendarIcon size={10} style={{ color: T.text3 }} />{format(ev.date, 'EEE, MMM d')}
                              </span>
                              {ev.startTime && (
                                <span style={{ fontSize: 11, color: T.text2, display: 'flex', alignItems: 'center', gap: 3 }}>
                                  <Clock size={10} style={{ color: T.text3 }} />{ev.startTime}–{ev.endTime}
                                </span>
                              )}
                              {ev.course && (
                                <span style={{ fontSize: 11, color: T.text2, display: 'flex', alignItems: 'center', gap: 3 }}>
                                  <BookOpen size={10} style={{ color: T.text3 }} />{ev.course}
                                </span>
                              )}
                            </div>
                            ...
                            {/* ── Row 4: progress bar (AI sessions only) ── */}
                            {hasProgressBar && (
                              <div style={{ padding: '10px 14px 0' }}>
                                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 5 }}>
                                  <span style={{ fontSize: 10, color: T.text3, fontWeight: 600 }}>PROGRESS</span>
                                  <span style={{ fontSize: 10, fontWeight: 700, color: pct > 0 ? '#fbbf24' : T.text3 }}>{pct}%</span>
                                </div>
                                <div style={{ display: 'flex', gap: 3, height: 6, borderRadius: 6, overflow: 'hidden', cursor: 'pointer' }}>
                                  {[25,50,75,100].map(p => (
                                    <div
                                      key={p}
                                      onClick={() => handleUpdateCompletionPercent(ev, p)}
                                      title={p === 100 ? 'Mark complete' : `Mark ${p}% done`}
                                      style={{
                                        flex: 1, borderRadius: 3, transition: 'background 0.15s',
                                        background: pct >= p ? (p === 100 ? '#10b981' : '#f59e0b') : T.divider,
                                      }}
                                    />
                                  ))}
                                </div>
                              ...
                              {canEdit && (<>
                                <button
                                  ...
                                  style={{
                                    ...
                                    padding: '7px 14px', borderRadius: 10, border: `1px solid ${T.btnSecBorder}`,
                                    cursor: 'pointer', background: T.btnSecBg, color: T.text2,
                                    fontFamily: "'Outfit',sans-serif", fontSize: 12, fontWeight: 600,
                                    transition: 'background 0.15s',
                                  }}
                                >
                                ...
                                <button
                                  ...
                                  style={{
                                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5,
                                    padding: '7px 14px', borderRadius: 10, border: `1px solid rgba(239,68,68,0.20)`,
      ...
      {view === 'analytics' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(2,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
            {[
              { icon: <CalendarIcon size={13} color="#60a5fa" />, label: 'Total Events',  value: events.length,              sub: 'all time',      accent: '#60a5fa', delay: 0   },
              { icon: <CheckCircle2  size={13} color="#4ade80" />, label: 'Completed',    value: completedCount,             sub: 'sessions done', accent: '#4ade80', delay: 40  },
              { icon: <Clock         size={13} color="#fcd34d" />, label: 'Upcoming',     value: upcomingEvents.length,      sub: 'remaining',     accent: '#fcd34d', delay: 80  },
              { icon: <Target        size={13} color="#c4b5fd" />, label: 'Active Goals', value: goals.length,               sub: 'in progress',   accent: '#c4b5fd', delay: 120 },
              { icon: <Flame         size={13} color="#fb923c" />, label: 'Study Streak', value: `${streak.current}d`,       sub: 'day streak',    accent: '#fb923c', delay: 160 },
              { icon: <Timer         size={13} color="#f9a8d4" />, label: 'Pomodoros',    value: streak.totalSessions,       sub: 'sessions',      accent: '#f9a8d4', delay: 200 },
            ].map((s, i) => (
              <Card key={s.label} accent={s.accent} enterDelay={s.delay} padding="sm">
                <div style={{ display: 'flex', flexDirection: 'column', gap: 5 }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                    {s.icon}
                    <span style={{ fontSize: 9, fontWeight: 700, color: T.text3, textTransform: 'uppercase' as const, letterSpacing: '0.07em' }}>{s.label}</span>
                  </div>
          ...
          <NoAnimCard>
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
              ...
              <span style={{ fontSize: 24, fontWeight: 700, color: T.text, width: 56, textAlign: 'right' as const }}>{completionRate}%</span>
            </div>
      ...
      {view === 'chat' && (
        <NoAnimCard>
          ...
          <div style={{ display: 'flex', gap: 8 }}>
            ...
            <button onClick={() => chatImageInputRef.current?.click()} disabled={chatLoading || chatImageOCRLoading} title="Upload exam routine or class schedule" style={{ width: 44, height: 44, padding: 0, background: T.surface, border: `1px solid ${T.border}`, color: T.text2, borderRadius: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: chatLoading ? 0.4 : 1 }}><Paperclip size={15} /></button>
            ...
            <button onClick={handleSendChat} disabled={(!chatInput.trim() && !chatImageFile) || chatLoading || chatImageOCRLoading || !aiReady} style={{ width: 44, height: 44, padding: 0, background: T.gradient, color: '#fff', border: 'none', borderRadius: 12, cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, opacity: ((!chatInput.trim() && !chatImageFile) || chatLoading || !aiReady) ? 0.5 : 1 }}><Send size={15} /></button>
          </div>
      ...
      {showPomodoroSettings && (
        <ModalShell onClose={() => setShowPomodoroSettings(false)}>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '18px 20px 14px', borderBottom: `1px solid ${T.divider}`, flexShrink: 0 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
              <div style={{ width: 34, height: 34, borderRadius: 10, background: `rgba(${T.pRgb},0.15)`, display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                <Timer size={15} style={{ color: T.primaryColor }} />
              </div>
              <div>
                <h3 style={{ color: T.text, fontWeight: 700, fontSize: 15, margin: 0 }}>Pomodoro Settings</h3>
                <p style={{ color: T.text3, fontSize: 11, margin: '2px 0 0' }}>Sound · volume · timer preferences</p>
              </div>
            </div>
            <button onClick={() => setShowPomodoroSettings(false)} style={{ padding: 6, color: T.text2, background: 'none', border: 'none', cursor: 'pointer', borderRadius: 8, display: 'flex' }}><X size={16} /></button>
          </div>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: T.text, margin: '0 0 10px', display: 'flex', alignItems: 'center', gap: 6 }}>
                <Volume2 size={13} style={{ color: T.primaryColor }} />Ambient Sound
              </p>
              <select
                value={pomodoroSoundUrl ?? '__none__'}
                onChange={e => {
                  const val = e.target.value;
                  const newUrl = val === '__none__' ? null : val;
                  setPomodoroSoundUrl(newUrl);
                  // Save to Firestore immediately
                  if (user?.uid) {
                    const selectedSound = pomodoroSounds.find(s => s.url === newUrl);
                    saveUserPomodoroSettings(user.uid, {
                      selectedSoundId: selectedSound ? selectedSound.id : (newUrl ? null : 'none'),
                      volume: pomodoroSoundVolume,
                    }).catch(() => {});
                  }
                }}
                className={inputCls}
              ...
              >
                <option value="__none__">No Sound — Silent</option>
                {pomodoroSounds.map(sound => (
                  <option key={sound.id} value={sound.url}>
                    {sound.title}{sound.isDefault ? ' (Default)' : ''}
                  </option>
                ))}
              </select>
              {pomodoroSounds.length === 0 && (
                <p style={{ fontSize: 11, color: T.text3, margin: '6px 0 0' }}>No sounds uploaded by admin yet.</p>
              )}
            </div>

            {/* Volume Slider — saves to Firestore debounced */}
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px' }}>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
                <p style={{ fontSize: 12, fontWeight: 700, color: T.text, margin: 0 }}>Volume</p>
                <span style={{ fontSize: 13, fontWeight: 700, color: T.primaryColor }}>{Math.round(pomodoroSoundVolume * 100)}%</span>
              </div>
              <input
                type="range"
                min={0} max={1} step={0.01}
                value={pomodoroSoundVolume}
                onChange={e => {
                  const v = parseFloat(e.target.value);
                  setPomodoroSoundVolume(v);
                  if (ambientAudioRef.current) ambientAudioRef.current.volume = v;
                  // Debounce Firestore save — only write 800ms after user stops dragging
                  if (volumeSaveTimerRef.current) clearTimeout(volumeSaveTimerRef.current);
                  volumeSaveTimerRef.current = setTimeout(() => {
                    if (user?.uid) {
                      const selectedSound = pomodoroSounds.find(s => s.url === pomodoroSoundUrl);
                      saveUserPomodoroSettings(user.uid, {
                        selectedSoundId: selectedSound ? selectedSound.id : (pomodoroSoundUrl ? null : 'none'),
                        volume: v,
                      }).catch(() => {});
                    }
                  }, 800);
                }}
                style={{ width: '100%', accentColor: T.primaryColor, cursor: 'pointer' }}
              />
              <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 4 }}>
                <span style={{ fontSize: 10, color: T.text3 }}>0%</span>
                <span style={{ fontSize: 10, color: T.text3 }}>100%</span>
              </div>
            </div>

            {/* Ambient Music Toggle (only silences music, NOT end-chime) */}
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12 }}>
              <div style={{ flex: 1, minWidth: 0 }}>
                <p style={{ fontSize: 13, fontWeight: 700, color: T.text, margin: 0 }}>Ambient Music</p>
                <p style={{ fontSize: 11, color: T.text3, margin: '3px 0 0', lineHeight: 1.4 }}>
                  {pomodoroAmbientEnabled ? 'Music plays during focus sessions' : 'Music muted — end-chime still plays'}
                </p>
              </div>
              {/* Professional iOS-style toggle */}
              <div
                onClick={() => setPomodoroAmbientEnabled(p => !p)}
                style={{
                  width: 48, height: 28, borderRadius: 14, flexShrink: 0, cursor: 'pointer',
                  background: pomodoroAmbientEnabled ? T.primaryColor : T.isLight ? '#d1d5db' : '#374151',
                  position: 'relative', transition: 'background 0.25s ease',
                  boxShadow: pomodoroAmbientEnabled ? `0 0 0 3px rgba(${T.pRgb},0.20)` : 'none',
                }}
              >
                <span style={{
                  position: 'absolute',
                  top: 3,
                  left: pomodoroAmbientEnabled ? 23 : 3,
      ...
      {detailEvent && createPortal(
        <>
          {/* Backdrop */}
          <div style={{ position: 'fixed', inset: 0, zIndex: 199, background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)', WebkitBackdropFilter: 'blur(4px)' }}
            onClick={() => setDetailEvent(null)} />
          {/* Bottom sheet */}
          <div style={{ position: 'fixed', left: 0, right: 0, bottom: 0, top: 60, zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', pointerEvents: 'none' }}>
            <div style={{
              width: '100%', maxWidth: 576, maxHeight: '92%',
              ...
              {(() => {
                const typeColors: Record<string,string> = { assignment:'#f59e0b', exam:'#ef4444', class:'#6366f1', study_session:'#10b981', deadline:'#ec4899', personal:'#8b5cf6' };
                const bc = typeColors[detailEvent.eventType] || T.text3;
                const Icon = TYPE_ICON[detailEvent.eventType] || CalendarIcon;
                const d = differenceInDays(detailEvent.date, new Date());
                const urgencyColor = d === 0 ? '#f87171' : d <= 3 ? '#fbbf24' : T.text3;
                const urgencyLabel = d === 0 ? 'Today' : d === 1 ? 'Tomorrow' : d < 0 ? 'Past' : `In ${d}d`;
                const pct = detailEvent.completionPercent || 0;
                const hasProgressBar = detailEvent.isAIGenerated && detailEvent.isPersonal && !detailEvent.completed;
                const canEdit = detailEvent.isPersonal && detailEvent.studentId === user?.uid;
                return (
                  <>
                    <div style={{ flexShrink: 0, padding: '10px 16px 14px', borderBottom: `1px solid ${T.border}` }}>
                      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', gap: 10, minWidth: 0, flex: 1 }}>
                          <div style={{ width: 38, height: 38, borderRadius: 12, background: `${bc}1a`, border: `1px solid ${bc}33`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                            <Icon size={16} style={{ color: bc }} />
                          </div>
                          <div style={{ minWidth: 0 }}>
                            <span style={{ fontSize: 10, fontWeight: 600, color: bc, display: 'block', marginBottom: 2, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>
                              {detailEvent.eventType.replace('_', ' ')}
                            </span>
                            <h2 style={{ fontSize: 15, fontWeight: 700, color: T.text, margin: 0, lineHeight: 1.3, wordBreak: 'break-word' }}>{detailEvent.title}</h2>
                            <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5, marginTop: 6 }}>
                              {detailEvent.isAIGenerated && (
                                <span style={{ fontSize: 10, background: 'rgba(139,92,246,0.15)', color: '#c4b5fd', padding: '2px 8px', borderRadius: 20, display: 'flex', alignItems: 'center', gap: 3, fontWeight: 600 }}>
                                  <Sparkles size={8} />AI Generated
                                </span>
                              )}
                              {detailEvent.completed && (
                                <span style={{ fontSize: 10, background: 'rgba(16,185,129,0.15)', color: '#4ade80', padding: '2px 8px', borderRadius: 20, fontWeight: 600 }}>✓ Completed</span>
                              )}
                              <span style={{ fontSize: 10, fontWeight: 700, color: urgencyColor, background: `${urgencyColor}18`, padding: '2px 8px', borderRadius: 20 }}>{urgencyLabel}</span>
                            </div>
                          </div>
                        </div>
                        <button onClick={() => setDetailEvent(null)} style={{ padding: 6, borderRadius: 8, color: T.text2, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', flexShrink: 0 }}><X size={16} /></button>
                      </div>
                    </div>
                    ...
                    <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '16px 16px 0' }}>
                      ...
                      {/* Date / Time / Course row */}
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10, marginBottom: 14 }}>
                        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '10px 12px' }}>
                          <span style={{ fontSize: 10, color: T.text3, fontWeight: 600, display: 'block', marginBottom: 3, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>Date</span>
                          <span style={{ fontSize: 13, fontWeight: 600, color: T.text, display: 'flex', alignItems: 'center', gap: 5 }}>
                            <CalendarIcon size={12} style={{ color: T.text3 }} />{format(detailEvent.date, 'EEE, MMM d, yyyy')}
                          </span>
                        </div>
                        {detailEvent.startTime && (
                          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '10px 12px' }}>
                            <span style={{ fontSize: 10, color: T.text3, fontWeight: 600, display: 'block', marginBottom: 3, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>Time</span>
                            <span style={{ fontSize: 13, fontWeight: 600, color: T.text, display: 'flex', alignItems: 'center', gap: 5 }}>
                              <Clock size={12} style={{ color: T.text3 }} />{detailEvent.startTime} – {detailEvent.endTime}
                            </span>
                          </div>
                        )}
                        {detailEvent.course && (
                          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '10px 12px', gridColumn: detailEvent.startTime ? 'unset' : 'span 2' }}>
                            <span style={{ fontSize: 10, color: T.text3, fontWeight: 600, display: 'block', marginBottom: 3, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>Course</span>
                            <span style={{ fontSize: 13, fontWeight: 600, color: T.text, display: 'flex', alignItems: 'center', gap: 5 }}>
                              <BookOpen size={12} style={{ color: T.text3 }} />{detailEvent.course}
                            </span>
                          </div>
                        )}
                        {detailEvent.priority && (
                          <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '10px 12px' }}>
                            <span style={{ fontSize: 10, color: T.text3, fontWeight: 600, display: 'block', marginBottom: 3, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>Priority</span>
                            <span style={{ fontSize: 13, fontWeight: 600, color: detailEvent.priority === 'high' ? '#f87171' : detailEvent.priority === 'medium' ? '#fbbf24' : T.text2, textTransform: 'capitalize' as const }}>
                              {detailEvent.priority}
                            </span>
                          </div>
                        )}
                      ...
                      {detailEvent.description && (
                        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '12px 14px', marginBottom: 14 }}>
                          <span style={{ fontSize: 10, color: T.text3, fontWeight: 600, display: 'block', marginBottom: 6, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>Description</span>
                          <p style={{ fontSize: 13, color: T.text2, margin: 0, lineHeight: 1.6 }}>{detailEvent.description}</p>
                        </div>
                      )}

                      {/* AI Tips */}
                      {detailEvent.aiTips && detailEvent.aiTips.length > 0 && (
                        <div style={{ background: 'rgba(139,92,246,0.08)', border: '1px solid rgba(139,92,246,0.20)', borderRadius: 12, padding: '12px 14px', marginBottom: 14 }}>
                          <span style={{ fontSize: 10, color: '#c4b5fd', fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, marginBottom: 8, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>
                            <Sparkles size={10} />AI Tips
                          </span>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                            {detailEvent.aiTips.map((tip, i) => (
                              <div key={i} style={{ display: 'flex', gap: 8, alignItems: 'flex-start' }}>
                                <div style={{ width: 5, height: 5, borderRadius: '50%', background: '#c4b5fd', marginTop: 6, flexShrink: 0 }} />
                                <p style={{ fontSize: 12, color: '#c4b5fd', margin: 0, lineHeight: 1.6 }}>{tip}</p>
                              </div>
                            ))}
                          </div>
                        </div>
                      ...
                      {/* Topics */}
                      {(detailEvent.topicContext?.length > 0 || detailEvent.topicNames?.length > 0) && (
                        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '12px 14px', marginBottom: 14 }}>
                          <span style={{ fontSize: 10, color: T.text3, fontWeight: 600, display: 'flex', alignItems: 'center', gap: 5, marginBottom: 8, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>
                            <Tag size={10} />Topics
                          </span>
                          {renderSessionTopics(detailEvent)}
                        </div>
                      )}
                      ...
                      {hasProgressBar && (
                        <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '12px 14px', marginBottom: 14 }}>
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                            <span style={{ fontSize: 10, color: T.text3, fontWeight: 600, textTransform: 'uppercase' as const, letterSpacing: '0.04em' }}>Progress</span>
                            <span style={{ fontSize: 12, fontWeight: 700, color: pct > 0 ? '#fbbf24' : T.text3 }}>{pct > 0 ? `${pct}% done` : 'Not started'}</span>
                          </div>
                          <div style={{ display: 'flex', gap: 4, height: 10, borderRadius: 8, overflow: 'hidden', cursor: 'pointer', marginBottom: 6 }}>
                            {[25,50,75,100].map(p => (
                              <div key={p} onClick={() => { handleUpdateCompletionPercent(detailEvent, p); setDetailEvent(ev => ev ? { ...ev, completionPercent: (ev.completionPercent||0) === p ? 0 : p, completed: ((ev.completionPercent||0) === p ? 0 : p) === 100 } : ev); }}
                                title={p === 100 ? 'Mark complete' : `Mark ${p}% done`}
                                style={{ flex: 1, borderRadius: 4, transition: 'background 0.15s', background: pct >= p ? (p===100?'#10b981':'#f59e0b') : T.divider, cursor: 'pointer' }}
                              />
                            ))}
                          </div>
                          <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: T.text3, padding: '0 2px' }}>
                            <span>25%</span><span>50%</span><span>75%</span><span style={{ display: 'flex', alignItems: 'center' }}><CheckCircle2 size={10} /></span>
                          </div>
                        </div>
                      )}

                      {/* Spacer so buttons don't overlap last content */}
                      <div style={{ height: 16 }} />
                    </div>

                    {/* ── Footer actions (never scrolls) ── */}
                    <div style={{ flexShrink: 0, padding: '12px 16px 20px', borderTop: `1px solid ${T.border}`, display: 'flex', gap: 8 }}>
                      <button
                        onClick={() => { handleToggleComplete(detailEvent); setDetailEvent(ev => ev ? { ...ev, completed: !ev.completed, completionPercent: !ev.completed ? 100 : 0 } : ev); }}
                        style={{
                          flex: 1, display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                          padding: '10px 0', borderRadius: 12, border: 'none', cursor: 'pointer',
                          fontFamily: "'Outfit',sans-serif", fontSize: 13, fontWeight: 600,
                          background: detailEvent.completed ? 'rgba(16,185,129,0.15)' : T.gradient,
                          color: detailEvent.completed ? '#4ade80' : '#fff',
                        }}
                      >
                        <CheckCircle2 size={14} />{detailEvent.completed ? 'Mark Incomplete' : 'Mark Complete'}
                      </button>
                      {canEdit && (<>
                        <button
                          onClick={() => { setEditingEvent(detailEvent); setShowModal(true); setDetailEvent(null); }}
                          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '10px 16px', borderRadius: 12, border: `1px solid ${T.btnSecBorder}`, cursor: 'pointer', background: T.btnSecBg, color: T.text2, fontFamily: "'Outfit',sans-serif", fontSize: 13, fontWeight: 600 }}
                        >
                          <Edit size={13} />Edit
                        </button>
                        <button
                          onClick={() => { handleDelete(detailEvent.id); setDetailEvent(null); }}
                          style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 5, padding: '10px 16px', borderRadius: 12, border: '1px solid rgba(239,68,68,0.25)', cursor: 'pointer', background: 'rgba(239,68,68,0.08)', color: '#f87171', fontFamily: "'Outfit',sans-serif", fontSize: 13, fontWeight: 600 }}
                        >
