...
const StudentStudyPlan = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, fontFamily: "'Outfit',sans-serif" }}>
      ...
      {/* ════════════════════════════════ LIST VIEW ═══════════════════════════════ */}
      {view === 'list' && (
        <div style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, width: '100%', minWidth: 0 }}>
          <style>{`@media(min-width:1024px){.sp-list-grid{grid-template-columns:2fr 1fr!important}}`}</style>
          <div className="sp-list-grid" style={{ display: 'grid', gridTemplateColumns: '1fr', gap: 16, width: '100%', minWidth: 0 }}>
            <NoAnimCard>
              {/* Sub-tabs row — full-width tab strip */}
              <div style={{ display: 'flex', width: '100%', borderBottom: `1px solid ${T.border}`, marginBottom: 16, alignItems: 'center', justifyContent: 'space-between', flexWrap: 'wrap', gap: 0 }}>
                <div style={{ display: 'flex', gap: 0 }}>
                  {[{id:'upcoming',label:'Upcoming',Icon:ListTodo},{id:'history',label:'History',Icon:History}].map(t => (
                    <button key={t.id} onClick={() => setListSubView(t.id as any)} style={{ display: 'flex', alignItems: 'center', gap: 5, padding: '8px 18px', fontSize: 13, fontWeight: 600, border: 'none', borderBottom: listSubView === t.id ? `2px solid ${T.primaryColor}` : '2px solid transparent', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", background: 'transparent', color: listSubView === t.id ? T.primaryColor : T.text2, transition: 'color 0.15s, border-color 0.15s', marginBottom: -1, whiteSpace: 'nowrap' as const }}>
                ...
                {listSubView === 'history' && (
                  <div style={{ position: 'relative', padding: '0 0 4px' }}>
                    ...
                    <input value={historySearch} onChange={e => setHistorySearch(e.target.value)} placeholder="Search history…" className={inputCls} style={{ ...inputStyle, paddingLeft: 28, width: 150, padding: '6px 10px 6px 28px', fontSize: 12 }} />
                  </div>
                )}
              </div>
