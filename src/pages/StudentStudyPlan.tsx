...
const StudentStudyPlan = () => {
  ...
  const savePomodoroToStorage = useCallback((forcePaused = false) => {
    ...
    if (s.active || s.phase !== 'idle' || s.sessionCount > 0) {
      localStorage.setItem('pomo_state', JSON.stringify({
        ...
        active: forcePaused ? false : s.active,
  ...
  // ── Bulletproof pause/resume logic ──────────────────────────────────────────
  //
  // PROBLEM: beforeunload is unreliable on Android (Kiwi/Chrome mobile often
  // skips it). So we cannot rely on it to detect "user left the browser".
  //
  // SOLUTION: Two-signal approach using visibilitychange + a localStorage
  // heartbeat. Every second while active, we write the current timestamp to
  // localStorage ('pomo_heartbeat'). On visibilitychange→hidden we also write
  // 'pomo_hidden_at'. On visibilitychange→visible:
  //   • If _inAppNavigating flag is set → in-app nav, don't pause, just catch up.
  //   • Else if hidden time < 10 min → screen lock / quick tab switch → catch up.
  //   • Else (hidden ≥ 10 min) → user left the browser → auto-reset.
  //
  // For tab close / navigate-away / browser quit: when the page reloads fresh,
  // 'pomo_hidden_at' is still in localStorage from the last hidden event.
  // The restore-on-mount code reads it and decides whether to restore paused.
  //
  // SCREEN LOCK (Case 1): visibilitychange hidden → visible, _inAppNavigating=false,
  //   elapsed < 10 min → just fast-forward. Timer + music kept running. ✅
  //
  // IN-APP NAV (Case 2): _inAppNavigating=true set before unmount, cleared
  //   after remount → visibilitychange (if any) skips pause logic. ✅
  //
  // TAB CLOSE / OTHER SITE / BROWSER QUIT (Cases 3 & 4): page is destroyed,
  //   pomo_hidden_at stays in localStorage. On next open, restore-on-mount
  //   sees savedAt + hidden_at gap ≥ threshold → restores paused or resets. ✅

  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Always record when we went hidden
        const hiddenAt = Date.now();
        pomodoroHiddenAtRef.current = hiddenAt;
        // Write to localStorage so next page load can detect "we were hidden"
        localStorage.setItem('pomo_hidden_at', String(hiddenAt));
        // Do NOT pause timer or audio. Keep running.
      } else {
        // Page became visible again
        const hiddenAt = pomodoroHiddenAtRef.current;
        localStorage.removeItem('pomo_hidden_at');

        if (_inAppNavigating) {
          // Came back from in-app navigation — music is still playing via module-level
          // ref, timer kept ticking. Just clear the flag, no action needed.
        ...
        if (hiddenAt !== null) {
          ...
          if (elapsedMs >= POMODORO_RESET_MS) {
            ...
            // Screen lock / quick switch → fast-forward any OS-throttled seconds
  ...
  // ── On in-app nav unmount: set flag + save active state ──────────────────────
  useEffect(() => {
    return () => {
      _inAppNavigating = true; // tell visibilitychange: this is in-app nav
      savePomodoroToStorage(false); // preserve active:true
    };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // ── beforeunload: best-effort pause save (fires on desktop, sometimes mobile) ─
  useEffect(() => {
    const handleBeforeUnload = () => {
      _inAppNavigating = false; // real unload, not in-app nav
      savePomodoroToStorage(true); // force paused
    };
  ...
  useEffect(() => {
    ...
    // Clear _inAppNavigating on fresh mount (page was reloaded, not in-app nav)
    _inAppNavigating = false;

    const raw = localStorage.getItem('pomo_state');
    if (!raw) { localStorage.removeItem('pomo_hidden_at'); return; }
    ...
    try {
      ...
      // Check if pomo_hidden_at indicates page was killed without beforeunload.
      // This happens when Android kills the browser tab or user force-closes.
      // If hidden_at exists and saved.active was true → treat as paused restore.
      const hiddenAtStr = localStorage.getItem('pomo_hidden_at');
      const hiddenAt = hiddenAtStr ? parseInt(hiddenAtStr, 10) : null;
      localStorage.removeItem('pomo_hidden_at');

      // If the page was hidden (browser quit/killed) and timer was "active",
      // we now treat it as paused (active:false). The elapsed time since
      // hiddenAt tells us how long they were gone.
      const wasKilledWhileActive = saved.active && hiddenAt !== null;
      const goneMs = hiddenAt ? (now - hiddenAt) : (elapsed * 1000);
      const goneSecs = Math.floor(goneMs / 1000);

      if (goneMs >= POMODORO_RESET_MS || elapsed * 1000 >= POMODORO_RESET_MS) {
        // Gone too long → full reset
        localStorage.removeItem('pomo_state');
        return;
      }

      // Restore state
      // If killed while active: restore as paused at adjusted time
      const treatAsActive = saved.active && !wasKilledWhileActive;
      const adjustedTime = saved.active
        ? Math.max(0, saved.time - goneSecs)
        : saved.time;
      ...
      // Only auto-resume if: was genuinely active AND page was NOT killed externally
      if (treatAsActive && adjustedTime > 0 && saved.phase !== 'idle') {
        ...
        if (isFocusSession && saved.soundUrl) {
          setTimeout(() => {
            // If audio already playing (returned from in-app nav), don't restart
            ...
            audio.play().catch(() => {});
            ambientAudioRef.current = audio;
            // Suppress OS media-session notification
            try {
              if ('mediaSession' in navigator) {
                navigator.mediaSession.metadata = null;
                navigator.mediaSession.playbackState = 'none';
      ...
      // else: restore in paused state — student resumes manually
