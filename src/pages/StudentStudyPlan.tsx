        ...
        <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: 8 }}>
            ...
            {aiReady && goals.length > 0 && (
              <div style={{ position: 'relative' }} ref={freeTimePanelRef}>
                <button onClick={() => setShowFreeTimePanel(p => !p)} style={{ ...btnSec, gap: 6 }} title="Set your free time">
                  <Clock size={12} style={{ color: T.text3 }} />
                  <span style={{ fontSize: 12, color: T.text2 }}>Free time:</span>
                  <span style={{ fontSize: 12, fontWeight: 700, color: !freeTimeConfigured ? '#fbbf24' : T.text }}>
                    {!freeTimeConfigured ? 'Set free time ▸'
                      : freeTimeMode === 'range'
                  ...
                  <ChevronDown size={11} style={{ color: T.text3, transform: showFreeTimePanel ? 'rotate(180deg)' : 'none', transition: 'transform 0.15s' }} />
                </button>

                {showFreeTimePanel && (
                  <div style={{ position: 'absolute', right: 0, top: '100%', marginTop: 8, zIndex: 40, width: 300, maxWidth: 'calc(100vw - 1rem)', background: T.baseBg, border: `1px solid ${T.border}`, borderRadius: 18, boxShadow: '0 12px 40px rgba(0,0,0,0.4)', padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 12 }}>
                    <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                      <p style={{ fontSize: 12, fontWeight: 600, color: T.text2, margin: 0 }}>When are you free each day?</p>
                      <button onClick={() => setShowFreeTimePanel(false)} style={{ color: T.text3, background: 'none', border: 'none', cursor: 'pointer', padding: 4 }}><X size={13} /></button>
                    </div>
                    ...
                    <button onClick={handleSaveFreeTime} disabled={freeTimeSaving} style={{ ...btnPrimary, justifyContent: 'center', padding: '9px 0', fontSize: 12, opacity: freeTimeSaving ? 0.6 : 1 }}>
                      {freeTimeSaving ? <Loader size={12} className="animate-spin" /> : freeTimeSaved ? <CheckCircle2 size={12} /> : <Download size={12} />}
            ...
            {goals.length > 0 && (
              <button onClick={hasExistingAIPlan ? handleOpenReschedule : handleGenerateSchedule} disabled={scheduleLoading}
                style={{ ...btnPrimary, background: hasExistingAIPlan ? 'linear-gradient(135deg,#d97706,#ea580c)' : 'linear-gradient(135deg,#7c3aed,#6366f1)', opacity: scheduleLoading ? 0.6 : 1, cursor: scheduleLoading ? 'not-allowed' : 'pointer' }}>
            ...
            <button onClick={() => { setEditingEvent(null); setShowModal(true); }} style={btnPrimary}>
              <Plus size={14} /> Add Event
            </button>
            {helpVideoUrl && (
              <button onClick={handleOpenHelpVideo} style={btnSec} title="Watch the guide video">
                <Video size={14} style={{ color: T.primaryColor }} /> Watch Guide
              </button>
            )}
            {events.length > 0 && (
              <button onClick={handleExportICS} style={btnSec} title="Export to .ics">
