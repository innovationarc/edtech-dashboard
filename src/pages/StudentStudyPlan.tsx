...
const StudentStudyPlan = () => {
  ...
  const startAmbientSound = () => {
    if (!pomodoroSoundEnabled || !pomodoroSoundUrl) return;
    try {
      ...
      // Only set crossOrigin for GitHub raw / CDN URLs that support CORS.
      // GDrive direct-download URLs block CORS preflight — leave it unset.
      ...
      audio.play().catch(() => {
        // Autoplay policy — will start on next user gesture; mark as pending
        ...
        <ModalShell onClose={() => setShowPomodoroSettings(false)}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: T.text, margin: '0 0 4px', display: 'flex', alignItems: 'center', gap: 6 }}><Volume2 size={13} style={{ color: T.primaryColor }} />Ambient Sound</p>
              {pomodoroSoundUrl ? (
                <p style={{ fontSize: 12, color: T.text2, margin: 0 }}>
                  {pomodoroSounds.find(s => s.url === pomodoroSoundUrl)?.title ?? 'Custom sound'} · playing during focus
                </p>
              ) : (
                <p style={{ fontSize: 12, color: T.text3, margin: 0 }}>No sound selected — timer runs silently</p>
              )}
            </div>

            {/* Volume display */}
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px' }}>
              <p style={{ fontSize: 12, fontWeight: 700, color: T.text, margin: '0 0 8px' }}>Volume: {Math.round(pomodoroSoundVolume * 100)}%</p>
              <div style={{ height: 6, background: T.divider, borderRadius: 4, overflow: 'hidden' }}>
                <div style={{ height: '100%', width: `${pomodoroSoundVolume * 100}%`, background: T.gradient, borderRadius: 4 }} />
              </div>
              <p style={{ fontSize: 11, color: T.text3, margin: '6px 0 0' }}>Adjust volume in Settings → Pomodoro Settings</p>
            </div>

            {/* Quick sound toggle */}
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px', display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
              <div>
                <p style={{ fontSize: 12, fontWeight: 700, color: T.text, margin: 0 }}>Sound enabled</p>
                <p style={{ fontSize: 11, color: T.text3, margin: '2px 0 0' }}>Toggle ambient + completion sounds</p>
              </div>
              <button
                onClick={() => setPomodoroSoundEnabled(p => !p)}
                style={{
                  width: 42, height: 24, borderRadius: 12, border: 'none', cursor: 'pointer',
                  background: pomodoroSoundEnabled ? T.primaryColor : T.divider,
                  position: 'relative', transition: 'background 0.2s', flexShrink: 0,
                }}
              >
                <span style={{
                  position: 'absolute', top: 3, left: pomodoroSoundEnabled ? 20 : 3,
            ...
            <p style={{ fontSize: 12, color: T.text3, margin: 0, textAlign: 'center' }}>
              To change sound or volume, go to{' '}
              <button
                onClick={() => {
                  setShowPomodoroSettings(false);
                  // Navigate to settings page — use window.location for SPA
                  const settingsLink = document.querySelector<HTMLElement>('[data-nav-settings]');
                  if (settingsLink) { settingsLink.click(); }
                  else { window.location.hash = '#/settings'; }
                }}
                style={{ color: T.primaryColor, background: 'none', border: 'none', cursor: 'pointer', fontWeight: 700, fontSize: 12, padding: 0, textDecoration: 'underline', fontFamily: "'Outfit',sans-serif" }}
              >
