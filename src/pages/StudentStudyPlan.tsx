...
const StudentStudyPlan = () => {
  ...
  // Deadline-based: stores wall-clock ms when timer should reach 0.
  ...
  const savePomodoroToStorage = useCallback((saveAsActive: boolean) => {
    ...
    if (s.active || s.phase !== 'idle' || s.sessionCount > 0) {
      localStorage.setItem('pomo_state', JSON.stringify({
        ...
        savedAt: Date.now(),
        deadline: pomodoroDeadlineRef.current,
  ...
  // ── Save + restore timer state across navigation / page reload ───────────────
  // On unmount (in-app nav): save state including the deadline timestamp.
  // On mount: restore state; if it was running, restore the deadline so the
  // deadline-based interval picks up exactly where it left off.
  useEffect(() => {
    return () => { savePomodoroToStorage(true); };
  ...
  useEffect(() => {
    ...
    try {
      ...
      // Compute remaining from the saved deadline (wall-clock accurate)
      const remaining = saved.deadline
        ? Math.max(0, Math.ceil((saved.deadline - Date.now()) / 1000))
        : saved.time;

      if (remaining <= 0) { localStorage.removeItem('pomo_state'); return; }

      if (saved.mode === 'custom') POMODORO_DURATIONS.custom = saved.time;
      ...
      if (saved.active && saved.phase !== 'idle') {
        // Restore deadline so the interval reads wall-clock correctly
        pomodoroDeadlineRef.current = saved.deadline ?? (Date.now() + remaining * 1000);
        setPomodoroActive(true);
        const isFocusSession = saved.phase === 'focus' || saved.mode === 'focus' ||
  ...
  // ── Deadline-based Pomodoro tick ─────────────────────────────────────────────
  // Instead of counting down 1 second per tick (which drifts when the screen locks
  // because the OS freezes setInterval), we store the absolute wall-clock deadline
  // (Date.now() + remainingMs) when the timer starts. Every tick just reads
  // deadline - Date.now(). Screen lock, tab switch, burst ticks — all irrelevant.
  useEffect(() => {
    if (!pomodoroActive) {
    ...
    // Set (or restore) deadline from current displayed time
    if (!pomodoroDeadlineRef.current) {
      pomodoroDeadlineRef.current = Date.now() + pomodoroTime * 1000;
    }

    pomodoroRef.current = setInterval(() => {
      const remaining = Math.max(0, Math.ceil((pomodoroDeadlineRef.current! - Date.now()) / 1000));
      setPomodoroTime(remaining);
      ...
      if (remaining <= 0) {
        ...
        if (pomodoroMode === 'focus' && user) {
          ...
          const sessionMins = Math.round(POMODORO_DURATIONS[pomodoroMode] / 60) || 25;
          ...
          if (pomodoroLinkedEvent) {
            ...
            if (linkedEv && !linkedEv.completed) {
              handleUpdateCompletionPercent(linkedEv, Math.min(100, (linkedEv.completionPercent || 0) + 25));
            }
          }
      ...
      if (pomodoroMode === 'focus' && user) {
        ...
        if (elapsedMins > 0 && elapsedMins % 5 === 0 && elapsedMins !== pomodoroLastDbWriteRef.current) {
          ...
          studyPlanService.savePomodoroSession({
            ...
            startTime: sessionStart, duration: 5, completed: false,
  ...
  const startPomodoro = () => {
    if (!pomodoroActive) {
      // Starting: set the deadline from current pomodoroTime so the interval
      // always reads from wall clock — never drifts on screen lock.
      pomodoroDeadlineRef.current = Date.now() + pomodoroTime * 1000;
      ...
      // Pausing: clear deadline so it gets recalculated on resume
      pomodoroDeadlineRef.current = null;
      stopAmbientSound();
    }
