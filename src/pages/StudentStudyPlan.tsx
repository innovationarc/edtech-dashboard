        ...
        <div className="ssp-btn-grid">
          ...
          {aiReady && goals.length > 0 ? (
            <div style={{ position: 'relative', gridColumn: 'auto' }}>
                  ...
                  {!freeTimeConfigured ? 'Set ▸'
                    : freeTimeMode === 'range'
                      ? freeTimeRanges.length === 1
                        ? `${effectiveFreeHours}h`
                        : `${effectiveFreeHours}h·${freeTimeRanges.length}r`
                      : `${freeHoursPerDay}h/d`}
                </span>
              ...
              {showFreeTimePanel && (
                <ModalShell onClose={() => setShowFreeTimePanel(false)}>
                  <div style={{ padding: '0 0 4px' }}>
                    {/* Header — mirrors task modal header style */}
                    <div style={{ padding: '16px 16px 14px', borderBottom: `1px solid ${T.divider}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 4 }}>
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10, minWidth: 0 }}>
                          <div style={{ width: 34, height: 34, borderRadius: 12, background: `rgba(${T.pRgb},0.15)`, border: `1px solid rgba(${T.pRgb},0.25)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                          ...
                          <div style={{ minWidth: 0 }}>
                            ...
                            <h2 style={{ fontSize: 14, fontWeight: 700, color: T.text, margin: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' as const }}>Free Time Settings</h2>
                          </div>
                        </div>
                    ...
                    {/* Body — inner content card matching exam task block */}
                    <div style={{ padding: '14px 16px', display: 'flex', flexDirection: 'column', gap: 14 }}>
                      {/* Inner styled card — mirrors secBg from task modal */}
                      <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '18px 16px', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, textAlign: 'center' }}>
                        <div style={{ width: 44, height: 44, background: `rgba(${T.pRgb},0.12)`, border: `1px solid rgba(${T.pRgb},0.22)`, borderRadius: '50%', display: 'flex', alignItems: 'center', justifyContent: 'center' }}>
                          <Clock size={20} style={{ color: T.primaryColor }} />
                        </div>
                        <div>
                          <p style={{ fontWeight: 600, color: T.text, fontSize: 14, margin: '0 0 4px' }}>When are you free each day?</p>
                      ...
                      {/* Mode toggle */}
                      <div style={{ display: 'flex', gap: 6, padding: 4, background: T.divider, borderRadius: 12 }}>
                        {(['hours', 'range'] as const).map(m => (
                          <button key={m} onClick={() => setFreeTimeMode(m)} style={{ flex: 1, fontSize: 12, padding: '7px 0', borderRadius: 9, border: 'none', cursor: 'pointer', fontWeight: 600, fontFamily: "'Outfit',sans-serif", background: freeTimeMode === m ? T.primaryColor : 'transparent', color: freeTimeMode === m ? '#fff' : T.text2, transition: 'all 0.15s' }}>
                      ...
                      {freeTimeMode === 'hours' ? (
                        <div style={{ display: 'flex', alignItems: 'center', gap: 10 }}>
                          <label style={{ fontSize: 13, color: T.text2, flex: 1 }}>Free hours per day:</label>
                        ...
                        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
                          ...
                          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 4 }}>
                            <button onClick={() => setFreeTimeRanges(prev => [...prev, { start: '18:00', end: '20:00' }])} style={{ fontSize: 12, color: T.primaryColor, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4 }}><Plus size={12} /> Add time range</button>
                            <span style={{ fontSize: 11, color: T.text2 }}>= <strong style={{ color: T.text }}>{effectiveFreeHours}h</strong> free</span>
                          </div>
                        </div>
                      )}

                      {/* Save button — matches task modal primary action */}
                      <button onClick={handleSaveFreeTime} disabled={freeTimeSaving} style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '11px 0', background: freeTimeSaved ? 'linear-gradient(135deg,#16a34a,#15803d)' : `linear-gradient(135deg,${T.primaryColor},${T.accentColor})`, color: '#fff', border: 'none', borderRadius: 12, fontSize: 13, fontWeight: 600, cursor: freeTimeSaving ? 'not-allowed' : 'pointer', opacity: freeTimeSaving ? 0.6 : 1, fontFamily: "'Outfit',sans-serif", transition: 'opacity 0.15s' }}>
                        {freeTimeSaving ? <Loader size={14} className="animate-spin" /> : freeTimeSaved ? <CheckCircle2 size={14} /> : <Download size={14} />}
                        {freeTimeSaving ? 'Saving…' : freeTimeSaved ? 'Saved!' : 'Save Free Time'}
                      </button>
                    </div>
                  </div>
                </ModalShell>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              ...
              <NoAnimCard>
                <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 14 }}>
                  ...
                  <button onClick={() => setPomodoroSoundEnabled(p => !p)} title={pomodoroSoundEnabled ? 'Sound on' : 'Sound off'} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: pomodoroSoundEnabled ? `rgba(${T.pRgb},0.15)` : T.surface, color: pomodoroSoundEnabled ? T.primaryColor : T.text3 }}>
                          ...
                          <div key={ev.id} style={{ ...surfaceCard, borderLeft: `4px solid ${borderColor}`, opacity: ev.completed ? 0.65 : 1 }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                                <button onClick={() => handleToggleComplete(ev)} title={ev.completed ? 'Mark incomplete' : 'Mark complete'} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: ev.completed ? 'rgba(16,185,129,0.15)' : T.surface, color: ev.completed ? '#4ade80' : T.text2, display: 'flex' }}><CheckCircle2 size={13} /></button>
                                {ev.isPersonal && ev.studentId === user?.uid && (<>
                                  <button onClick={() => { setEditingEvent(ev); setShowModal(true); }} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: T.surface, color: T.text2, display: 'flex' }}><Edit size={12} /></button>
                                  <button onClick={() => handleDelete(ev.id)} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: T.surface, color: T.text2, display: 'flex' }}><Trash2 size={12} /></button>
                                </>)}
                              </div>
                            </div>
                          </div>
                            ...
                            <>
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8, flexWrap: 'wrap', gap: 6 }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                                  ...
                                  <button onClick={() => handleOpenEditGoal(g)} style={{ padding: 4, color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><Edit size={11} /></button>
                                  <button onClick={() => handleDeleteGoal(g.id)} style={{ padding: 4, color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><Trash2 size={11} /></button>
                                </div>
                              </div>
                            ...
                            {editingActivity?.id === a.id ? (
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                                ...
                                <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0, marginLeft: 8 }}>
                                  <button onClick={() => handleOpenEditActivity(a)} style={{ padding: 6, color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><Edit size={12} /></button>
                                  <button onClick={() => handleDeleteActivity(a.id)} style={{ padding: 6, color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><Trash2 size={13} /></button>
                                </div>
                              </div>
                            )}
                          </div>
                          ...
                          <div key={ev.id} style={{ ...surfaceCard, borderLeft: `4px solid ${bc}` }}>
                            <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 10 }}>
                              ...
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                                ...
                                <button onClick={() => handleToggleComplete(ev)} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: ev.completed ? 'rgba(16,185,129,0.15)' : T.surface, color: ev.completed ? '#4ade80' : T.text2, display: 'flex' }}><CheckCircle2 size={13} /></button>
                                {ev.isPersonal && ev.studentId === user?.uid && (<>
                                  <button onClick={() => { setEditingEvent(ev); setShowModal(true); }} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: T.surface, color: T.text2, display: 'flex' }}><Edit size={12} /></button>
                                  <button onClick={() => handleDelete(ev.id)} style={{ padding: 6, borderRadius: 8, border: 'none', cursor: 'pointer', background: T.surface, color: T.text2, display: 'flex' }}><Trash2 size={12} /></button>
                                </>)}
                  ...
                  <div key={g.id} style={{ marginBottom: 12 }}>
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 4 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                        ...
                        <button onClick={() => handleOpenEditGoal(g)} style={{ padding: 2, color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><Edit size={10} /></button>
                        <button onClick={() => handleDeleteGoal(g.id)} style={{ padding: 2, color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex' }}><Trash2 size={10} /></button>
                      </div>
                    </div>
