...
// ── Module-level audio ref — survives React unmount/remount (in-app navigation) ──
// Stored outside the component so the <audio> element keeps playing when the
// student navigates to another page within the app and comes back.
// React's useRef resets on unmount, but this module variable persists for the
// entire browser session (as long as the JS bundle stays loaded).
...
const StudentStudyPlan = () => {
  ...
  const startAmbientSound = (isFocus = true) => {
    ...
    try {
      ...
      // Suppress OS media-session notification (lock-screen player controls).
      // Without this, Android/iOS shows an "Admin Dashboard" now-playing card.
      try {
        if ("mediaSession" in navigator) {
          navigator.mediaSession.metadata = null;
          const noopActions: MediaSessionAction[] = ["play","pause","stop","seekbackward","seekforward","previoustrack","nexttrack"];
          noopActions.forEach(a => { try { navigator.mediaSession.setActionHandler(a, null); } catch { /* ok */ } });
        }
  ...
  // ── Visibility change: screen-lock aware ────────────────────────────────────
  //
  // Desired behaviour:
  //   • Screen locked (power button) while browser/tab stays open
  //       → visibilitychange fires (hidden=true) but we do NOT pause.
  //         The setInterval + <audio> keep running in the background.
  //         On return we fast-forward any seconds the OS may have throttled.
  //   • User switches to another browser tab / app (tab still open)
  //       → same as above — do NOT pause. Timer keeps running.
  //   • Tab is closed or user navigates to a different website
  //       → beforeunload fires → we save state as PAUSED (active:false).
  //         On next visit the timer restores in paused state.
  //   • User quits the browser entirely
  //       → beforeunload fires → same as above (paused save).
  //   • User navigates within the app (React Router)
  //       → component unmounts → we save state as ACTIVE (active: true).
  //         Audio element is not explicitly stopped so it keeps playing via GC
  //         until the browser decides (effectively keeps running briefly).
  //         On remount the state is restored as running.
  //
  // Key rule: visibilitychange NEVER pauses. Only beforeunload pauses.
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Record timestamp for catch-up calculation on return.
        // Do NOT stop audio or pause the timer.
        ...
        if (hiddenAt !== null) {
          ...
          if (elapsedMs >= POMODORO_RESET_MS) {
            // Hidden for >10 min → auto-reset (e.g. user left for a long time)
            ...
            // Fast-forward: compensate for any seconds setInterval missed
            // while the OS throttled the tab (common on mobile when screen is off)
  ...
  // ── Always-current ref so save callbacks read latest state ──────────────────
   ...
   * forcepaused=true  → saves active:false (used on tab close / browser quit)
   * forcepaused=false → saves active as-is (used on in-app navigation unmount)
   */
  const savePomodoroToStorage = useCallback((forcePaused = false) => {
    ...
    if (s.active || s.phase !== 'idle' || s.sessionCount > 0) {
      localStorage.setItem('pomo_state', JSON.stringify({
        ...
        // If forcePaused (tab close/browser quit), store as not-active so it
        // restores paused. For in-app navigation, preserve running state.
  ...
  // ── Save on in-app navigation (React Router unmounts the component) ──────────
  // Audio is NOT explicitly stopped here — the browser GCs the <audio> element
  // on its own schedule, which usually means it keeps playing briefly.
  // The restored state will restart audio on remount.
  useEffect(() => {
    return () => {
      savePomodoroToStorage(false); // preserve active flag for in-app nav
    };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps

  // ── Save as PAUSED on tab close / page refresh / browser quit ───────────────
  // beforeunload fires when: tab closed, page navigated away (other site),
  // browser quit, or page refreshed. In all these cases we want the timer
  // to restore in a PAUSED state so the student can resume manually.
  useEffect(() => {
    ...
    try {
      ...
      if (elapsed < POMODORO_RESET_MS / 1000) {
        ...
        if (saved.active && adjustedTime > 0 && saved.phase !== 'idle') {
          ...
          // Restart ambient sound if it was a focus session.
          // Use a small timeout so state setters above have flushed before we try to play.
          const isFocusSession = saved.phase === 'focus' ||
            (saved.mode === 'focus') ||
            (saved.mode === 'custom' && saved.customType === 'focus');
          if (isFocusSession && saved.soundUrl) {
            setTimeout(() => {
              // If audio is already playing (student returned from in-app navigation),
              // don't create a new Audio — the module-level element is still running.
              ...
              <NoAnimCard>
                ...
                <div style={{ display: 'flex', marginBottom: 12, borderBottom: `1px solid ${T.divider}` }}>
                  {([
                    ...
                    // Active = running mode OR custom panel is open (for the Custom tab).
                    const showActive = (pomodoroMode as string) === key ||
                      (key === 'custom' && showCustomTimer);
                    return (
                      <button
                        key={key}
                        onClick={() => {
