...
const StudentStudyPlan = () => {
  ...
  /** True if two time ranges [s1,e1) and [s2,e2) overlap */
  const rangesOverlap = (s1: string, e1: string, s2: string, e2: string) => {
    const toMin = (t: string) => { const [h, m] = t.split(':').map(Number); return h * 60 + m; };
    return toMin(s1) < toMin(e2) && toMin(s2) < toMin(e1);
  };

  /** Merge and deduplicate ranges, sort by start time, remove zero-length */
  const mergeRanges = (ranges: { start: string; end: string }[]) => {
    const toMin = (t: string) => { const [h, m] = t.split(':').map(Number); return h * 60 + m; };
    ...
    for (const r of sorted) {
      ...
      if (toMin(r.start) <= toMin(last.end)) {
        merged[merged.length - 1] = { start: last.start, end: toStr(Math.max(toMin(last.end), toMin(r.end))) };
      } else {
        merged.push(r);
      }
    }
  ...
  const nextFreeSlot = (ranges: { start: string; end: string }[]) => {
    ...
    // Try slots starting from 06:00 in 30-min increments
    for (let start = 6 * 60; start <= 22 * 60; start += 30) {
      const end = start + 120;
      if (end > 24 * 60) break;
      const overlaps = sorted.some(r => toMin(r.start) < end && start < toMin(r.end));
      if (!overlaps) return { start: toStr(start), end: toStr(end) };
    }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20, fontFamily: "'Outfit',sans-serif" }}>
                ...
                <>
                  ...
                  <div style={{
                    ...
                    {freeTimeMode === 'range' && (
                      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '10px 16px 4px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                        {freeTimeRanges.map((r, idx) => {
                          // Check if this range overlaps any other range
                          const hasOverlap = freeTimeRanges.some((other, oi) =>
                            oi !== idx && rangesOverlap(r.start, r.end, other.start, other.end)
                          );
                          return (
                          <div key={idx} style={{ display: 'flex', alignItems: 'center', gap: 6, flexWrap: 'wrap', padding: hasOverlap ? '6px 8px' : '0', borderRadius: 8, background: hasOverlap ? 'rgba(239,68,68,0.08)' : 'transparent', border: hasOverlap ? '1px solid rgba(239,68,68,0.25)' : '1px solid transparent', transition: 'all 0.2s' }}>
                            <label style={{ fontSize: 11, color: T.text3, width: 30 }}>From</label>
                            <input type="time" value={r.start}
                              onChange={e => {
                                const newStart = e.target.value;
                                setFreeTimeRanges(prev => {
                                  const updated = prev.map((x, i) => i === idx ? { ...x, start: newStart } : x);
                                  return updated;
                                });
                                setFreeTimeConfigured(true);
                              }}
                              onBlur={() => setFreeTimeRanges(prev => mergeRanges(prev).length < prev.length ? mergeRanges(prev) : prev)}
                              className={inputCls} style={{ ...inputStyle, flex: 1, minWidth: 90, padding: '6px 8px', fontSize: 12 }} />
                            <label style={{ fontSize: 11, color: T.text3 }}>To</label>
                            <input type="time" value={r.end}
                              onChange={e => {
                                const newEnd = e.target.value;
                                setFreeTimeRanges(prev => {
                                  const updated = prev.map((x, i) => i === idx ? { ...x, end: newEnd } : x);
                                  return updated;
                                });
                                setFreeTimeConfigured(true);
                              }}
                              onBlur={() => setFreeTimeRanges(prev => mergeRanges(prev).length < prev.length ? mergeRanges(prev) : prev)}
                              className={inputCls} style={{ ...inputStyle, flex: 1, minWidth: 90, padding: '6px 8px', fontSize: 12 }} />
                            <span style={{ fontSize: 11, color: hasOverlap ? '#f87171' : T.text3, fontWeight: hasOverlap ? 700 : 400 }}>
                              {hasOverlap ? '⚠ overlap' : `${Math.max(0, Math.round(rangeMinutes(r.start, r.end) / 60 * 10) / 10)}h`}
                            </span>
                            {freeTimeRanges.length > 1 && <button onClick={() => setFreeTimeRanges(prev => prev.filter((_, i) => i !== idx))} style={{ color: T.text3, background: 'none', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center' }}><X size={13} /></button>}
                          </div>
                          );
                        })}
                      </div>
                    ...
                    <div style={{ flexShrink: 0, padding: freeTimeMode === 'range' ? '8px 16px 16px' : 16 }}>
                      <button onClick={handleSaveFreeTime} disabled={freeTimeSaving || effectiveFreeHours > 24 || freeTimeRanges.some((r, i) => freeTimeRanges.some((o, oi) => oi !== i && rangesOverlap(r.start, r.end, o.start, o.end)))}
                        ...
                        {freeTimeSaving ? 'Saving…' : freeTimeSaved ? 'Saved!' : effectiveFreeHours > 24 ? 'Total exceeds 24h' : freeTimeRanges.some((r, i) => freeTimeRanges.some((o, oi) => oi !== i && rangesOverlap(r.start, r.end, o.start, o.end))) ? 'Fix overlapping ranges' : 'Save Free Time'}
                      </button>
                    </div>
      ...
      {/* ── View Tabs ── */}
      <div style={{ display: 'flex', gap: 4, background: T.surface, border: `1px solid ${T.border}`, padding: 4, borderRadius: 16, width: '100%' }}>
        {([
