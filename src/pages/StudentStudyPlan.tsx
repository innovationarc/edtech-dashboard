    ...
    const handleVisibilityChange = () => {
      if (document.hidden) {
        ...
        if (_inAppNavigating) {
          ...
          // Fast-forward any OS-throttled seconds
          if (hiddenAt !== null) {
        ...
        // Timer was ticking the whole time. Just fast-forward any throttled secs.
        if (hiddenAt !== null) {
          ...
          if (elapsedMs >= POMODORO_RESET_MS) {
            ...
            // Fast-forward seconds the setInterval may have missed while screen was off
    ...
    if (pomodoroActive) {
      pomodoroRef.current = setInterval(() => {
        setPomodoroTime(t => {
          if (t <= 1) {
            ...
            if (pomodoroMode === 'focus' && user) {
              ...
              setPomodoroTodayMinutes(prev => {
                ...
                localStorage.setItem(`pomo_today_${user.uid}`, JSON.stringify({ mins: next, date: new Date().toISOString().slice(0, 10) }));
                return next;
              });
          ...
          if (pomodoroMode === 'focus' && user) {
            const totalSecs    = POMODORO_DURATIONS[pomodoroMode];
            ...
            if (elapsedMins > 0 && elapsedMins % 5 === 0 && elapsedMins !== pomodoroLastDbWriteRef.current) {
              ...
              const sessionStart = pomodoroStartTime || new Date(Date.now() - elapsedMins * 60 * 1000);
              studyPlanService.savePomodoroSession({
                studentId: user.uid,
                subject: pomodoroSubject || 'General',
                startTime: sessionStart,
