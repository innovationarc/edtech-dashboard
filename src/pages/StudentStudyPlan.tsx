          ...
          {aiReady && goals.length > 0 ? (
            <div style={{ position: 'relative', gridColumn: 'auto' }}>
              ...
              {showFreeTimePanel && createPortal(
                <div className="fixed inset-0 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm" style={{ zIndex: 200 }}
                  onClick={e => { if (e.target === e.currentTarget) setShowFreeTimePanel(false); }}>
                  <NoAnimCard className="w-full sm:max-w-xl !rounded-t-2xl sm:!rounded-2xl shadow-2xl flex flex-col !p-0" style={{ maxHeight: 'calc(100dvh - 70px)', display: 'flex', flexDirection: 'column' }}>
                    {/* Header — exact TaskModal header pattern */}
                    <div className="p-4 flex-shrink-0" style={{ borderBottom: `1px solid ${T.border}` }}>
                      <div className="flex items-start justify-between gap-2 mb-3">
                        <div className="flex items-center gap-2 min-w-0">
                          <div className="w-8 h-8 rounded-xl flex items-center justify-center shrink-0"
                            style={{ background: `${T.primaryColor}18` }}>
                            <Clock size={16} style={{ color: T.primaryColor }} />
                          </div>
                          <div className="min-w-0">
                            <span className="text-xs font-medium" style={{ color: T.primaryColor }}>Study Planner</span>
                            <h2 className="text-sm font-bold truncate" style={{ color: T.text }}>Free Time Settings</h2>
                          </div>
                        </div>
                        <button onClick={() => setShowFreeTimePanel(false)} className="p-1.5 rounded-lg hover:bg-white/10 shrink-0 transition-colors" style={{ color: '#9ca3af' }}>
                          <X size={16} />
                        </button>
                      </div>
                    ...
                    <div className="flex-1 p-4 space-y-4" style={{ overflowY: 'auto', minHeight: 0 }}>
                      {/* Inner content card — exact exam task secBg pattern */}
                      <div className="rounded-xl p-6 text-center space-y-4" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.border}` }}>
                        <Clock size={40} className="mx-auto" style={{ color: T.primaryColor }} />
                        <div>
                          <p className="font-semibold" style={{ color: T.text }}>When are you free each day?</p>
                          <p className="text-sm mt-1" style={{ color: T.text2 }}>Set your daily free time so the AI can schedule study sessions that fit your life.</p>
                        </div>
                      </div>

                      {/* Mode toggle */}
                      <div style={{ display: 'flex', gap: 6, padding: 4, background: T.divider, borderRadius: 10 }}>
                      ...
                      {/* Save button — sticky footer outside scroll */}
                    </div>
                    <div className="flex-shrink-0 p-4 pt-0">
                      <button onClick={handleSaveFreeTime} disabled={freeTimeSaving}
                        className="w-full inline-flex items-center justify-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium text-white"
                        style={{ background: freeTimeSaved ? '#16a34a' : T.primaryColor, opacity: freeTimeSaving ? 0.6 : 1, cursor: freeTimeSaving ? 'not-allowed' : 'pointer', border: 'none' }}>
