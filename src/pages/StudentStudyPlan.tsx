...
const StudentStudyPlan = () => {
  ...
  // Save free time prefs to Firestore on change (debounced via ref)
  const freeTimeSaveRef = useRef<ReturnType<typeof setTimeout> | null>(null);
  useEffect(() => {
    if (!user || !freeTimeConfigured) return;
    if (freeTimeSaveRef.current) clearTimeout(freeTimeSaveRef.current);
    freeTimeSaveRef.current = setTimeout(() => {
      studyPlanService.saveFreeTimePrefs(user.uid, { mode: freeTimeMode, hours: freeHoursPerDay, ranges: freeTimeRanges }).catch(() => {});
    }, 800);
    return () => { if (freeTimeSaveRef.current) clearTimeout(freeTimeSaveRef.current); };
  }, [user, freeTimeMode, freeHoursPerDay, freeTimeRanges]); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  useEffect(() => {
    if (!user) return;
    studyPlanService.getStreakFreeze(user.uid).then(({ count }) => setStreakFreezeCount(count)).catch(() => setStreakFreezeCount(0));
  }, [user]);
  ...
  const handleOpenRecovery = () => {
    ...
    const { sessions, hasSlots } = recoverTodayPlan({
      goals,
      existingEvents: eventsForReschedule,
      customActivities,
      freeTimeMode,
  ...
  const handleAcceptRecovery = async () => {
    ...
    try {
      ...
      // Delete only today's uncompleted AI sessions
  ...
  const handleUseStreakFreeze = async () => {
    ...
    const newCount = await studyPlanService.useStreakFreeze(user.uid);
    setStreakFreezeCount(newCount);
  };
    ...
    <div className="space-y-6">
      ...
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-3">
        {[
          ...
          { label: 'Goals',       value: goals.length,           extra: `${goals.filter(g => g.currentProgress >= 100).length} completed`, cls: 'from-purple-600/20 to-violet-600/20 border-purple-500/20', id: 'goals' },
        ].map(s => (
          <div key={s.label} className={`bg-gradient-to-br ${s.cls} border rounded-2xl p-4 relative`}>
            ...
            {s.id === 'streak' && (
              <button
                ...
                className={`absolute top-3 right-3 flex items-center gap-1 border text-xs px-1.5 py-0.5 rounded-full transition-colors ${streakFreezeCount > 0 ? 'bg-cyan-500/20 border-cyan-500/30 text-cyan-300 hover:bg-cyan-500/30' : 'bg-background-700 border-background-600 text-gray-500 cursor-default'}`}
              >
          ...
          <div className="bg-background-900 border border-background-700 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              <div className="flex items-center gap-3">
                ...
                <div>
                  <h3 className="text-sm font-bold text-white">Recover Today's Plan</h3>
                  <p className="text-xs text-gray-400">Reschedules only today's missed &amp; upcoming sessions</p>
                </div>
              </div>
              <button onClick={() => { setShowRecoveryModal(false); setRecoveryPreview([]); setRecoveryNoSlots(false); }} className="text-gray-400 hover:text-white p-1"><X size={16} /></button>
            </div>
            ...
            {recoveryNoSlots ? (
              <div className="bg-red-500/10 border border-red-500/30 rounded-xl p-4 text-center space-y-2">
                ...
                <p className="text-sm font-semibold text-red-300">No free time slots left today</p>
                <p className="text-xs text-gray-400">There isn't enough time today to fit any sessions. Use the <span className="text-amber-300 font-medium">Replan</span> button to reschedule sessions across all remaining days.</p>
              </div>
            ) : (
              <>
                <p className="text-xs text-gray-400">The following sessions will <span className="text-white font-medium">replace today's uncompleted sessions only</span>. Other days and completed sessions are untouched.</p>
                <div className="max-h-52 overflow-y-auto space-y-2 pr-1">
                  {recoveryPreview.map((s, i) => (
                    <div key={i} className="flex items-center gap-3 p-2.5 bg-background-800 rounded-lg border border-background-700">
                      <BookOpen size={13} className="text-emerald-400 flex-shrink-0" />
                      <div className="flex-1 min-w-0">
                        <p className="text-xs font-medium text-white truncate">{s.title}</p>
                        <p className="text-xs text-gray-400">{s.startTime} – {s.endTime} · {s.durationMins} min</p>
                      </div>
                      <span className={`text-xs px-1.5 py-0.5 rounded font-medium ${s.priority === 'high' ? 'bg-red-500/15 text-red-300' : s.priority === 'medium' ? 'bg-amber-500/15 text-amber-300' : 'bg-gray-500/15 text-gray-400'}`}>{s.priority}</span>
                    </div>
                  ))}
                </div>
                <div className="flex gap-2 pt-1">
                  <button onClick={() => { setShowRecoveryModal(false); setRecoveryPreview([]); }} className="flex-1 bg-background-700 hover:bg-background-600 text-gray-300 py-2.5 rounded-xl text-sm transition-colors">Cancel</button>
                  <button
                    onClick={handleAcceptRecovery}
                    disabled={recoveryApplying}
                    className="flex-1 bg-amber-600 hover:bg-amber-700 text-white py-2.5 rounded-xl text-sm font-semibold transition-all disabled:opacity-50 flex items-center justify-center gap-2"
                  >
      ...
      {showFreezePrompt && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-background-900 border border-background-700 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h3 className="text-base font-bold text-white">Streak Freeze</h3>
                <p className="text-xs text-gray-400 mt-0.5">You have {streakFreezeCount} freeze{streakFreezeCount !== 1 ? 's' : ''} remaining · +1 added monthly (max 10)</p>
              </div>
            </div>
            <p className="text-sm text-gray-300">Use a streak freeze to protect your <span className="font-semibold text-orange-400">{streak.current}-day streak</span> when you miss a day of study.</p>
            <div className="flex gap-2">
              <button
                onClick={handleUseStreakFreeze}
                disabled={streakFreezeCount <= 0}
                className="flex-1 bg-cyan-600 hover:bg-cyan-700 text-white py-2.5 rounded-xl text-sm font-semibold transition-all disabled:opacity-40 flex items-center justify-center gap-2"
              >
                <Snowflake size={14} />Use Freeze
              </button>
              <button onClick={() => setShowFreezePrompt(false)} className="px-4 bg-background-700 hover:bg-background-600 text-gray-300 py-2.5 rounded-xl text-sm transition-colors">Cancel</button>
            </div>
          </div>
        </div>
      )}
      ...
      {celebratingGoal && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/70 backdrop-blur-sm px-4">
          <div className="bg-background-900 border border-yellow-500/30 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4 text-center">
            ...
            <div className="flex justify-center gap-3 mb-2">
              {['text-yellow-400','text-orange-400','text-amber-300'].map((c, i) => (
                <Star key={i} size={22} className={`${c} star-float`} style={{ animationDelay: `${i * 0.2}s`, fill: 'currentColor' }} />
              ))}
            </div>
            <div className="celeb-pop">
              <div className="w-14 h-14 bg-gradient-to-br from-yellow-500 to-orange-500 rounded-full flex items-center justify-center mx-auto mb-3 shadow-lg shadow-yellow-500/30">
                <Award size={24} className="text-white" />
              </div>
            </div>
            <h3 className="text-lg font-bold text-white">Goal Complete! 🎉</h3>
            <p className="text-sm text-gray-300">
              You finished <span className="font-semibold text-yellow-400">{celebratingGoal.subject}</span>. Excellent work — keep the momentum going!
            </p>
            <div className="flex gap-2">
              <button
                onClick={() => { setCelebratingGoal(null); setView('calendar'); }}
                className="flex-1 bg-gradient-to-r from-yellow-600 to-orange-600 hover:from-yellow-700 hover:to-orange-700 text-white py-2.5 rounded-xl text-sm font-semibold transition-all"
              >
                Set New Goal
              </button>
              <button onClick={() => setCelebratingGoal(null)} className="px-4 bg-background-700 hover:bg-background-600 text-gray-300 py-2.5 rounded-xl text-sm transition-colors">Close</button>
            </div>
          </div>
        </div>
      )}
