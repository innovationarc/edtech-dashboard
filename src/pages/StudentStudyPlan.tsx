            ...
            <div style={{ position: 'relative', gridColumn: 'auto' }}>
              ...
              {showFreeTimePanel && createPortal(
                <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}
                  ...
                  {/* Direct styled div — NOT NoAnimCard, which adds an unconstrained wrapper div that breaks height */}
                  <div style={{
                    ...
                    <div style={{ flexShrink: 0, padding: '12px 16px 0', display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {/* Info card */}
                      <div style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.border}`, borderRadius: 12, padding: '14px 16px', textAlign: 'center' }}>
                    ...
                    {/* ── Scrollable time ranges (range mode only, flex: 1 so it fills remaining space) ── */}
                    {freeTimeMode === 'range' && (
                      <div style={{ flex: 1, overflowY: 'auto', minHeight: 0, padding: '10px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>
                    ...
                    <div style={{ flexShrink: 0, padding: 16 }}>
                      <button onClick={handleSaveFreeTime} disabled={freeTimeSaving}
                        style={{ width: '100%', display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 8, padding: '12px 0', background: freeTimeSaved ? '#16a34a' : T.primaryColor, color: '#fff', border: 'none', borderRadius: 14, fontSize: 14, fontWeight: 600, cursor: freeTimeSaving ? 'not-allowed' : 'pointer', opacity: freeTimeSaving ? 0.6 : 1, fontFamily: "'Outfit',sans-serif" }}>
