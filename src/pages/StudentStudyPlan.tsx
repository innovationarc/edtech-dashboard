...
const StudentStudyPlan = () => {
  ...
  const [pomodoroSoundUrl,      setPomodoroSoundUrl]      = useState<string | null>(null); // resolved url to play
  const [pomodoroSoundVolume,   setPomodoroSoundVolume]   = useState(0.5);
  // Separate toggle: controls ambient music only (not the end-of-session chime)
  ...
  const pomodoroHiddenAtRef    = useRef<number | null>(null); // timestamp when tab hidden
  const POMODORO_RESET_MS      = 15 * 60 * 1000;             // 15 min idle → auto-reset
  // Per-minute DB write accumulator
  const pomodoroLastDbWriteRef = useRef<number>(0);           // minutes already written this session
  const pomodoroLastCacheMinRef= useRef<number>(0);           // last time cache written (seconds elapsed)
  // All-time minutes cache key
   ...
   * Uses a standard <audio> element with loop=true so it keeps playing even
   * when the screen is off but the browser/tab is open.
   * The URL is fetched with crossOrigin='anonymous' only when needed — for
   * GDrive direct-download links (uc?export=download) CORS is handled server-
   * side; for GitHub raw URLs it works natively. We never set crossOrigin for
   * opaque (non-CORS) URLs to avoid blocking the request.
   */
  const startAmbientSound = () => {
    // Master switch off → total silence; ambient toggle off → no music (chime still plays via playPomodoroSound)
    if (!pomodoroSoundEnabled || !pomodoroAmbientEnabled || !pomodoroSoundUrl) return;
    // Only play ambient during focus sessions, not breaks
    const isFocusMode = (pomodoroMode as string) === 'focus' || (pomodoroMode as string) === 'custom';
  ...
  // ── Visibility change: pause on tab switch, reset after 15-min idle ────────
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Tab hidden — note the time but keep internal timer running via Page Visibility API
        // Only pause if it's a tab-switch to another site (handled by beforeunload too)
        ...
        if (hiddenAt !== null) {
          ...
          // Else: was away < 15min — timer continued via setInterval (which fires even in bg)
        }
  ...
  // ── Save timer state to localStorage on unload so it persists across navigations ──
  useEffect(() => {
    const handleBeforeUnload = () => {
      if (pomodoroActive) {
        localStorage.setItem('pomo_state', JSON.stringify({
          ...
          linkedEvent: pomodoroLinkedEvent,
          sessionCount: pomodoroSessionCount,
          startTime: pomodoroStartTime?.toISOString() ?? null,
          savedAt: Date.now(),
          soundUrl: pomodoroSoundUrl,
  ...
  }); // run every render so closure captures latest state

  // ── Restore timer state on mount (navigating within the app) ──────────────
  useEffect(() => {
    ...
    try {
      ...
      // Only restore if within the 15-minute reset window
      if (elapsed < POMODORO_RESET_MS / 1000) {
        const adjustedTime = Math.max(0, saved.time - elapsed);
        setPomodoroMode(saved.mode);
        setPomodoroTime(adjustedTime);
        ...
        // Restore sound url/volume if not yet loaded from Firestore
        ...
        if (adjustedTime > 0 && saved.phase !== 'idle') {
  ...
  const startPomodoro = () => {
    if (!pomodoroActive) {
      ...
      setPomodoroPhase(pomodoroMode === 'focus' ? 'focus' : 'break');
      pomodoroLastDbWriteRef.current = 0;
      pomodoroLastCacheMinRef.current = 0;
              ...
              <NoAnimCard>
                ...
                <div style={{ display: 'flex', gap: 6, marginBottom: 12 }}>
                  {(['focus','short','long'] as const).map(m => (
                    <button key={m} onClick={() => resetPomodoro(m)} style={{ flex: 1, padding: '7px 0', borderRadius: 10, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", background: 'transparent', color: pomodoroMode === m ? T.primaryColor : T.text2, transition: 'color 0.15s', position: 'relative' as const }}>
                      {m === 'focus' ? '25 min' : m === 'short' ? '5 min' : '15 min'}
                      {pomodoroMode === m && <span style={{ position: 'absolute', bottom: 1, left: '50%', transform: 'translateX(-50%)', width: 16, height: 2, borderRadius: 2, background: T.gradient }} />}
                    </button>
                  ))}
                  {/* Custom tab */}
                  <button
                    onClick={() => setShowCustomTimer(p => !p)}
                    style={{ flex: 1, padding: '7px 0', borderRadius: 10, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", background: 'transparent', color: (pomodoroMode as string) === 'custom' ? T.primaryColor : T.text2, transition: 'color 0.15s', position: 'relative' as const }}
                  >
                    Custom
                    {(pomodoroMode as string) === 'custom' && <span style={{ position: 'absolute', bottom: 1, left: '50%', transform: 'translateX(-50%)', width: 16, height: 2, borderRadius: 2, background: T.gradient }} />}
                  </button>
                </div>

                {/* Custom timer panel */}
                {showCustomTimer && (
                  <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 12, padding: '10px 12px', marginBottom: 12, display: 'flex', flexDirection: 'column', gap: 10 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                      <div style={{ flex: 1 }}>
                        <label style={{ fontSize: 11, color: T.text2, fontWeight: 600, display: 'block', marginBottom: 4 }}>Duration (minutes)</label>
                        <input
                          type="number"
                          min={1} max={120}
                    ...
                    <button
                      onClick={() => { resetPomodoro('custom'); setShowCustomTimer(false); }}
                      style={{ padding: '7px 0', borderRadius: 10, fontSize: 12, fontWeight: 600, border: 'none', cursor: 'pointer', fontFamily: "'Outfit',sans-serif", background: T.gradient, color: '#fff' }}
                    >
                      Set {pomodoroCustomMinutes} min {pomodoroCustomType}
                    </button>
                  </div>
                )}
        ...
        <ModalShell onClose={() => setShowPomodoroSettings(false)}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <div style={{ background: T.surface, border: `1px solid ${T.border}`, borderRadius: 14, padding: '12px 14px' }}>
              ...
              {/* None option */}
              <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                <label style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '8px 10px', borderRadius: 10, background: !pomodoroSoundUrl ? `rgba(${T.pRgb},0.10)` : 'transparent', border: `1px solid ${!pomodoroSoundUrl ? T.primaryColor : 'transparent'}`, transition: 'all 0.15s' }}>
                  <input type="radio" name="pomo-sound" checked={!pomodoroSoundUrl} onChange={() => setPomodoroSoundUrl(null)} style={{ accentColor: T.primaryColor, width: 15, height: 15, flexShrink: 0 }} />
                  <span style={{ fontSize: 13, color: !pomodoroSoundUrl ? T.text : T.text2, fontWeight: !pomodoroSoundUrl ? 600 : 400 }}>No Sound</span>
                  <span style={{ fontSize: 11, color: T.text3, marginLeft: 'auto' }}>Silent</span>
                </label>
                {pomodoroSounds.map(sound => (
                  <label key={sound.id} style={{ display: 'flex', alignItems: 'center', gap: 10, cursor: 'pointer', padding: '8px 10px', borderRadius: 10, background: pomodoroSoundUrl === sound.url ? `rgba(${T.pRgb},0.10)` : 'transparent', border: `1px solid ${pomodoroSoundUrl === sound.url ? T.primaryColor : 'transparent'}`, transition: 'all 0.15s' }}>
                    <input type="radio" name="pomo-sound" checked={pomodoroSoundUrl === sound.url} onChange={() => setPomodoroSoundUrl(sound.url)} style={{ accentColor: T.primaryColor, width: 15, height: 15, flexShrink: 0 }} />
                    <span style={{ fontSize: 13, color: pomodoroSoundUrl === sound.url ? T.text : T.text2, fontWeight: pomodoroSoundUrl === sound.url ? 600 : 400, flex: 1, minWidth: 0, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{sound.title}</span>
                    {sound.isDefault && <span style={{ fontSize: 10, color: '#fbbf24', background: 'rgba(251,191,36,0.12)', border: '1px solid rgba(251,191,36,0.25)', padding: '2px 6px', borderRadius: 20, flexShrink: 0 }}>Default</span>}
                  </label>
                ))}
                {pomodoroSounds.length === 0 && (
                  <p style={{ fontSize: 12, color: T.text3, margin: 0, padding: '4px 0' }}>No sounds uploaded by admin yet.</p>
                )}
              </div>
            </div>
