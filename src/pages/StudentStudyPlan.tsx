...
const StudentStudyPlan = () => {
  ...
  useEffect(() => {
    return () => {
      ...
      // Stop audio on unmount (will restart via state restore on remount)
      stopAmbientSound();
    };
  }, []); // eslint-disable-line react-hooks/exhaustive-deps
  ...
  // Runs once per mount. Restores state from localStorage after React Router
  // navigation causes this component to unmount and remount.
  useEffect(() => {
    if (pomodoroRestoredRef.current) return; // already restored this mount cycle
    ...
    try {
      ...
      if (elapsed < POMODORO_RESET_MS / 1000) {
        // Adjust time for elapsed seconds only if timer was actively running when saved
        ...
        // Timer was running before nav — re-activate (sound will restart via startAmbientSound)
              ...
              <NoAnimCard>
                ...
                <div style={{ display: 'flex', marginBottom: 12, borderBottom: `1px solid ${T.divider}` }}>
                  {([
                    ...
                    const isActive = pomodoroMode === key || (key === 'custom' && showCustomTimer && pomodoroMode !== 'focus' && pomodoroMode !== 'short' && pomodoroMode !== 'long')
                      || (key === 'custom' && (pomodoroMode as string) === 'custom');
                    const showActive = key === 'custom'
                      ? ((pomodoroMode as string) === 'custom' || showCustomTimer)
                      : pomodoroMode === key;
                    return (
                      <button
                        key={key}
                        onClick={() => {
                          if (key === 'custom') {
                            ...
                            if (!pomodoroActive) resetPomodoro(key);
                              ...
                              // If active, just highlight the tab without resetting
                              setPomodoroMode(key);
                            }
                          }
                        }}
