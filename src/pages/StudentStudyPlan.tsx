        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm px-4">
          <div className="bg-background-900 border border-background-700 rounded-2xl shadow-2xl w-full max-w-md p-6 space-y-4">
            <div className="flex items-center justify-between">
              ...
              <button onClick={() => { setShowRecoveryModal(false); setRecoveryPreview([]); setRecoveryNoSlots(false); setRecoveryUnscheduled([]); }} className="text-gray-400 hover:text-white p-1"><X size={16} /></button>
            </div>
            ...
            {recoveryNoSlots ? (
              ...
              <>
                ...
                {/* Partial coverage warning — some subjects couldn't fit today */}
                {recoveryUnscheduled.length > 0 && (
                  <div className="bg-amber-500/10 border border-amber-500/30 rounded-xl p-3 flex items-start gap-2.5">
                    ...
                    <div>
                      <p className="text-xs font-semibold text-amber-300 mb-1">Not enough time today for all sessions</p>
                      <p className="text-xs text-gray-400">
                        No free slots for <span className="text-amber-300 font-medium">{recoveryUnscheduled.join(', ')}</span> — those sessions will be <span className="text-white font-medium">kept as-is</span>.
                        Use <span className="text-amber-300 font-medium">Replan</span> to redistribute them across upcoming days.
                      </p>
                    </div>
                  </div>
                )}
                ...
                <div className="flex gap-2 pt-1">
                  <button onClick={() => { setShowRecoveryModal(false); setRecoveryPreview([]); setRecoveryUnscheduled([]); }} className="flex-1 bg-background-700 hover:bg-background-600 text-gray-300 py-2.5 rounded-xl text-sm transition-colors">Cancel</button>
                  <button
          ...
          <div className="bg-background-900 border border-background-700 rounded-2xl shadow-2xl w-full max-w-sm p-6 space-y-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                <p className="text-xs text-gray-400 mt-0.5">You have {streakFreezeCount} freeze{streakFreezeCount !== 1 ? 's' : ''} remaining · +2/month · +1/week streak (max 10)</p>
              </div>
            </div>
            {freezeActiveUntil && (
              <div className="bg-cyan-500/10 border border-cyan-500/30 rounded-xl px-3 py-2 flex items-center gap-2">
                <Snowflake size={13} className="text-cyan-400 flex-shrink-0" />
                <p className="text-xs text-cyan-300">Freeze active — streak protected until <strong>{freezeActiveUntil.toLocaleString([], { weekday: 'short', hour: '2-digit', minute: '2-digit' })}</strong></p>
              </div>
            )}
            <p className="text-sm text-gray-300">Use a streak freeze to protect your <span className="font-semibold text-orange-400">{streak.current}-day streak</span> for <span className="font-semibold text-cyan-300">48 hours</span>. One freeze covers up to 2 missed days.</p>
            <div className="text-xs text-gray-500 space-y-0.5">
              <p>🗓 +2 freezes added every month</p>
              <p>🔥 +1 freeze for every 7-day streak with no freeze used</p>
            </div>
            <div className="flex gap-2">
              <button
