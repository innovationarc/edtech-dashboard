...
const StudentStudyPlan = () => {
  ...
  // ── Visibility change: pause on tab/app switch, reset after 10-min idle ────
  useEffect(() => {
    const handleVisibilityChange = () => {
      if (document.hidden) {
        // Tab hidden (switch to another app/tab) — pause timer & sound immediately
        pomodoroHiddenAtRef.current = Date.now();
        if (pomodoroActive) {
          setPomodoroActive(false);
          stopAmbientSound();
          // Persist the paused state so restore works on return
          const saveState = () => {
            const raw = localStorage.getItem('pomo_state');
            if (raw) {
              try {
                const s = JSON.parse(raw);
                localStorage.setItem('pomo_state', JSON.stringify({ ...s, savedAt: Date.now() }));
              } catch { /* silent */ }
            }
          };
          saveState();
        ...
        if (hiddenAt !== null) {
          ...
          if (elapsed >= POMODORO_RESET_MS) {
            // Idle too long — auto-reset, clear saved state
            setPomodoroActive(false);
          ...
          // If < 10 min they can manually resume; timer is paused waiting for them
        }
  ...
  // ── Save timer state: on unmount (React Router nav) AND on browser unload ──
  // This is the key fix: PageTransition unmounts this component on navigation.
  // We save to localStorage on cleanup so state survives the remount.
  ...
  const savePomodoroToStorage = useCallback(() => {
    ...
    if (s.active || s.phase !== 'idle' || s.sessionCount > 0) {
      localStorage.setItem('pomo_state', JSON.stringify({
        ...
        active: s.active, customType: s.customType,
      }));
    } else {
      localStorage.removeItem('pomo_state');
    }
  ...
  // Save on unmount — fires when React Router navigates away
  // IMPORTANT: do NOT stop ambient audio here — the <audio> element is GC'd anyway on unmount,
  // but stopping it explicitly prevents the browser from continuing playback in background tabs.
  // Instead we just save state and let the browser handle audio lifecycle.
  useEffect(() => {
    return () => {
  ...
  // Save on browser tab close / refresh
  useEffect(() => {
    window.addEventListener('beforeunload', savePomodoroToStorage);
    return () => window.removeEventListener('beforeunload', savePomodoroToStorage);
  }, [savePomodoroToStorage]);
      ...
      if (elapsed < POMODORO_RESET_MS / 1000) {
        ...
        if (saved.active && adjustedTime > 0 && saved.phase !== 'idle') {
          ...
          if (isFocusSession && saved.soundUrl) {
            setTimeout(() => {
              // Re-read the flags fresh from refs/state since this runs async
              const audio = new Audio();
              audio.loop = true;
              ...
              <NoAnimCard>
                ...
                <div style={{ display: 'flex', marginBottom: 12, borderBottom: `1px solid ${T.divider}` }}>
                  {([
                    ...
                    // Active = the actual current running mode. Never driven by showCustomTimer.
