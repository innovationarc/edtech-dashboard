          ...
          {aiReady && goals.length > 0 ? (
            <div style={{ position: 'relative', gridColumn: 'auto' }}>
              ...
              {showFreeTimePanel && createPortal(
                <div style={{ position: 'fixed', inset: 0, zIndex: 200, display: 'flex', alignItems: 'flex-end', justifyContent: 'center', background: 'rgba(0,0,0,0.6)', backdropFilter: 'blur(4px)' }}
                  onClick={e => { if (e.target === e.currentTarget) setShowFreeTimePanel(false); }}>
                  <NoAnimCard className="w-full sm:max-w-xl !rounded-t-2xl sm:!rounded-2xl shadow-2xl !p-0" style={{ height: '88dvh', maxHeight: '88dvh', display: 'flex', flexDirection: 'column', overflow: 'hidden' }}>

                    {/* ── Header (fixed, never scrolls) ── */}
                    <div className="flex-shrink-0 p-4" style={{ borderBottom: `1px solid ${T.border}` }}>
                      <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 10 }}>
                        ...
                        <button onClick={() => setShowFreeTimePanel(false)} style={{ padding: 6, borderRadius: 8, color: '#9ca3af', background: 'none', border: 'none', cursor: 'pointer', display: 'flex', flexShrink: 0 }}><X size={16} /></button>
                      </div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', alignItems: 'center', gap: '4px 12px', fontSize: 12, color: '#9ca3af' }}>
                    ...
                    <div className="flex-shrink-0 p-4 pb-0" style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                      {/* Info card */}
                      <div style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${T.border}`, borderRadius: 12, padding: '16px', textAlign: 'center' }}>
                        <Clock size={32} style={{ color: T.primaryColor, margin: '0 auto 10px' }} />
                    ...
                    {/* ── Scrollable time ranges (range mode only) ── */}
                    {freeTimeMode === 'range' && (
                    ...
                    {/* ── Save button (fixed, never scrolls) ── */}
