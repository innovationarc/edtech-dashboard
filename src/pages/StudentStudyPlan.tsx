...
const StudentStudyPlan = () => {
  ...
  useEffect(() => {
    if (!user) return;
    // Small delay to ensure Firestore auth token is ready
    const t = setTimeout(() => {
      studyPlanService.getStreakFreeze(user.uid)
        .then(({ count }) => setStreakFreezeCount(count))
        .catch(() => {
          // Retry once after 2s in case of auth timing issue
          setTimeout(() => {
            studyPlanService.getStreakFreeze(user.uid)
              .then(({ count }) => setStreakFreezeCount(count))
              .catch(() => {});
