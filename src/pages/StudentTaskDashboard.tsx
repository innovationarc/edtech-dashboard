...
// ─── NoAnimCard — same pattern as Settings.tsx, blocks card animations ────────
  ...
  return (
    <div data-no-anim>
      <style>{`
        ...
        .std-task-btn { -webkit-tap-highlight-color: rgba(0,0,0,0) !important; outline: none !important; }
        .std-task-btn:focus, .std-task-btn:active { outline: none !important; background: transparent !important; }
        [data-no-anim] div[style*="shimPass"],
        [data-no-anim] div[style*="skewX"] { animation: none !important; opacity: 0 !important; }
      `}</style>
...
// ─── Simple theme tokens (non-card elements) ──────────────────────────────────
...
const useThemeTokens = () => {
  ...
  return { isLight, primaryColor, accentColor, border, headC, subC };
};

// ─── Kill tap highlight globally for this page ────────────────────────────────
if (typeof document !== 'undefined') {
  const styleId = 'std-task-no-tap';
...
const TaskModal = ({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm">
      <NoAnimCard className="w-full sm:max-w-xl !rounded-t-2xl sm:!rounded-2xl shadow-2xl flex flex-col !p-0" style={{ maxHeight: '92dvh' }}>
        {/* Header */}
...
const GroupCard = ({
  ...
  const { primaryColor, border, headC, subC } = useThemeTokens();

  return (
    <NoAnimCard className="overflow-hidden transition-all w-full !p-0">
      <button
        onClick={() => setExpanded(x => !x)}
        className="std-task-btn w-full text-left p-4 transition-colors bg-transparent active:bg-transparent focus:bg-transparent"
        style={{ WebkitTapHighlightColor: 'transparent', outline: 'none', border: 'none', WebkitAppearance: 'none' }}
                ...
                <div key={task.id} className="flex items-center gap-2 p-3 rounded-xl transition-colors mt-2"
                  ...
                  <div className="flex items-center gap-1.5 shrink-0">
                    ...
                    <button onClick={() => onOpenTask(task, sub)}
                      className="std-task-btn px-2.5 py-1.5 rounded-lg text-xs font-medium text-white transition-opacity hover:opacity-80"
                      style={{ background: primaryColor, WebkitTapHighlightColor: 'transparent' }}>
...
const StudentTaskDashboard: React.FC = () => {
  ...
  const { isLight, primaryColor, border, headC, subC } = useThemeTokens();

  return (
    <div className="student-task-dashboard w-full max-w-full overflow-x-hidden box-border" style={{ padding: '16px 12px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      <style>{`
        .student-task-dashboard button {
          -webkit-tap-highlight-color: transparent !important;
          outline: none !important;
        }
        .student-task-dashboard button:focus,
        .student-task-dashboard button:active {
          outline: none !important;
      ...
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {[
          ...
          <NoAnimCard key={s.label} className="min-w-0 !p-3">
