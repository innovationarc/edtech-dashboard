...
const useThemeTokens = () => {
  const { theme, primaryColor, accentColor, cardStyle } = useDashboard();
  ...
  const getCardStyle = (): React.CSSProperties => {
    ...
    switch (cardStyle) {
      case 'liquid':
        return { background: isL ? 'rgba(255,255,255,0.65)' : 'rgba(255,255,255,0.05)', backdropFilter: 'blur(16px)', border: `1px solid ${border}` };
      case 'crystal':
        return { background: isL ? 'rgba(255,255,255,0.75)' : 'rgba(22,26,37,0.82)', backdropFilter: 'blur(12px)', border: `1px solid ${border}` };
      case 'glassmorphism': {
        const [r, g, b] = hexToRgb(primaryColor);
        return { background: `rgba(${r},${g},${b},0.10)`, backdropFilter: 'blur(12px)', border: `1px solid rgba(${r},${g},${b},0.25)` };
      }
      case 'frost': {
        const [r, g, b] = hexToRgb(primaryColor);
        return { background: `rgba(${r},${g},${b},0.08)`, backdropFilter: 'blur(10px)', border: `1px solid ${border}` };
      }
      case 'neon':
        return { background: isL ? '#ffffff' : 'rgba(8,8,20,0.88)', border: `1px solid ${primaryColor}55`, boxShadow: `0 0 12px ${primaryColor}22` };
      case 'vintage':
        return { background: isL ? 'rgba(253,246,227,0.97)' : 'rgba(45,35,20,0.92)', border: `1px solid ${isL ? 'rgba(180,140,60,0.2)' : 'rgba(180,140,60,0.15)'}` };
      case 'matte':
        return { background: isL ? 'rgba(255,255,255,0.90)' : 'rgba(22,26,37,0.82)', backdropFilter: 'blur(8px)', border: `1px solid ${border}` };
      case 'solid':
      default:
        return { background: isL ? '#ffffff' : 'var(--color-card, #1a1f2e)', border: `1px solid ${border}` };
    }
  };

  return { isLight, primaryColor, accentColor, border, getCardStyle };
};
...
const SubmissionForm = ({
  ...
  // Resolve courseId for exam route: /content-library/exam/:courseId/:contentId
  // Route defined in AppRoutes.tsx — /exam/:contentId does NOT exist
...
const TaskModal = ({
  ...
  const { getCardStyle, primaryColor, accentColor, border } = useThemeTokens();
  const modalBg = getCardStyle();

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm p-0 sm:p-4">
      <div className="rounded-t-2xl sm:rounded-2xl shadow-2xl w-full sm:max-w-xl max-h-[92dvh] flex flex-col" style={modalBg}>
        {/* Header */}
...
const GroupCard = ({
  ...
  const { getCardStyle, primaryColor } = useThemeTokens();
  const cardStyle = getCardStyle();

  return (
    <div style={cardStyle} className="rounded-2xl overflow-hidden transition-all"
      onMouseEnter={e => { (e.currentTarget as HTMLElement).style.borderColor = `${primaryColor}55`; }}
      onMouseLeave={e => { (e.currentTarget as HTMLElement).style.borderColor = (cardStyle.border as string ?? '').replace('1px solid ', ''); }}>
      <button onClick={() => setExpanded(x => !x)} className="w-full text-left p-4 sm:p-5 hover:bg-white/5 transition-colors">
...
const StudentTaskDashboard: React.FC = () => {
  ...
  const { getCardStyle, primaryColor, border } = useThemeTokens();
  const cardStyle = getCardStyle();
  ...
  return (
    <div className="p-3 sm:p-6 space-y-4 sm:space-y-6">
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: 'var(--color-text, #f1f5f9)' }}>My Tasks</h1>
        <p className="text-sm mt-0.5" style={{ color: 'var(--color-text2, #94a3b8)' }}>View and submit your assigned tasks</p>
      </div>

      {/* Stats — 2 col on all mobile, 4 col on md+ */}
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2 sm:gap-3">
        {[
          ...
          <div key={s.label} style={cardStyle} className="rounded-2xl p-3 sm:p-4 min-w-0">
      ...
      {/* Search + refresh — full width, no max-w truncation */}
      <div className="flex items-center gap-2">
        <div className="relative flex-1 min-w-0">
          <Search className="absolute left-3 top-1/2 -translate-y-1/2 w-4 h-4" style={{ color: '#6b7280' }} />
          <input
            className="w-full pl-9 pr-4 py-2 text-sm rounded-xl border bg-transparent outline-none focus:ring-1"
            style={{ borderColor: border, color: isLight ? '#374151' : '#e5e7eb', focusRingColor: primaryColor } as React.CSSProperties}
