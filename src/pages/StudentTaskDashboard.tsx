...
// ─── Theme helpers — mirrors Settings.tsx card style system ──────────────────
...
const useThemeTokens = () => {
  const { theme, primaryColor, accentColor, cardStyle: cs } = useDashboard();
  const isLight = theme === 'light';
  const border  = isLight ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.08)';
  const headC   = isLight ? '#111827' : '#f1f5f9';
  const subC    = isLight ? '#6b7280' : '#94a3b8';

  const card = (): React.CSSProperties => {
    // base card bg from context CSS var (theme-aware: dark=#1f2937, light=rgba(255,255,255,0.82) etc.)
    const base = isLight ? 'rgba(255,255,255,0.90)' : 'var(--color-card,#1f2937)';
    switch (cs) {
      case 'liquid':        return { background: isLight ? 'rgba(255,255,255,0.65)' : 'rgba(255,255,255,0.07)', backdropFilter:'blur(16px)', WebkitBackdropFilter:'blur(16px)', border:`1px solid ${border}` };
      case 'crystal':       return { background: isLight ? 'rgba(255,255,255,0.75)' : 'var(--color-card,#1a1f2e)', backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)', border:`1px solid ${border}` };
      case 'glassmorphism': { const [r,g,b]=hexToRgb(primaryColor); return { background:`rgba(${r},${g},${b},0.12)`, backdropFilter:'blur(12px)', WebkitBackdropFilter:'blur(12px)', border:`1px solid rgba(${r},${g},${b},0.30)` }; }
      case 'frost':         { const [r,g,b]=hexToRgb(primaryColor); return { background:`rgba(${r},${g},${b},0.10)`, backdropFilter:'blur(10px)', WebkitBackdropFilter:'blur(10px)', border:`1px solid ${border}` }; }
      case 'neon':          return { background: isLight ? '#ffffff' : 'var(--color-card,#1f2937)', border:`1px solid ${primaryColor}66`, boxShadow:`0 0 16px ${primaryColor}28` };
      case 'vintage':       return { background: isLight ? 'rgba(253,246,227,0.97)' : 'rgba(45,35,20,0.92)', border:`1px solid ${isLight?'rgba(180,140,60,0.2)':'rgba(180,140,60,0.15)'}` };
      case 'matte':         return { background: base, backdropFilter:'blur(8px)', WebkitBackdropFilter:'blur(8px)', border:`1px solid ${border}` };
      case 'solid':
      default:              return { background: base, border:`1px solid ${border}` };
    }
  };

  return { isLight, primaryColor, accentColor, border, headC, subC, card };
};
...
const FileDropZone = ({
  ...
  const { primaryColor, border } = useThemeTokens();
  ...
  return (
    <div className="space-y-2">
      <div onClick={() => ref.current?.click()}
        ...
        style={{ borderColor: over ? primaryColor : border, background: over ? `${primaryColor}08` : 'transparent' }}>
        {uploading
...
const SimpleEditor = ({ value, onChange, placeholder, wordLimit }: {
  ...
  const { isLight, border } = useThemeTokens();
  const words = value.trim() ? value.trim().split(/\s+/).length : 0;
  const over = wordLimit ? words > wordLimit : false;
  return (
    <div className="rounded-xl overflow-hidden" style={{ border: `1px solid ${border}` }}>
      <div className="flex items-center gap-1 px-3 py-2" style={{ background: 'rgba(255,255,255,0.03)', borderBottom: `1px solid ${border}` }}>
...
const SubmissionForm = ({
  ...
  const { isLight, border, primaryColor } = useThemeTokens();
  const examCourseId = group.assignedTo.type === 'course'
    ? (group.assignedTo as any).courseId
    : '_';
  ...
  const submit = async () => {
    ...
    try {
      ...
      notificationService.createNotification({
        userId: studentId,
        title: existing ? 'Task Resubmitted' : 'Task Submitted',
        message: `Your ${existing ? 're' : ''}submission for "${task.title}" in "${group.title}" has been received.`,
        type: 'assignment',
        priority: 'low',
        isPermanent: false,
        relatedId: task.id,
        relatedType: 'task',
        metadata: { taskTitle: task.title, groupTitle: group.title, taskType: task.type },
      });
      onSubmitted();
  ...
  if (task.type === 'exam') {
    return (
      <div style={secBg} className="rounded-xl p-6 text-center space-y-4">
        ...
        {task.contentId ? (
          <button
            onClick={() => { window.location.href = `/content-library/exam/${examCourseId}/${task.contentId}`; }}
...
const TaskModal = ({
  ...
  const { primaryColor, border, card } = useThemeTokens();
  const modalBg = card();

  return (
    <div className="fixed inset-0 z-50 flex items-end sm:items-center justify-center bg-black/60 backdrop-blur-sm">
      <div className="w-full sm:max-w-xl rounded-t-2xl sm:rounded-2xl shadow-2xl flex flex-col" style={{ ...modalBg, maxHeight: '92dvh' }}>
        {/* Header */}
        <div className="p-4 flex-shrink-0" style={{ borderBottom: `1px solid ${border}` }}>
          <div className="flex items-start justify-between gap-2 mb-3">
            <div className="flex items-center gap-2 min-w-0">
              ...
              <div className="min-w-0">
                ...
                <h2 className="text-sm font-bold truncate" style={{ color: '#f1f5f9' }}>{task.title}</h2>
              </div>
            </div>
            <button onClick={onClose} className="p-1.5 rounded-lg hover:bg-white/10 shrink-0 transition-colors" style={{ color: '#9ca3af' }}>
              <X className="w-4 h-4" />
            </button>
          </div>
          <div className="flex flex-wrap items-center gap-x-3 gap-y-1 text-xs" style={{ color: '#9ca3af' }}>
            ...
            <span className="flex items-center gap-1" style={{ color: urgent ? '#fcd34d' : undefined }}>
              <Clock className="w-3.5 h-3.5" />
              {isPastDue && isLateAllowed && !isPastLate ? `Late: ${countdown}` : isPastDue ? 'Overdue' : `Due in ${countdown}`}
            </span>
            <span className="flex items-center gap-1"><Calendar className="w-3.5 h-3.5" />{fmtDate(group.dueDate)}</span>
          </div>
          {isPastDue && isLateAllowed && !isPastLate && (
            <div className="mt-2 flex items-center gap-1.5 text-xs px-2.5 py-1.5 rounded-lg"
              ...
              <AlertTriangle className="w-3.5 h-3.5 shrink-0" />Late accepted until {fmtDate(group.lateSubmissionDeadline)}
            </div>
          )}
        </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-4">
          {task.description && (
            <div>
              <p className="text-xs font-semibold mb-1" style={{ color: '#9ca3af' }}>Instructions</p>
              <p className="text-sm leading-relaxed" style={{ color: '#d1d5db' }}>{task.description}</p>
            </div>
          )}

          {task.rubric?.length ? (
            <div className="rounded-xl p-3" style={{ background: 'rgba(255,255,255,0.03)', border: `1px solid ${border}` }}>
              <p className="text-xs font-semibold mb-2" style={{ color: '#9ca3af' }}>Grading Rubric</p>
              {task.rubric.map(r => (
                <div key={r.id} className="flex items-center justify-between py-1 border-b last:border-0"
                  style={{ borderColor: 'rgba(255,255,255,0.05)' }}>
                  <div className="min-w-0 flex-1">
                    <span className="text-xs font-medium" style={{ color: '#e5e7eb' }}>{r.criterion}</span>
                    {r.description && <span className="text-xs ml-2" style={{ color: '#6b7280' }}>{r.description}</span>}
                  </div>
                  <span className="text-xs font-bold shrink-0 ml-2" style={{ color: '#d1d5db' }}>{r.maxPoints} pts</span>
                </div>
              ))}
            </div>
          ) : null}
          ...
          {showForm ? (
            ...
            <button onClick={() => setShowForm(true)}
              ...
              style={{ color: '#d1d5db', border: `1px solid ${border}` }}>
...
const GroupCard = ({
  ...
  const { primaryColor, border, headC, subC, card } = useThemeTokens();
  const cardSt = card();

  return (
    <div style={cardSt} className="rounded-2xl overflow-hidden transition-all w-full">
      <button onClick={() => setExpanded(x => !x)} className="w-full text-left p-4 hover:bg-white/5 transition-colors">
        <div className="flex items-start justify-between gap-2 mb-3">
          <div className="flex-1 min-w-0">
            <h3 className="font-bold text-base truncate" style={{ color: headC }}>{group.title}</h3>
            {group.description && <p className="text-xs mt-0.5 line-clamp-1" style={{ color: subC }}>{group.description}</p>}
          </div>
          {expanded ? <ChevronUp className="w-4 h-4 shrink-0 mt-0.5" style={{ color: subC }} /> : <ChevronDown className="w-4 h-4 shrink-0 mt-0.5" style={{ color: subC }} />}
        </div>

        <div className="flex flex-wrap gap-x-3 gap-y-1 text-xs mb-3" style={{ color: subC }}>
          <span className="flex items-center gap-1"><Layers className="w-3 h-3" />{tasks.length} tasks</span>
          <span className="flex items-center gap-1"><Award className="w-3 h-3" />{group.totalPoints} pts total</span>
          {earned > 0 && <span className="flex items-center gap-1" style={{ color: '#a5b4fc' }}><Star className="w-3 h-3" />{earned} pts earned</span>}
          <span className="flex items-center gap-1" style={{ color: urgent ? '#fcd34d' : undefined }}>
            <Clock className="w-3 h-3" />{countdown}
          </span>
          <span className="flex items-center gap-1"><Calendar className="w-3 h-3" />{fmtDate(group.dueDate)}</span>
        </div>

        <div className="space-y-1">
          <div className="flex justify-between text-xs" style={{ color: subC }}>
          ...
          <div className="h-1.5 rounded-full" style={{ background: 'rgba(255,255,255,0.06)' }}>
            <div className="h-full rounded-full transition-all" style={{
              width: `${progress}%`,
              background: progress === 100 ? '#10b981' : primaryColor,
            }} />
          </div>
        </div>
      </button>
      ...
      {expanded && (
        <div className="px-3 pb-3 space-y-2" style={{ borderTop: `1px solid ${border}` }}>
          {tasks.length === 0 ? (
            <p className="text-sm text-center py-4" style={{ color: subC }}>No tasks in this group.</p>
          ) : (
            tasks.map(task => {
              const meta = TYPE_META[task.type];
              const TIcon = meta.icon;
              ...
              return (
                <div key={task.id} className="flex items-center gap-2 p-3 rounded-xl transition-colors mt-2"
                  ...
                  {/* Info — truncates, never pushes right side off */}
                  <div className="flex-1 min-w-0 overflow-hidden">
                    <div className="flex items-center gap-1.5 flex-wrap">
                      <span className="text-sm font-semibold truncate" style={{ color: '#e2e8f0' }}>{task.title}</span>
                      <span className="text-xs px-1.5 py-0.5 rounded shrink-0"
                        style={{ background: `${meta.color}18`, color: meta.color }}>{meta.label}</span>
                    </div>
                    <div className="flex items-center gap-1.5 mt-0.5 text-xs flex-wrap" style={{ color: subC }}>
                  ...
                  {/* Actions — always visible, never wraps off-screen */}
...
const StudentTaskDashboard: React.FC = () => {
  ...
  const load = useCallback(async () => {
    ...
    try {
      // 1. Get student's classGrade from their profile
      const classGrade = (user as any).classGrade as string | undefined;

      // 2. Fetch student's enrolled course IDs from the enrollments collection
      //    (courseEnrollmentService checks the 'enrollments' collection by studentId)
      const enrolledCourseIds = await courseEnrollmentService.getStudentEnrolledCourseIds(user.uid);

      // 3. Collect all relevant task groups using a Map to deduplicate by group ID.
      //    getTaskGroupsForStudent filters client-side, but can only match ONE courseId per call.
      //    Strategy:
      //      (a) Call with no courseId  → catches 'all', 'students', and 'class' scoped groups
      //      (b) Call per enrolled course → catches 'course' scoped groups for each course
      const groupMap = new Map<string, TaskGroup>();

      // (a) catches 'all', 'students', 'class'
      const baseGroups = await taskService.getTaskGroupsForStudent(user.uid, undefined, classGrade);
      baseGroups.forEach(g => groupMap.set(g.id, g));

      // (b) catches 'course' scoped groups — one call per enrolled course
      for (const cId of enrolledCourseIds) {
        const courseGroups = await taskService.getTaskGroupsForStudent(user.uid, cId, classGrade);
      ...
      const [fetchedSubs] = await Promise.all([
  ...
  const { isLight, primaryColor, border, headC, subC, card } = useThemeTokens();
  const cardSt = card();

  return (
    <div className="w-full max-w-full overflow-x-hidden box-border" style={{ padding: '16px 12px', display: 'flex', flexDirection: 'column', gap: 16 }}>
      {/* Header */}
      <div>
        <h1 className="text-xl font-bold" style={{ color: headC }}>My Tasks</h1>
        <p className="text-sm mt-0.5" style={{ color: subC }}>View and submit your assigned tasks</p>
      </div>

      {/* Stats — strict 2-col always, no overflow */}
      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 10 }}>
        {[
          ...
          <div key={s.label} style={{ ...cardSt, borderRadius: 16, padding: '12px 14px', minWidth: 0 }}>
            <p className="text-xs mb-1 truncate" style={{ color: subC }}>{s.label}</p>
            <p className="text-2xl font-bold" style={{ color: s.color }}>{s.value}</p>
          </div>
        ))}
      </div>

      {/* Search + refresh — full width, contained */}
      <div className="flex items-center gap-2">
        <div className="relative min-w-0" style={{ flex: 1 }}>
          ...
          <input
            className="w-full pl-9 pr-3 py-2 text-sm rounded-xl border bg-transparent outline-none"
            style={{ borderColor: border, color: isLight ? '#374151' : '#e5e7eb', boxSizing: 'border-box' }}
            placeholder="Search tasks..." value={search} onChange={e => setSearch(e.target.value)} />
        </div>
        <button onClick={load} className="p-2 rounded-xl hover:bg-white/5 shrink-0 transition-colors" style={{ color: '#9ca3af' }} title="Refresh">
      ...
      {loading ? (
        ...
        <div style={{ ...cardSt, borderRadius: 16, padding: '48px 24px', textAlign: 'center' }}>
          <BookOpen className="w-12 h-12 mx-auto mb-4 text-gray-600" />
          <p className="font-medium" style={{ color: subC }}>No tasks found</p>
          <p className="text-sm mt-1" style={{ color: '#6b7280' }}>Tasks assigned by your instructor will appear here.</p>
        </div>
      ) : (
