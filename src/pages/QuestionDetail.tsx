...
const QuestionDetail = () => {
  ...
  const loadAllQuestions = async () => {
    try {
      ...
      // Filter: student's own questions + answered questions from others
      const filteredQuestions = allQs.filter(q => 
        q.studentId === user?.uid || 
        (q.studentId !== user?.uid && q.status === 'answered')
      );
  ...
  const loadQuestionAndAnswers = async () => {
    ...
    try {
      ...
      // Mark as viewed if it's the student's question and it's answered
...
// Follow-up Question Modal (same as before)
