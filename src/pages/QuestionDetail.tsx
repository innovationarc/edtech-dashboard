// src/pages/QuestionDetail.tsx - Part 1 of 3 (Enhanced with Rating Edit, Image/PDF Viewer, Voice Support)
            ...
            {question.audioUrl && (
              <div className="bg-background-800 rounded-lg p-4">
                <div className="flex items-center gap-3 mb-3">
                  ...
                  <span className="text-white font-medium">Voice Question</span>
                </div>
                <audio controls className="w-full">
                  <source src={question.audioUrl} type="audio/webm" />
...
const FollowUpQuestionCard = ({ question, courseId, onViewDocument, onViewImage }: FollowUpQuestionCardProps) => {
  const { user } = useDashboard();
  const [answers, setAnswers] = useState<Answer[]>([]);
  const [loading, setLoading] = useState(true);
...
// src/pages/QuestionDetail.tsx - Part 3 of 3 (Modals and Viewers)
...
const FollowUpAnswerCard = ({ answer, questionId, isOwnQuestion, courseId, onViewDocument, onViewImage }: FollowUpAnswerCardProps) => {
  const [rating, setRating] = useState<number>(0);
  const [hoveredRating, setHoveredRating] = useState<number>(0);
  const [isEditingRating, setIsEditingRating] = useState(false);
      ...
      {answer.audioUrl && (
        <div className="bg-background-700 rounded-lg p-3">
          <div className="flex items-center gap-2 mb-2">
            ...
            <span className="text-sm text-white">Voice Answer</span>
          </div>
          <audio controls className="w-full">
            <source src={answer.audioUrl} type="audio/webm" />
            Your browser does not support the audio element.
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-background-900 rounded-xl w-full max-w-2xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div>
            <label className="block text-sm text-gray-400 mb-1">Voice Message (Optional - Replace Current)</label>
            <div className="flex gap-2">
              <button
                type="button"
              ...
              >
                ...
                <span>{isRecording ? 'Stop Recording' : 'Record New Voice'}</span>
              </button>
              {audioFile && (
          ...
          <div className="bg-background-800 rounded-lg p-3 text-sm text-gray-400">
            ...
            <p className="mt-2"><strong>Locked:</strong> Subject, Course, and File attachments cannot be changed.</p>
            <p className="mt-1"><strong>Editable:</strong> Question text and voice message can be updated.</p>
          </div>
...
const DocumentViewer = ({ url, fileName, type, onClose }: DocumentViewerProps) => {
  return (
    <div className="fixed inset-0 bg-black bg-opacity-90 flex items-center justify-center z-50 p-4">
      <div className="bg-background-900 rounded-xl w-full max-w-6xl h-[90vh] flex flex-col">
        <div className="flex items-center justify-between p-4 border-b border-background-700">
          <h2 className="text-xl font-bold text-white truncate flex-1">{fileName}</h2>
          <div className="flex items-center gap-2">
            <button
              onClick={() => {
                fetch(url)
