// src/pages/QuestionDetail.tsx - Part 1 (Enhanced with Ratings, Edit, Save, Document Viewer)
  ...
  return (
    <div className="flex gap-6">
      ...
      <div className="flex-1 space-y-6">
        ...
        // src/pages/QuestionDetail.tsx - Part 2 (Enhanced - Continuation)
            ...
            {question.imageUrl && (
              <div>
                <img 
                  ...
                  className="max-h-96 object-contain rounded-lg border border-background-700 cursor-pointer"
...
const AnswerCard = ({ answer, questionId, isStudentQuestion, onViewDocument }: AnswerCardProps) => {
  const { user } = useDashboard();
  ...
  return (
    <Card>
      <div className="space-y-4">
        ...
        {isStudentQuestion && (
          <div className="pt-4 border-t border-background-700">
            <p className="text-sm text-gray-400 mb-2">Rate this answer:</p>
            <div className="flex items-center gap-1">
              ...
              {hasRated && (
                <span className="ml-3 text-sm text-green-400">✓ Rated</span>
              )}
            </div>
          </div>
        )}
...
const FollowUpQuestionCard = ({ question, onViewDocument }: FollowUpQuestionCardProps) => {
  const [answers, setAnswers] = useState<Answer[]>([]);
  const [loading, setLoading] = useState(true);
      ...
      <div className="space-y-4">
        ...
        {loading ? (
          ...
          <div className="space-y-3 pt-4 border-t border-background-700">
            ...
            {answers.map((answer) => (
              <div key={answer.id} className="bg-background-800 rounded-lg p-4 space-y-3">
                <div className="flex items-center gap-2">
                  {answer.type === 'ai' ? (
                    <>
...
// Continue with remaining components in next artifact...

 // Add these components to the end of QuestionDetail.tsx (after Part 2)
...
const EditQuestionModal = ({ question, onClose, onSuccess }: EditQuestionModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await qaService.updateQuestion(question.id, {
      ...
      <div className="bg-background-900 rounded-xl w-full max-w-2xl p-6">
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-background-800 rounded-lg p-3 text-sm text-gray-400">
            ...
            <p className="mt-1"><strong>Note:</strong> You can only edit the question text. Subject and attachments cannot be changed.</p>
          </div>
