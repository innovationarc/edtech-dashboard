// src/pages/QuestionDetail.tsx - Part 1
...
const QuestionDetail = () => {
  ...
  const loadCoursesWithQnA = async () => {
    try {
      const allCourses = await courseService.getPublishedCourses();
      const coursesWithQnA = allCourses.filter(course => course.hasQnA === true);
      setAvailableCourses(coursesWithQnA);
    } catch (err) {
  ...
  const loadAllQuestions = async () => {
    try {
      ...
      // Filter: student's own questions + answered questions from others (exclude closed questions from others)
      const filteredQuestions = allQs.filter(q => {
        if (q.studentId === user?.uid) {
          // Show all questions (including closed) for the student who asked
          return true;
        } else {
          // For other students, only show answered questions that are not closed
          return q.status === 'answered' && q.status !== 'closed';
        }
      });
      
      // Only show root questions (not follow-ups) in the menu
  ...
  const loadQuestionAndAnswers = async () => {
    ...
    try {
      ...
      // Check if question is closed and user is not the owner
      if (questionData.status === 'closed' && questionData.studentId !== user?.uid) {
        setError('This question is not accessible');
        navigate('/student/qa');
        return;
      }
      ...
      // Load answers for each follow-up
      const followUpsWithAnswers = await Promise.all(
        followUps.map(async (fq) => {
          const fqAnswers = await qaService.getAnswersForQuestion(fq.id);
          return { question: fq, answers: fqAnswers };
        })
      );
      
      setFollowUpQuestions(followUps);

      // Mark as viewed if it's the student's question and it's answered (but not closed)
      if (questionData.studentId === user?.uid && questionData.status === 'answered' && !questionData.viewedByStudent) {
        ...
        // Reload questions to update unread status
        loadAllQuestions();
      }

      // Check follow-ups for unread answers
      ...
      setTimeout(() => navigate('/student/qa'), 2000);
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleDeleteQuestion = async () => {
    ...
    try {
      ...
      for (const answer of answers) {
        await qaService.deleteAnswer(answer.id);
      }

      // Delete all follow-up questions and their answers
      for (const followUp of followUpQuestions) {
        const followUpAnswers = await qaService.getAnswersForQuestion(followUp.id);
        for (const answer of followUpAnswers) {
          await qaService.deleteAnswer(answer.id);
        }
        await qaService.deleteQuestion(followUp.id);
      }
  ...
  const hasUnreadAnswer = (q: Question): boolean => {
    ...
    // Check if main question has unread answer
    if (q.status === 'answered' && !q.viewedByStudent) return true;
    
    // Check if any follow-up has unread answer
    const allQs = allQuestions;
    const hasUnreadFollowUp = allQs.some(aq => 
...
// src/pages/QuestionDetail.tsx - Part 2 (continued from Part 1)
        ...
        {/* Closed Question Notice - Show before answers if closed */}
        {isClosed && (
          <div className="bg-red-900 border border-red-700 text-red-200 px-4 py-3 rounded-lg flex items-center gap-2">
            <XCircle size={20} />
            <span className="font-medium">This question is closed by admin</span>
          </div>
        )}

        {/* Answers Section or Admin Comment for Closed Questions */}
        <div className="space-y-4">
          <h2 className="text-2xl font-bold text-white flex items-center gap-2">
            <MessageSquare size={24} />
            {isClosed ? 'Admin Comment' : `Answers (${answers.length})`}
          </h2>

          {isClosed ? (
            // Show admin comment for closed questions
            <Card>
              <div className="space-y-4">
                <div className="flex items-center gap-2">
                  <XCircle size={20} className="text-red-400" />
                  <span className="text-lg font-medium text-white">Reason for Closing</span>
                </div>
                <div className="pt-4 border-t border-background-700">
                  <p className="text-gray-300 whitespace-pre-wrap leading-relaxed">
                    {question.adminComment || 'No reason provided'}
                  </p>
                </div>
              </div>
            </Card>
        ...
        {/* Red note after answers for closed questions */}
        {isClosed && (
          <div className="bg-red-900 border border-red-700 text-red-200 px-4 py-3 rounded-lg flex items-center gap-2">
            <AlertCircle size={20} />
            <span>This question has been closed by the admin and cannot accept further responses.</span>
          </div>
        )}

        {/* Follow-up Questions and Answers */}
        {followUpQuestions.length > 0 && !isClosed && (
          <div className="space-y-4">
            <h2 className="text-2xl font-bold text-white flex items-center gap-2">
              <MessageSquare size={24} />
        ...
        {/* Action Buttons - Only for student who asked */}
        {isStudentQuestion && isAnswered && !isClosed && (
          <Card>
            <div className="space-y-4">
        ...
        {/* Delete Button for Closed Questions */}
        {isStudentQuestion && isClosed && (
          <Card>
            <button
              onClick={() => setShowDeleteConfirm(true)}
              className="w-full flex items-center justify-center gap-2 bg-error-dark hover:bg-error-DEFAULT text-error-light px-4 py-2 rounded-lg transition-colors font-medium"
            >
              <Trash2 size={16} />
              <span>Delete Question</span>
            </button>
...
// Follow-up Question Card Component
interface FollowUpQuestionCardProps {
  question: Question;
  isOwnQuestion: boolean;
}

const FollowUpQuestionCard = ({ question, isOwnQuestion }: FollowUpQuestionCardProps) => {
  const [answers, setAnswers] = useState<Answer[]>([]);
  const [loading, setLoading] = useState(true);
...
const FollowUpQuestionModal = ({ parentQuestion, onClose, onSuccess }: FollowUpQuestionModalProps) => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      // If parent was answered by AI, handle follow-up with AI
...
Provide a clear, step-by-step answer to their follow-up question.`;
        ...
        // If parent was answered by teacher, submit to teacher
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-background-900 rounded-xl w-full max-w-2xl p-6">
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-2xl font-bold text-white">Ask Follow-up Question</h2>
          <button
            onClick={onClose}
