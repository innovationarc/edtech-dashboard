...
// BUG FIX 1 & 2: statusBadge now shows 'expired' as error, 'scheduled' as info
const statusBadge = (s: string) => {
  const m: Record<string, 'success' | 'error' | 'warning' | 'info'> = {
    active: 'success',
    inactive: 'error',
    expired: 'warning',
    scheduled: 'info',
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 mt-1">
              ...
              If set, coupon shows as <strong>Scheduled</strong> until this date, even if Start Date has passed.
            </p>
          </div>
        </div>
      </Section>
          ...
          <div className="flex items-center justify-between p-5 border-b border-gray-700 flex-shrink-0">
            <div className="flex items-center gap-3">
              ...
              <div>
                ...
                {/* BUG FIX 4: Show live count from coupons array, not stale group.couponCount */}
                <p className="text-xs text-gray-400">
                  ID: <span className="text-indigo-400 font-mono">{group.groupId}</span>
                  {' · '}
                  <span className={coupons && coupons.length !== group.couponCount ? 'text-yellow-400' : ''}>
                    {coupons ? coupons.length : group.couponCount} coupons
                  </span>
                  {' · '}Created {fmt(group.createdAt)}
                </p>
              </div>
            </div>
                ...
                <div key={log.id} className="bg-gray-800 rounded-lg border border-gray-700 overflow-hidden">
                  ...
                  <div className="p-3">
                    <div className="flex items-center justify-between mb-1.5">
                      <Badge variant={
                        log.actionType.includes('create') ? 'success' :
                          log.actionType === 'delete' || log.actionType === 'delete_group' ? 'error' :
                    ...
                    <p className="text-gray-300 text-xs">
                      ...
                      {log.groupName && <><span className="text-gray-400"> · Group Name: </span><span className="text-purple-300">{log.groupName}</span></>}
                      ...
                      {log.metadata?.deletedTokenCount != null && <><span className="text-gray-400"> · Tokens deleted: </span><span className="text-red-400">{log.metadata.deletedTokenCount}</span></>}
                    </p>
...
const CouponManagement: React.FC = () => {
  ...
  const buildCouponPayloadBase = (form: FormState) => {
    ...
    return {
      ...
      actorUserId: `${(user as any).userId || user!.uid}`,
  ...
  const handleToggle = async (c: Coupon) => {
    ...
    try {
     await couponService.toggleCouponStatus(c.id, next, `${(user as any).userId || user!.uid}`, `${user!.surname}`);
  ...
  const handleDelete = async (c: Coupon) => {
    ...
    try {
      await couponService.deleteCoupon(c.id, `${(user as any).userId || user!.uid}`, `${user!.surname}`);
      setSingleCoupons(p => p.filter(x => x.id !== c.id));
      setGroupCoupons(p => {
        const n = { ...p };
        Object.keys(n).forEach(k => { n[k] = n[k].filter(x => x.id !== c.id); });
        return n;
      });
      // BUG FIX 4: Update group couponCount in local state when a token is deleted individually
      if (c.bulkGroupId) {
        setBulkGroups(p => p.map(g =>
          g.id === c.bulkGroupId
            ? { ...g, couponCount: Math.max(0, g.couponCount - 1) }
            : g
        ));
      }
  ...
  const handleDeleteGroup = async (group: BulkGroup) => {
    ...
    try {
      await couponService.deleteBulkGroup(group.id, `${(user as any).userId || user!.uid}`, `${user!.surname}`);
                  ...
                  <React.Fragment key={c.id}>
                    <tr className="hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3 text-xs text-gray-400 whitespace-nowrap">
                        ...
                        {c.activationDate && (
                          <div className="text-blue-400/70 flex items-center gap-0.5 mt-0.5">
                            <Clock size={9} />Act: {fmt(c.activationDate)}
                          </div>
                        )}
                      </td>
            ...
            <Card key={group.id} className="overflow-hidden">
              <div className="w-full flex items-center justify-between p-4 hover:bg-gray-700/30 transition-colors">
                ...
                >
                  ...
                  <div>
                    ...
                    <p className="text-xs text-gray-400">
                      ID: <span className="text-indigo-400 font-mono">{group.groupId}</span>
                      {' · '}
                      {/* BUG FIX 4: Show live count from cached groupCoupons if loaded, else fall back to stored count */}
                      <span className={groupCoupons[group.id] && groupCoupons[group.id].length !== group.couponCount ? 'text-yellow-400' : ''}>
                        {groupCoupons[group.id] ? groupCoupons[group.id].length : group.couponCount} coupons
                      </span>
                      {' · '}{fmt(group.createdAt)}
                    </p>
                  </div>
                ...
                <div className="flex items-center gap-2 ml-3">
                  <Badge variant="info">{groupCoupons[group.id] ? groupCoupons[group.id].length : group.couponCount} coupons</Badge>
                  <Btn
                    variant="ghost"
                    size="sm"
                    onClick={() => {
