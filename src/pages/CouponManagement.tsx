    ...
    <div>
      ...
      {!isAll && (
        <div className="relative">
                ...
                <span className="flex flex-wrap gap-1">
                  {selected.length <= 2
                    ? selected.map(v => {
                      ...
                      return opt ? (
                        <span key={v} className="bg-indigo-900/60 text-indigo-300 border border-indigo-700 px-1.5 py-0.5 rounded text-xs">{opt.label}</span>
                      ) : null;
                    })
          ...
          {open && (
            <div className="absolute z-50 w-full mt-1 bg-gray-800 border border-gray-600 rounded-xl shadow-2xl overflow-hidden">
              {/* Search bar */}
              {searchable && (
                  ...
                  : filtered.map(o => {
                    ...
                    return (
                      <button
                        ...
                        className={`w-full flex items-center gap-2.5 px-3 py-2 text-sm text-left transition-colors ${isSelected ? 'bg-indigo-900/30 text-white' : 'text-gray-300 hover:bg-gray-700'}`}
                      >
                        <span className={`w-4 h-4 rounded border-2 flex items-center justify-center flex-shrink-0 transition-all ${isSelected ? 'border-indigo-500 bg-indigo-600' : 'border-gray-500'}`}>
                          {isSelected && (
...
const Section: React.FC<{ title: string; icon?: React.ReactNode; children: React.ReactNode; className?: string }> = ({ title, icon, children, className = '' }) => (
  <div className={`rounded-xl border border-gray-700 overflow-hidden ${className}`}>
    <div className="flex items-center gap-2 px-4 py-3 bg-gray-900/60 border-b border-gray-700">
