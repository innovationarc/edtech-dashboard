...
const CouponForm: React.FC<{
  ...
  onClose: () => void;
}> = ({ courses, users, isBulk, saving, initialForm, initialBulkGroupName = '', initialBulkGroupId = '', initialBulkQuantity = '10', isEdit = false, onSubmit, onClose }) => {
...
}> = ({ group, coupons, onClose, onToggle, onDelete, onEdit, notify }) => {
  const exportTxt = () => {
...
const CouponManagement: React.FC = () => {
  ...
  const handleEditCouponSubmit = async (form: FormState, _bulk: any) => {
    ...
    try {
      ...
      // Update state in all relevant places
  ...
  // Edit group name submit
  const handleEditGroupSubmit = async (_form: FormState, bulk: { groupName: string; groupId: string; quantity: number }) => {
    if (!editGroup) return;
    setSaving(true);
    try {
                ...
                {filteredSingle.map(c => (
                  <React.Fragment key={c.id}>
                    <tr className="hover:bg-gray-700/30 transition-colors">
                      ...
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-1">
                          <Btn variant="ghost" size="sm" onClick={() => setShowDetailId(showDetailId === c.id ? null : c.id)} title="View details"><Eye size={13} /></Btn>
        ...
        <div className="fixed inset-0 bg-black/70 backdrop-blur-sm z-50 flex items-center justify-center p-4">
          <div className="bg-gray-900 border border-gray-700 rounded-2xl w-full max-w-md shadow-2xl">
            ...
            <div className="p-5 space-y-4">
              <div>
                <Lbl>Group ID</Lbl>
                <input className="w-full bg-gray-600/50 border border-gray-600 text-gray-400 rounded-lg px-3 py-2 text-sm cursor-not-allowed font-mono" value={editGroup.groupId} readOnly />
                <p className="text-xs text-gray-500 mt-1">Group ID cannot be changed</p>
              </div>
              <EditGroupNameForm
                group={editGroup}
                ...
                onSave={async (newName) => {
                  setSaving(true);
                  try {
                    const input: UpdateBulkGroupInput = { groupName: newName, actorUserId: user!.uid, actorName: user!.name };
                    const updated = await couponService.updateBulkGroup(editGroup.id, input);
                    setBulkGroups(p => p.map(g => g.id === updated.id ? updated : g));
                    setEditGroup(null);
                    notify('Group updated successfully');
                ...
                : auditLogs.map(log => (
                  <div key={log.id} className="bg-gray-800 rounded-lg p-3 text-sm border border-gray-700">
                    <div className="flex items-center justify-between mb-1">
                      <Badge variant={log.actionType.includes('create') ? 'success' : log.actionType === 'delete' ? 'error' : log.actionType.includes('update') ? 'info' : log.actionType === 'deactivate' ? 'warning' : 'info'}>
...
// ── EDIT GROUP NAME FORM (small inline form) ──────────────────────────────────

const EditGroupNameForm: React.FC<{
  group: BulkGroup;
  saving: boolean;
  onSave: (name: string) => Promise<void>;
  onClose: () => void;
}> = ({ group, saving, onSave, onClose }) => {
  ...
  return (
    <div className="space-y-4">
      ...
      <div className="flex justify-end gap-3">
        <Btn variant="secondary" onClick={onClose} disabled={saving}>Cancel</Btn>
        <Btn variant="primary" loading={saving} onClick={handleSave}><Pencil size={14} />Save Changes</Btn>
      </div>
    </div>
  );
};
