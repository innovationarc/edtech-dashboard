...
const CouponManagement: React.FC = () => {
  ...
  const handleToggle = async (c: Coupon) => {
    ...
    try {
      await couponService.toggleCouponStatus(c.id, next, user!.uid, `${user!.name} [${(user as any).userId || user!.uid}]`);
  ...
  const handleDelete = async (c: Coupon) => {
    ...
    try {
      await couponService.deleteCoupon(c.id, user!.uid, `${user!.name} [${(user as any).userId || user!.uid}]`);
  ...
  const handleDeleteGroup = async (group: BulkGroup) => {
    ...
    try {
      await couponService.deleteBulkGroup(group.id, user!.uid, `${user!.name} [${(user as any).userId || user!.uid}]`);
