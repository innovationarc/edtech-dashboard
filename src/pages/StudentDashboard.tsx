...
function hmapLevel(v: number): 0|1|2|3|4|5|6|7 {
  // Fixed daily thresholds (minutes) — 30-min intervals, same scale for all students
...
function hmapLevelForView(v: number, view: HeatmapView): 0|1|2|3|4|5|6|7 {
  if (!v) return 0;
  if (view === 'weeks') {
    // Weekly = daily × 7, same 30-min-interval spirit scaled up
    // 0 / <3.5h / <7h / <10.5h / <14h / <17.5h / <21h / 21h+
    if (v < 210)  return 1;
    if (v < 420)  return 2;
    if (v < 630)  return 3;
    if (v < 840)  return 4;
...
const StudentDashboard = () => {
  ...
  const loadKPI = async () => {
    ...
    try {
      ...
      // watchedDuration is cumulative seconds for that content item.
      // We bucket by lastWatchedAt date — best approximation without schema changes.
      const weekAgoTs = Timestamp.fromDate(weekAgoDate);
      ...
      // Uses durationMins written by attendanceService.leave().
      // joinedAt is the anchor date for bucketing.
      ...
      // Use e.date (the scheduled event date) for bucketing, not completedAt.
      // This ensures a "Physics session on Monday" counts on Monday, not whenever
      // the student tapped complete.
      ...
      // Count ALL completed sessions EXCEPT those explicitly linked to a calendar
      // event slot via eventId (those are AI-scheduler-generated sessions where the
      // event duration is already counted in Source 3, so skipping avoids double-counting).
      // Manual subject names typed into either pomodoro widget do NOT set eventId —
      // those are independent sessions and must be counted here.
      const pomSessions = await studyPlanService.getPomodoroSessions(user.uid, 200);
      pomSessions.forEach(s => {
        if (!s.completed) return;
        // Skip only AI-scheduler-linked sessions (have an explicit eventId)
        if (s.eventId) return;
        const dateStr = ymd(s.startTime instanceof Date ? s.startTime : new Date(s.startTime));
        if (dateStr >= weekAgoStr) weekMins += s.duration;
        if (dateStr === todayStr)  todayMins += s.duration;
      });

      setKpiTodayMins(todayMins);
      setKpiWeekMins(weekMins);

      // Objectives completed (unchanged)
  ...
  const loadStudyHeatmap = async () => {
    ...
    try {
      ...
      for (let i = WEEKS*7-1; i >= 0; i--) {
        ...
        days.push({ date: k, value: v, level: hmapLevel(v) });
      }
  ...
  const loadAppHeatmap = async () => {
    ...
    try {
      ...
      // Recalculate levels with fixed thresholds (service uses its own intensityLevel)
      const normalised = heatmap.map(d => ({ ...d, level: hmapLevel(d.value) }));
      ...
      if (liveMins >= 1) {
        ...
        if (todayIdx !== -1) {
          ...
          updated[todayIdx] = { ...updated[todayIdx], value: combined, level: hmapLevel(combined) };
...
const HeatmapCard = ({
  ...
  days: { date: string; value: number; level: 0|1|2|3|4|5|6|7 }[];
  ...
  const agg = (() => {
    ...
    return Array.from(map.entries()).map(([date,value]) => ({
      date, value,
      level: hmapLevelForView(value, view),
    }));
  })();

  const fmtTooltip = (date: string, value: number) => {
    const d = new Date(date);
    const label = view === 'months'
    ...
    const display = value === 0 ? 'No activity' : `${hrs}h (${value}${unit})`;
    return { label, display };
  };

  const active = agg.filter(d => d.level > 0).length;

  const showTip = (e: React.MouseEvent | React.TouchEvent, d: { date: string; value: number }) => {
    const el = e.currentTarget as HTMLDivElement;
    const rect = el.getBoundingClientRect();
    setTooltip({ x: rect.left + rect.width / 2, y: rect.top, date: d.date, value: d.value });
  };
  const hideTip = () => setTooltip(null);
  const toggleTip = (e: React.TouchEvent, d: { date: string; value: number }) => {
    if (tooltip?.date === d.date) { hideTip(); } else { showTip(e, d); }
  };

  const cellHandlers = (d: { date: string; value: number; level: 0|1|2|3|4|5|6|7 }) => ({
    onMouseEnter: (e: React.MouseEvent) => showTip(e, d),
    onMouseLeave: hideTip,
    onClick: (e: React.MouseEvent | React.TouchEvent) => toggleTip(e as any, d),
  });
  ...
  return (
    <Card title={title} subtitle={subtitle} icon={icon} accent={accent} enterDelay={enterDelay}>
        ...
        <div style={{ overflowX:'auto' }}>
          {view === 'days' ? (
            <div style={{ display:'grid', gridTemplateColumns:`repeat(${Math.ceil(agg.length/7)},${CELL}px)`, gridTemplateRows:`repeat(7,${CELL}px)`, gap:GAP, width:'fit-content' }}>
              {agg.map(d => (
                <div key={d.date} {...cellHandlers(d)}
                  style={{ width:CELL, height:CELL, borderRadius:2, background:colors[d.level], transition:'transform 0.1s', cursor:'pointer' }}
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform='scale(1.3)'; showTip(e, d); }}
            ...
            <div style={{ display:'flex', gap:GAP, flexWrap:'wrap' }}>
              {agg.map(d => (
                <div key={d.date} {...cellHandlers(d)} style={{ width:CELL, height:CELL, borderRadius:view==='months'?4:3, background:colors[d.level], cursor:'pointer', flexShrink:0 }} />
              ))}
            </div>
          )}
        </div>
        <div style={{ display:'flex', alignItems:'center', justifyContent:'space-between' }}>
          <div style={{ display:'flex', alignItems:'center', gap:3 }}>
            ...
            <span style={{ fontSize:9, color:T.text3, marginLeft:2 }}>
              {view === 'weeks'
                ? '3.5h·7h·10.5h·14h·17.5h·21h+'
                : view === 'months'
                ? '15h·30h·45h·60h·75h·90h+'
                : '30m·1h·1.5h·2h·2.5h·3h+'}
            </span>
          </div>
      ...
      {/* GitHub-style tooltip via portal — escapes Card's overflow:hidden */}
      {tooltip && createPortal((() => {
        const { label, display } = fmtTooltip(tooltip.date, tooltip.value);
        return (
          <div style={{ position:'fixed', left:tooltip.x, top:tooltip.y - 8, transform:'translate(-50%,-100%)', zIndex:99999, pointerEvents:'none',
            background: dark ? 'rgba(15,17,26,0.97)' : 'rgba(255,255,255,0.97)',
            border: `1px solid ${dark?'rgba(255,255,255,0.12)':'rgba(0,0,0,0.10)'}`,
            borderRadius:8, padding:'7px 11px', boxShadow:'0 4px 20px rgba(0,0,0,0.35)',
            minWidth:140, textAlign:'center', fontFamily:"'Outfit',sans-serif" }}>
            <p style={{ fontSize:11, fontWeight:700, color: dark?'rgba(255,255,255,0.9)':'#111', margin:'0 0 2px' }}>{display}</p>
            <p style={{ fontSize:10, color: dark?'rgba(255,255,255,0.45)':'rgba(0,0,0,0.45)', margin:0 }}>{label}</p>
            <div style={{ position:'absolute', bottom:-5, left:'50%', width:8, height:8,
              background: dark?'rgba(15,17,26,0.97)':'rgba(255,255,255,0.97)',
              borderRight:`1px solid ${dark?'rgba(255,255,255,0.12)':'rgba(0,0,0,0.10)'}`,
              borderBottom:`1px solid ${dark?'rgba(255,255,255,0.12)':'rgba(0,0,0,0.10)'}`,
