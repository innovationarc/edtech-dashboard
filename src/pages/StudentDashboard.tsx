...
const StudentDashboard = () => {
  ...
  // Heatmaps
  const [studyHeatmap, setStudyHeatmap]     = useState<{ date: string; value: number; level: 0|1|2|3|4 }[]>([]);
  const [appHeatmap, setAppHeatmap]         = useState<{ date: string; value: number; level: 0|1|2|3|4 }[]>([]);
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(18px,2.8vw,30px)', fontFamily: "'Outfit',sans-serif" }}>
      ...
      {/* ── KPI Stats Row ─────────────────────────────────────────────────────── */}
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,130px),1fr))', gap: 'clamp(8px,1.2vw,12px)' }}>
        {[
          { icon: <Clock size={14} color="#6366f1" />, label: 'Today', value: kpiLoading ? '…' : fmtMins(kpiTodayMins), sub: 'study time', accent: '#6366f1' },
          { icon: <TrendingUp size={14} color="#10b981" />, label: 'This Week', value: kpiLoading ? '…' : fmtMins(kpiWeekMins), sub: 'study time', accent: '#10b981' },
          { icon: <CheckCircle size={14} color="#f59e0b" />, label: 'Done', value: kpiLoading ? '…' : `${kpiTasksDone}`, sub: 'objectives', accent: '#f59e0b' },
          { icon: <Flame size={14} color="#ef4444" />, label: 'Streak', value: kpiLoading ? '…' : `${streak.current}`, sub: 'day streak', accent: '#ef4444' },
        ].map((s, i) => (
          <div key={i} style={{ padding: 'clamp(10px,1.4vw,14px)', borderRadius: 14, background: T.surface, border: `1px solid ${T.border}`, display: 'flex', flexDirection: 'column', gap: 6 }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
              <div style={{ width: 26, height: 26, borderRadius: 7, display: 'flex', alignItems: 'center', justifyContent: 'center', background: `${s.accent}18`, border: `1px solid ${s.accent}28`, flexShrink: 0 }}>{s.icon}</div>
              <span style={{ fontSize: 9, fontWeight: 700, color: T.text3, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{s.label}</span>
            </div>
            <div>
              <p style={{ fontSize: 'clamp(1rem,1.8vw,1.3rem)', fontWeight: 800, color: T.text, margin: 0, lineHeight: 1.1 }}>{s.value}</p>
              <p style={{ fontSize: 9, color: T.text3, margin: '2px 0 0' }}>{s.sub}</p>
            </div>
          </div>
        ))}
      </div>
      ...
      {continueLearning && (
        <div style={{ padding: 'clamp(12px,1.8vw,16px)', borderRadius: 16, background: `linear-gradient(135deg,${primaryColor}12 0%,${primaryColor}06 100%)`, border: `1px solid ${primaryColor}28`, display: 'flex', alignItems: 'center', gap: 14 }}>
          ...
          <button onClick={() => navigate(`/student-courses/${continueLearning.courseId}`)} style={{ flexShrink: 0, display: 'flex', alignItems: 'center', gap: 5, padding: '8px 14px', borderRadius: 9, border: 'none', cursor: 'pointer', fontSize: 12, fontWeight: 700, fontFamily: "'Outfit',sans-serif", background: primaryColor, color: '#fff' }}>
      ...
      {/* ── Pending Tasks ─────────────────────────────────────────────────────── */}
      <Card title="Pending Tasks" subtitle="Due within 2 weeks" icon={<AlertTriangle size={15} color="#f59e0b" />} accent="#f59e0b" enterDelay={560}>
      ...
      {/* ── Available Courses ─────────────────────────────────────────────────── */}
      <Card title="Available Courses" subtitle="All published courses" icon={<BookOpen size={15} color="#10b981" />} accent="#10b981" enterDelay={620}>
        {coursesLoading ? (
          ...
          <div style={{ textAlign:'center', padding:'20px 0' }}><BookOpen size={26} color={T.dimIcon} style={{ margin:'0 auto 6px' }} /><p style={{ fontSize:12, color:T.text3, margin:0 }}>No courses available yet</p></div>
        ) : (
          <div style={{ display:'grid', gridTemplateColumns:'repeat(auto-fill,minmax(min(100%,220px),1fr))', gap:'clamp(8px,1.2vw,12px)' }}>
            {allCourses.map(c => (
              <div key={c.id} style={{ padding:'12px 14px', borderRadius:14, background:T.surface, border:`1px solid ${T.border}`, display:'flex', gap:10, alignItems:'flex-start' }}>
                <div style={{ width:36, height:36, borderRadius:9, flexShrink:0, overflow:'hidden', background:'rgba(16,185,129,0.1)', border:'1px solid rgba(16,185,129,0.18)', display:'flex', alignItems:'center', justifyContent:'center' }}>
                  {c.thumbnailUrl ?? c.thumbnail ? <img src={c.thumbnailUrl ?? c.thumbnail} alt="" style={{ width:'100%', height:'100%', objectFit:'cover' }} /> : <BookOpen size={16} color="#10b981" />}
                </div>
                <div style={{ flex:1, minWidth:0 }}>
                  <p style={{ fontSize:'clamp(0.7rem,1.05vw,0.78rem)', fontWeight:700, color:T.text, margin:'0 0 2px', overflow:'hidden', textOverflow:'ellipsis', whiteSpace:'nowrap' }}>{c.title}</p>
                  <p style={{ fontSize:9, color:T.text3, margin:0 }}>{c.instructor}</p>
                  <div style={{ display:'flex', alignItems:'center', gap:4, marginTop:4 }}>
                    <span style={{ fontSize:9, fontWeight:600, background:'rgba(16,185,129,0.12)', color:'#10b981', borderRadius:4, padding:'1px 6px', textTransform:'capitalize' }}>{c.level}</span>
                    {c.price === 0 && <span style={{ fontSize:9, fontWeight:600, background:'rgba(99,102,241,0.12)', color:'#818cf8', borderRadius:4, padding:'1px 6px' }}>Free</span>}
                  </div>
                </div>
              </div>
            ))}
  ...
  return (
    <Card title={title} subtitle={subtitle} icon={icon} accent={accent} enterDelay={enterDelay}>
      <div style={{ display:'flex', flexDirection:'column', gap:10 }}>
        ...
        {days.length === 0 ? (
          <div style={{ textAlign:'center', padding:'20px 0' }}><Zap size={22} color={T.dimIcon} style={{ margin:'0 auto 6px' }} /><p style={{ fontSize:11, color:T.text3, margin:0 }}>No data yet</p></div>
        ) : (
