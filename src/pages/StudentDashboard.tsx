          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {frozen && (
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 7, padding: '7px 14px', borderRadius: 10,
                background: 'linear-gradient(90deg,rgba(147,197,253,0.14),rgba(125,211,252,0.09))',
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
              {cards.map((s, i) => frozen ? (
                ...
                <div key={i} style={{
                  ...
                  padding: '10px 12px',
                  // Card's own dark background — desaturated and dimmed by the ice filter
                  background: isLight ? 'rgba(0,0,0,0.045)' : 'rgba(255,255,255,0.06)',
                  // Ice border replaces normal accent border
                  border: '1px solid rgba(147,197,253,0.55)',
                  boxShadow: '0 0 0 1px rgba(186,230,253,0.20), inset 0 0 20px rgba(147,197,253,0.10)',
                }}>
                  {/* ── Full-card ice overlay ── */}
                  {/* Frosted glass wash — covers entire card including padding */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none',
                    background: 'linear-gradient(135deg, rgba(186,230,253,0.28) 0%, rgba(147,197,253,0.16) 35%, rgba(224,242,254,0.22) 65%, rgba(125,211,252,0.12) 100%)',
                    backdropFilter: 'blur(2px) saturate(0.45) brightness(0.85)',
                    WebkitBackdropFilter: 'blur(2px) saturate(0.45) brightness(0.85)',
                  }} />
                  {/* Ice crack lines — full card coverage */}
                  <svg viewBox="0 0 100 100" preserveAspectRatio="none"
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 1, pointerEvents: 'none', opacity: 0.40 }}>
                    <path d={iceCracks[i]} stroke="rgba(186,230,253,0.95)" strokeWidth="0.9" fill="none" strokeLinecap="round"/>
                    <path d={iceCracks[(i+2)%4]} stroke="rgba(224,242,254,0.65)" strokeWidth="0.55" fill="none" strokeLinecap="round" opacity="0.7"/>
                  </svg>
                  {/* Top-left shimmer — light catching the ice surface */}
                  <div style={{ position: 'absolute', top: -8, left: -8, width: 52, height: 52, zIndex: 1, pointerEvents: 'none',
                    background: 'radial-gradient(circle, rgba(224,242,254,0.60) 0%, transparent 65%)',
                    borderRadius: '50%',
                  }} />
                  {/* Bottom-right shimmer for depth */}
                  ...
                  {/* Snowflake watermark — bottom right corner */}
                  <svg width="36" height="36" viewBox="0 0 24 24" fill="none" stroke="rgba(147,197,253,0.28)" strokeWidth="1.1" strokeLinecap="round"
                  ...
                  {/* Card content — above all ice layers, desaturated */}
                  <div style={{ position: 'relative', zIndex: 2, display: 'flex', flexDirection: 'column', gap: 5,
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      ...
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase',
                        color: 'rgba(186,230,253,0.90)' }}>{s.label}</span>
                    </div>
                    <p style={{ fontSize: 'clamp(1.1rem,2vw,1.35rem)', fontWeight: 800, margin: 0, lineHeight: 1,
                      color: '#dbeafe', textShadow: '0 1px 10px rgba(147,197,253,0.70)' }}>{s.value}</p>
                    <p style={{ fontSize: 9, margin: 0, color: 'rgba(147,197,253,0.80)' }}>{s.sub}</p>
                  </div>
                </div>
              ) : (
