          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
              {cards.map((s, i) => frozen ? (
                // ── Frozen card: real frost crystal formations on edges ──
                <div key={i} style={{
                  position: 'relative', overflow: 'hidden', borderRadius: 14,
                  padding: '10px 12px',
                  background: isLight ? 'rgba(210,235,255,0.60)' : 'rgba(12,22,45,0.82)',
                  border: 'none', boxShadow: 'none',
                }}>

                  {/* Frost crystal SVG — organic ice growing inward from all 4 edges */}
                  <svg viewBox="0 0 200 120" preserveAspectRatio="none"
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 1, pointerEvents: 'none' }}>
                    <defs>
                      <filter id={`frostf${i}`}>
                        <feTurbulence type="fractalNoise" baseFrequency="0.9" numOctaves="4" seed={i * 11 + 5}/>
                        <feDisplacementMap in="SourceGraphic" scale="3" xChannelSelector="R" yChannelSelector="G"/>
                      </filter>
                    </defs>

                    {/* Top frost layer 1 — deepest crystals */}
                    <path d="M0,0 L0,32 C5,24 9,34 14,22 C18,13 22,28 27,17 C31,8 35,24 40,14 C44,6 48,22 53,13 C57,5 61,20 66,11 C70,4 74,18 79,9 C83,3 87,16 92,8 C96,2 100,14 105,7 C109,2 113,13 118,6 C122,1 126,11 131,5 C135,1 139,10 144,4 C148,0 152,9 157,4 C161,0 165,8 170,3 C174,0 178,7 183,2 C187,0 191,5 196,2 L200,0 Z"
                      fill={isLight ? 'rgba(100,160,240,0.65)' : 'rgba(147,197,253,0.55)'}
                      filter={`url(#frostf${i})`} />
                    {/* Top frost layer 2 — sharper edge detail */}
                    <path d="M0,0 L0,18 C4,14 8,20 13,11 C17,5 21,16 26,8 C30,2 34,13 39,7 C43,2 47,12 52,6 C56,1 60,10 65,5 C69,1 73,9 78,4 C82,0 86,8 91,3 C95,0 99,7 104,3 C108,0 112,6 117,2 C121,0 125,5 130,2 C134,0 138,5 143,2 C147,0 151,4 156,2 C160,0 164,4 169,1 C173,0 177,3 182,1 C186,0 190,3 195,1 L200,0 Z"
                      fill={isLight ? 'rgba(147,197,253,0.55)' : 'rgba(186,230,253,0.45)'} />

                    {/* Bottom frost layer 1 */}
                    <path d="M0,120 L0,88 C5,96 9,86 14,98 C18,107 22,92 27,103 C31,112 35,96 40,106 C44,114 48,98 53,107 C57,115 61,100 66,109 C70,116 74,102 79,111 C83,117 87,104 92,112 C96,118 100,106 105,113 C109,118 113,107 118,114 C122,119 126,109 131,115 C135,119 139,110 144,116 C148,120 152,111 157,116 C161,120 165,112 170,117 C174,120 178,113 183,118 C187,120 191,115 196,118 L200,120 Z"
                      fill={isLight ? 'rgba(100,160,240,0.65)' : 'rgba(147,197,253,0.55)'}
                      filter={`url(#frostf${i})`} />
                    {/* Bottom frost layer 2 */}
                    <path d="M0,120 L0,108 C4,112 8,106 13,114 C17,120 21,110 26,116 C30,120 34,112 39,118 C43,120 47,113 52,119 C56,122 60,115 65,120 C69,122 73,116 78,121 C82,122 86,117 91,121 C95,122 99,117 104,121 C108,122 112,118 117,121 C121,122 125,118 130,121 C134,122 138,118 143,121 C147,122 151,118 156,121 C160,122 164,118 169,121 C173,122 177,118 182,121 C186,122 190,118 195,121 L200,120 Z"
                      fill={isLight ? 'rgba(147,197,253,0.55)' : 'rgba(186,230,253,0.45)'} />

                    {/* Left frost layer 1 */}
                    <path d="M0,0 L32,0 C24,5 34,9 22,14 C13,18 28,22 17,27 C8,31 24,35 14,40 C6,44 22,48 13,53 C5,57 20,61 11,66 C4,70 18,74 9,79 C3,83 16,87 8,92 C2,96 14,100 7,105 C1,109 12,113 5,118 L0,120 Z"
                      fill={isLight ? 'rgba(100,160,240,0.62)' : 'rgba(147,197,253,0.52)'}
                      filter={`url(#frostf${i})`} />
                    {/* Left frost layer 2 */}
                    <path d="M0,0 L18,0 C14,4 20,8 11,13 C5,17 16,21 8,26 C2,30 13,34 7,39 C1,43 12,47 6,52 C1,56 10,60 5,65 C1,69 9,73 4,78 C0,82 8,86 3,91 C0,95 7,99 3,104 C0,108 6,112 2,117 L0,120 Z"
                      fill={isLight ? 'rgba(147,197,253,0.52)' : 'rgba(186,230,253,0.42)'} />

                    {/* Right frost layer 1 */}
                    <path d="M200,0 L168,0 C176,5 166,9 178,14 C187,18 172,22 183,27 C192,31 176,35 186,40 C194,44 178,48 187,53 C195,57 180,61 189,66 C196,70 182,74 191,79 C197,83 184,87 192,92 C198,96 186,100 193,105 C199,109 188,113 195,118 L200,120 Z"
                      fill={isLight ? 'rgba(100,160,240,0.62)' : 'rgba(147,197,253,0.52)'}
                      filter={`url(#frostf${i})`} />
                    {/* Right frost layer 2 */}
                    <path d="M200,0 L182,0 C186,4 180,8 189,13 C195,17 184,21 192,26 C198,30 187,34 193,39 C199,43 188,47 194,52 C199,56 190,60 195,65 C199,69 191,73 196,78 C200,82 192,86 197,91 C200,95 193,99 197,104 C200,108 194,112 198,117 L200,120 Z"
                      fill={isLight ? 'rgba(147,197,253,0.52)' : 'rgba(186,230,253,0.42)'} />
                  </svg>

                  {/* Subtle backdrop blur — preserves content readability */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 2, pointerEvents: 'none',
                    backdropFilter: 'blur(4px) saturate(0.45) brightness(0.88)',
                    WebkitBackdropFilter: 'blur(4px) saturate(0.45) brightness(0.88)',
                  }} />

                  {/* Card content */}
                  <div style={{ position: 'relative', zIndex: 3, display: 'flex', flexDirection: 'column', gap: 5,
                    filter: isLight ? 'saturate(0.25) brightness(0.75)' : 'saturate(0.30) brightness(0.82)' }}>
