...
function hmapLevel(v: number): 0|1|2|3|4|5|6|7 {
  // Fixed daily thresholds (minutes) — 1-hour intervals up to 8h+
  // 0=none 1=<1h 2=1h 3=2h 4=3h 5=4h 6=5h–6h 7=6h–7h (8=8h+ shown as 7)
  // 8 distinct colors: empty + 7 filled levels
  if (!v)       return 0;
  if (v < 60)   return 1;
  if (v < 120)  return 2;
  if (v < 180)  return 3;
    ...
    try {
      ...
      allEvs.forEach(e => {
        ...
        try {
          ...
          addMins(dateStr, Math.max(0, (eh * 60 + em) - (sh * 60 + sm)));
        } catch {}
      });
      ...
      // Any previous day with 0 mins immediately breaks it.
      let currentStreak = 0;
      ...
      for (let i = 0; i < 90; i++) {
        ...
        if (i === 0) {
          // Today: count it if active, but a zero day doesn't break the streak
      ...
      // 10-week rolling window rendered as a grid of daily cells.
      const WEEKS = 10;
      ...
      for (let i = WEEKS * 7 - 1; i >= 0; i--) {
        ...
        heatmapDays.push({ date: k, value: v, level: hmapLevel(v) });
      }
