              ...
              <div style={{ position: 'relative', height: 130, borderRadius: 10, background: T.starBg, border: `1px solid ${T.border}`, overflow: 'hidden' }}>
                ...
                {c.stars.map(s => (
                  <div key={s.id} style={{ position: 'absolute', left: `${s.position.x}%`, top: `${s.position.y}%`, transform: 'translate(-50%,-50%)' }} title={`${s.name} – ${s.progress}%`}>
                    <Star size={s.mastered ? 16 : 12} color={s.mastered ? '#fbbf24' : s.progress > 50 ? (dark ? `${c.color}88` : c.color) : T.dimIcon} fill={s.mastered ? '#fbbf24' : s.progress > 50 ? (dark ? 'none' : `${c.color}22`) : 'none'} style={{ filter: s.mastered ? 'drop-shadow(0 0 4px rgba(251,191,36,0.6))' : 'none' }} />
                  </div>
                ))}
              </div>
                ...
                {c.stars.map(s => (
                  <div key={s.id} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                      <Star size={9} color={s.mastered ? '#fbbf24' : (dark ? T.dimIcon : 'rgba(99,102,241,0.40)')} fill={s.mastered ? '#fbbf24' : 'none'} />
