          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
              {cards.map((s, i) => frozen ? (
                // ── Frozen card: dense opaque frost at edges, clear center ──
                <div key={i} style={{
                  position: 'relative', overflow: 'hidden', borderRadius: 14,
                  padding: '10px 12px',
                  // Dark cool base so frost reads as cold
                  background: dark ? 'rgba(6,16,38,0.90)' : 'rgba(220,240,255,0.70)',
                  border: 'none', boxShadow: 'none',
                }}>

                  {/* ── Dense solid frost border — high opacity, NO blur, sharp inner edge ── */}
                  {/* Top */}
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 18, zIndex: 1, pointerEvents: 'none',
                    background: dark
                      ? 'linear-gradient(to bottom, rgba(147,197,253,0.95) 0%, rgba(147,197,253,0.85) 40%, rgba(186,230,253,0.50) 70%, transparent 100%)'
                      : 'linear-gradient(to bottom, rgba(59,130,246,0.90) 0%, rgba(96,165,250,0.75) 40%, rgba(147,197,253,0.40) 70%, transparent 100%)',
                  }} />
                  {/* Bottom */}
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 18, zIndex: 1, pointerEvents: 'none',
                    background: dark
                      ? 'linear-gradient(to top, rgba(147,197,253,0.95) 0%, rgba(147,197,253,0.85) 40%, rgba(186,230,253,0.50) 70%, transparent 100%)'
                      : 'linear-gradient(to top, rgba(59,130,246,0.90) 0%, rgba(96,165,250,0.75) 40%, rgba(147,197,253,0.40) 70%, transparent 100%)',
                  }} />
                  {/* Left */}
                  <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 18, zIndex: 1, pointerEvents: 'none',
                    background: dark
                      ? 'linear-gradient(to right, rgba(147,197,253,0.95) 0%, rgba(147,197,253,0.85) 40%, rgba(186,230,253,0.50) 70%, transparent 100%)'
                      : 'linear-gradient(to right, rgba(59,130,246,0.90) 0%, rgba(96,165,250,0.75) 40%, rgba(147,197,253,0.40) 70%, transparent 100%)',
                  }} />
                  {/* Right */}
                  <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 18, zIndex: 1, pointerEvents: 'none',
                    background: dark
                      ? 'linear-gradient(to left, rgba(147,197,253,0.95) 0%, rgba(147,197,253,0.85) 40%, rgba(186,230,253,0.50) 70%, transparent 100%)'
                      : 'linear-gradient(to left, rgba(59,130,246,0.90) 0%, rgba(96,165,250,0.75) 40%, rgba(147,197,253,0.40) 70%, transparent 100%)',
                  }} />

                  {/* ── Corner caps — extra dense, covers the corner gap between edge strips ── */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 2, pointerEvents: 'none',
                    background: dark
                      ? 'radial-gradient(ellipse 55% 55% at 0% 0%, rgba(147,197,253,0.98) 0%, rgba(147,197,253,0.70) 20%, transparent 55%), radial-gradient(ellipse 55% 55% at 100% 0%, rgba(147,197,253,0.98) 0%, rgba(147,197,253,0.70) 20%, transparent 55%), radial-gradient(ellipse 55% 55% at 0% 100%, rgba(147,197,253,0.98) 0%, rgba(147,197,253,0.70) 20%, transparent 55%), radial-gradient(ellipse 55% 55% at 100% 100%, rgba(147,197,253,0.98) 0%, rgba(147,197,253,0.70) 20%, transparent 55%)'
                      : 'radial-gradient(ellipse 55% 55% at 0% 0%, rgba(59,130,246,0.92) 0%, rgba(96,165,250,0.65) 20%, transparent 55%), radial-gradient(ellipse 55% 55% at 100% 0%, rgba(59,130,246,0.92) 0%, rgba(96,165,250,0.65) 20%, transparent 55%), radial-gradient(ellipse 55% 55% at 0% 100%, rgba(59,130,246,0.92) 0%, rgba(96,165,250,0.65) 20%, transparent 55%), radial-gradient(ellipse 55% 55% at 100% 100%, rgba(59,130,246,0.92) 0%, rgba(96,165,250,0.65) 20%, transparent 55%)',
                  }} />

                  {/* ── Crystal texture: feTurbulence noise on top of the solid frost ── */}
                  <svg viewBox="0 0 200 100" preserveAspectRatio="none"
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 3, pointerEvents: 'none', opacity: dark ? 0.22 : 0.18 }}>
                    <defs>
                      <filter id={`ice${i}`} x="0" y="0" width="100%" height="100%">
                        <feTurbulence type="fractalNoise" baseFrequency="1.2 0.8" numOctaves="4" seed={i * 13 + 7} result="noise"/>
                        <feColorMatrix type="saturate" values="0" in="noise" result="grey"/>
                        <feBlend in="SourceGraphic" in2="grey" mode="overlay"/>
                      </filter>
                    </defs>
                    <rect width="200" height="100" fill={dark ? 'rgba(186,230,253,0.9)' : 'rgba(147,197,253,0.9)'} filter={`url(#ice${i})`}/>
                  </svg>

                  {/* ── Slight global frosted-glass — keeps content readable ── */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 4, pointerEvents: 'none',
                    backdropFilter: 'blur(2px) saturate(0.45)',
                    WebkitBackdropFilter: 'blur(2px) saturate(0.45)',
                  }} />

                  {/* Card content */}
                  <div style={{ position: 'relative', zIndex: 5, display: 'flex', flexDirection: 'column', gap: 5,
                    filter: dark ? 'saturate(0.30) brightness(0.82)' : 'saturate(0.25) brightness(0.78)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {s.icon}
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase',
                        color: dark ? 'rgba(186,230,253,0.90)' : 'rgba(30,58,138,0.85)' }}>{s.label}</span>
                    </div>
                    <p style={{ fontSize: 'clamp(1.1rem,2vw,1.35rem)', fontWeight: 800, margin: 0, lineHeight: 1,
                      color: dark ? '#dbeafe' : '#1e3a5f',
                      textShadow: dark ? '0 1px 10px rgba(147,197,253,0.65)' : 'none' }}>{s.value}</p>
                    <p style={{ fontSize: 9, margin: 0,
                      color: dark ? 'rgba(147,197,253,0.80)' : 'rgba(30,58,138,0.70)' }}>{s.sub}</p>
                  </div>
                </div>
