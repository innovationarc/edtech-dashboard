...
const StudentDashboard = () => {
  ...
  const loadAppHeatmap = async () => {
    if (!user) return;
    try {
      const raw = await dashboardStatsService.getAppUsageHeatmap(user.uid, 10);
      // Service stores durationSeconds — convert to minutes for display (unit="min")
      const heatmap = raw.map(d => ({ ...d, value: Math.round(d.value / 60) }));
      // Apply fixed thresholds after conversion
