          ...
          <div className="flex flex-col gap-2">
            ...
            <div className="relative flex justify-center items-center h-24">
              <svg width="96" height="96" className="-rotate-90" style={{ position: 'absolute' }}>
                <circle cx="48" cy="48" r="40" strokeWidth="7" stroke="#1f2937" fill="none" />
                <circle cx="48" cy="48" r="40" strokeWidth="7" stroke={pomodoroMode === 'focus' ? '#6366f1' : pomodoroMode === 'short' ? '#10b981' : '#f59e0b'} fill="none" strokeDasharray={circumference} strokeDashoffset={circumference * (1 - pct / 100)} strokeLinecap="round" style={{ transition: 'stroke-dashoffset 1s linear' }} />
              </svg>
              <div className="relative text-center z-10">
                <span className="text-xl font-bold font-mono" style={{ color: T.text }}>{pMins}:{pSecs}</span>
...
const HeatmapCard = ({
  ...
  return (
    <Card title={title} subtitle={subtitle} icon={icon} accent={accent} enterDelay={enterDelay}>
      ...
      {/* GitHub-style tooltip via portal */}
      {tooltip && (() => {
        ...
        return (
          <div style={{ position:'fixed', left:tooltip.x, top:tooltip.y - 8, transform:'translate(-50%,-100%)', zIndex:99999, pointerEvents:'none',
            ...
            {/* arrow */}
            <div style={{ position:'absolute', bottom:-5, left:'50%', transform:'translateX(-50%)', width:8, height:8,
              background: dark?'rgba(15,17,26,0.97)':'rgba(255,255,255,0.97)',
