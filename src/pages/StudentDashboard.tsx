...
const StudentDashboard = () => {
  ...
  // Objectives — Firebase-backed via dashboardService
  const [objectives, setObjectives]                         = useState<DashboardObjective[]>([]);
  const [objectivesLoading, setObjectivesLoading]           = useState(true);

  // Goals — from studyPlanService (student's real Study Planner goals)
  const [goals, setGoals]                                   = useState<StudyGoal[]>([]);
  const [goalsLoading, setGoalsLoading]                     = useState(true);

  // Inspiration — admin-seeded from Firestore, fallback to local quote
  ...
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      ...
      loadLiveClasses(); loadLiveStreams(); loadLiveExams();
    }, 30000);
  ...
  // Timer tick + pomodoro save on completion
  useEffect(() => {
    ...
    const iv = setInterval(() => {
      if (timerSec > 0) {
        ...
        if (timerMode === 'focus') {
          ...
          if (user) {
            gamificationService.recordActivity(user.uid, 'study_session', { duration: 25 });
            studyPlanService.savePomodoroSession({
              ...
              duration: 25,
              completed: true,
              notes: 'Completed via dashboard focus timer',
            }).catch(() => {});
          }
          setTimerMode('break');
          setTimerMin(5);
  ...
  const loadObjectives = async () => {
    ...
    try {
      ...
      setObjectives(await dashboardService.getObjectives(user.uid));
  ...
  const loadInspiration = async () => {
    try {
      ...
      const q = await inspirationService.getRandom();
      setInspiration(q);
    } catch { /* silent — fallback to local quote */ } finally { setInspirationLoading(false); }
  };
  ...
  const loadCalendarEvents = async () => {
    ...
    try {
      ...
      setCalendarEvents(
        all
          .filter(e => { const d = new Date(e.date.getFullYear(), e.date.getMonth(), e.date.getDate()); return d >= today && d <= next; })
  ...
  const loadLiveClasses = async () => {
    try {
      ...
      const now = new Date(); const in48h = new Date(now.getTime() + 48 * 60 * 60 * 1000);
      setLiveClasses(all.filter(c => {
        if (c.status === 'live') return true;
        if (c.status === 'scheduled') { const t = c.scheduledAt.toDate(); return t >= now && t <= in48h; }
        return false;
      }));
    } catch {}
  };
  ...
  const loadLiveStreams = async () => {
    try {
      ...
      setLiveStreams(all.filter(s => {
        ...
        if (s.status === 'scheduled' && s.scheduledAt) { const t = s.scheduledAt.toDate(); return t >= now && t <= in48h; }
        return false;
      }));
    } catch {}
  };
  ...
  const toggleObj = async (id: string, current: boolean) => {
    ...
    try {
      await dashboardService.toggleObjective(id, !current);
    } catch {
      // Revert on failure
      setObjectives(prev => prev.map(o => o.id === id ? { ...o, completed: current } : o));
    }
  ...
  const addObj = async (title: string, priority: 'high'|'medium'|'low') => {
    ...
    try {
      const newObj = await dashboardService.addObjective(user.uid, title, priority);
  ...
  const pct      = objectives.length > 0 ? Math.round((done / objectives.length) * 100) : 0;
  const circ     = 2 * Math.PI * 42;
  const timerPct = (timerMin * 60 + timerSec) / ((timerMode === 'focus' ? 25 : 5) * 60);
  const hour     = new Date().getHours();
  const greeting = hour < 12 ? 'Good morning' : hour < 17 ? 'Good afternoon' : 'Good evening';

  // Inspiration display — prefer Firestore, fallback to local quote
  const inspirationText   = inspiration ? inspiration.text   : dailyQuote.text;
  const inspirationAuthor = inspiration ? inspiration.author : dailyQuote.author;
    ...
    <div style={{display:'flex',flexDirection:'column',gap:'clamp(18px,2.8vw,30px)',fontFamily:"'Outfit',sans-serif",transformStyle:'preserve-3d'}}>
      ...
      <div style={{animation:'cardReveal 500ms cubic-bezier(0.22,1,0.36,1) 0ms both'}}>
        <div className="flex items-start justify-between">
          ...
          <div className="text-right" style={{flexShrink:0,marginLeft:12,display:'flex',flexDirection:'column',alignItems:'flex-end',paddingTop:isMobile?2:4}}>
            <div style={{fontSize:11,color:T.text3,fontWeight:600,textTransform:'uppercase',letterSpacing:'0.08em'}}>Today</div>
            {isMobile ? <>
              <div style={{fontSize:16,fontWeight:700,color:T.text,whiteSpace:'nowrap'}}>{new Date().toLocaleDateString('en-US',{weekday:'long'})},</div>
              <div style={{fontSize:16,fontWeight:700,color:T.text,whiteSpace:'nowrap'}}>{new Date().toLocaleDateString('en-US',{month:'short',day:'numeric'})}</div>
            </> :
              <div style={{fontSize:16,fontWeight:700,color:T.text,whiteSpace:'nowrap'}}>{new Date().toLocaleDateString('en-US',{weekday:'long',month:'short',day:'numeric'})}</div>
            }
          </div>
        </div>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(100%,280px),1fr))',gap:'clamp(8px,1.2vw,14px)'}}>
            {liveStreams.map(s => {
              ...
              const diffH = Math.floor(diffMs / 3600000); const diffM = Math.floor((diffMs % 3600000) / 60000);
              const timeLabel = isLive ? 'Live now' : diffH > 0 ? `In ${diffH}h ${diffM}m` : `In ${diffM}m`;
              return (
                <div key={s.id} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',borderRadius:13,background:isLive?'rgba(99,102,241,0.08)':T.surface,border:isLive?`1px solid rgba(99,102,241,0.28)`:`1px solid ${T.border}`,transition:'all 0.15s'}}>
                  ...
                  <button onClick={()=>navigate('/student-streams')} style={{flexShrink:0,display:'flex',alignItems:'center',gap:5,padding:'6px 12px',borderRadius:8,border:'none',cursor:'pointer',fontSize:11,fontWeight:700,fontFamily:"'Outfit',sans-serif",background:`${primaryColor}cc`,color:'#fff',boxShadow:`0 2px 8px ${primaryColor}40`}}>
        ...
        <div style={{display:'flex',flexDirection:'column',gap:8}}>
          ...
          <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(min(100%,280px),1fr))',gap:'clamp(8px,1.2vw,14px)'}}>
            {liveClasses.map(cls => {
              ...
              return (
                <div key={cls.id} style={{display:'flex',alignItems:'center',gap:12,padding:'11px 14px',borderRadius:13,background:isLive?'rgba(239,68,68,0.08)':T.surface,border:isLive?'1px solid rgba(239,68,68,0.28)':`1px solid ${T.border}`,transition:'all 0.15s'}}>
                  ...
                  <button onClick={()=>navigate('/student-live-classes')} style={{flexShrink:0,display:'flex',alignItems:'center',gap:5,padding:'6px 12px',borderRadius:8,border:'none',cursor:'pointer',fontSize:11,fontWeight:700,fontFamily:"'Outfit',sans-serif",background:isLive?'#ef4444':'rgba(99,102,241,0.85)',color:'#fff',boxShadow:isLive?'0 2px 10px rgba(239,68,68,0.35)':'0 2px 8px rgba(99,102,241,0.3)'}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:6}}>
              {liveExams.map(exam => (
                <button key={exam.id} onClick={()=>navigate(`/student-live-exams?highlight=${exam.id}`)}
                  style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px',borderRadius:10,background:T.surface,border:`1px solid ${T.border}`,cursor:'pointer',textAlign:'left',width:'100%',fontFamily:"'Outfit',sans-serif",transition:'all 0.15s'}}
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,230px),1fr))',gap:'clamp(14px,1.8vw,22px)',transformStyle:'preserve-3d'}}>
          ...
          <div style={{display:'flex',flexDirection:'column',gap:8}}>
            {objectivesLoading ? (
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'24px 0'}}>
                <Loader size={15} color="#6366f1" className="animate-spin"/>
              ...
              <div key={o.id} onClick={()=>toggleObj(o.id, o.completed)}
                ...
                onMouseEnter={e=>{e.currentTarget.style.background=dark?'rgba(255,255,255,0.07)':'rgba(0,0,0,0.04)';e.currentTarget.style.transform='translateX(3px)';}}
                onMouseLeave={e=>{e.currentTarget.style.background=o.completed?(dark?'rgba(16,185,129,0.07)':'rgba(16,185,129,0.06)'):T.surface;e.currentTarget.style.transform='translateX(0)';}}
              >
                {o.completed
                ...
                <span style={{fontSize:9,fontWeight:700,letterSpacing:'0.05em',textTransform:'uppercase',padding:'3px 8px',borderRadius:99,flexShrink:0,background:dark?'rgba(255,255,255,0.07)':'rgba(0,0,0,0.06)',color:PC(o.priority),border:'1px solid rgba(0,0,0,0.08)'}}>{o.priority}</span>
              </div>
            ))}
            <button onClick={()=>setShowObjModal(true)} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:6,padding:'9px 14px',borderRadius:12,marginTop:4,background:isLight?'rgba(99,102,241,0.07)':'rgba(99,102,241,0.10)',border:'1px solid rgba(99,102,241,0.20)',color:'#818cf8',fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:"'Outfit',sans-serif",transition:'all 0.18s'}}
          ...
          {announcementsLoading ? (
            ...
            <div style={{display:'flex',flexDirection:'column',gap:9}}>
              {announcements.slice(0,3).map(a=>(
                <div key={a.id} style={{padding:'11px 13px',borderRadius:14,background:T.surface,border:`1px solid ${T.border}`,transition:'background 0.18s'}}>
        ...
        <Card title="Daily Inspiration" icon={<Star size={15} color="#f59e0b"/>} accent="#f59e0b" enterDelay={270}>
          <div style={{display:'flex',flexDirection:'column',alignItems:'center',textAlign:'center',gap:14}}>
            ...
            {inspirationLoading ? (
              <Loader size={16} color="#f59e0b" className="animate-spin"/>
            ) : (
              <>
            ...
            <div style={{width:'100%',borderTop:`1px solid ${T.border}`,paddingTop:9}}>
              <button
                ...
                style={{display:'inline-flex',alignItems:'center',gap:5,fontSize:11,fontWeight:600,color:T.muted,background:T.surface,border:`1px solid ${T.border}`,borderRadius:20,padding:'5px 11px',cursor:'pointer',fontFamily:"'Outfit',sans-serif"}}
              >
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(min(100%,290px),1fr))',gap:'clamp(14px,1.8vw,22px)',transformStyle:'preserve-3d'}}>
        ...
        {/* Weekly Schedule — unchanged, already dynamic */}
        <Card title="Weekly Schedule" subtitle="Events this week" icon={<Calendar size={15} color="#06b6d4"/>} accent="#06b6d4" enterDelay={360}>
          <div style={{display:'flex',flexDirection:'column',gap:11}}>
            ...
            <div style={{display:'flex',flexDirection:'column',gap:5}}>
              ...
              {calendarLoading
                ? <div style={{display:'flex',alignItems:'center',gap:5,padding:'6px 0'}}><Loader size={13} color="#6366f1" className="animate-spin"/><span style={{fontSize:11,color:T.text2}}>Loading…</span></div>
                : calendarEvents.length===0
                  ...
                  : calendarEvents.slice(0,4).map(ev=>(
                    <div key={ev.id} style={{display:'flex',alignItems:'center',gap:10,padding:'9px 12px',borderRadius:14,background:T.surface,border:`1px solid ${T.border}`,transition:'background 0.18s'}}>
              ...
              <button onClick={()=>setShowEventModal(true)} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:5,padding:'8px 14px',borderRadius:12,marginTop:4,background:isLight?'rgba(6,182,212,0.07)':'rgba(6,182,212,0.10)',border:'1px solid rgba(6,182,212,0.20)',color:'#22d3ee',fontSize:11,fontWeight:600,cursor:'pointer',fontFamily:"'Outfit',sans-serif",transition:'all 0.18s'}}
        ...
        {/* Goals — from studyPlanService */}
        <Card title="My Goals" subtitle="From your Study Planner" icon={<Award size={15} color="#f97316"/>} accent="#f97316" enterDelay={450}>
          <div style={{display:'flex',flexDirection:'column',gap:9}}>
            {goalsLoading ? (
              <div style={{display:'flex',alignItems:'center',justifyContent:'center',gap:8,padding:'24px 0'}}>
                <Loader size={15} color="#f97316" className="animate-spin"/>
                <span style={{fontSize:12,color:T.text2}}>Loading…</span>
              </div>
            ) : goals.length === 0 ? (
              <div style={{textAlign:'center',padding:'20px 0'}}>
                <Award size={28} color={T.dimIcon} style={{margin:'0 auto 7px'}}/>
                <p style={{fontSize:12,color:T.text3,margin:'0 0 6px'}}>No active goals yet</p>
                <button onClick={()=>navigate('/student-study-plan')} style={{fontSize:11,color:'#fb923c',background:'none',border:'none',cursor:'pointer'}}>Open Study Planner →</button>
              </div>
            ) : goals.map(g => {
              const progress = Math.round(g.hoursCompleted > 0 ? (g.hoursCompleted / g.hoursNeeded) * 100 : g.currentProgress ?? 0);
              return (
                <div key={g.id} style={{padding:'12px 14px',borderRadius:14,background:T.surface,border:`1px solid ${T.border}`,transition:'all 0.18s'}}>
                  ...
                  <div style={{display:'flex',alignItems:'center',gap:8}}>
                    <div style={{flex:1,height:5,borderRadius:99,background:T.trackBg,overflow:'hidden'}}>
                      <div style={{height:'100%',width:`${Math.min(progress,100)}%`,borderRadius:99,background:'linear-gradient(90deg,#6366f1,#8b5cf6,#06b6d4)'}}/>
                    </div>
                    <span style={{fontSize:11,fontWeight:700,color:progress>70?'#10b981':progress>40?'#f59e0b':T.muted,flexShrink:0,minWidth:30,textAlign:'right'}}>{Math.min(progress,100)}%</span>
                  </div>
            ...
            <button onClick={()=>navigate('/student-study-plan')} style={{display:'flex',alignItems:'center',justifyContent:'center',gap:5,padding:'9px 14px',borderRadius:12,background:isLight?'rgba(249,115,22,0.07)':'rgba(249,115,22,0.10)',border:'1px solid rgba(249,115,22,0.20)',color:'#fb923c',fontSize:12,fontWeight:600,cursor:'pointer',fontFamily:"'Outfit',sans-serif",transition:'all 0.18s'}}
              ...
              <Plus size={11}/> Add goal in Study Planner
            </button>
          </div>
        </Card>
      </div>
      ...
      {showEventModal && (
        <StudyPlanEventModal
          ...
          onSave={()=>{setShowEventModal(false);loadCalendarEvents();}}
        />
      )}
    </div>
  );
