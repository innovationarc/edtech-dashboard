...
const StudentDashboard = () => {
  ...
  useEffect(() => {
    ...
    // Poll every 5 minutes (was 30s) — paused when tab is hidden to avoid quota exhaustion
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
              {cards.map((s, i) => frozen ? (
                // ── Frozen card: solid frost BAND at edges, clean center ──
                <div key={i} style={{
                  position: 'relative', overflow: 'hidden', borderRadius: 14,
                  padding: '10px 12px',
                  // Clean icy center background
                  background: dark ? 'rgba(10,22,50,0.85)' : 'rgba(245,252,255,0.92)',
                  border: 'none', boxShadow: 'none',
                }}>

                  {/* ── SOLID FROST BAND — top edge, full opacity, jagged inner boundary ── */}
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, zIndex: 2, pointerEvents: 'none',
                    height: 22,
                    background: dark
                      ? 'linear-gradient(180deg, rgba(214,240,255,0.98) 0%, rgba(186,230,253,0.95) 35%, rgba(147,197,253,0.80) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)'
                      : 'linear-gradient(180deg, rgba(255,255,255,0.99) 0%, rgba(214,240,255,0.97) 35%, rgba(147,197,253,0.82) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)',
                  }} />
                  {/* ── SOLID FROST BAND — bottom edge ── */}
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, zIndex: 2, pointerEvents: 'none',
                    height: 22,
                    background: dark
                      ? 'linear-gradient(0deg, rgba(214,240,255,0.98) 0%, rgba(186,230,253,0.95) 35%, rgba(147,197,253,0.80) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)'
                      : 'linear-gradient(0deg, rgba(255,255,255,0.99) 0%, rgba(214,240,255,0.97) 35%, rgba(147,197,253,0.82) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)',
                  }} />
                  {/* ── SOLID FROST BAND — left edge ── */}
                  <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, zIndex: 2, pointerEvents: 'none',
                    width: 22,
                    background: dark
                      ? 'linear-gradient(90deg, rgba(214,240,255,0.98) 0%, rgba(186,230,253,0.95) 35%, rgba(147,197,253,0.80) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)'
                      : 'linear-gradient(90deg, rgba(255,255,255,0.99) 0%, rgba(214,240,255,0.97) 35%, rgba(147,197,253,0.82) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)',
                  }} />
                  {/* ── SOLID FROST BAND — right edge ── */}
                  <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, zIndex: 2, pointerEvents: 'none',
                    width: 22,
                    background: dark
                      ? 'linear-gradient(270deg, rgba(214,240,255,0.98) 0%, rgba(186,230,253,0.95) 35%, rgba(147,197,253,0.80) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)'
                      : 'linear-gradient(270deg, rgba(255,255,255,0.99) 0%, rgba(214,240,255,0.97) 35%, rgba(147,197,253,0.82) 65%, rgba(147,197,253,0.20) 85%, transparent 100%)',
                  }} />

                  {/* ── CORNER CAPS — fill the corner gaps between bands, extra dense ── */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 3, pointerEvents: 'none',
                    background: dark
                      ? 'radial-gradient(circle 38px at 0px 0px, rgba(224,242,254,0.99) 0%, rgba(186,230,253,0.90) 40%, transparent 72%), radial-gradient(circle 38px at 100% 0px, rgba(224,242,254,0.99) 0%, rgba(186,230,253,0.90) 40%, transparent 72%), radial-gradient(circle 38px at 0px 100%, rgba(224,242,254,0.99) 0%, rgba(186,230,253,0.90) 40%, transparent 72%), radial-gradient(circle 38px at 100% 100%, rgba(224,242,254,0.99) 0%, rgba(186,230,253,0.90) 40%, transparent 72%)'
                      : 'radial-gradient(circle 38px at 0px 0px, rgba(255,255,255,1.0) 0%, rgba(214,240,255,0.95) 40%, transparent 72%), radial-gradient(circle 38px at 100% 0px, rgba(255,255,255,1.0) 0%, rgba(214,240,255,0.95) 40%, transparent 72%), radial-gradient(circle 38px at 0px 100%, rgba(255,255,255,1.0) 0%, rgba(214,240,255,0.95) 40%, transparent 72%), radial-gradient(circle 38px at 100% 100%, rgba(255,255,255,1.0) 0%, rgba(214,240,255,0.95) 40%, transparent 72%)',
                  }} />

                  {/* ── CRYSTAL TEXTURE on the frost band — SVG jagged/noise on edges only ── */}
                  <svg viewBox="0 0 100 100" preserveAspectRatio="none"
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 4, pointerEvents: 'none',
                      opacity: dark ? 0.55 : 0.45 }}>
                    <defs>
                      <filter id={`tex${i}`}>
                        <feTurbulence type="fractalNoise" baseFrequency="1.8 1.2" numOctaves="4" seed={i * 9 + 3}/>
                        <feColorMatrix type="matrix" values="0 0 0 0 0.85  0 0 0 0 0.93  0 0 0 0 1  0 0 0 1.5 -0.3"/>
                      </filter>
                      {/* Mask: only apply texture at edges, not center */}
                      <mask id={`em${i}`}>
                        <rect width="100" height="100" fill="white"/>
                        <ellipse cx="50" cy="50" rx="36" ry="34" fill="black"/>
                      </mask>
                    </defs>
                    <rect width="100" height="100" filter={`url(#tex${i})`} mask={`url(#em${i})`}/>
                  </svg>

                  {/* ── CRACK LINES — originate from edge frost into center ── */}
                  <svg viewBox="0 0 100 100" preserveAspectRatio="none"
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 5, pointerEvents: 'none' }}>
                    {[
                      ["M5,8 L22,18 L18,32 L35,28 M22,18 L30,12 M18,32 L12,45","M85,5 L75,20 L82,35 M75,20 L65,28 M3,75 L18,68 L28,78","M70,80 L80,70 L95,75 M80,70 L78,58"],
                      ["M10,5 L25,22 L20,38 M25,22 L38,18 M38,18 L42,8","M80,10 L68,25 L75,40 M68,25 L60,18","M5,85 L20,75 L30,85 M20,75 L15,62","M75,90 L85,78 L95,82 M85,78 L82,65"],
                      ["M8,12 L20,25 L15,40 M20,25 L32,20 M88,8 L72,22 L78,38","M72,22 L62,15 M5,78 L22,70 L18,85","M80,85 L88,72 L95,78 M88,72 L85,60"],
                      ["M12,5 L28,20 L22,35 M28,20 L40,15 M85,12 L70,28 L80,42","M70,28 L58,20 M8,80 L25,72 L20,88","M72,88 L82,75 L95,80 M82,75 L80,62"],
                    ][i % 4].map((p, ci) => (
                      <g key={ci}>
                        <path d={p} stroke="rgba(255,255,255,0.92)" strokeWidth="2.2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                        <path d={p} stroke={dark ? "rgba(96,165,250,0.85)" : "rgba(59,130,246,0.75)"} strokeWidth="0.9" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                        <path d={p} stroke={dark ? "rgba(29,78,216,0.55)" : "rgba(29,78,216,0.50)"} strokeWidth="0.35" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                      </g>
                    ))}
                  </svg>

                  {/* Specular glints */}
                  <div style={{ position: 'absolute', top: 5, left: 7, width: 26, height: 10, zIndex: 6, pointerEvents: 'none',
                    background: 'radial-gradient(ellipse, rgba(255,255,255,0.95) 0%, transparent 70%)',
                    borderRadius: '50%', transform: 'rotate(-20deg)', filter: 'blur(0.8px)',
                  }} />

                  {/* Frosted glass — light, just desaturates content */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 7, pointerEvents: 'none',
                    backdropFilter: 'blur(2.5px) saturate(0.42) brightness(0.84)',
                    WebkitBackdropFilter: 'blur(2.5px) saturate(0.42) brightness(0.84)',
                  }} />

                  {/* Card content */}
                  <div style={{ position: 'relative', zIndex: 8, display: 'flex', flexDirection: 'column', gap: 5 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {s.icon}
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase',
                        color: dark ? 'rgba(186,230,253,0.92)' : 'rgba(30,58,138,0.88)' }}>{s.label}</span>
                    </div>
                    <p style={{ fontSize: 'clamp(1.1rem,2vw,1.35rem)', fontWeight: 800, margin: 0, lineHeight: 1,
                      color: dark ? '#e0f2fe' : '#1e3a5f',
                      textShadow: dark ? '0 0 12px rgba(147,197,253,0.65)' : 'none' }}>{s.value}</p>
                    <p style={{ fontSize: 9, margin: 0,
                      color: dark ? 'rgba(147,197,253,0.82)' : 'rgba(30,58,138,0.72)' }}>{s.sub}</p>
                  </div>
                </div>
        ...
        <Card title="Latest Updates" icon={<Megaphone size={15} color="#a78bfa" />} accent="#8b5cf6" enterDelay={200}>
          {announcementsLoading ? (
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 9 }}>
              ...
              {announcements.length > 3 && <button onClick={() => navigate('/notifications')} style={{ fontSize: 11, color: '#6366f1', background: 'none', border: 'none', cursor: 'pointer', padding: '3px 0' }}>View all {announcements.length} →</button>}
            </div>
          )}
        </Card>
