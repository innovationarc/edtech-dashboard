...
const StudentDashboard = () => {
  ...
  const loadAppHeatmap = async () => {
    ...
    try {
      ...
      const maxMins = Math.max(...heatmap.map(d => d.value), 1);
      const normalised = heatmap.map(d => ({ ...d, level: hmapLevel(d.value, maxMins) }));
      ...
      if (liveMins >= 1) {
        ...
        if (todayIdx !== -1) {
          ...
          const newMax = Math.max(...updated.map(d => d.value), combined, 1);
          updated[todayIdx] = { ...updated[todayIdx], value: combined, level: hmapLevel(combined, newMax) };
...
const HeatmapCard = ({
  ...
  const cellHandlers = (d: { date: string; value: number; level: 0|1|2|3|4 }) => ({
