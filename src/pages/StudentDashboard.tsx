      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit,minmax(280px,1fr))', gap: 'clamp(14px,1.8vw,22px)' }}>
        ...
        <Card title="Focus Timer" icon={<Timer size={15} color="#6366f1" />} accent="#6366f1" enterDelay={160}>
          <div className="flex flex-col gap-2">
            {/* Header: sound + settings buttons */}
            <div className="flex justify-end gap-1.5 -mt-1 mb-0.5">
              <button onClick={() => setPomodoroSoundEnabled(p => !p)} title={pomodoroSoundEnabled ? 'Sound on' : 'Sound off'}
                className={`p-1.5 rounded-lg transition-colors ${pomodoroSoundEnabled ? 'text-primary-400 bg-primary-500/10' : 'text-gray-600 bg-background-700 hover:text-gray-400'}`}>
