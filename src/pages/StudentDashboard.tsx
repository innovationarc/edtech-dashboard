          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
              {cards.map((s, i) => frozen ? (
                // ── Frozen card: edge ice crystals + foggy center ──
                <div key={i} style={{
                  position: 'relative', overflow: 'hidden', borderRadius: 14,
                  padding: '10px 12px',
                  background: isLight ? 'rgba(224,242,254,0.60)' : 'rgba(10,20,40,0.75)',
                  border: 'none', boxShadow: 'none',
                }}>

                  {/* ── Layer 1: Fog — central frosted glass wash ── */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 0, pointerEvents: 'none',
                    backdropFilter: 'blur(6px) saturate(0.35) brightness(0.88)',
                    WebkitBackdropFilter: 'blur(6px) saturate(0.35) brightness(0.88)',
                    background: isLight
                      ? 'radial-gradient(ellipse at 50% 50%, rgba(255,255,255,0.55) 0%, rgba(186,230,253,0.25) 60%, transparent 100%)'
                      : 'radial-gradient(ellipse at 50% 50%, rgba(186,230,253,0.10) 0%, rgba(147,197,253,0.04) 60%, transparent 100%)',
                  }} />

                  {/* ── Layer 2: Edge ice — heavy crystal buildup around all 4 edges ── */}
                  {/* Top edge */}
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: '45%', zIndex: 1, pointerEvents: 'none',
                    background: isLight
                      ? 'linear-gradient(to bottom, rgba(147,197,253,0.75) 0%, rgba(186,230,253,0.45) 30%, transparent 100%)'
                      : 'linear-gradient(to bottom, rgba(147,197,253,0.55) 0%, rgba(186,230,253,0.28) 30%, transparent 100%)',
                    backdropFilter: 'blur(3px)', WebkitBackdropFilter: 'blur(3px)',
                  }} />
                  {/* Bottom edge */}
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: '40%', zIndex: 1, pointerEvents: 'none',
                    background: isLight
                      ? 'linear-gradient(to top, rgba(147,197,253,0.70) 0%, rgba(186,230,253,0.40) 30%, transparent 100%)'
                      : 'linear-gradient(to top, rgba(147,197,253,0.50) 0%, rgba(186,230,253,0.25) 30%, transparent 100%)',
                    backdropFilter: 'blur(3px)', WebkitBackdropFilter: 'blur(3px)',
                  }} />
                  {/* Left edge */}
                  <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: '35%', zIndex: 1, pointerEvents: 'none',
                    background: isLight
                      ? 'linear-gradient(to right, rgba(147,197,253,0.65) 0%, rgba(186,230,253,0.30) 40%, transparent 100%)'
                      : 'linear-gradient(to right, rgba(147,197,253,0.45) 0%, rgba(186,230,253,0.20) 40%, transparent 100%)',
                  }} />
                  {/* Right edge */}
                  <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: '35%', zIndex: 1, pointerEvents: 'none',
                    background: isLight
                      ? 'linear-gradient(to left, rgba(147,197,253,0.65) 0%, rgba(186,230,253,0.30) 40%, transparent 100%)'
                      : 'linear-gradient(to left, rgba(147,197,253,0.45) 0%, rgba(186,230,253,0.20) 40%, transparent 100%)',
                  }} />

                  {/* ── Layer 3: Corner crystal clusters — densest buildup at corners ── */}
                  {['top:0;left:0', 'top:0;right:0', 'bottom:0;left:0', 'bottom:0;right:0'].map((pos, ci) => {
                    const style: React.CSSProperties = { position: 'absolute', width: 48, height: 48, zIndex: 2, pointerEvents: 'none', borderRadius: ci === 0 ? '14px 0 100% 0' : ci === 1 ? '0 14px 0 100%' : ci === 2 ? '100% 0 0 14px' : '0 100% 14px 0' };
                    if (pos.includes('top:0')) style.top = 0; else style.bottom = 0;
                    if (pos.includes('left:0')) style.left = 0; else style.right = 0;
                    return (
                      <div key={ci} style={{ ...style,
                        background: isLight
                          ? 'radial-gradient(circle at corner, rgba(255,255,255,0.90) 0%, rgba(186,230,253,0.80) 30%, rgba(147,197,253,0.50) 55%, transparent 75%)'
                          : 'radial-gradient(circle at corner, rgba(224,242,254,0.65) 0%, rgba(186,230,253,0.45) 30%, rgba(147,197,253,0.25) 55%, transparent 75%)',
                      }} />
                    );
                  })}

                  {/* ── Layer 4: Crystal texture SVG — jagged edge detail ── */}
                  <svg viewBox="0 0 100 100" preserveAspectRatio="none"
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 2, pointerEvents: 'none',
                      opacity: isLight ? 0.60 : 0.45 }}>
                    {/* Top edge crystals */}
                    <polyline points="0,0 8,12 15,4 22,14 30,3 38,11 45,2 53,13 60,4 67,10 74,2 82,13 90,3 100,8 100,0" fill={isLight ? 'rgba(147,197,253,0.60)' : 'rgba(186,230,253,0.40)'} stroke={isLight ? 'rgba(96,165,250,0.70)' : 'rgba(147,197,253,0.60)'} strokeWidth="0.5"/>
                    {/* Bottom edge crystals */}
                    <polyline points="0,100 7,88 14,97 21,86 29,98 37,87 44,97 52,86 59,97 66,88 73,98 81,87 88,97 95,89 100,100" fill={isLight ? 'rgba(147,197,253,0.60)' : 'rgba(186,230,253,0.40)'} stroke={isLight ? 'rgba(96,165,250,0.70)' : 'rgba(147,197,253,0.60)'} strokeWidth="0.5"/>
                    {/* Left edge crystals */}
                    <polyline points="0,0 11,7 2,15 13,22 1,30 12,37 2,44 11,52 1,59 13,66 2,74 12,81 1,89 10,96 0,100" fill={isLight ? 'rgba(147,197,253,0.55)' : 'rgba(186,230,253,0.35)'} stroke={isLight ? 'rgba(96,165,250,0.65)' : 'rgba(147,197,253,0.55)'} strokeWidth="0.5"/>
                    {/* Right edge crystals */}
                    <polyline points="100,0 89,8 98,16 87,23 99,31 88,38 98,45 89,53 99,60 87,67 98,75 89,82 99,90 90,97 100,100" fill={isLight ? 'rgba(147,197,253,0.55)' : 'rgba(186,230,253,0.35)'} stroke={isLight ? 'rgba(96,165,250,0.65)' : 'rgba(147,197,253,0.55)'} strokeWidth="0.5"/>
                    {/* Crack lines from corners */}
                    <path d={iceCracks[i]} stroke={isLight ? 'rgba(96,165,250,0.50)' : 'rgba(186,230,253,0.40)'} strokeWidth="0.7" fill="none" strokeLinecap="round"/>
                  </svg>

                  {/* ── Layer 5: Specular highlights — light glinting off ice surface ── */}
                  <div style={{ position: 'absolute', top: 6, left: 8, width: 30, height: 14, zIndex: 3, pointerEvents: 'none',
                    background: 'linear-gradient(135deg, rgba(255,255,255,0.70) 0%, rgba(255,255,255,0.30) 50%, transparent 100%)',
                    borderRadius: '50% 30% 50% 30%', transform: 'rotate(-8deg)',
                    filter: 'blur(1.5px)',
                  }} />
                  <div style={{ position: 'absolute', bottom: 8, right: 10, width: 20, height: 10, zIndex: 3, pointerEvents: 'none',
                    background: 'linear-gradient(135deg, rgba(255,255,255,0.50) 0%, transparent 100%)',
                    borderRadius: '50%', filter: 'blur(1px)',
                  }} />

                  {/* Card content — sits above all ice layers */}
                  <div style={{ position: 'relative', zIndex: 4, display: 'flex', flexDirection: 'column', gap: 5,
                    filter: isLight ? 'saturate(0.20) brightness(0.75)' : 'saturate(0.30) brightness(0.80)' }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {s.icon}
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase',
                        color: isLight ? 'rgba(30,58,138,0.85)' : 'rgba(186,230,253,0.90)' }}>{s.label}</span>
                    </div>
                    <p style={{ fontSize: 'clamp(1.1rem,2vw,1.35rem)', fontWeight: 800, margin: 0, lineHeight: 1,
                      color: isLight ? '#1e3a5f' : '#dbeafe',
                      textShadow: isLight ? '0 1px 3px rgba(147,197,253,0.60)' : '0 1px 12px rgba(147,197,253,0.70)' }}>{s.value}</p>
                    <p style={{ fontSize: 9, margin: 0,
                      color: isLight ? 'rgba(30,58,138,0.70)' : 'rgba(147,197,253,0.80)' }}>{s.sub}</p>
                  </div>
                </div>
              ) : (
