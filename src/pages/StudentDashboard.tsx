          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
              {cards.map((s, i) => frozen ? (
                // ── Frozen card: reference-matched blue-white ice ──
                <div key={i} style={{
                  position: 'relative', overflow: 'hidden', borderRadius: 14,
                  padding: '10px 12px',
                  // Pale blue-white center — like the inside of the reference
                  background: dark ? 'rgba(160,210,240,0.22)' : 'rgba(210,238,255,0.70)',
                  border: 'none', boxShadow: 'none',
                }}>

                  {/* ── VIVID BLUE FROST BANDS — top/bottom/left/right, solid at edge ── */}
                  <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 24, zIndex: 2, pointerEvents: 'none',
                    background: 'linear-gradient(180deg, rgba(58,130,200,0.98) 0%, rgba(80,160,225,0.92) 25%, rgba(100,180,235,0.70) 55%, rgba(140,200,240,0.30) 78%, transparent 100%)',
                  }} />
                  <div style={{ position: 'absolute', bottom: 0, left: 0, right: 0, height: 24, zIndex: 2, pointerEvents: 'none',
                    background: 'linear-gradient(0deg, rgba(58,130,200,0.98) 0%, rgba(80,160,225,0.92) 25%, rgba(100,180,235,0.70) 55%, rgba(140,200,240,0.30) 78%, transparent 100%)',
                  }} />
                  <div style={{ position: 'absolute', top: 0, left: 0, bottom: 0, width: 24, zIndex: 2, pointerEvents: 'none',
                    background: 'linear-gradient(90deg, rgba(58,130,200,0.98) 0%, rgba(80,160,225,0.92) 25%, rgba(100,180,235,0.70) 55%, rgba(140,200,240,0.30) 78%, transparent 100%)',
                  }} />
                  <div style={{ position: 'absolute', top: 0, right: 0, bottom: 0, width: 24, zIndex: 2, pointerEvents: 'none',
                    background: 'linear-gradient(270deg, rgba(58,130,200,0.98) 0%, rgba(80,160,225,0.92) 25%, rgba(100,180,235,0.70) 55%, rgba(140,200,240,0.30) 78%, transparent 100%)',
                  }} />

                  {/* ── CORNER CAPS — brightest, most opaque, lightest blue-white ── */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 3, pointerEvents: 'none',
                    background:
                      'radial-gradient(circle 42px at 0px 0px, rgba(180,225,248,0.99) 0%, rgba(100,175,230,0.88) 32%, rgba(70,145,210,0.55) 52%, transparent 70%), ' +
                      'radial-gradient(circle 42px at 100% 0px, rgba(180,225,248,0.99) 0%, rgba(100,175,230,0.88) 32%, rgba(70,145,210,0.55) 52%, transparent 70%), ' +
                      'radial-gradient(circle 42px at 0px 100%, rgba(180,225,248,0.99) 0%, rgba(100,175,230,0.88) 32%, rgba(70,145,210,0.55) 52%, transparent 70%), ' +
                      'radial-gradient(circle 42px at 100% 100%, rgba(180,225,248,0.99) 0%, rgba(100,175,230,0.88) 32%, rgba(70,145,210,0.55) 52%, transparent 70%)',
                  }} />

                  {/* ── CRYSTAL CRACKS — white frost line + vivid blue line + dark crack ── */}
                  <svg viewBox="0 0 100 100" preserveAspectRatio="none"
                    style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 4, pointerEvents: 'none' }}>
                    {[
                      ["M2,6 L18,16 L14,30 L30,26 M18,16 L26,10 M14,30 L8,44","M88,3 L76,18 L84,33 M76,18 L64,26 M2,74 L16,66 L26,76","M72,82 L82,72 L97,76 M82,72 L80,58"],
                      ["M8,3 L24,20 L18,36 M24,20 L38,16 M38,16 L44,6","M82,8 L68,24 L76,38 M68,24 L58,16","M3,86 L18,74 L28,84 M18,74 L14,60","M76,92 L86,78 L97,82 M86,78 L84,64"],
                      ["M6,10 L20,24 L14,40 L26,46 M20,24 L32,18","M90,6 L72,20 L80,36 M72,20 L60,14","M3,76 L20,68 L16,84","M82,87 L90,72 L98,78 M90,72 L88,58"],
                      ["M10,3 L26,18 L20,34 M26,18 L40,14 M40,14 L36,4","M86,10 L70,26 L82,40 L68,48 M70,26 L58,18","M6,80 L24,70 L18,88 M24,70 L34,62","M74,90 L84,76 L98,80 M84,76 L82,62"],
                    ][i % 4].map((p, ci) => (
                      <g key={ci}>
                        {/* White frost deposit — thick glow around crack */}
                        <path d={p} stroke="rgba(220,242,255,0.95)" strokeWidth="3.0" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                        {/* Vivid blue line — the ice color */}
                        <path d={p} stroke="rgba(60,140,210,0.90)" strokeWidth="1.2" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                        {/* Dark navy crack — the actual fracture */}
                        <path d={p} stroke="rgba(20,60,130,0.80)" strokeWidth="0.45" fill="none" strokeLinecap="round" strokeLinejoin="round"/>
                      </g>
                    ))}
                  </svg>

                  {/* Specular glint — light on ice */}
                  <div style={{ position: 'absolute', top: 5, left: 6, width: 22, height: 9, zIndex: 5, pointerEvents: 'none',
                    background: 'radial-gradient(ellipse, rgba(255,255,255,0.92) 0%, transparent 70%)',
                    borderRadius: '50%', transform: 'rotate(-22deg)', filter: 'blur(0.8px)',
                  }} />
                  <div style={{ position: 'absolute', bottom: 6, right: 8, width: 14, height: 6, zIndex: 5, pointerEvents: 'none',
                    background: 'radial-gradient(ellipse, rgba(255,255,255,0.75) 0%, transparent 70%)',
                    borderRadius: '50%', filter: 'blur(0.5px)',
                  }} />

                  {/* Frosted glass — desaturates content only */}
                  <div style={{ position: 'absolute', inset: 0, zIndex: 6, pointerEvents: 'none',
                    backdropFilter: 'blur(2px) saturate(0.40) brightness(0.83)',
                    WebkitBackdropFilter: 'blur(2px) saturate(0.40) brightness(0.83)',
                  }} />

                  {/* Card content */}
                  <div style={{ position: 'relative', zIndex: 7, display: 'flex', flexDirection: 'column', gap: 5 }}>
                    <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
                      {s.icon}
                      <span style={{ fontSize: 9, fontWeight: 700, letterSpacing: '0.07em', textTransform: 'uppercase',
                        color: dark ? 'rgba(186,230,253,0.95)' : 'rgba(20,50,120,0.90)' }}>{s.label}</span>
                    </div>
                    <p style={{ fontSize: 'clamp(1.1rem,2vw,1.35rem)', fontWeight: 800, margin: 0, lineHeight: 1,
                      color: dark ? '#e0f2fe' : '#1a3a70',
                      textShadow: dark ? '0 0 10px rgba(147,197,253,0.60)' : 'none' }}>{s.value}</p>
                    <p style={{ fontSize: 9, margin: 0,
                      color: dark ? 'rgba(147,197,253,0.85)' : 'rgba(20,60,140,0.75)' }}>{s.sub}</p>
                  </div>
                </div>
