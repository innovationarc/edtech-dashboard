...
const StudentDashboard = () => {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(18px,2.8vw,30px)', fontFamily: "'Outfit',sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'repeat(auto-fit,minmax(280px,1fr))', gridAutoRows: isMobile ? 'auto' : '1fr', gap: 'clamp(14px,1.8vw,22px)' }}>
...
const HeatmapCard = ({
  ...
  const cellProps = (d: { date: string; value: number; level: 0|1|2|3|4 }, style: React.CSSProperties) => ({
    style,
        ...
        <div style={{ overflowX:'auto' }}>
          {view === 'days' ? (
            <div style={{ display:'grid', gridTemplateColumns:`repeat(${Math.ceil(agg.length/7)},${CELL}px)`, gridTemplateRows:`repeat(7,${CELL}px)`, gap:GAP, width:'fit-content' }}>
              {agg.map(d => (
                <div key={d.date} {...cellProps(d, { width:CELL, height:CELL, borderRadius:2, background:colors[d.level], transition:'transform 0.1s', cursor:'pointer' })}
                  onMouseEnter={e => { (e.currentTarget as HTMLDivElement).style.transform='scale(1.3)'; const rect=(e.currentTarget as HTMLDivElement).getBoundingClientRect(); setTooltip({x:rect.left+rect.width/2,y:rect.top,date:d.date,value:d.value}); }}
            ...
            <div style={{ display:'flex', gap:GAP, flexWrap:'wrap' }}>
              {agg.map(d => (
                <div key={d.date} {...cellProps(d, { width:CELL, height:CELL, borderRadius:view==='months'?4:3, background:colors[d.level], cursor:'pointer', flexShrink:0 })} />
              ))}
            </div>
          )}
        </div>
