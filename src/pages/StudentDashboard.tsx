  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 'clamp(18px,2.8vw,30px)', fontFamily: "'Outfit',sans-serif" }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit,minmax(min(100%,130px),1fr))', gap: 'clamp(8px,1.2vw,12px)' }}>
        {([
          { icon: <Clock size={14} color="#6366f1" />, label: 'Today', value: kpiLoading ? '…' : fmtMins(kpiTodayMins), sub: 'study time', accent: '#6366f1', delay: 0 },
          { icon: <TrendingUp size={14} color="#10b981" />, label: 'This Week', value: kpiLoading ? '…' : fmtMins(kpiWeekMins), sub: 'study time', accent: '#10b981', delay: 40 },
          { icon: <CheckCircle size={14} color="#f59e0b" />, label: 'Done', value: kpiLoading ? '…' : `${kpiTasksDone}`, sub: 'objectives', accent: '#f59e0b', delay: 80 },
          { icon: <Flame size={14} color="#ef4444" />, label: 'Streak', value: kpiLoading ? '…' : `${streak.current}`, sub: 'day streak', accent: '#ef4444', delay: 120 },
        ] as const).map((s, i) => (
          <Card key={i} title="" icon={s.icon} accent={s.accent} enterDelay={s.delay}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginTop: -8 }}>
              <span style={{ fontSize: 9, fontWeight: 700, color: T.text3, textTransform: 'uppercase', letterSpacing: '0.07em' }}>{s.label}</span>
              <p style={{ fontSize: 'clamp(1.1rem,2vw,1.4rem)', fontWeight: 800, color: T.text, margin: 0, lineHeight: 1.1 }}>{s.value}</p>
