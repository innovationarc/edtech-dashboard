      ...
      {(() => {
        ...
        return (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
            ...
            {frozen && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 7, padding: '6px 12px', borderRadius: 9,
                background: 'linear-gradient(90deg,rgba(147,197,253,0.12),rgba(125,211,252,0.08))',
                border: '1px solid rgba(147,197,253,0.30)', alignSelf: 'flex-start',
                backdropFilter: 'blur(8px)' }}>
                <svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="#93c5fd" strokeWidth="2.5" strokeLinecap="round">
                  <line x1="12" y1="2" x2="12" y2="22"/><line x1="2" y1="12" x2="22" y2="12"/>
                  <line x1="4.93" y1="4.93" x2="19.07" y2="19.07"/><line x1="19.07" y1="4.93" x2="4.93" y2="19.07"/>
                  <line x1="12" y1="2" x2="9" y2="5"/><line x1="12" y1="22" x2="15" y2="19"/>
                </svg>
                <span style={{ fontSize: 10, fontWeight: 600, color: '#bae6fd', letterSpacing: '0.01em' }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? 'repeat(2,1fr)' : 'repeat(4,1fr)', gap: 'clamp(8px,1.2vw,12px)' }}>
              {cards.map((s, i) => (
                <Card key={i} accent={s.accent} enterDelay={s.delay} padding="sm">
                  {/* Frozen card wrapper — ice overlay rendered inside Card content */}
                  <div style={{ position: 'relative', display: 'flex', flexDirection: 'column', gap: 5, overflow: 'hidden', borderRadius: 8 }}>

                    {/* ── Ice layer (only when frozen) ── */}
                    {frozen && (
                      <>
                        {/* Frosted glass base */}
                        <div style={{ position: 'absolute', inset: -4, zIndex: 0, pointerEvents: 'none',
                          background: 'linear-gradient(135deg, rgba(186,230,253,0.22) 0%, rgba(147,197,253,0.14) 40%, rgba(224,242,254,0.18) 70%, rgba(125,211,252,0.10) 100%)',
                          backdropFilter: 'blur(1.5px) saturate(0.55) brightness(0.92)',
                          WebkitBackdropFilter: 'blur(1.5px) saturate(0.55) brightness(0.92)',
                          borderRadius: 10,
                          border: '1px solid rgba(186,230,253,0.45)',
                          boxShadow: 'inset 0 0 18px rgba(147,197,253,0.15), 0 0 12px rgba(125,211,252,0.12)',
                        }} />
                        {/* Ice crack lines SVG */}
                        <svg viewBox="0 0 100 100" preserveAspectRatio="none"
                          style={{ position: 'absolute', inset: 0, width: '100%', height: '100%', zIndex: 1, pointerEvents: 'none', opacity: 0.35 }}>
                          <path d={iceCracks[i]} stroke="rgba(186,230,253,0.9)" strokeWidth="0.8" fill="none" strokeLinecap="round"/>
                          <path d={iceCracks[(i+2)%4]} stroke="rgba(224,242,254,0.6)" strokeWidth="0.5" fill="none" strokeLinecap="round" opacity="0.6"/>
                        </svg>
                        {/* Shimmer highlight — top-left bright spot like light on ice */}
                        <div style={{ position: 'absolute', top: -6, left: -6, width: 40, height: 40, zIndex: 1, pointerEvents: 'none',
                          background: 'radial-gradient(circle, rgba(224,242,254,0.55) 0%, transparent 70%)',
                          borderRadius: '50%',
                        }} />
                        {/* Snowflake watermark — bottom right */}
                        <svg width="28" height="28" viewBox="0 0 24 24" fill="none" stroke="rgba(147,197,253,0.30)" strokeWidth="1.2" strokeLinecap="round"
