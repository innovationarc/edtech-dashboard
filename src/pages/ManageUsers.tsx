                ...
                >
                    ...
                    <div className="flex items-center gap-3">
                      <div className="relative">
                        {user.profilePictureUrl ? (
                          ...
                          <div className={`h-11 w-11 rounded-xl flex items-center justify-center bg-gradient-to-br ${
                            user.role === 'admin' ? 'from-gray-700 to-gray-800' :
                            user.role === 'manager' ? 'from-red-600 to-rose-700' :
                            user.role === 'coordinator' ? 'from-orange-600 to-red-700' :
                            user.role === 'teacher' ? 'from-green-600 to-emerald-700' :
                            user.role === 'parent' ? 'from-purple-600 to-pink-700' :
                      ...
                      <div className="min-w-0">
                        ...
                        <div className="flex items-center gap-2 mt-1 md:hidden">
                          <span className={`px-2 py-0.5 rounded text-xs font-semibold ${
                            user.role === 'admin' ? 'bg-gray-800/60 text-gray-300' :
                            user.role === 'manager' ? 'bg-red-800/60 text-red-300' :
                            user.role === 'coordinator' ? 'bg-orange-800/60 text-orange-300' :
                            user.role === 'teacher' ? 'bg-green-800/60 text-green-300' :
                            user.role === 'parent' ? 'bg-purple-800/60 text-purple-300' :
                  ...
                  <td className="p-4 hidden md:table-cell">
                    <span className={`inline-flex items-center gap-1.5 px-2.5 py-1 rounded-lg text-xs font-semibold ${
                      ...
                      {user.role === 'manager' && <Briefcase size={11} />}
                      {user.role === 'coordinator' && <UserCog size={11} />}
                      {user.role === 'teacher' && <UsersRound size={11} />}
                      {user.role === 'parent' && <Users size={11} />}
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-black/70 backdrop-blur-sm animate-in fade-in duration-200">
          <div className="bg-background-800/95 backdrop-blur-xl w-full max-w-md rounded-2xl overflow-hidden border border-red-700/30">
            <div className="p-7 text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Delete User Account</h3>
        ...
        <form onSubmit={handleSubmit} className="p-5 space-y-4">
          <div className="bg-blue-900/20 border border-blue-700/30 rounded-xl p-3.5">
            ...
            <p className="text-white font-semibold capitalize flex items-center gap-2">
              ...
              {user.role === 'manager' && <Briefcase size={16} />}
              {user.role === 'coordinator' && <UserCog size={16} />}
              {user.role === 'teacher' && <UsersRound size={16} />}
              {user.role === 'parent' && <Users size={16} />}
              {user.role === 'student' && <GraduationCap size={16} />}
              <span>{user.role}</span>
            </p>
          </div>
