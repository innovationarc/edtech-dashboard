...
const RegisterModal = ({ onClose, onSuccess }: RegisterModalProps) => {
  ...
  // Handle form submission (validate and check uniqueness before sending OTP)
  const handleFormSubmit = async () => {
    ...
    try {
      ...
      // Check if user already exists (phone number and email) - BEFORE sending OTP
      console.log('🔍 Validating uniqueness before sending OTP...');
      const existsCheck = await authService.checkUserExists(
        normalizedPhone, 
        formData.email.trim() || undefined
      );

      if (existsCheck.exists) {
        // Show specific error based on which field exists
        ...
        } else if (existsCheck.field === 'email') {
          setError('This email is already registered. Please use a different email or sign in.');
        } else {
          setError(existsCheck.message || 'This account already exists. Please sign in.');
        }
        setLoading(false);
        return;
      }

      console.log('✅ User is unique, sending OTP...');
      
      // Send OTP only after confirming user is unique
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      // Double-check uniqueness right before creating account
      console.log('🔍 Final uniqueness check before account creation...');
      const finalCheck = await authService.checkUserExists(
        normalizedPhone, 
        formData.email.trim() || undefined
      );

      if (finalCheck.exists) {
        if (finalCheck.field === 'phone') {
          setOtpError('This phone number was just registered by another user. Please use a different phone number.');
        } else if (finalCheck.field === 'email') {
          setOtpError('This email was just registered by another user. Please use a different email.');
        } else {
          setOtpError('This account was just registered. Please sign in instead.');
        }
        setOtpLoading(false);
        return;
      }

      // OTP verified and user is unique, create user account
        ...
        <div className="relative">
          ...
          <div className="space-y-4 sm:space-y-5">
            ...
            <button
              ...
              disabled={loading || !captchaLoaded || !agreedToTerms || !agreedToPrivacy}
              className="w-full bg-gradient-to-r from-primary-600 via-purple-600 to-primary-600 hover:from-primary-700 hover:via-purple-700 hover:to-primary-700 disabled:from-gray-700 disabled:to-gray-800 disabled:cursor-not-allowed text-white py-4 rounded-xl transition-all duration-300 active:scale-95 flex items-center justify-center gap-2 font-semibold shadow-2xl hover:shadow-primary-500/50"
            >
              {loading && <Loader size={20} className="animate-spin" />}
              <span>{loading ? 'Validating...' : !captchaLoaded ? 'Loading Security...' : 'Continue to Verification'}</span>
            </button>
          </div>
