...
import { X, Lock, Loader, Shield, AlertCircle, CheckCircle, CreditCard, Eye, EyeOff } from 'lucide-react';
...
const ForgotPasswordModal = ({ onClose, onSuccess }: ForgotPasswordModalProps) => {
  ...
  const handleVerifyOTP = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        setTimeout(() => setCurrentStep('reset'), 500);
      } else {
  ...
  // Show Forgot User ID modal
  if (showForgotUserId) {
    return (
      <ForgotUserIdModal 
        onClose={() => setShowForgotUserId(false)}
        onUserIdFound={(foundUserId) => {
    ...
    return (
      <div className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-4">
        <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-3xl w-full max-w-md p-8 relative shadow-2xl border border-gray-700/50">
          <div className="absolute inset-0 bg-gradient-to-br from-green-500/10 via-emerald-500/5 to-green-500/10 rounded-3xl pointer-events-none"></div>
          ...
          <div className="relative text-center">
            ...
            <button
              ...
              className="w-full bg-gradient-to-r from-green-600 via-emerald-600 to-green-600 hover:from-green-700 hover:via-emerald-700 hover:to-green-700 text-white py-4 rounded-xl transition-all duration-300 active:scale-95 flex items-center justify-center gap-2 font-semibold shadow-2xl hover:shadow-green-500/50"
            >
                  ...
                  <div className="mt-2">
                    ...
                    {passwordStrength.issues.length > 0 && (
                      <div className="mt-2 text-xs text-gray-400">
                        Missing: {passwordStrength.issues.join(', ')}
                      </div>
                    )}
                  </div>
                )}
