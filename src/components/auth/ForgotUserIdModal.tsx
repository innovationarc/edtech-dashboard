...
const ForgotUserIdModal = ({ onClose, onSignInClick }: ForgotUserIdModalProps) => {
  ...
  const handleSearchUsers = async () => {
    ...
    try {
      ...
      try {
        await executeRecaptcha('user_search');
      } catch {
  ...
  if (currentStep === 'results') {
    return (
      <div data-fuid="" className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-3 md:p-4">
      ...
      <div data-fuid="" className="fixed inset-0 bg-black/70 backdrop-blur-md flex items-center justify-center z-50 p-3 md:p-4">
        <div className="bg-gradient-to-br from-gray-900 via-gray-800 to-gray-900 rounded-2xl md:rounded-3xl w-full max-w-[95%] sm:max-w-md p-4 sm:p-6 md:p-8 relative shadow-2xl border border-gray-700/50">
          ...
          <div className="relative">
            ...
            <div className="text-xs sm:text-sm text-center" style={{color:"#9ca3af"}}>
              {canResendOTP ? (
                <button
        ...
        <div className="relative">
          <div className="flex justify-center mb-4 md:mb-6">
            <div className="relative">
              ...
              <div className="relative bg-gradient-to-br from-primary-500 to-purple-600 rounded-full p-3 md:p-4 shadow-2xl shadow-primary-500/50">
                <UserSearch size={32} className="md:w-12 md:h-12" style={{color:"#ffffff"}} />
              </div>
            </div>
          </div>
          ...
          <div className="space-y-3 sm:space-y-4 md:space-y-5">
            ...
            {userCount === 0 && (
              ...
              >
                {loading && <Loader size={18} className="sm:w-5 sm:h-5 animate-spin" />}
                <span>{loading ? 'Searching...' : !captchaReady ? 'Loading Security...' : 'Search User'}</span>
              </button>
            )}
          </div>
          ...
          <div className="mt-4 sm:mt-6 text-center">
            ...
            <button 
              onClick={onClose}
              className="text-xs sm:text-sm transition-colors" style={{color:"#9ca3af"}}
            >
              Back to Sign In
            </button>
          </div>
