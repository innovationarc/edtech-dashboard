...
import { useDashboard } from '../../contexts/DashboardContext'; // adjust path if needed
...
function darkenColor(hex: string, factor: number) {
  ...
  if (!rgb) return hex;
  const r = Math.max(0, Math.min(255, Math.floor(rgb.r * (1 - factor))));
  const g = Math.max(0, Math.min(255, Math.floor(rgb.g * (1 - factor))));
  const b = Math.max(0, Math.min(255, Math.floor(rgb.b * (1 - factor))));
...
function lightenColor(hex: string, factor: number) {
  const rgb = hexToRgb(hex);
  if (!rgb) return hex;
  const r = Math.max(0, Math.min(255, Math.floor(rgb.r + (255 - rgb.r) * factor)));
  const g = Math.max(0, Math.min(255, Math.floor(rgb.g + (255 - rgb.g) * factor)));
  const b = Math.max(0, Math.min(255, Math.floor(rgb.b + (255 - rgb.b) * factor)));
  return rgbToHex(r, g, b);
}
...
const GhostIcon: React.FC<GhostIconProps> = ({ size = 72, isActive = false }) => {
  ...
  const darkPrimary = darkenColor(primaryColor, 0.7);
  const mediumPrimary = darkenColor(primaryColor, 0.4);
  const lightPrimary = lightenColor(primaryColor, 0.5);
  ...
  }, [isActive, primaryColor, accentColor]); // re‑run if colors change (to update gradient stops – handled by render)
  ...
  return (
    <div
      ...
      style={{
        ...
        // Expose colors as CSS variables for use in SVG
        ['--ghost-primary' as any]: primaryColor,
        ['--ghost-accent' as any]: accentColor,
        ['--ghost-dark' as any]: darkPrimary,
        ['--ghost-medium' as any]: mediumPrimary,
        ['--ghost-light' as any]: lightPrimary,
      }}
    >
      <svg
        width={size}
      ...
      >
        <defs>
          ...
          {/* Body gradient – now using theme colors */}
          <radialGradient id="g-body" cx="40%" cy="20%" r="78%" fx="40%" fy="20%">
            <stop offset="0%" stopColor="#ffffff" />
            <stop offset="30%" stopColor={lightPrimary} />
            <stop offset="65%" stopColor={primaryColor} />
            <stop offset="100%" stopColor={accentColor} />
          </radialGradient>

          {/* Aura gradient – uses accent color */}
        ...
        <g filter="url(#g-glow)">
          ...
          {/* Body (fill uses dynamic gradient) */}
          ...
          {/* Open mouth (active state) */}
          {isActive && (
            <>
              <ellipse cx="80" cy="104" rx="9" ry="11" fill={darkPrimary} opacity="0.92" />
              <ellipse cx="80" cy="106" rx="6.5" ry="7.5" fill={mediumPrimary} opacity="0.5" />
            </>
          )}
