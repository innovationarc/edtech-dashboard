...
const GhostIcon: React.FC<GhostIconProps> = ({ size = 72, isActive = false }) => {
  ...
  const leftBlinkRef  = useRef<SVGAnimateElement>(null);
  const rightBlinkRef = useRef<SVGAnimateElement>(null);
  ...
  useEffect(() => {
    ...
    const setBlinkPaused = (paused: boolean) => {
      // Pause/resume SVG blink animation
      if (leftBlinkRef.current)  (leftBlinkRef.current  as any)[paused ? 'beginElement' : 'endElement']?.();
      if (rightBlinkRef.current) (rightBlinkRef.current as any)[paused ? 'beginElement' : 'endElement']?.();
      // Keep eyes wide open during drag
      if (leftEyeRef.current)  leftEyeRef.current.setAttribute('ry',  paused ? '13' : (isActive ? '14' : '12'));
      if (rightEyeRef.current) rightEyeRef.current.setAttribute('ry', paused ? '13' : (isActive ? '14' : '12'));
    };
    ...
    // rAF loop — only updates tilt & syncs pupils to tilt in same frame
    const tick = () => {
      ...
      // Sync pupils to tilt directly — same frame, zero lag
      if (active.current) {
    ...
    const onMove = (e: MouseEvent | TouchEvent) => {
      ...
      prev.current = pos;
      // Only set target — rAF loop applies lerp and syncs pupils same frame
      targetTilt.current = clamp(targetTilt.current + dx * 2.8, 30);
      if (resetTimer.current) clearTimeout(resetTimer.current);
      resetTimer.current = setTimeout(() => { targetTilt.current = 0; }, 120);
    };
  ...
  return (
    <div ref={wrapRef} style={{ display: 'inline-block', transformOrigin: 'center bottom', willChange: 'transform' }}>
      <svg width={size} height={size} viewBox="0 0 160 170" fill="none" xmlns="http://www.w3.org/2000/svg" style={{ overflow: 'visible' }}>
        <defs>
        ...
        <g filter="url(#g-glow)">
          ...
          <ellipse ref={leftEyeRef} cx="60" cy="72" rx="11.5" ry={isActive ? 14 : 12} fill="#1c0b30" filter="url(#g-eye)">
            {!isActive && (
              <animate ref={leftBlinkRef} attributeName="ry" values="12;1.2;12" dur="5s" begin="2s"
          ...
          <ellipse ref={rightEyeRef} cx="100" cy="72" rx="11.5" ry={isActive ? 14 : 12} fill="#1c0b30" filter="url(#g-eye)">
            {!isActive && (
              <animate ref={rightBlinkRef} attributeName="ry" values="12;1.2;12" dur="5s" begin="2s"
                repeatCount="indefinite" calcMode="spline" keySplines="0.5 0 0.5 1;0.5 0 0.5 1" />
            )}
          </ellipse>
          ...
          {/* Smile — idle */}
          <path ref={mouthSmileRef}
            d="M67,102 Q80,116 93,102" stroke="#1c0b30" strokeWidth="5"
