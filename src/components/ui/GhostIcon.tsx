...
const GhostIcon: React.FC<GhostIconProps> = ({ size = 72, isActive = false }) => (
  ...
  >
    ...
    <g filter="url(#g-glow)">
      ...
      {/*
        ONE continuous cubic bezier path — no corners anywhere.

        The trick: every segment shares its endpoint tangent with
        the next segment. Specifically the side-to-hem transition
        uses a control point that curves OUTWARD so the path arrives
        at the first scallop peak already moving sideways/downward,
        not vertically. This eliminates the pinch corner entirely.

        Path reads clockwise:
          top-center → right arch → right side (curves out) →
          right-outer scallop → inner scallop → centre scallop →
          inner scallop → left-outer scallop →
          left side (curves out back up) → left arch → top-center

        Scallop anatomy (each pair of cubics):
          First cubic:  goes DOWN into the valley
          Second cubic: comes back UP to next peak
        The control points are symmetric so valleys are round-bottomed.
      */}
      ...
      <ellipse cx="64" cy="66" rx="3" ry="4" fill="white" opacity="0.55" />
      ...
      <ellipse cx="104" cy="66" rx="3" ry="4" fill="white" opacity="0.55" />
