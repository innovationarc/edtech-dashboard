 ...
 *
 * Uses the same portal + position:fixed approach as ChatbotWidget to
 * completely bypass the DashboardLayout's padding/overflow constraints.
 *
 * On mobile  → position:fixed, top:60px (header height), bottom:64px (nav height)
 *              own scroll container, cards truly full-width, swipe works perfectly
 * On desktop → normal in-flow rendering (layout padding is fine on desktop)
 *
 * Features:
...
const SNAP_BOTH = 160;   // snap position when unread (Read + Delete)
const SNAP_ONE  = 80;    // snap position when already read (Delete only)
const THRESH    = 48;    // minimum drag to trigger snap
const FULL_DEL  = 230;   // drag to trigger full-swipe delete
const MSG_LIMIT = 120;

const NotifCard: React.FC<CardProps> = ({ notif,dark,pRgb,onRead,onDelete,idx }) => {
  const [expanded, setExpanded] = useState(false);
  const [tx,       setTx]       = useState(0);
  const [anim,     setAnim]     = useState(false);  // true = CSS transition on tx
  ...
  return (
    /* height-collapse shell — only clips during delete exit */
    <div style={{
      overflow: 'hidden',
      maxHeight: gone ? 0 : 1000,
      opacity:   gone ? 0 : 1,
      transition: gone
      ...
      {/* swipe viewport — the ONE overflow:hidden that clips horizontally */}
      <div style={{ position:'relative', overflow:'hidden', borderRadius:16, touchAction:'pan-y' }}>
        ...
        <div
          ...
          style={{
            ...
            userSelect:'none',
            background: dark
              ? (notif.isRead ? '#161c28' : '#1a2235')
              : (notif.isRead ? '#f8fafc' : '#ffffff'),
            border:`1px solid ${!notif.isRead
              ? `rgba(${pRgb},0.25)`
              : (dark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.08)')}`,
            boxShadow: dark
              ? (notif.isRead ? '0 2px 12px rgba(0,0,0,0.25)' : `0 2px 16px rgba(0,0,0,0.3), 0 0 0 0.5px rgba(${pRgb},0.12)`)
        ...
        >
          ...
          <div style={{ display:'flex',gap:12,alignItems:'flex-start',padding:'14px 14px 12px 16px' }}>
            ...
            {/* desktop hover actions — CSS hides on touch devices */}
...
const ConfirmSheet: React.FC<{
  dark:boolean; title:string; body:string;
  ...
  <div onClick={onCancel} style={{
    ...
    <div onClick={e=>e.stopPropagation()} style={{
      background: dark ? 'rgba(13,16,26,0.96)' : 'rgba(255,255,255,0.96)',
...
const NotificationsPage: React.FC = () => {
  ...
  /* ── Layout constants (match DashboardLayout) ─────────────────────────────
   *  Mobile  top nav:    60px  (paddingTop in DashboardLayout main)
   *  Mobile  bottom nav: 64px  (MobileNavigation height)
   *  Desktop sidebar:    64px  wide (ml-[64px] on the flex child)
   *  Desktop top nav:    64px  (paddingTop)
   *
   *  On mobile we use position:fixed (portal to body) like ChatbotWidget does,
   *  so we escape the layout's p-3 padding and overflow-y-auto scroll container.
   *  On desktop we render normally in-flow — layout padding is fine there.
   * ─────────────────────────────────────────────────────────────────────── */
  const TOP_NAV    = isMobile ? 60  : 64;
  const BOTTOM_NAV = isMobile ? 64  : 0;

  /* ── Content (same JSX for both mobile portal and desktop in-flow) ──────── */
  const content = (
    <div style={{
      fontFamily:"'Outfit',sans-serif",
      display:'flex', flexDirection:'column', gap:12,
      /* On mobile: fill the fixed viewport; on desktop: normal flow */
      ...(isMobile ? {
        ...
        padding: '16px 0 24px',
        /* match the dashboard background exactly */
        background: baseBg,
      } : {
        paddingBottom: 40,
        width: '100%',
        ...
        <div style={{ display:'flex',alignItems:'center',gap:5,flexShrink:0 }}>
          ...
          <div style={{ position:'relative' }}>
            <button
              ...
              <SlidersHorizontal size={13} color={filterOpen ? `rgb(${pRgb})` : dark?'#94a3b8':'#6b7280'}/>
              ...
              <>
                ...
                <div style={{
                  ...
                  {TABS.map(t => {
                    ...
                    const cfg2 = t.id !== 'all' && t.id !== 'unread' ? TYPE_CFG[t.id as NotificationType] : null;
                    return (
                      <button key={t.id}
                        ...
                        style={{
                          ...
                          background: active
                            ? (dark?`rgba(${pRgb},0.18)`:`rgba(${pRgb},0.1)`)
                            : 'transparent',
                        ...
                        <span style={{
                          ...
                          background: cfg2 ? cfg2.color : active ? `rgb(${pRgb})` : dark?'#475569':'#94a3b8',
                        }}/>
                        <span style={{
                          flex:1,fontSize:13,fontWeight:active?700:400,
                          color: active ? `rgb(${pRgb})` : dark?'#e2e8f0':'#1e293b',
                        }}>{t.label}</span>
                        {count > 0 && (
                          <span style={{
                            fontSize:10,fontWeight:700,padding:'1px 7px',
                            borderRadius:99,lineHeight:'17px',
                            background: active ? `rgb(${pRgb})` : dark?'rgba(255,255,255,0.10)':'rgba(0,0,0,0.07)',
          ...
          {/* Mark all read — icon only on mobile, icon+text on desktop */}
          {unread > 0 && (
          ...
          {/* Clear all — icon only on mobile, icon+text on desktop */}
          {notifs.length > 0 && (
      ...
      {/* ── active filter badge (shown below header when a filter is active) ── */}
      {tab !== 'all' && (
        <div style={{
          padding: isMobile ? '0 16px' : '0',
          ...
          <div style={{
            ...
            background:dark?`rgba(${pRgb},0.15)`:`rgba(${pRgb},0.10)`,
  ...
  /*
   * Mobile → portal into document.body (same as ChatbotWidget)
   *          position:fixed escapes ALL layout constraints
