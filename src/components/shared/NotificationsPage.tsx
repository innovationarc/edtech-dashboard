 ...
 *   • Same baseBg (THEME_BG), sparkle gradients, glitter overlays
 *   • Same sbBorder / sbShadow card surfaces
 *   • Same noise texture overlay
 *   • Same pRgb primary-color system throughout
 *   • Zero hardcoded background colours on cards
 *
 * Layout:
  ...
  return (
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 16, touchAction: 'pan-y' }}>
        ...
        {/* foreground sliding card — uses ModalShell surface vars */}
        <div
          onTouchStart={onTS}
          onTouchMove={onTM}
          onTouchEnd={onTE}
          ...
          style={{
            ...
            userSelect: 'none',
            isolation: 'isolate',
            /* ── card surface — same as ModalShell ── */
            backgroundColor: sv.baseBg,
            backgroundImage: sv.sbSparkle,
            backgroundSize: sv.sbBgSize,
            backdropFilter: 'blur(32px) saturate(200%)',
            WebkitBackdropFilter: 'blur(32px) saturate(200%)',
            border: !notif.isRead
              ? `1px solid rgba(${pRgb},0.30)`
              : sv.sbBorder,
            boxShadow: !notif.isRead
              ? `${sv.sbShadow}, 0 0 0 0.5px rgba(${pRgb},0.10)`
              : sv.sbShadow,
          }}
        >
          {/* noise overlay — same as ModalShell */}
