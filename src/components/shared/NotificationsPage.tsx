...
import {
  ...
  Star, Cpu, Check, CheckCheck, Trash2, ArrowLeft, RefreshCw, Inbox,
} from 'lucide-react';
...
const NotificationsPage: React.FC = () => {
  ...
  // Sync announcements on mount
  useEffect(() => {
    if (!isAuthenticated || !user?.id) return;
    const sync = async () => {
      setSyncing(true);
      await notificationService.syncAnnouncementsAsNotifications(
        user.id, user.role ?? 'student', (user as any).enrolledCourseIds ?? []
      );
      setSyncing(false);
    };
    sync();
  ...
  const handleMarkAllRead = useCallback(async () => { if (user?.id) await notificationService.markAllAsRead(user.id); }, [user?.id]);
  ...
  const hdrBg = darkMode ? 'rgba(13,16,23,0.97)' : 'rgba(255,255,255,0.97)';

  return (
    <div style={{
      minHeight: '100vh',
      background: darkMode ? '#0d1017' : '#f3f4f8',
      fontFamily: "'Outfit', sans-serif",
      paddingBottom: 80,
    }}>
        ...
        <div style={{ maxWidth: 760, margin: '0 auto', padding: '12px 16px 0' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 12 }}>
            ...
            <button
              ...
              onMouseEnter={e => { e.currentTarget.style.background = darkMode ? 'rgba(255,255,255,0.12)' : 'rgba(0,0,0,0.09)'; }}
              onMouseLeave={e => { e.currentTarget.style.background = darkMode ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)'; }}
            >
              <ArrowLeft size={17} color={darkMode ? '#94a3b8' : '#4b5563'} />
            </button>
            ...
            <div style={{ flex: 1, display: 'flex', alignItems: 'center', gap: 10 }}>
              ...
              <div>
                <h1 style={{ margin: 0, fontSize: 17, fontWeight: 800, color: darkMode ? '#f1f5f9' : '#111827', lineHeight: 1.2 }}>
                  Notifications
                </h1>
                <p style={{ margin: 0, fontSize: 12, color: '#64748b', lineHeight: 1.2 }}>
                  {loading ? 'Loading…' : syncing ? 'Syncing…' : unreadCount > 0 ? `${unreadCount} unread` : 'All caught up'}
                </p>
              </div>
              {syncing && (
                <RefreshCw size={13} color="#64748b" style={{ animation: 'nspin 1.2s linear infinite', marginLeft: 2 }} />
              )}
            </div>
            ...
            {TABS.map(tab => {
              ...
              return (
                <button
                  ...
                  style={{
                    height: 34, padding: '0 13px', borderRadius: 99, flexShrink: 0,
                    background: isActive
      ...
      {/* ── Content ── */}
      <div style={{ maxWidth: 760, margin: '0 auto', padding: isMobile ? '12px 12px 0' : '16px 16px 0' }}>
        {loading ? (
          <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
            {Array.from({ length: 5 }).map((_, i) => <SkeletonCard key={i} darkMode={darkMode} />)}
          </div>
        ) : filtered.length === 0 ? (
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: isMobile ? 8 : 6 }}>
            ...
            {filtered.length >= 10 && (
              <p style={{ textAlign: 'center', padding: '20px 0 0', fontSize: 12, color: '#64748b', fontWeight: 500 }}>
