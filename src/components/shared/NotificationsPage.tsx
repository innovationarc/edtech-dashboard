  ...
  return (
    ...
    <div style={{
      ...
      <div style={{ position: 'relative', overflow: 'hidden', borderRadius: 16, touchAction: 'pan-y' }}>
        ...
        {/*
         * Foreground: Card component provides the exact same surface as
         * ComingSoon feature cards — background, border, shadow, sparkle.
         * Touch/swipe handlers sit on the outer wrapper div so Card stays clean.
         */}
        <div
          onTouchStart={onTS}
          onTouchMove={onTM}
        ...
        >
          {/* ── Card component: identical surface to ComingSoon feature cards ── */}
          <Card style={{
            position: 'relative',
            isolation: 'isolate',
            overflow: 'hidden',
            borderRadius: 16,
            /* unread state: stronger primary-color border + glow */
            ...((!notif.isRead) ? {
              border: `1px solid rgba(${pRgb},0.30)`,
              boxShadow: `0 8px 40px rgba(0,0,0,0.5), inset 0 0 0 1px rgba(${pRgb},0.12), 0 0 60px rgba(${pRgb},0.06), 0 0 0 0.5px rgba(${pRgb},0.10)`,
            } : {}),
          }}>
            <NoiseOverlay dark={dark} />

            {/* primary colour accent glow (top) — same as ModalShell */}
            {!notif.isRead && (
              <div style={{
            ...
            {/* content row */}
            <div style={{ position: 'relative', zIndex: 1, display: 'flex', gap: 12, alignItems: 'flex-start', padding: '14px 14px 12px 16px' }}>
