...
// ─── Theme helpers (mirrors StudentStudyPlan.tsx pattern) ────────────────────
...
const StudyPlanEventModal: React.FC<StudyPlanEventModalProps> = ({
  ...
  // ── Theme tokens (same pattern as StudentStudyPlan / StudentQA) ─────────────
  ...
  const T = {
    ...
    listBg:       isLight ? 'rgba(0,0,0,0.03)' : 'rgba(255,255,255,0.03)',
    listBorder:   isLight ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.06)',
  };

  // ── Modal shell styles (mirrors ModalShell in StudentStudyPlan.tsx) ──────────
...
["full detailed tip 1", "full detailed tip 2", "full detailed tip 3", "full detailed tip 4", "full detailed tip 5"]`;
  ...
  // ── Input styles (themed) ──────────────────────────────────────────────────
  const inputCls = 'w-full rounded-lg py-3 px-4 focus:outline-none focus:ring-2 transition-colors text-sm placeholder-gray-500';
      ...
      >
        ...
        <div style={{ padding: '24px 24px 20px' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 22 }}>
            ...
            <div style={{ flex: 1, minWidth: 0 }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' }}>
                <h2 style={{ color: T.text, fontWeight: 700, fontSize: 18, margin: 0, fontFamily: "'Outfit',sans-serif" }}>
                  {isEditing ? 'Edit Event' : `Create ${isPersonalEvent ? 'Personal ' : ''}Event`}
                </h2>
                {aiReady && (
                  <span style={{
                    fontSize: 11, display: 'flex', alignItems: 'center', gap: 4,
                    background: `rgba(${pRgb},0.15)`, border: `1px solid rgba(${pRgb},0.30)`,
                    color: primaryColor, padding: '2px 8px', borderRadius: 999,
                  }}>
                    <Sparkles size={9} /> AI Enhanced
                  </span>
                )}
              </div>
              <p style={{ color: T.text2, fontSize: 13, margin: '3px 0 0', fontFamily: "'Outfit',sans-serif" }}>
          ...
          <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: 18 }}>
            ...
            {draft && !draftApplied && (
              <div style={{ borderRadius: 14, border: `1px solid rgba(${pRgb},0.30)`, background: `rgba(${pRgb},0.08)`, padding: '14px 16px' }}>
                <div style={{ display: 'flex', alignItems: 'flex-start', gap: 12 }}>
                  <div style={{ width: 32, height: 32, borderRadius: 10, background: `rgba(${pRgb},0.25)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0, marginTop: 2 }}>
                    <Brain size={15} style={{ color: primaryColor }} />
                  </div>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <p style={{ fontSize: 13, fontWeight: 600, color: T.text, margin: '0 0 3px', fontFamily: "'Outfit',sans-serif" }}>AI Draft Ready</p>
                    <p style={{ fontSize: 12, color: T.text2, lineHeight: 1.5, margin: 0, fontFamily: "'Outfit',sans-serif" }}>
                      Suggested: <span style={{ color: T.text, fontWeight: 600 }}>"{draft.title}"</span>
                      {' · '}{draft.estimatedDuration}min · {draft.priority} priority
                    </p>
                    {draft.suggestedPrep.length > 0 && (
                      <p style={{ fontSize: 11, color: T.text3, marginTop: 4, margin: '4px 0 0', fontFamily: "'Outfit',sans-serif" }}>Prep: {draft.suggestedPrep.slice(0, 2).join(' · ')}</p>
                    )}
                  </div>
                  <div style={{ display: 'flex', gap: 8, flexShrink: 0 }}>
                    <button type="button" onClick={applyDraft}
                      style={{ fontSize: 12, background: primaryColor, color: '#fff', border: 'none', padding: '6px 12px', borderRadius: 8, fontWeight: 600, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 4, fontFamily: "'Outfit',sans-serif" }}>
                      <Check size={11} /> Apply
                    </button>
                    <button type="button" onClick={() => setDraft(null)}
                      style={{ fontSize: 12, background: T.btnSecBg, border: `1px solid ${T.btnSecBorder}`, color: T.text2, padding: '6px 12px', borderRadius: 8, cursor: 'pointer', fontFamily: "'Outfit',sans-serif" }}>
                      Skip
                    </button>
                  </div>
                </div>
            ...
            {draftLoading && (
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: primaryColor, background: `rgba(${pRgb},0.08)`, border: `1px solid rgba(${pRgb},0.20)`, borderRadius: 10, padding: '10px 16px', fontFamily: "'Outfit',sans-serif" }}>
            ...
            <div>
              ...
              <div style={{ position: 'relative' }}>
                ...
                />
                {draftLoading && (
                  <Loader size={14} className="absolute right-3 top-1/2 -translate-y-1/2 animate-spin" style={{ position: 'absolute', right: 12, top: '50%', transform: 'translateY(-50%)', color: primaryColor }} />
                )}
              </div>
            </div>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <label className={labelCls} style={{ ...labelStyle, marginBottom: 0 }}>Date & Time *</label>
                {aiReady && (
                  <button type="button" onClick={handleGetSlots} disabled={slotsLoading}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, background: `rgba(${pRgb},0.12)`, border: `1px solid rgba(${pRgb},0.28)`, color: primaryColor, padding: '6px 12px', borderRadius: 8, fontWeight: 600, cursor: slotsLoading ? 'not-allowed' : 'pointer', opacity: slotsLoading ? 0.5 : 1, fontFamily: "'Outfit',sans-serif" }}>
              ...
              {/* Applied slot badge */}
              {appliedSlot && (
                <div style={{ marginTop: 8, display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#4ade80', background: 'rgba(34,197,94,0.10)', border: '1px solid rgba(34,197,94,0.20)', borderRadius: 10, padding: '8px 12px', fontFamily: "'Outfit',sans-serif" }}>
                  <CheckCircle2 size={12} /> AI slot applied · {appliedSlot.reason}
                  <button type="button" onClick={() => setAppliedSlot(null)} style={{ marginLeft: 'auto', background: 'none', border: 'none', cursor: 'pointer', color: T.text3, display: 'flex' }}><X size={11} /></button>
                </div>
              )}
            </div>
            ...
            {/* AI Time Slot Panel */}
            {aiPanel === 'slots' && (
              <div style={{ borderRadius: 14, border: `1px solid rgba(${pRgb},0.25)`, background: T.surface, overflow: 'hidden' }}>
                <div style={{ padding: '12px 16px', borderBottom: `1px solid ${T.divider}`, display: 'flex', alignItems: 'center', justifyContent: 'space-between' }}>
                  <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
                    ...
                    <span style={{ fontSize: 13, fontWeight: 600, color: T.text, fontFamily: "'Outfit',sans-serif" }}>AI Suggested Time Slots</span>
                  </div>
                  <button type="button" onClick={() => setAiPanel(null)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: T.text3, display: 'flex' }}><X size={14} /></button>
                </div>

                {slotsLoading ? (
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '28px 16px', gap: 8, color: primaryColor, fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>
                    <Loader size={16} className="animate-spin" /> Analyzing your schedule…
                  </div>
                ) : slots.length > 0 ? (
                  <div>
                    {slots.map((slot, i) => (
                      <div key={i} style={{ padding: '14px 16px', borderBottom: i < slots.length - 1 ? `1px solid ${T.divider}` : 'none' }}>
                        <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: 12 }}>
                          <div style={{ flex: 1, minWidth: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 8, flexWrap: 'wrap' as const, marginBottom: 6 }}>
                              <span style={{ fontSize: 13, fontWeight: 700, color: T.text, fontFamily: "'Outfit',sans-serif" }}>{slot.startTime} – {slot.endTime}</span>
                              <span style={{ fontSize: 11, color: T.text3, fontFamily: "'Outfit',sans-serif" }}>{slot.date}</span>
                              ...
                              <span style={{ fontSize: 11, color: T.text3, background: T.surface, padding: '2px 8px', borderRadius: 999, textTransform: 'capitalize' as const, fontFamily: "'Outfit',sans-serif" }}>{slot.sessionType}</span>
                            </div>
                            <p style={{ fontSize: 12, color: T.text2, lineHeight: 1.5, margin: 0, fontFamily: "'Outfit',sans-serif" }}>{slot.reason}</p>
                            {slot.conflictWarning && (
                              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginTop: 6, fontSize: 12, color: '#fbbf24', fontFamily: "'Outfit',sans-serif" }}>
                          ...
                          <div style={{ display: 'flex', flexDirection: 'column' as const, alignItems: 'flex-end', gap: 8, flexShrink: 0 }}>
                            <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                              ...
                              <span style={{ fontSize: 12, fontWeight: 700, color: '#4ade80', fontFamily: "'Outfit',sans-serif" }}>{slot.estimatedProductivity}%</span>
                            </div>
                            <button type="button" onClick={() => applySlot(slot)}
                              style={{ fontSize: 12, background: primaryColor, color: '#fff', border: 'none', padding: '6px 14px', borderRadius: 8, fontWeight: 600, cursor: 'pointer', fontFamily: "'Outfit',sans-serif" }}>
                              Use This
                            </button>
                          </div>
                        </div>
                  ...
                  <div style={{ padding: '20px 16px', display: 'flex', flexDirection: 'column' as const, alignItems: 'center', gap: 10, textAlign: 'center' as const }}>
                    <AlertTriangle size={20} style={{ color: '#f87171' }} />
                    <div>
                      <p style={{ fontSize: 13, fontWeight: 600, color: '#fca5a5', margin: '0 0 4px', fontFamily: "'Outfit',sans-serif" }}>AI request failed</p>
                      <p style={{ fontSize: 11, color: '#fca5a580', margin: 0, fontFamily: 'monospace' }}>{slotsError}</p>
                    </div>
                    <p style={{ fontSize: 12, color: T.text3, marginTop: 4, fontFamily: "'Outfit',sans-serif" }}>
                      Go to <strong style={{ color: T.text2 }}>Admin → AI Model Settings</strong> to verify your provider and API key.
                    </p>
                  </div>
                ) : (
                  <p style={{ padding: '24px 16px', textAlign: 'center' as const, color: T.text3, fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>No suggestions available — enter a title first</p>
                )}
              </div>
            )}
            ...
            {/* Course */}
            <div>
              <label className={labelCls} style={labelStyle}>Course / Subject</label>
              {allCourses.length > 0 ? (
                ...
                <input type="text" value={formData.course} onChange={e => setField('course', e.target.value)}
                  className={inputCls} style={inputStyle} placeholder="Enter course or subject name…" />
              )}
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 14 }}>
              <div>
                <label className={labelCls} style={labelStyle}>Event Type</label>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: 14 }}>
              <div>
                <label className={labelCls} style={labelStyle}>Reminder</label>
            ...
            <div>
              <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 8 }}>
                <label className={labelCls} style={{ ...labelStyle, marginBottom: 0 }}>Study Tips</label>
                {aiReady && (
                  <button type="button" onClick={handleGetTips} disabled={tipsLoading}
                    style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.25)', color: '#fbbf24', padding: '6px 12px', borderRadius: 8, fontWeight: 600, cursor: tipsLoading ? 'not-allowed' : 'pointer', opacity: tipsLoading ? 0.5 : 1, fontFamily: "'Outfit',sans-serif" }}>
              ...
              {aiPanel === 'tips' && tipsLoading && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#fbbf24', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.20)', borderRadius: 10, padding: '10px 16px', fontFamily: "'Outfit',sans-serif" }}>
              ...
              {tips.length === 0 && !tipsLoading && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: T.text3, background: T.surface, border: `1px solid ${T.divider}`, borderRadius: 10, padding: '10px 16px', fontFamily: "'Outfit',sans-serif" }}>
                  <Lightbulb size={11} /> Click <span style={{ color: '#fbbf24', fontWeight: 600, margin: '0 3px' }}>AI Tips</span> to get study suggestions for this topic
                </div>
              )}

              {tips.length > 0 && !tipsLoading && (
                <div style={{ borderRadius: 12, border: '1px solid rgba(245,158,11,0.20)', background: 'rgba(245,158,11,0.05)', padding: '14px 16px', display: 'flex', flexDirection: 'column' as const, gap: 8 }}>
                  <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 4 }}>
                    <p style={{ fontSize: 12, fontWeight: 600, color: '#fbbf24', display: 'flex', alignItems: 'center', gap: 6, margin: 0, fontFamily: "'Outfit',sans-serif" }}>
                      <Lightbulb size={12} /> AI Study Tips
                      <span style={{ color: T.text3, fontWeight: 400 }}>· {formData.course || 'General'}</span>
                    </p>
                    {tipsSource === 'draft' && (
                      <span style={{ fontSize: 11, color: T.text3, fontStyle: 'italic', fontFamily: "'Outfit',sans-serif" }}>Click AI Tips for detailed tips</span>
                    )}
                  </div>
                  {tips.map((tip, i) => (
                    <div key={i} style={{ display: 'flex', alignItems: 'flex-start', gap: 10, fontSize: 13, color: T.text2, fontFamily: "'Outfit',sans-serif" }}>
                      <span style={{ color: '#fbbf24', fontWeight: 700, flexShrink: 0, marginTop: 2 }}>{i + 1}.</span>
            ...
            {!isPersonalEvent && formData.targetAudience === 'specific_student' && (
              <div>
                <label className={labelCls} style={labelStyle}>Select Students</label>
                <div style={{ background: T.listBg, borderRadius: 10, padding: 12, maxHeight: 192, overflowY: 'auto', border: `1px solid ${T.listBorder}` }}>
                  {allStudents.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '16px 0', color: T.text3 }}>
                      <Users size={24} style={{ margin: '0 auto 8px', display: 'block' }} />
                      <p style={{ fontSize: 13, margin: 0, fontFamily: "'Outfit',sans-serif" }}>No students available</p>
                    </div>
                  ) : allStudents.map(s => (
                    <label key={s.uid} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 6px', borderRadius: 8, cursor: 'pointer' }}>
                      <input type="checkbox"
                        ...
                        style={{ width: 16, height: 16, accentColor: primaryColor, borderRadius: 4 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ color: T.text, fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>{s.name}</div>
                      ...
                      {formData.targetStudentIds.includes(s.uid) && <Check size={16} style={{ color: '#4ade80' }} />}
                    </label>
                  ))}
                </div>
                {formData.targetStudentIds.length > 0 && (
                  <p style={{ marginTop: 8, fontSize: 12, color: T.text2, fontFamily: "'Outfit',sans-serif" }}>{formData.targetStudentIds.length} student{formData.targetStudentIds.length !== 1 ? 's' : ''} selected</p>
                )}
              </div>
            )}
            ...
            {!isPersonalEvent && formData.targetAudience === 'course_students' && (
              <div>
                <label className={labelCls} style={labelStyle}>Select Courses</label>
                <div style={{ background: T.listBg, borderRadius: 10, padding: 12, maxHeight: 192, overflowY: 'auto', border: `1px solid ${T.listBorder}` }}>
                  {allCourses.length === 0 ? (
                    <div style={{ textAlign: 'center', padding: '16px 0', color: T.text3 }}>
                      <BookOpen size={24} style={{ margin: '0 auto 8px', display: 'block' }} />
                      <p style={{ fontSize: 13, margin: 0, fontFamily: "'Outfit',sans-serif" }}>No courses available</p>
                    </div>
                  ) : allCourses.map(c => (
                    <label key={c.id} style={{ display: 'flex', alignItems: 'center', gap: 12, padding: '8px 6px', borderRadius: 8, cursor: 'pointer' }}>
                      <input type="checkbox"
                        ...
                        style={{ width: 16, height: 16, accentColor: primaryColor, borderRadius: 4 }} />
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ color: T.text, fontSize: 13, fontFamily: "'Outfit',sans-serif" }}>{c.title}</div>
                      ...
                      {formData.targetCourseIds.includes(c.id) && <Check size={16} style={{ color: '#4ade80' }} />}
                    </label>
                  ))}
                </div>
                {formData.targetCourseIds.length > 0 && (
                  <p style={{ marginTop: 8, fontSize: 12, color: T.text2, fontFamily: "'Outfit',sans-serif" }}>{formData.targetCourseIds.length} course{formData.targetCourseIds.length !== 1 ? 's' : ''} selected</p>
                )}
              </div>
            )}

            {/* Footer */}
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', paddingTop: 16, borderTop: `1px solid ${T.divider}`, flexWrap: 'wrap' as const, gap: 10 }}>
              <div style={{ display: 'flex', gap: 8 }}>
                {aiReady && aiPanel === null && (
                  <>
                    <button type="button" onClick={handleGetSlots}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, background: `rgba(${pRgb},0.10)`, border: `1px solid rgba(${pRgb},0.25)`, color: primaryColor, padding: '8px 14px', borderRadius: 10, fontWeight: 600, cursor: 'pointer', fontFamily: "'Outfit',sans-serif" }}>
                      <Brain size={12} /> AI Times
                    </button>
                    <button type="button" onClick={handleGetTips}
                      style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 12, background: 'rgba(245,158,11,0.10)', border: '1px solid rgba(245,158,11,0.25)', color: '#fbbf24', padding: '8px 14px', borderRadius: 10, fontWeight: 600, cursor: 'pointer', fontFamily: "'Outfit',sans-serif" }}>
              ...
              <div style={{ display: 'flex', gap: 10 }}>
                <button type="button" onClick={onClose}
                  style={{ padding: '10px 20px', background: T.btnSecBg, border: `1px solid ${T.btnSecBorder}`, color: T.text2, borderRadius: 12, fontSize: 13, fontWeight: 600, cursor: 'pointer', fontFamily: "'Outfit',sans-serif" }}>
                  Cancel
                </button>
                <button type="submit" disabled={isSaving}
                  style={{ padding: '10px 22px', background: T.gradient, color: '#fff', border: 'none', borderRadius: 12, fontSize: 13, fontWeight: 600, cursor: isSaving ? 'not-allowed' : 'pointer', opacity: isSaving ? 0.7 : 1, display: 'flex', alignItems: 'center', gap: 8, fontFamily: "'Outfit',sans-serif", boxShadow: `0 4px 14px rgba(${pRgb},0.30)` }}>
                  {isSaving
                    ? <><Loader size={14} className="animate-spin" />{isEditing ? 'Updating…' : 'Creating…'}</>
                    : <><CheckCircle2 size={14} />{isEditing ? 'Update Event' : 'Create Event'}</>
                  }
                </button>
              </div>
            </div>
