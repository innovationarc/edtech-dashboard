...
const StudyPlanEventModal: React.FC<StudyPlanEventModalProps> = ({
  ...
  // ── Theme tokens — exact match to ComingSoon.tsx ModalShell ─────────────────
  ...
  // ── Glitter image map — exact copy from ComingSoon.tsx ModalShell ────────────
  ...
  // ── Shell background vars — exact match to ComingSoon.tsx ModalShell ─────────
  ...
  return createPortal(
    ...
    <style>{`
      @media (min-width: 480px) {
        .spe-footer { flex-direction: row !important; align-items: center !important; }
        .spe-footer-ai { grid-template-columns: auto auto !important; width: auto !important; }
        .spe-footer-actions { grid-template-columns: auto auto !important; width: auto !important; margin-left: auto; }
        .spe-footer-actions button:first-child { min-width: 88px !important; }
        .spe-footer-actions button:last-child { min-width: 140px !important; }
      }
    `}</style>
    <div
      style={{
    ...
    >
      {/* Shell — exact match to ComingSoon.tsx ModalShell */}
      <div
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
          ...
          <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            {/* Date & Time — responsive: date full width, times on second row */}
            <div>
              ...
              <input
                ...
                className={inputCls} style={{ ...inputStyle, width: '100%', marginBottom: 8 }}
              />
              {/* Start & End time — side by side, full width */}
            ...
            <div>
              ...
              {tips.length === 0 && !tipsLoading && (
                <div style={{ borderRadius: 10, border: `1px solid ${T.border}`, background: T.surface, padding: '12px 14px', display: 'flex', alignItems: 'center', gap: 8 }}>
                  <Lightbulb size={14} style={{ color: T.text3, flexShrink: 0 }} />
                  <p style={{ fontSize: 12, color: T.text3, margin: 0, fontFamily: "'Outfit',sans-serif" }}>
                    Click <span style={{ color: '#fbbf24', fontWeight: 600 }}>AI Tips</span> to get study suggestions for this topic
                  </p>
                </div>
              )}
              {tipsLoading && (
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, color: '#fbbf24', background: 'rgba(245,158,11,0.08)', border: '1px solid rgba(245,158,11,0.18)', borderRadius: 10, padding: '9px 14px', fontFamily: "'Outfit',sans-serif" }}>
                  <Loader size={12} className="animate-spin" /> Generating study tips…
                </div>
              )}
              {tips.length > 0 && !tipsLoading && (
          ...
          {/* ── FOOTER ── */}
          <div className="spe-footer" style={{ borderTop: `1px solid ${T.divider}`, flexShrink: 0, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 10 }}>

            {/* Row 1 (mobile): AI utility buttons — equal width, only shown when aiReady & no panel open */}
            {aiReady && aiPanel === null && (
              <div className="spe-footer-ai" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
                <button type="button" onClick={handleGetSlots}
                  style={{
                    display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 6,
                    height: 44, borderRadius: 12, fontWeight: 600, cursor: 'pointer',
            ...
            {/* Row 2: Cancel + Primary action — always visible */}
            <div className="spe-footer-actions" style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 8 }}>
              ...
              <button type="submit" form="study-event-form" disabled={isSaving}
                style={{
                  ...
                  boxShadow: `0 4px 16px rgba(${pRgb},0.30)`,
