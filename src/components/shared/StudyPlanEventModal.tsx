...
["full detailed tip 1", "full detailed tip 2", "full detailed tip 3", "full detailed tip 4", "full detailed tip 5"]`;
  ...
  return createPortal(
    <>
    <style>{`
      .spe-btn-full { display: none; }
      .spe-btn-short { display: inline; }
      @media (min-width: 480px) {
        .spe-btn-full { display: inline; }
        .spe-btn-short { display: none; }
        .spe-footer { flex-direction: row !important; align-items: center !important; gap: 8px !important; }
        .spe-footer > div { display: contents; }
        .spe-footer button { flex: 1; min-width: 0; }
      }
    `}</style>
    <div
      style={{
    ...
    >
      ...
      >
        ...
        <div style={{ position: 'relative', zIndex: 1, display: 'flex', flexDirection: 'column', flex: 1, minHeight: 0 }}>
          ...
          {/* ── FOOTER ── */}
          <div className="spe-footer" style={{ borderTop: `1px solid ${T.divider}`, flexShrink: 0, padding: '12px 16px', display: 'flex', flexDirection: 'column', gap: 8 }}>

            {/* Row 1: AI utility buttons — equal width */}
            ...
            {/* Row 2: Cancel + Submit — equal width */}
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8 }}>
              ...
              <button type="submit" form="study-event-form" disabled={isSaving}
                ...
                {isSaving ? (
                  <><Loader size={13} className="animate-spin" /><span className="spe-btn-full">{isEditing ? 'Updating…' : 'Creating…'}</span></>
                ) : (
                  <><CheckCircle2 size={14} /><span className="spe-btn-short">{isEditing ? 'Update' : 'Create'}</span><span className="spe-btn-full">{isEditing ? 'Update Event' : 'Create Event'}</span></>
                )}
              </button>
            </div>
          </div>
