...
export interface CouponUsageRecord {
  ...
  courseId: string;
  usedAt: Date;
  // These will be populated from enrollment data when available
  userName?: string;
  courseName?: string;
  amountPaid?: number;
...
export interface CouponStats {
  ...
  // Time-series (daily usage count for last 30 days)
  dailyUsages: { date: string; count: number }[];

  // Top courses used with
  ...
  daysTotal: number;
  usageRate: number; // percent of usageLimit consumed
...
interface Props {
  /** Pass a single Coupon for individual coupon stats */
  coupon?: Coupon;
  /** Pass a BulkGroup + its coupons for group stats */
  group?: BulkGroup;
  groupCoupons?: Coupon[];
...
// Minimal bar chart using pure divs
...
async function fetchCouponUsageRecords(couponIds: string[]): Promise<CouponUsageRecord[]> {
  ...
  // Firestore 'in' operator supports up to 30 items; chunk if needed
  const CHUNK = 30;
  for (let i = 0; i < couponIds.length; i += CHUNK) {
    ...
    snap.docs.forEach(d => {
      records.push({
        ...
        // Future: populate from enrollments
...
function computeStats(
  ...
  // Days lifecycle (use first coupon as reference)
  const ref = coupons[0];
  const now = new Date();
  const daysTotal = ref
  ...
  const usageRate = isFinite(totalUsageLimit) && totalUsageLimit > 0
    ...
    : totalUsageCount > 0 ? -1 : 0; // -1 = unlimited
...
const CouponStatisticsModal: React.FC<Props> = ({
  ...
  const allCoupons: Coupon[] = isGroup ? groupCoupons : coupon ? [coupon] : [];
  ...
  useEffect(() => {
    if (couponIds.length > 0) {
      ...
      // No coupons yet (e.g. group with 0 coupons loaded)
      setStats({
        totalUsages: 0, uniqueUsers: 0, uniqueCourses: 0,
        estimatedDiscountGiven: 0, dailyUsages: last30Days().map(date => ({ date, count: 0 })),
            ...
            <>
              ...
              {activeSection === 'overview' && (
                <div className="space-y-6">
                  {/* Notice: Enrollment integration pending */}
