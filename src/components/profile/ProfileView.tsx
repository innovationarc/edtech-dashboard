...
/* ─── Glass card wrapper (moved inside component for theme access) ─── */

/* ══════════════════════════════════════════════
   MAIN COMPONENT
══════════════════════════════════════════════ */
const ProfileView = ({ user, onEdit, onChangePassword, onClose }: ProfileViewProps) => {
  const { primaryColor = '#6366f1', theme } = useDashboard();
  ...
  const cardBgSolid = isLightTheme ? '#ffffff' : '#161a28';
  ...
  const GlassCard = ({
    ...
    >
      ...
      <div style={{
        ...
        background: NOISE, opacity: isLightTheme ? 0.02 : 0.04, mixBlendMode: 'overlay',
      }} />
      {children}
    </div>
  );
  ...
  /* ─── Theme-aware InfoRow ─── */
  const InfoRow = ({
    icon: Icon, label, value, accent = accentBg,
  }: { icon: React.ElementType; label: string; value?: string; accent?: string }) => {
    if (!value) return null;
    return (
      <div style={{
        display: 'flex', alignItems: 'flex-start', gap: 10,
        padding: '7px 0', borderBottom: `1px solid ${isLightTheme ? 'rgba(0,0,0,0.04)' : 'rgba(255,255,255,0.04)'}`,
        ...
        <div>
          <div style={{ fontSize: 9.5, fontWeight: 700, color: textTertiary, textTransform: 'uppercase', letterSpacing: '0.07em', marginBottom: 2 }}>{label}</div>
          <div style={{ fontSize: 12.5, fontWeight: 600, color: textSecondary }}>{value}</div>
        </div>
      </div>
    );
  };
            ...
            <div className="pv-name-block" style={{ paddingBottom: 16, flex: 1, minWidth: 0 }}>
              ...
              <div className="pv-badges-row" style={{ display: 'flex', alignItems: 'center', gap: 7, flexWrap: 'wrap', marginBottom: 13 }}>
                ...
                <span style={{
                  ...
                  background: `rgba(${pRgb},0.18)`, color: isLightTheme ? primaryColor : '#a5b4fc',
                ...
                {user.classGrade && (
                  <span style={{
                    display: 'inline-flex', alignItems: 'center', gap: 4,
                    padding: '3px 11px', borderRadius: 20, fontSize: 11, fontWeight: 700,
                    background: isLightTheme ? 'rgba(0,0,0,0.06)' : 'rgba(255,255,255,0.07)', color: textTertiary,
                    border: `1px solid ${borderColor}`,
            ...
            {[
              ...
              >
                ...
                <div style={{ position: 'relative', zIndex: 4 }}>
                  ...
                  <div className="pv-stat-num" style={{ fontSize: 26, fontWeight: 900, color: textPrimary, letterSpacing: '-0.03em', lineHeight: 1 }}>{num}</div>
          ...
          <div className="pv-id-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, padding: '14px 28px 0' }}>
            {[
              ...
              <div key={lbl} style={{
                ...
                <div style={{ fontSize: 14, fontWeight: 800, color: textSecondary, letterSpacing: '0.04em', fontFamily: "'Courier New',monospace" }}>{val || '—'}</div>
              </div>
            ))}
          </div>
              ...
              <button key={label}
                ...
                style={{
                  ...
                  ...(primary
                    ...
                    : { background: hoverBg, color: textSecondary, boxShadow: `0 2px 10px ${shadowColor}`, border: `1px solid ${borderColor}` }
                  ),
                }}
                onMouseEnterCapture={e => {
                  ...
                  if (primary) {
                    ...
                    el.style.boxShadow = `0 8px 24px ${isLightTheme ? 'rgba(0,0,0,0.15)' : 'rgba(0,0,0,0.35)'}`;
                    el.style.background = isLightTheme ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.08)';
                  }
                }}
        ...
        <div className="pv-badge-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 14 }}>
          {[
            ...
            <div key={i}
              ...
              onMouseLeaveCapture={e => { e.currentTarget.style.transform = ''; e.currentTarget.style.boxShadow = `0 4px 20px ${shadowColor},inset 0 1px 0 ${shimmerColor}`; }}
            >
              {/* Gold orb */}
              ...
              <div style={{ position: 'relative', zIndex: 4 }}>
                <div style={{ fontSize: 12.5, fontWeight: 800, color: textSecondary }}>{title}</div>
                <div style={{ fontSize: 10, color: textTertiary, fontWeight: 600, marginTop: 2 }}>{sub}</div>
              </div>
            </div>
          ))}
        </div>
          ...
          <div style={{ position: 'relative', zIndex: 2 }}>
            ...
            <div className="pv-subject-grid" style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
              {[
                ...
                >
                  <div style={{ fontSize: 10, fontWeight: 700, color: textTertiary, marginBottom: 4 }}>{name}</div>
          ...
          {[
            ...
            <div key={title}
              ...
              onMouseEnterCapture={e => {
                ...
                e.currentTarget.style.boxShadow = `0 20px 48px ${isLightTheme ? 'rgba(0,0,0,0.15)' : 'rgba(0,0,0,0.45)'},inset 0 1px 0 ${shimmerColor}`;
                e.currentTarget.style.borderColor = isLightTheme ? 'rgba(0,0,0,0.12)' : 'rgba(255,255,255,0.13)';
              }}
              onMouseLeaveCapture={e => {
                e.currentTarget.style.transform = '';
                e.currentTarget.style.boxShadow = `0 4px 20px ${shadowColor},inset 0 1px 0 ${shimmerColor}`;
            ...
            >
              ...
              {/* Top shimmer */}
              <div style={{ position: 'absolute', top: 0, left: 0, right: 0, height: 1, background: isLightTheme ? 'linear-gradient(90deg,transparent,rgba(0,0,0,0.06)40%,rgba(0,0,0,0.12)50%,rgba(0,0,0,0.06)60%,transparent)' : 'linear-gradient(90deg,transparent,rgba(255,255,255,0.10)40%,rgba(255,255,255,0.20)50%,rgba(255,255,255,0.10)60%,transparent)', zIndex: 2 }} />
              {/* Noise */}
              <div style={{ position: 'absolute', inset: 0, borderRadius: 'inherit', pointerEvents: 'none', zIndex: 1, background: NOISE, opacity: isLightTheme ? 0.02 : 0.04, mixBlendMode: 'overlay' }} />
              {/* Header */}
