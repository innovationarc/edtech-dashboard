// src/components/profile/Profile-3.tsx — Teacher Profile v3 Matte Crystal Sparkle
import { useState, useEffect, useCallback } from 'react';
import { useDashboard } from '../../contexts/DashboardContext';
import {
  User, Mail, Phone, MapPin, Calendar, BookOpen,
  Lock, Loader, X, Clock, CheckCircle2,
  GraduationCap, Shield, Zap, Award, Star, Users as UsersIcon,
  BarChart2
} from 'lucide-react';
...
const yearsSince = (d?: Date | string) => {
  if (!d) return 0;
  const start = new Date(d).getTime();
  const now = Date.now();
  return Math.max(0, Math.floor((now - start) / (1000 * 60 * 60 * 24 * 365)));
};
...
const Profile3 = ({ onClose }: Profile3Props) => {
  ...
  useEffect(() => {
    ...
    const fields = [
      ...
      (user as any).religion, user.profilePictureUrl,
  ...
  /* fetch teacher's courses for live stats */
  useEffect(() => {
    if (!user) return;
  ...
  const avgRating = ratedCourses.length
    ...
    : 0;
  const teachingYears = yearsSince(user?.createdAt);
  const subjectsTaught = Array.from(new Set(courses.flatMap(c => c.subjects || []))).slice(0, 4);
          ...
          <div className="pv-print" style={{ maxWidth:900,margin:'0 auto',width:'100%',padding:'0 20px 40px',display:'flex',flexDirection:'column',gap:16 }}>
            ...
            <GlassCard>
              ...
              <div className="stat-grid" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14,padding:'20px 28px 0' }}>
                {[
                  { icon:BookOpen,  num:coursesLoading ? '—' : String(activeCourses), lbl:'Active Courses',   delta:'Currently teaching', dC:'#34d399', dB:'rgba(16,185,129,0.12)', glow:`rgba(${pRgb},0.5)`, iconBg:`rgba(${pRgb},0.14)` },
                  { icon:UsersIcon, num:coursesLoading ? '—' : String(totalStudents), lbl:'Total Students',   delta:'Across all courses', dC:'#60a5fa', dB:'rgba(59,130,246,0.12)', glow:'rgba(59,130,246,0.45)', iconBg:'rgba(59,130,246,0.14)' },
                  { icon:Star,      num:coursesLoading ? '—' : (avgRating > 0 ? avgRating.toFixed(1) : 'N/A'), lbl:'Avg. Course Rating', delta:'From student reviews', dC:'#fbbf24', dB:'rgba(245,158,11,0.12)', glow:'rgba(245,158,11,0.45)', iconBg:'rgba(245,158,11,0.14)' },
              ...
              <div className="id-chips" style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14,padding:'14px 28px 0' }}>
                {[
                  { lbl:'Teacher ID',   val:user.userId },
            ...
            <div className="achievements-grid" style={{ display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:14 }}>
              {[
                { em:'🏆', title: avgRating >= 4.5 ? 'Top Rated Educator' : 'Highly Rated', sub:avgRating > 0 ? `${avgRating.toFixed(1)} ★ average` : 'Building reputation', bg:'rgba(245,158,11,0.08)', border:'rgba(245,158,11,0.22)', glow:'rgba(245,158,11,0.3)' },
                { em:'📖', title:`${teachingYears > 0 ? teachingYears : '<1'} yr${teachingYears === 1 ? '' : 's'} Teaching`, sub:'On this platform', bg:'rgba(239,68,68,0.08)', border:'rgba(239,68,68,0.20)', glow:'rgba(239,68,68,0.25)' },
                { em:'📚', title:'Course Creator', sub:`${activeCourses} course${activeCourses === 1 ? '' : 's'} published`, bg:`rgba(${pRgb},0.08)`, border:`rgba(${pRgb},0.22)`, glow:`rgba(${pRgb},0.25)` },
              ...
              <div style={{ position:'relative',zIndex:2 }}>
                <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:16,flexWrap:'wrap',gap:8 }}>
                  <span style={{ fontSize:13.5,fontWeight:800,color:textSecondary,display:'flex',alignItems:'center',gap:8 }}>
                    <Zap size={15} color={primaryColor} /> Courses Overview
                  </span>
                  <span style={{ fontSize:11,fontWeight:600,color:textTertiary }}>{activeCourses} active</span>
                </div>
                <div style={{ height:10,borderRadius:6,background:isLightTheme ? 'rgba(0,0,0,0.08)' : 'rgba(255,255,255,0.06)',overflow:'hidden',marginBottom:10 }}>
                  <div style={{ height:'100%',width:`${Math.min(100, avgRating > 0 ? (avgRating/5)*100 : 0)}%`,borderRadius:6,
                    background:`linear-gradient(90deg,${primaryColor},#8b5cf6,#a855f7)`,
                ...
                <div style={{ display:'flex',justifyContent:'space-between',marginBottom:16 }}>
                  <span style={{ fontSize:10,fontWeight:600,color:textTertiary }}>0 ★</span>
                  <span style={{ fontSize:10,fontWeight:800,color:primaryColor }}>{avgRating > 0 ? `${avgRating.toFixed(1)} ★ overall rating` : 'No ratings yet'}</span>
                  <span style={{ fontSize:10,fontWeight:600,color:textTertiary }}>5 ★</span>
                </div>

                {coursesLoading ? (
                  <div style={{ textAlign:'center', padding:'20px 0', color:textTertiary, fontSize:12.5 }}>Loading courses…</div>
                ) : courses.length > 0 ? (
                  <div className="subject-grid" style={{ display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:10 }}>
                    {courses.slice(0, 4).map((c) => {
                      const pct = c.rating > 0 ? Math.round((c.rating/5)*100) : 0;
                      const good = c.rating >= 4;
                      const bg = good ? 'rgba(16,185,129,0.10)' : 'rgba(59,130,246,0.10)';
                      const border = good ? 'rgba(16,185,129,0.18)' : 'rgba(59,130,246,0.18)';
                      const col = good ? '#34d399' : '#60a5fa';
                      const fill = good ? '#10b981' : '#3b82f6';
                      return (
                        <div key={c.id} style={{ borderRadius:12,padding:'10px 12px',textAlign:'center',background:bg,border:`1px solid ${border}`,transition:'transform 0.2s ease' }}
                          onMouseEnter={e=>{e.currentTarget.style.transform='translateY(-3px)'}}
                          onMouseLeave={e=>{e.currentTarget.style.transform=''}}>
                          <div style={{ fontSize:10,fontWeight:700,color:textTertiary,marginBottom:4, whiteSpace:'nowrap',overflow:'hidden',textOverflow:'ellipsis' }}>{c.title}</div>
                          <div style={{ fontSize:17,fontWeight:900,color:col }}>{c.rating > 0 ? c.rating.toFixed(1) : '—'}</div>
                          <div style={{ fontSize:9,fontWeight:600,color:textTertiary,marginTop:2 }}>{c.studentCount || 0} students</div>
                  ...
                  <div style={{ textAlign:'center', padding:'20px 0', color:textTertiary, fontSize:12.5 }}>No courses created yet.</div>
                )}
              </div>
            </GlassCard>
            ...
            <div className="info-grid" style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:14 }}>
              {[
                ...
                { title:'Professional Information', Icon:GraduationCap, glow:'rgba(16,185,129,0.12)', accent:'rgba(16,185,129,0.14)',
                  rows:[
                    { icon:BookOpen,      label:'Subjects Taught',     value:subjectsTaught.length ? subjectsTaught.join(', ') : undefined },
                    { icon:GraduationCap, label:'Institution',         value:(user as any).college || (user as any).school },
                    { icon:Award,         label:'Active Courses',      value:coursesLoading ? undefined : String(activeCourses) },
                ...
                { title:'Account Information', Icon:Shield, glow:'rgba(59,130,246,0.12)', accent:'rgba(59,130,246,0.14)',
                  rows:[
                    { icon:Shield,       label:'Account Type',    value:'Teacher' },
