...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ eyeOffset: _eyeOffset }) => {
  ...
  // voiceConversation = true means fully hands-free loop (speak → auto-send → AI speaks → listen again)
  const [voiceConversation, setVoiceConversation] = useState(false);
  // voiceStatus drives the UI label during conversation
  ...
  // ── Core STT: starts one recognition cycle ────────────────────────────
  const startListening = useCallback(()=>{
    ...
    rec.onresult = (e: SpeechRecognitionEvent) => {
      ...
      if (last.isFinal && t.length > 0) {
        if (voiceConvRef.current) {
          // Stop recognition immediately — we have what we need
    ...
    rec.onerror = (e: any) => {
      setIsListening(false);
      // 'no-speech' and 'aborted' are normal — only retry on real errors
      if (voiceConvRef.current && e.error !== 'aborted' && e.error !== 'no-speech') {
        voiceLoopTimer.current = setTimeout(()=>{ if(voiceConvRef.current) startListening(); }, 600);
      } else if (voiceConvRef.current && e.error === 'no-speech') {
        // No speech heard — just restart listening quietly
    ...
    rec.onend = () => {
      setIsListening(false);
      // Only restart if we're still in conversation AND not mid-think or mid-speak
      // Use voiceStatusRef to avoid stale closure on voiceStatus state
      if (voiceConvRef.current && voiceStatusRef.current === 'listening') {
        voiceLoopTimer.current = setTimeout(()=>{ if(voiceConvRef.current) startListening(); }, 150);
      }
    };
  ...
  const startVoiceConversation = useCallback(()=>{
    // Prime speechSynthesis with a silent utterance on first use
    // This satisfies Android/iOS requirement for user-gesture before audio
  ...
  const isExplicitNavIntent = (msg: string): boolean => {
    ...
    return [
      'go to','open the','open my','take me to','navigate to','bring me to',
      'redirect me','switch to','jump to','get me to','head to',
      'show me the page','show me the tab',
    ].some(t => l.includes(t));
  };
  ...
  const handleSendMessage = useCallback(async (overrideText?: string, speakReply = false) => {
    ...
    try {
      ...
      if (res.navigateTo){
        if (explicitNav){
          // User explicitly asked to go somewhere — auto-navigate
                ...
                return (
                  <div key={msg.id} className="nv-msg-enter"
                    ...
                    <div style={{ display:'flex',flexDirection:'column',gap:3,maxWidth:'min(78%,480px)',alignItems:isUser?'flex-end':'flex-start' }}>
                      ...
                      {/* Open [Page] button — only shown when AI suggested navigation but didn't auto-navigate */}
                      {!isUser && navSuggestions.has(msg.id) && (
                        <button
                          ...
                          style={{ display:'flex',alignItems:'center',gap:6,marginTop:2,
                            ...
                            boxShadow:`0 2px 10px rgba(${pr},0.35)`,
                            fontFamily:"'Outfit',sans-serif",
                            transition:'opacity .15s' }}
                          onMouseEnter={e=>(e.currentTarget as HTMLElement).style.opacity='0.85'}
                          onMouseLeave={e=>(e.currentTarget as HTMLElement).style.opacity='1'}>
                          ↗ Open {pathName(navSuggestions.get(msg.id)!)}
                        </button>
                      )}
                      {(()=>{
