...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ eyeOffset: _eyeOffset }) => {
  ...
  // ── Core STT: requests mic permission then starts one recognition cycle ──
  const startListening = useCallback(()=>{
    ...
    // DO NOT call getUserMedia here — it conflicts with SpeechRecognition's internal
    // mic acquisition on every loop restart (especially on Android/mobile).
    // Permission is already requested once in startVoiceConversation before we ever
    // reach this function. Calling it again here causes the recognition session to
    // open but capture nothing (onresult never fires).
    ...
    rec.lang           = 'en-US';
    rec.continuous     = false;  // false is the only reliable option on Android
    rec.interimResults = true;
    rec.maxAlternatives = 1;

    // Track the last transcript so onend can use it as fallback
    let lastTranscript = '';
    ...
    rec.onresult = (e: SpeechRecognitionEvent) => {
      // Accumulate all results for display
      ...
      if (latest.isFinal && full.length > 0 && !finalFired) {
        finalFired = true;
        if (voiceConvRef.current) {
          setVoiceTranscript('');
          setVoiceStatus('thinking');
          sendVoiceMessageRef.current(full);
        } else {
    ...
    rec.onerror = (e: any) => {
      ...
      if (voiceConvRef.current && e.error !== 'aborted') {
    ...
    rec.onend = () => {
      setIsListening(false);
      // Fallback: if we have a transcript but isFinal never fired (common on Android)
      if (!finalFired && lastTranscript.length > 0) {
        finalFired = true;
        if (voiceConvRef.current) {
          setVoiceTranscript('');
          setVoiceStatus('thinking');
          sendVoiceMessageRef.current(lastTranscript);
          return; // don't restart — sendVoiceMessage will trigger it after AI responds
        } else {
          setInputMessage(lastTranscript);
          setVoiceTranscript('');
        }
      }
      // Restart listening loop if in conversation mode and still listening phase
      if (voiceConvRef.current && voiceStatusRef.current === 'listening') {
        voiceLoopTimer.current = setTimeout(()=>{ if(voiceConvRef.current) startListening(); }, 200);
      }
    };
  ...
  // ── TTS: speak AI reply, then restart listening if in conversation mode ─
  const speakText = useCallback((text: string, onDone?: ()=>void)=>{
    if (!window.speechSynthesis){ onDone?.(); return; }
    window.speechSynthesis.cancel();

    // Strip markdown symbols that get read aloud literally
    const clean = text
      .replace(/\*\*(.*?)\*\*/g, '$1')   // bold
      .replace(/\*(.*?)\*/g, '$1')        // italic
    ...
    // For voice mode, truncate very long replies to keep conversation snappy
    // If reply is over 300 chars, speak only the first ~2 sentences
    ...
    const doSpeak = () => {
      ...
      utt.rate  = 1.25;  // Faster — more natural conversation pace
      utt.pitch = 1.05;
      utt.volume = 1;
      // Use best picked voice if available
      ...
      utt.onerror = (e) => {
        ...
        onDone?.();
        if (voiceConvRef.current){
          voiceLoopTimer.current = setTimeout(()=>{ if(voiceConvRef.current) startListening(); }, 600);
        }
      };
      window.speechSynthesis.speak(utt);

      // Android Chrome workaround: speechSynthesis pauses after ~15s — keep it alive
      ...
      utt.onend = () => {
        ...
        if (voiceConvRef.current){
          setVoiceStatus('listening');
          // Reduced from 200ms → 50ms for snappier conversation loop
          voiceLoopTimer.current = setTimeout(()=>{ if(voiceConvRef.current) startListening(); }, 50);
        } else {
          setVoiceStatus('idle');
        }
      };
    ...
    if (!preferredVoiceRef.current && !window.speechSynthesis.getVoices().length){
      ...
      // Fallback: speak after 500ms even if event never fires
  ...
  // ── Send a voice message through the AI pipeline ─────────────────────
  // Defined as ref so it can be called from inside STT callback without stale closure
  ...
  const handleSendMessage = useCallback(async (overrideText?: string, speakReply = false) => {
    ...
    // Auto-scroll to bottom immediately when user sends
  ...
  // Wire sendVoiceMessageRef after handleSendMessage is defined
  useEffect(()=>{
          ...
          <div style={{
            ...
            <div style={{ flex:1,minWidth:0 }}>
              ...
              <div style={{ fontSize:11,color: dark ? '#a78bfa' : `rgb(${pr})`,fontWeight:500,marginTop:1 }}>
                {voiceStatus==='listening' ? 'Listening…' : voiceStatus==='thinking' ? 'Thinking…' : voiceStatus==='speaking' ? 'Speaking…' : voiceConversation ? 'Conversation mode' : 'Voice Mode'}
              </div>
            </div>
            {isSpeaking && (
          ...
          <div style={{ position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:0,paddingTop:60,paddingBottom:100,zIndex:2 }}>
            ...
            <div style={{ fontFamily:"'Sora',sans-serif",fontSize:isMobile?22:18,fontWeight:700,color:txtPri,marginTop:20,textAlign:'center',minHeight:32 }}>
              {voiceStatus==='thinking'
                ? <><span style={{ color:accent }}>Aura</span> is thinking…</>
