...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ eyeOffset: _eyeOffset }) => {
  ...
  const startListening = useCallback(()=>{
    ...
    navigator.mediaDevices?.getUserMedia({ audio: true })
      .catch(()=>{}) // permission may already be granted — ignore error
      .finally(()=>{
        ...
        rec.onerror = (e: any) => {
          ...
          if (voiceConvRef.current && e.error !== 'aborted') {
            const delay = e.error === 'no-speech' ? 200 : 700;
        ...
        rec.onend = () => {
          ...
          if (!finalFired && lastTranscript.length > 0) {
            ...
            if (voiceConvRef.current) {
              setVoiceTranscript('');
              setVoiceStatus('thinking');
              sendVoiceMessage(lastTranscript);
  ...
  const startVoiceConversation = useCallback(()=>{
    // Request mic permission + prime TTS — both triggered by user gesture here
    navigator.mediaDevices?.getUserMedia({ audio: true })
      .then(stream => {
      ...
      .finally(()=>{
        // Prime TTS with silent utterance to unlock Android autoplay
