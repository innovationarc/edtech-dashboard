...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ eyeOffset }) => {
  ...
  // Load persistent chat history when user is known
  useEffect(() => {
    if (!user?.uid) {
      setHistoryLoaded(true);
      return;
    }
    novaChatHistoryService.getHistory(user.uid).then(history => {
      setMessages(history);
      setHistoryLoaded(true);
  ...
  // Scroll to bottom instantly when panel opens (show latest)
  useEffect(() => {
    if (isOpen) {
  ...
  const handleSendMessage = async () => {
    ...
    const optimisticUserMsg: NovaChatMessage = {
    ...
    try {
      ...
      const errMsg: NovaChatMessage = {
        id: `tmp-err-${Date.now()}`,
        text: 'Something went wrong. Tap the info icon for details.',
        sender: 'ai',
        timestamp: new Date(),
        sessionId: sessionId.current,
      };
  ...
  const displayItems: Array<
    ...
    { type: 'message'; msg: NovaChatMessage }
  > = [];
  let lastDateStr = '';
  for (const msg of messages) {
    const dateStr = msg.timestamp.toDateString();
    if (dateStr !== lastDateStr) {
      displayItems.push({ type: 'separator', label: formatDateSeparator(msg.timestamp), key: `sep-${dateStr}` });
      lastDateStr = dateStr;
    }
    ...
    <>
      <style>{`
        ...
        .nova-ghost-btn {
          ...
          cursor: pointer; display: block; width: 72px; height: 72px;
        }
        ...
        .nova-panel {
          position: fixed; z-index: 49; right: 20px;
        ...
        @media (max-width: 1023px) {
          .nova-panel {
            left: 12px; right: 12px; width: auto; border-radius: 18px;
            height: min(520px, calc(100dvh - 220px));
          }
        }
        @media (max-height: 580px) {
          .nova-panel { height: calc(100dvh - ${isMobile ? 220 : 130}px); }
        }
        ...
        @keyframes nova-status-pulse { 0%,100% { opacity: 1; } 50% { opacity: 0.5; } }
        ...
        .nova-date-sep::before, .nova-date-sep::after {
          content: ''; flex: 1; height: 1px; background: var(--nova-border);
        }
        .nova-date-sep span { font-size: 10.5px; color: var(--nova-muted); white-space: nowrap; padding: 0 4px; }

        .nova-history-loading { display: flex; flex-direction: column; gap: 10px; padding: 8px 0; }
        .nova-shimmer {
          height: 36px; border-radius: 12px;
          background: linear-gradient(90deg, var(--nova-surface2) 25%, var(--nova-border) 50%, var(--nova-surface2) 75%);
          background-size: 200% 100%;
          animation: nova-shimmer 1.4s infinite;
        }
        .nova-shimmer.short { width: 55%; }
        .nova-shimmer.long  { width: 80%; align-self: flex-end; }
        @keyframes nova-shimmer { 0% { background-position: 200% 0; } 100% { background-position: -200% 0; } }
        ...
        .nova-empty-icon {
          ...
          background: var(--nova-accent-dim); border: 1px solid var(--nova-accent-mid);
          display: flex; align-items: center; justify-content: center; color: var(--nova-accent);
        }
        .nova-chips { display: flex; flex-wrap: wrap; gap: 8px; justify-content: center; margin-top: 4px; }
        .nova-chip {
        ...
        .nova-bubble.ai {
          background: var(--nova-surface2); border: 1px solid var(--nova-border);
          color: var(--nova-text); border-bottom-left-radius: 4px;
        }
        .nova-bubble.user {
          background: var(--nova-accent); color: #fff;
          border-bottom-right-radius: 4px; box-shadow: 0 4px 16px var(--nova-accent-mid);
        }
        .nova-timestamp { font-size: 10px; color: var(--nova-muted); padding: 0 4px; margin-top: 2px; align-self: flex-end; }
        .nova-mini-avatar {
        ...
        .nova-typing-indicator { display: flex; gap: 4px; align-items: center; padding: 12px 16px; }
        ...
        .nova-input-area {
          ...
          background: var(--nova-surface); border-top: 1px solid var(--nova-border);
        }
        .nova-input-row {
          display: flex; align-items: center; gap: 10px;
          background: var(--nova-surface2); border: 1px solid var(--nova-border);
          border-radius: 14px; padding: 6px 6px 6px 14px; transition: border-color 0.2s ease;
        }
        .nova-input-row:focus-within { border-color: var(--nova-accent-mid); }
        .nova-input {
          flex: 1; background: transparent; border: none; outline: none;
          color: var(--nova-text); font-size: 13.5px; font-family: inherit; padding: 5px 0;
        }
      ...
      {isOpen && (
        <div className={`nova-panel nova-widget ${isVisible ? 'entering' : 'exiting'}`} style={{ bottom: panelBottom }} role="dialog" aria-label="Nova assistant">
          ...
          <div className="nova-messages">
            {/* Loading shimmer while fetching history */}
            {!historyLoaded && (
              <div className="nova-history-loading">
                <div className="nova-shimmer short" />
                <div className="nova-shimmer long" />
                <div className="nova-shimmer short" />
              </div>
            )}

            {/* Empty state — only after history loaded with no messages */}
            {historyLoaded && messages.length === 0 && (
              <div className="nova-empty">
                <div className="nova-empty-icon"><Sparkles size={26} /></div>
                <div>
            ...
            {historyLoaded && displayItems.map(item =>
              item.type === 'separator' ? (
                <div key={item.key} className="nova-date-sep"><span>{item.label}</span></div>
              ) : (
                <div key={item.msg.id} className={`nova-msg-row ${item.msg.sender}`}>
                  {item.msg.sender === 'ai' && (
                    <div className="nova-mini-avatar"><Zap size={13} color="#fff" strokeWidth={2.5} /></div>
                  )}
                  <div style={{ display: 'flex', flexDirection: 'column', gap: 3, alignItems: item.msg.sender === 'user' ? 'flex-end' : 'flex-start', maxWidth: '78%' }}>
                    <div className={`nova-bubble ${item.msg.sender}`}>{item.msg.text}</div>
                    <span className="nova-timestamp">{formatTime(item.msg.timestamp)}</span>
                  </div>
                </div>
              )
            )}
            ...
            {isLoading && (
              <div className="nova-msg-row ai">
                <div className="nova-mini-avatar"><Zap size={13} color="#fff" strokeWidth={2.5} /></div>
                <div className="nova-bubble ai" style={{ padding: '4px 6px' }}><TypingIndicator /></div>
              </div>
            )}
