...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ eyeOffset: _eyeOffset }) => {
  ...
  // Text colors — match Navigation's darkMode text system
  const txtPri = dark ? '#f1f5f9' : '#111827';
  const txtMut = dark ? '#94a3b8' : '#6b7280';
  const txtDim = dark ? '#475569' : '#9ca3af';
  const bbAi   = dark ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.05)';
  const bbAiBdr = dark ? `rgba(${pr},0.25)` : `rgba(${pr},0.15)`;
  const surf2  = dark ? 'rgba(255,255,255,0.05)' : 'rgba(0,0,0,0.04)';

  const mBg  = dark ? `${baseBg}` : 'rgba(255,255,255,0.98)';
  const mBdr = dark ? `rgba(${pr},0.22)` : 'rgba(0,0,0,0.09)';
  const mTxt = dark ? '#f1f5f9' : '#111827';
  const mMut = dark ? '#94a3b8' : '#6b7280';
  const mDim = dark ? '#64748b' : '#9ca3af';
