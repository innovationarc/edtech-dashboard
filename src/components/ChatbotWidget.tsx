...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ eyeOffset: _eyeOffset }) => {
  ...
  const handleSendMessage = useCallback(async (overrideText?: string, speakReply = false) => {
    ...
    try {
      // Pass current in-memory messages as chat history — eliminates getRecentForContext Firestore read
      const historyForContext = messages
        .filter(m => !m.id.startsWith('tmp-'))
        .slice(-12)
        .map(m => ({ sender: m.sender as 'user' | 'ai', text: m.text }));
      const res = await novaRAGService.sendMessage(text, user??null, sessionId.current, siteName, speakReply, historyForContext);
      ...
      {voiceMode && (
        <div style={{
          ...
          <div style={{
            ...
            <p style={{ fontSize:10,color: dark ? `rgba(${ar},0.45)` : `rgba(${pr},0.55)`,letterSpacing:'0.08em',fontFamily:"'Outfit',sans-serif",margin:'0 0 3px' }}>
              AURA · VOICE ASSISTANT · {voiceConversation ? 'CONVERSATION' : isListening ? 'LIVE' : 'READY'}
            </p>
            <p style={{ fontSize:9.5, color:txtDim, fontFamily:"'Outfit',sans-serif", margin:0, lineHeight:1.4 }}>
              Voice chat is less accurate than text — use text for best results.
            </p>
          </div>
        </div>
      )}
      ...
      {isOpen && (
        ...
        >
          ...
          <div style={{
            ...
            {/* AI disclaimer */}
            <p style={{ margin:'6px 0 0', textAlign:'center', fontSize:10, color:txtDim, fontFamily:"'Outfit',sans-serif", lineHeight:1.4 }}>
