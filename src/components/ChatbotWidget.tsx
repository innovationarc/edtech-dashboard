...
const ChatbotWidget: React.FC<ChatbotWidgetProps> = ({ eyeOffset: _eyeOffset }) => {
  ...
  // ── scroll lock: freeze page but restore EXACT position on close ──────
  useEffect(()=>{
    if (!isMobile) return;
    if (isOpen) {
  ...
  const closeVoiceMode = useCallback(()=>{ setVoiceModeVisible(false); setTimeout(()=>{ setVoiceMode(false); setVoiceTranscript(''); }, 320); },[]);
    ...
    <>
        ...
        <div onClick={()=>setShowClearConfirm(false)} style={{ position:'fixed',inset:0,zIndex:10001,background:'rgba(0,0,0,0.82)',backdropFilter:'blur(12px)',display:'flex',alignItems:'center',justifyContent:'center',padding:20,animation:'nv-fade .16s ease',fontFamily:"'Outfit',sans-serif" }}>
          <div onClick={e=>e.stopPropagation()} style={{ background:mBg,backdropFilter:'blur(40px)',border:`1px solid ${mBdr}`,borderRadius:24,width:'min(340px,100%)',overflow:'hidden',boxShadow:'0 32px 80px rgba(0,0,0,.85)',animation:'nv-in .22s cubic-bezier(.22,1,.36,1)' }}>
            ...
            <div style={{ padding:'16px 20px 20px',display:'flex',gap:10 }}>
              ...
              <button onClick={()=>{ setMessages([]); setShowClearConfirm(false); if (user?.uid) novaChatHistoryService.clearHistory?.(user.uid).catch(()=>{}); }}
      ...
      {voiceMode && (
        <div style={{
          ...
          <div style={{ position:'absolute',inset:0,pointerEvents:'none' }}>
            <div style={{ position:'absolute',width:'70%',height:'70%',left:'-15%',top:'-10%',borderRadius:'50%',background:`radial-gradient(circle,rgba(${pr},0.65) 0%,transparent 65%)`,filter:'blur(30px)',animation:'nv-aurora-1 6s ease-in-out infinite' }}/>
            <div style={{ position:'absolute',width:'60%',height:'60%',right:'-10%',top:'20%',borderRadius:'50%',background:`radial-gradient(circle,rgba(236,72,153,0.45) 0%,transparent 65%)`,filter:'blur(26px)',animation:'nv-aurora-2 7s ease-in-out infinite' }}/>
            <div style={{ position:'absolute',width:'55%',height:'55%',left:'10%',bottom:'5%',borderRadius:'50%',background:`radial-gradient(circle,rgba(59,130,246,0.35) 0%,transparent 65%)`,filter:'blur(28px)',animation:'nv-aurora-3 8s ease-in-out 1s infinite' }}/>
            {[{x:'12%',y:'18%',s:2.2,d:'1.9s'},{x:'84%',y:'10%',s:1.6,d:'2.5s'},{x:'72%',y:'38%',s:1.2,d:'1.7s'},{x:'6%',y:'58%',s:1.8,d:'3.1s'},{x:'91%',y:'62%',s:1.4,d:'2.2s'},{x:'50%',y:'88%',s:1,d:'2.8s'}].map((p,i)=>(
              <div key={i} style={{ position:'absolute',left:p.x,top:p.y,width:p.s,height:p.s,borderRadius:'50%',background:`rgba(${ar},0.7)`,animation:`nv-dot ${p.d} ease-in-out ${i*0.35}s infinite` }}/>
            ))}
          </div>
          ...
          <div style={{
            ...
            paddingTop: isMobile ? 'env(safe-area-inset-top, 12px)' : '12px',
            ...
            <button onClick={()=>{ stopListening(); closeVoiceMode(); }}
              style={{ width:38,height:38,borderRadius:12,background:'rgba(255,255,255,0.08)',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.65)',flexShrink:0,WebkitTapHighlightColor:'transparent' }}>
          ...
          <div style={{ position:'absolute',inset:0,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',gap:0,paddingTop:60,paddingBottom:100,zIndex:2 }}>
            <div style={{ position:'relative',display:'flex',alignItems:'center',justifyContent:'center' }}>
              {isListening && (<>
                <div style={{ position:'absolute',top:'50%',left:'50%',width:isMobile?200:170,height:isMobile?200:170,borderRadius:'50%',border:`1.5px solid rgba(${ar},0.35)`,animation:'nv-ring 1.8s ease-out infinite' }}/>
                <div style={{ position:'absolute',top:'50%',left:'50%',width:isMobile?200:170,height:isMobile?200:170,borderRadius:'50%',border:`1px solid rgba(${ar},0.2)`,animation:'nv-ring 1.8s ease-out 0.65s infinite' }}/>
                <div style={{ position:'absolute',top:'50%',left:'50%',width:isMobile?200:170,height:isMobile?200:170,borderRadius:'50%',border:`1px solid rgba(${ar},0.12)`,animation:'nv-ring 1.8s ease-out 1.3s infinite' }}/>
              </>)}
            ...
            {(voiceTranscript||isListening) && (
              <div style={{ fontSize:isMobile?15:13,color:'rgba(167,139,250,0.85)',textAlign:'center',maxWidth:isMobile?'80%':'85%',lineHeight:1.55,marginTop:10,padding:`0 ${isMobile?28:18}px`,minHeight:26,fontStyle:voiceTranscript?'normal':'italic' }}>
                {voiceTranscript || 'Say something…'}
              </div>
            )}
            <div style={{ marginTop:20 }}><AuroraWave accent={accent} active={isListening}/></div>
            <div style={{ display:'flex',gap:isMobile?20:14,alignItems:'center',marginTop:isMobile?32:24 }}>
              <button onClick={()=>{ stopListening(); closeVoiceMode(); setVoiceTranscript(''); }} className="nv-btn"
                style={{ width:isMobile?54:46,height:isMobile?54:46,borderRadius:'50%',background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.15)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.6)' }}>
                <X size={isMobile?20:17}/>
              </button>
              <button onClick={isListening ? handleVoiceSend : startListening}
                style={{ width:isMobile?76:64,height:isMobile?76:64,borderRadius:'50%',border:'none',cursor:'pointer',background:gradient,display:'flex',alignItems:'center',justifyContent:'center',color:'#fff',boxShadow:`0 4px 28px rgba(${pr},.65),0 0 50px rgba(${ar},.2)`,transition:'transform 0.15s cubic-bezier(.34,1.56,.64,1)',...(isListening && { boxShadow:`0 0 0 6px rgba(${ar},0.22),0 4px 28px rgba(${pr},.65)` }) }}
                onMouseEnter={e=>(e.currentTarget as HTMLElement).style.transform='scale(1.08)'}
                onMouseLeave={e=>(e.currentTarget as HTMLElement).style.transform='scale(1)'}>
                {isListening ? <Send size={isMobile?26:22}/> : <Mic size={isMobile?26:22}/>}
              </button>
              <button onClick={isListening ? stopListening : startListening} className="nv-btn"
                style={{ width:isMobile?54:46,height:isMobile?54:46,borderRadius:'50%',background:'rgba(255,255,255,0.08)',border:'1px solid rgba(255,255,255,0.15)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:isListening?accent:'rgba(255,255,255,0.5)' }}>
                {isListening ? <MicOff size={isMobile?20:17}/> : <Mic size={isMobile?20:17}/>}
              </button>
            </div>
            {isListening && voiceTranscript && (
              <p style={{ fontSize:11,color:`rgba(${ar},0.6)`,marginTop:14,textAlign:'center',letterSpacing:'0.04em' }}>Tap ↗ to send · tap mic to stop</p>
            )}
          </div>

          {/* Bottom bar — paddingBottom uses safe-area-inset-bottom */}
          <div style={{
      ...
      {isOpen && (
        <div style={{
          ...
          ...(isMobile
            ? { top:0, bottom:0, left:0, right:0 }
            : { bottom:16, right:24, width:desktopPanelW, top:desktopPanelTop, borderRadius:28, border:`1px solid rgba(${pr},0.38)` }),
          zIndex:9999,
          background:cosmicBg,
        ...
        >
          ...
          <div style={{ position:'absolute',inset:0,pointerEvents:'none',zIndex:0 }}>
            <div style={{ position:'absolute',top:'-15%',left:'-10%',width:'65%',height:'65%',borderRadius:'50%',background:`radial-gradient(circle,rgba(${pr},0.38) 0%,transparent 65%)`,filter:'blur(50px)' }}/>
            <div style={{ position:'absolute',bottom:'-10%',right:'-8%',width:'55%',height:'55%',borderRadius:'50%',background:`radial-gradient(circle,rgba(${ar},0.22) 0%,transparent 65%)`,filter:'blur(44px)' }}/>
            <div style={{ position:'absolute',top:'40%',right:'-5%',width:'40%',height:'40%',borderRadius:'50%',background:`radial-gradient(circle,rgba(99,102,241,0.2) 0%,transparent 65%)`,filter:'blur(36px)' }}/>
            {[{x:'10%',y:'8%',s:1.8,d:'2s'},{x:'85%',y:'6%',s:1.4,d:'2.6s'},{x:'68%',y:'22%',s:1,d:'1.7s'},{x:'6%',y:'42%',s:1.6,d:'3.2s'},{x:'92%',y:'55%',s:1.8,d:'2.1s'},{x:'42%',y:'78%',s:1,d:'2.9s'},{x:'75%',y:'85%',s:1.4,d:'1.9s'},{x:'22%',y:'66%',s:1.2,d:'3.4s'}].map((s,i)=>(
              <div key={i} style={{ position:'absolute',left:s.x,top:s.y,width:s.s,height:s.s,borderRadius:'50%',background:`rgba(${ar},0.65)`,animation:`nv-dot ${s.d} ease-in-out ${i*0.38}s infinite` }}/>
            ))}
          </div>
          ...
          <div style={{
            ...
            minHeight: isMobile ? 'calc(64px + env(safe-area-inset-top, 0px))' : '58px',
            ...
            <div style={{ display:'flex',gap:2,flexShrink:0,alignItems:'center',paddingRight:4 }}>
              ...
              <button onClick={()=>setShowInfo(true)} title="About Aura"
                style={{ width:34,height:34,borderRadius:10,background:'transparent',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.32)',WebkitTapHighlightColor:'transparent',transition:'color .15s' }}
              ...
              <button onClick={()=>setShowClearConfirm(true)} title="Clear chat"
                style={{ width:34,height:34,borderRadius:10,background:'transparent',border:'none',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.32)',WebkitTapHighlightColor:'transparent',transition:'color .15s,background .15s' }}
              ...
              <button onClick={()=>setIsOpen(false)} title={isMobile?'Close':'Minimize'}
                style={{ width:34,height:34,borderRadius:10,background:'rgba(255,255,255,0.06)',border:'1px solid rgba(255,255,255,0.1)',cursor:'pointer',display:'flex',alignItems:'center',justifyContent:'center',color:'rgba(255,255,255,0.55)',WebkitTapHighlightColor:'transparent',transition:'background .15s,color .15s' }}
          ...
          <div className="nv-msgs" ref={messagesRef}
                ...
                <div style={{ display:'flex',flexWrap:'wrap',gap:9,justifyContent:'center' }}>
                  {['My schedule','Study help','Submit exam?','Platform guide','🎙 Voice'].map((q,i)=>(
                    <button key={q} className="nv-chip-btn"
                      onClick={()=>{ if (q==='🎙 Voice'){ openVoiceMode(); startListening(); return; } setInputMessage(q); setTimeout(()=>handleSendMessage(q),0); }}
            ...
            {showScrollBtn && (
              <button onClick={()=>scrollToBottom()}
                style={{ position:'sticky',bottom:6,display:'flex',alignSelf:'center',alignItems:'center',gap:5,padding:'6px 14px 6px 11px',borderRadius:22,background:'rgba(8,5,26,0.95)',border:`1px solid rgba(${pr},0.4)`,color:txtMut,fontSize:12,fontWeight:600,cursor:'pointer',boxShadow:`0 6px 22px rgba(0,0,0,.8),0 0 14px rgba(${pr},0.24)`,animation:'nv-fab .18s ease',whiteSpace:'nowrap',fontFamily:"'Outfit',sans-serif",margin:'6px auto 0' }}>
          ...
          {/* ── INPUT BAR — paddingBottom accounts for home indicator ── */}
          <div style={{
            ...
            paddingBottom: '12px',
            background:cosmicInp,
            borderTop:`1px solid ${cosmicBdr}`,
            backdropFilter:'blur(24px)',
          }}>
            <div style={{ display:'flex',alignItems:'center',gap:8,background:surf2,border:`1px solid rgba(${pr},0.3)`,borderRadius:17,padding:'8px 8px 8px 14px' }}>
              <input ref={inputRef} type="text" className="nv-inp"
                ...
                style={{ flex:1,minWidth:0,background:'transparent',border:'none',color:txtPri,fontSize:14,fontFamily:"'Outfit',sans-serif",padding:'3px 0',lineHeight:1.5,outline:'none',WebkitTapHighlightColor:'transparent' }}/>
              <button className={`nv-mic ${isListening?'listening':''}`}
                onClick={()=>{ openVoiceMode(); if(!isListening) startListening(); }}
                title="Voice mode"
                style={{ width:36,height:36,borderRadius:11,border:'none',cursor:'pointer',flexShrink:0,background:isListening?`rgba(${ar},0.22)`:'rgba(255,255,255,0.07)',display:'flex',alignItems:'center',justifyContent:'center',color:isListening?accent:'rgba(255,255,255,0.45)' }}>
                <Mic size={15}/>
              </button>
              <button className="nv-send"
                onClick={()=>handleSendMessage()}
                disabled={isLoading||!inputMessage.trim()}
                style={{ width:36,height:36,borderRadius:11,border:'none',cursor:'pointer',flexShrink:0,background:(!isLoading&&inputMessage.trim())?gradient:'rgba(255,255,255,0.07)',color:'#fff',display:'flex',alignItems:'center',justifyContent:'center',opacity:(!isLoading&&inputMessage.trim())?1:0.32,boxShadow:(!isLoading&&inputMessage.trim())?`0 2px 16px rgba(${pr},.65)`:'none' }}>
