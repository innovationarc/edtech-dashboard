...
import {
  ...
  BrainCircuit, LayoutGrid, Bot, Video, Radio, Activity,
} from 'lucide-react';
...
const Navigation = () => {
  ...
  const [islandNotif, setIslandNotif] = useState<{ message: string; type: 'success'|'info'|'warning'|'error'; title?: string } | null>(null);
  ...
  const isFirstNotifSnapshot = useRef(true);
  ...
  const headerScrollTimer = useRef<ReturnType<typeof setTimeout> | null>(null);
  ...
  useEffect(() => {
    ...
    const onScroll = (e: Event) => {
      ...
      if (currentY > headerLastScrollY.current + 4) {
        setHeaderVisible(false);
      } else if (currentY < headerLastScrollY.current - 4) {
        setHeaderVisible(true);
      }
      headerLastScrollY.current = currentY;

      if (headerScrollTimer.current) clearTimeout(headerScrollTimer.current);
      headerScrollTimer.current = setTimeout(() => setHeaderVisible(true), 400);
    };

    let el = getScrollEl();
    if (el) {
      headerLastScrollY.current = el.scrollTop;
  ...
  const NAV_ALL = [
    ...
    { name:'Live Exams',    Icon:Radio,           path:'/live-exams',         roles:['admin','teacher'] },
    { name:'Live Exams',    Icon:Radio,           path:'/student-live-exams', roles:['student'] },
    ...
    { name:'Achievements',  Icon:Trophy,          path:'/achievements',       roles:['admin','teacher'] },
    ...
    { name:'Firebase Monitor', Icon:Activity,     path:'/firebase-monitor',   roles:['admin'] },
  ];
  ...
  const addNotification = useCallback((message: string, type: 'success'|'info'|'warning'|'error' = 'info', title?: string) => {
    ...
    setIslandNotif({ message, type, title });
  ...
  useEffect(() => {
    if (!isAuthenticated) {
      ...
      isFirstNotifSnapshot.current = true;
    }
  }, [isAuthenticated]);
  ...
  // Real-time unread count from Firestore + Dynamic Island for new notifications
  const addNotificationRef = useRef(addNotification);
  useEffect(() => { addNotificationRef.current = addNotification; }, [addNotification]);

  useEffect(() => {
    if (!isAuthenticated || !user?.uid) return;
    const unsub = notificationService.subscribeToNotifications(user.uid, notifs => {
      setUnreadCount(notifs.filter(n => !n.isRead).length);

      if (isFirstNotifSnapshot.current) {
        // Seed seenIds from the initial snapshot — don't toast existing notifications
        notifs.forEach(n => seenNotifIds.current.add(n.id));
        isFirstNotifSnapshot.current = false;
        return;
      }

      // Detect genuinely new notifications (within last 15s to guard page reloads)
      const cutoff = Date.now() - 15_000;
      notifs.forEach(n => {
        if (seenNotifIds.current.has(n.id)) return;
        seenNotifIds.current.add(n.id);
        if (n.createdAt.getTime() < cutoff) return;
        // Map notification type → island type
        const islandType =
          n.type === 'urgent'      ? 'error'   :
          n.type === 'grade'       ? 'success' :
          n.type === 'announcement'? 'info'    :
          n.type === 'assignment'  ? 'info'    :
          n.priority === 'high'    ? 'success' : 'info';
        addNotificationRef.current(n.title, islandType, n.message);
      });
    });
    return () => unsub();
  ...
  const glitterBgImage = glitterTheme.startsWith('data:')
    ? `url(${glitterTheme})`
    : (glitterImageMap[glitterTheme] ?? '');
  const glitterBgSize = glitterTheme.startsWith('data:')
    ? 'cover'
    : glitterTheme === 'silver'
    ...
    <>
      ...
      {isAuthenticated && <aside
        ...
        style={{
          ...
          bottom: 10,
          width: SIDEBAR_W,
          /* Matte sparkle — frosted glass with color tint */
          backgroundColor: sbBg,
          backgroundImage: sbSparkle,
          backgroundSize: isImageBg ? 'auto, cover' : glitterBgSize,
          backgroundPosition: isImageBg ? 'center, center center' : 'initial',
      ...
      >
        ...
        {isAuthenticated && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {/* + Create / Enroll — role gated, desktop only */}
            {user && ['admin','teacher','course_manager','student'].includes(user.role) && (
              <div ref={createMenuRef} style={{ position: 'relative' }}>
                {/* Student: simple enroll button */}
                {user.role === 'student' ? (
                  <button onClick={() => navigate('/course-enrollment')} style={{
                  ...
                  <>
                    <button onClick={() => setShowCreateMenu(v => !v)} style={{
                      display: 'flex', alignItems: 'center', gap: 5,
                      padding: '0 14px', height: 36, borderRadius: 10,
                      background: gradient, color: '#fff',
                      fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer',
                      ...
                      onMouseEnter={e => { e.currentTarget.style.boxShadow = `0 6px 18px rgba(${pRgb},0.5)`; }}
                      onMouseLeave={e => { e.currentTarget.style.boxShadow = `0 3px 10px rgba(${pRgb},0.35)`; }}
                    >
                      <Plus size={14} strokeWidth={2.5}
                        style={{ transition: 'transform 0.2s ease', transform: showCreateMenu ? 'rotate(45deg)' : 'rotate(0deg)' }}
                      /> Create
                    </button>
                    ...
                    {showCreateMenu && (
                      <div style={{
                        ...
                        minWidth: 200, borderRadius: 16,
                        background: darkMode ? 'rgba(13,16,23,0.97)' : 'rgba(255,255,255,0.98)',
                        border: `1px solid rgba(${pRgb},0.25)`,
                        boxShadow: `0 12px 40px rgba(0,0,0,0.25), 0 0 0 1px rgba(${pRgb},0.08)`,
                        backdropFilter: 'blur(20px)',
                        padding: '6px',
                        zIndex: 200,
                        animation: 'fadeIn 0.15s ease',
                      }}>
                        <p style={{ fontSize: 10, fontWeight: 700, color: darkMode ? '#475569' : '#9ca3af', textTransform: 'uppercase', letterSpacing: '0.08em', padding: '4px 10px 6px' }}>
                          Quick Create
                        </p>

                        {['admin','course_manager'].includes(user.role) && (
                          <button onClick={() => { navigate('/course-creation'); setShowCreateMenu(false); }} style={{
                            width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                            padding: '9px 10px', borderRadius: 10, border: 'none', cursor: 'pointer',
                            background: 'transparent', transition: 'background 0.15s ease', textAlign: 'left',
                          }}
                            onMouseEnter={e => { e.currentTarget.style.background = `rgba(${pRgb},0.10)`; }}
                            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                          >
                            <span style={{ width: 30, height: 30, borderRadius: 8, background: gradient, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <PlusCircle size={14} color="#fff" strokeWidth={2.2}/>
                            </span>
                            <div>
                              <p style={{ fontSize: 13, fontWeight: 600, color: darkMode ? '#f1f5f9' : '#111827', margin: 0 }}>New Course</p>
                              <p style={{ fontSize: 11, color: darkMode ? '#64748b' : '#9ca3af', margin: 0 }}>Set up a course</p>
                            </div>
                            <ChevronRight size={13} style={{ marginLeft: 'auto', color: darkMode ? '#475569' : '#d1d5db' }}/>
                          </button>
                        )}

                        {['admin','teacher','course_manager'].includes(user.role) && (
                          <button onClick={() => { navigate('/content'); setShowCreateMenu(false); }} style={{
                            width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                            padding: '9px 10px', borderRadius: 10, border: 'none', cursor: 'pointer',
                            background: 'transparent', transition: 'background 0.15s ease', textAlign: 'left',
                          }}
                            ...
                            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                          >
                            <span style={{ width: 30, height: 30, borderRadius: 8, background: `linear-gradient(135deg,#06b6d4,#0ea5e9)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <Upload size={14} color="#fff" strokeWidth={2.2}/>
                            </span>
                            <div>
                              <p style={{ fontSize: 13, fontWeight: 600, color: darkMode ? '#f1f5f9' : '#111827', margin: 0 }}>Upload Content</p>
                              <p style={{ fontSize: 11, color: darkMode ? '#64748b' : '#9ca3af', margin: 0 }}>Add videos, PDFs</p>
                            </div>
                            <ChevronRight size={13} style={{ marginLeft: 'auto', color: darkMode ? '#475569' : '#d1d5db' }}/>
                          </button>
                        )}
                        ...
                        {['admin','teacher'].includes(user.role) && (
                          <button onClick={() => { navigate('/teacher-tasks'); setShowCreateMenu(false); }} style={{
                            width: '100%', display: 'flex', alignItems: 'center', gap: 10,
                            padding: '9px 10px', borderRadius: 10, border: 'none', cursor: 'pointer',
                            background: 'transparent', transition: 'background 0.15s ease', textAlign: 'left',
                          }}
                            ...
                            onMouseLeave={e => { e.currentTarget.style.background = 'transparent'; }}
                          >
                            <span style={{ width: 30, height: 30, borderRadius: 8, background: `linear-gradient(135deg,#10b981,#059669)`, display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                              <FileText size={14} color="#fff" strokeWidth={2.2}/>
                            </span>
                            <div>
                              <p style={{ fontSize: 13, fontWeight: 600, color: darkMode ? '#f1f5f9' : '#111827', margin: 0 }}>New Task</p>
                              <p style={{ fontSize: 11, color: darkMode ? '#64748b' : '#9ca3af', margin: 0 }}>Assign to students</p>
                            </div>
                            <ChevronRight size={13} style={{ marginLeft: 'auto', color: darkMode ? '#475569' : '#d1d5db' }}/>
                          </button>
                        )}
      ...
      <header
        ...
        style={{
          ...
          fontFamily: "'Outfit', sans-serif",
          transform: (location.pathname === '/notifications' || !headerVisible) ? 'translateY(-100%)' : 'translateY(0)',
          opacity: (location.pathname === '/notifications' || !headerVisible) ? 0 : 1,
          pointerEvents: (location.pathname === '/notifications' || !headerVisible) ? 'none' : 'auto',
          transition: 'transform 0.3s cubic-bezier(0.34,1.15,0.64,1), opacity 0.25s ease',
        }}
      >
        {/* Hamburger + Search */}
        {isAuthenticated ? (
      ...
      {/* Dynamic Island — fixed floating overlay, never affects layout, all screen sizes */}
      {islandNotif && (
        <div ref={islandRef}
          onClick={() => {
          ...
          style={{
            ...
            width: islandFullExpand ? 'min(calc(100vw - 28px), 380px)' : islandExpanded ? 'min(calc(100vw - 28px), 340px)' : '44px',
        ...
        >
          {/* Pill row — title in pill for brevity */}
          <div style={{
          ...
          <div style={{
            ...
            <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
              ...
              <span style={{ fontSize: 11, fontWeight: 700, color: '#f1f5f9', flex: 1, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>
                {islandNotif.title ?? islandNotif.message}
              </span>
            </div>
            {islandNotif.title && (
              <p style={{ fontSize: 13, color: '#94a3b8', margin: 0, lineHeight: 1.4, overflow: 'hidden', display: '-webkit-box', WebkitLineClamp: 2, WebkitBoxOrient: 'vertical' }}>
                {islandNotif.message}
              </p>
            )}
            <p style={{ fontSize: 10, color: '#475569', margin: 0 }}>Tap to collapse</p>
          </div>
        </div>
      )}
