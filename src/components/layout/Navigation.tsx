...
const Navigation = () => {
  ...
  /* ─── Sidebar — matte sparkle frosted glass, color-tinted per theme ─── */
  // Primary color tint for the sparkle effect
  const sbBg = darkMode
    ? `rgba(13,16,23,0.92)`
    ...
    <>
      ...
      {isAuthenticated && <aside
        ...
        style={{
          ...
          /* Noise texture overlay via pseudo-element workaround — inline SVG filter */
          isolation: 'isolate',
        }}
      >
      ...
      >
        ...
        {isAuthenticated && (
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            ...
            {/* + Create / Enroll button — role gated */}
            {user && ['admin','teacher','course_manager','student'].includes(user.role) && (
              <div ref={createMenuRef} style={{ position: 'relative' }}>
                ...
                {user.role === 'student' ? (
                  ...
                  <>
                    {/* Admin / Teacher / course_manager: dropdown */}
                    <button onClick={() => setShowCreateMenu(v => !v)} style={{
                      display: 'flex', alignItems: 'center', gap: 5,
                      padding: '0 14px', height: 36, borderRadius: 10,
                      background: showCreateMenu ? `rgba(${pRgb},0.85)` : gradient,
                      color: '#fff',
                      fontSize: 13, fontWeight: 700, border: 'none', cursor: 'pointer',
                      boxShadow: `0 3px 10px rgba(${pRgb},0.35)`,
                      fontFamily: "'Outfit',sans-serif",
                    ...
                    {showCreateMenu && (
                      <div style={{
                        ...
                        {/* Course — admin & course_manager only (not teacher) */}
