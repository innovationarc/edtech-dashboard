  ...
  return (
    <Link
      ...
      style={{
        ...
        /* iDraft — clean neutral active/hover states */
        background: isActive
          ? darkMode
        ...
        boxShadow: isActive
          ? darkMode
            ? '0 4px 16px -4px rgba(0,0,0,0.5), 0 1px 0 rgba(255,255,255,0.08) inset'
            : '0 4px 12px -4px rgba(0,0,0,0.12), 0 1px 0 rgba(255,255,255,1) inset'
          : 'none',
        ...
        color: isActive
          ...
          : isHovered
            ...
            : darkMode ? 'rgba(148,163,184,0.8)' : 'rgba(107,114,128,0.9)',
      }}
    >
      {/* ── icon bubble — iDraft neutral ── */}
      <span
        className="flex items-center justify-center flex-shrink-0 rounded-xl"
        style={{
          width: 34, height: 34,
          background: isActive
            ? darkMode ? 'rgba(255,255,255,0.15)' : 'rgba(0,0,0,0.10)'
            : isHovered
              ? darkMode ? 'rgba(255,255,255,0.10)' : 'rgba(0,0,0,0.07)'
              : darkMode ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.05)',
          border: isActive
            ? darkMode ? '1px solid rgba(255,255,255,0.22)' : '1px solid rgba(0,0,0,0.15)'
            : darkMode ? '1px solid rgba(255,255,255,0.08)' : '1px solid rgba(0,0,0,0.08)',
          boxShadow: isActive
            ? '0 3px 10px rgba(0,0,0,0.3)'
            : 'none',
          transform: isActive ? 'scale(1.07)' : isHovered ? 'scale(1.04)' : 'scale(1)',
          transition: 'all 0.22s cubic-bezier(0.25,1,0.5,1)',
        }}
      >
        <Icon size={16} strokeWidth={isActive || isHovered ? 2.5 : 2}
          style={{ color: isActive ? (darkMode ? 'white' : '#111827') : 'inherit',
      ...
      {sidebarOpen && (
        <span style={{
          ...
          /* iDraft: plain white/dark text when active — no gradient */
          ...(isActive ? {
...
const Navigation = () => {
  ...
  const glassBtnHov: React.CSSProperties = {
    background:  darkMode ? 'rgba(255,255,255,0.11)' : 'rgba(0,0,0,0.08)',
    border:      darkMode ? '1px solid rgba(255,255,255,0.18)' : '1px solid rgba(0,0,0,0.14)',
    boxShadow:   darkMode ? '0 4px 16px -4px rgba(0,0,0,0.4)' : '0 4px 12px -4px rgba(0,0,0,0.12)',
  };
  ...
  return (
    <>
      ...
      <aside
        ...
        style={{
          /* iDraft Design System — clean neutral glass sidebar */
          background: darkMode
            ? 'rgba(13, 14, 18, 0.97)'
          ...
          boxShadow: darkMode
            ? '8px 0 40px -8px rgba(0,0,0,0.7)'
            : '8px 0 32px -8px rgba(0,0,0,0.1)',
          fontFamily: "'DM Sans', sans-serif",
        }}
      >
        ...
        <div className="flex-1 relative overflow-hidden">
          ...
          {/* iDraft: clean sidebar — no tinted gradient bar */}
      ...
      <header
        ...
        style={{
          /* iDraft Design System — clean frosted glass header */
          background: darkMode
            ? 'rgba(13, 14, 18, 0.88)'
      ...
      >
        {/* iDraft: no inner gradient strip — clean flat header */}
                ...
                <button onClick={() => setShowNotifications(v => !v)}
                  ...
                  {notifications.length > 0 && (
                    <span className="absolute top-1 right-1 text-white text-[9px] rounded-full min-w-[15px] h-[15px] px-1 flex items-center justify-center font-bold"
                      style={{ background: '#6366f1', boxShadow: '0 2px 6px rgba(99,102,241,0.6)' }}>
              ...
              <form onSubmit={handleSearchSubmit} className="relative">
                <input
                  ...
                  style={{
                    ...
                    border: darkMode ? '1px solid rgba(255,255,255,0.11)' : '1px solid rgba(0,0,0,0.10)',
                    paddingLeft: 42, paddingRight: 16,
                    color: darkMode ? '#f1f5f9' : '#111827',
                    backdropFilter: 'blur(16px)',
                    fontFamily: "'DM Sans', sans-serif",
                    boxShadow: darkMode ? 'none' : '0 2px 8px -2px rgba(0,0,0,0.08)',
                    transition: 'all 0.22s cubic-bezier(0.25,1,0.5,1)',
                  }}
                  onFocus={e => {
                    e.currentTarget.style.background = darkMode ? 'rgba(255,255,255,0.12)' : 'rgba(255,255,255,1)';
                    e.currentTarget.style.border = darkMode ? '1px solid rgba(255,255,255,0.22)' : '1px solid rgba(0,0,0,0.18)';
                    e.currentTarget.style.boxShadow = darkMode ? '0 0 0 3px rgba(255,255,255,0.06)' : '0 0 0 3px rgba(0,0,0,0.06)';
                  }}
                  onBlur={e => {
                    e.currentTarget.style.background = darkMode ? 'rgba(255,255,255,0.07)' : 'rgba(255,255,255,0.85)';
                    e.currentTarget.style.border = darkMode ? '1px solid rgba(255,255,255,0.11)' : '1px solid rgba(0,0,0,0.10)';
                    e.currentTarget.style.boxShadow = darkMode ? 'none' : '0 2px 8px -2px rgba(0,0,0,0.08)';
                  }}
                />
                <span className="absolute left-2.5 top-1/2 -translate-y-1/2 w-6 h-6 rounded-lg flex items-center justify-center pointer-events-none"
                  ...
                  <Search size={12} style={{ color: darkMode ? 'rgba(148,163,184,0.9)' : 'rgba(107,114,128,0.9)' }} strokeWidth={2.5}/>
                </span>
                {/* Search results dropdown */}
                {showSearchResults && searchQuery && (
                  <div className="search-results absolute top-full left-0 right-0 mt-2 rounded-2xl overflow-hidden z-50"
                    style={{
                      background: darkMode ? 'rgba(18,19,24,0.98)' : 'rgba(255,255,255,0.98)',
                      border: darkMode ? '1px solid rgba(255,255,255,0.10)' : '1px solid rgba(0,0,0,0.10)',
                      boxShadow: '0 16px 48px rgba(0,0,0,0.5)',
                      backdropFilter: 'blur(24px)',
                    }}>
                    <div className="px-3 py-2.5 border-b" style={{ borderColor: darkMode ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.07)' }}>
                      <p className="text-[11px] font-medium" style={{ color: darkMode ? 'rgba(148,163,184,0.7)' : 'rgba(75,85,99,0.7)' }}>
                        Results for <span style={{ color: darkMode ? 'white' : '#111827', fontWeight: 700 }}>"{searchQuery}"</span>
                      </p>
                    </div>
                    {(isStudent
                      ...
                      <button key={r}
                        ...
                        style={{ color: darkMode ? '#e2e8f0' : '#111827', borderColor: darkMode ? 'rgba(255,255,255,0.06)' : 'rgba(0,0,0,0.06)', fontFamily:"'DM Sans',sans-serif" }}
                        onMouseEnter={e => {
                          e.currentTarget.style.background = darkMode ? 'rgba(255,255,255,0.07)' : 'rgba(0,0,0,0.04)';
                          e.currentTarget.style.color = darkMode ? 'white' : '#111827';
          ...
          {isAuthenticated && (
            <div className="flex items-center gap-2 flex-shrink-0">
              ...
              <button onClick={() => setShowProfile(true)}
                ...
                style={{
                  background: darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.95)',
                  border: darkMode ? '1px solid rgba(255,255,255,0.13)' : '1px solid rgba(0,0,0,0.10)',
                  boxShadow: darkMode ? '0 4px 16px -4px rgba(0,0,0,0.5)' : '0 4px 14px -4px rgba(0,0,0,0.12)',
                  backdropFilter: 'blur(20px)',
                  transition: 'all 0.22s cubic-bezier(0.25,1,0.5,1)',
                }}
                onMouseEnter={e => {
                  e.currentTarget.style.background = darkMode ? 'rgba(255,255,255,0.13)' : 'rgba(255,255,255,1)';
                  e.currentTarget.style.boxShadow = darkMode ? '0 6px 20px -4px rgba(0,0,0,0.6)' : '0 6px 18px -4px rgba(0,0,0,0.16)';
                  e.currentTarget.style.transform = 'scale(1.02)';
                }}
                onMouseLeave={e => {
                  e.currentTarget.style.background = darkMode ? 'rgba(255,255,255,0.08)' : 'rgba(255,255,255,0.95)';
                  e.currentTarget.style.boxShadow = darkMode ? '0 4px 16px -4px rgba(0,0,0,0.5)' : '0 4px 14px -4px rgba(0,0,0,0.12)';
                  e.currentTarget.style.transform = 'scale(1)';
                }}>
                <div className="w-8 h-8 rounded-lg flex items-center justify-center text-white font-bold text-xs flex-shrink-0"
                  style={{ background: gradient, boxShadow: `0 3px 10px rgba(${pRgb},0.45)` }}>
                  {user && getInitials(user.name)}
                </div>
                <div className="text-left hidden xl:block">
                  ...
                  <p className="text-[11px] font-semibold capitalize"
                    style={{ color: darkMode ? 'rgba(148,163,184,0.75)' : 'rgba(107,114,128,0.9)' }}>
                    {user?.role}
                  </p>
                </div>
              </button>
      ...
      {showMobileSearch && (
        <div className="fixed inset-0 bg-black/70 backdrop-blur-md z-[90] lg:hidden"
          ...
          <div className="absolute top-0 left-0 right-0 px-3 py-3 border-b"
            ...
            style={{
              background: darkMode ? 'rgba(13,14,18,0.98)' : 'rgba(255,255,255,0.98)',
