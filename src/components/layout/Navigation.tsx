...
const Navigation = () => {
  ...
  const navItems = [
    ...
    { name: 'Course Management', icon: <PlusCircle size={20} strokeWidth={2} />, path: '/course-creation', roles: ['admin', 'teacher'] },
    { name: 'Coupon Management', icon: <Ticket size={20} strokeWidth={2} />, path: '/manage-coupon', roles: ['admin', 'manager'] },
  ...
  return (
    <>
      ...
      <aside
        className={clsx(
          "fixed top-0 left-0 h-screen flex flex-col transition-all duration-300 ease-in-out z-[100]",
      ...
      >
        ...
        <div className={clsx(
          "h-16 sm:h-[68px] lg:h-[72px] px-4 border-b flex items-center justify-between flex-shrink-0",
          darkMode ? "border-white/[0.06]" : "border-gray-200"
        )}>
          <div className="flex items-center gap-3 min-w-0">
            <Link
              to="/dashboard"
              className="w-10 h-10 rounded-2xl flex items-center justify-center flex-shrink-0 shadow-lg cursor-pointer transition-transform duration-200 hover:scale-105 active:scale-95"
              style={{ 
          ...
          {/* Mobile Hamburger Button - Inside Sidebar beside title */}
          <button
            onClick={toggleSidebarClick}
            className={clsx(
              "lg:hidden flex items-center justify-center w-9 h-9 rounded-lg transition-all duration-200 flex-shrink-0",
              darkMode 
          ...
          {/* Desktop Hamburger Button - Only shows when sidebar is open */}
          {sidebarOpen && (
            <button
              onClick={toggleSidebarClick}
              className={clsx(
                "hidden lg:flex items-center justify-center w-9 h-9 rounded-lg transition-all duration-200 flex-shrink-0",
                darkMode 
                  ? "hover:bg-white/[0.06] active:bg-white/[0.04]" 
                  : "hover:bg-gray-100 active:bg-gray-50"
              )}
        ...
        >
          ...
          >
            ...
            {!sidebarOpen && user && (
              <span className={clsx(
                "hidden lg:fixed px-3 py-2 rounded-lg text-xs font-semibold whitespace-nowrap",
        ...
        {/* Bottom Actions */}
        <div className="flex-shrink-0 pb-16 sm:pb-20 md:pb-24 lg:pb-0">
          {/* Theme Toggle */}
          <div 
            className={clsx(
              "px-3 py-2 border-t",
            ...
            >
              {sidebarOpen ? (
                ...
                <div className="flex items-center justify-center w-full">
                  {darkMode ? (
                    <Moon size={18} className="text-slate-400 transition-colors" strokeWidth={2} />
                  ) : (
                    <Sun size={18} className="text-gray-600 transition-colors" strokeWidth={2} />
                  )}
                </div>
              )}
      ...
      <header 
        className={clsx(
          "header-container fixed top-0 right-0 h-16 sm:h-[68px] lg:h-[72px] bg-gradient-to-b from-[#0a0e1a] to-[#0d1220] border-b border-white/[0.06] flex items-center justify-between px-4 sm:px-6 lg:px-8 xl:px-12 safe-area-top z-[60] backdrop-blur-xl bg-opacity-95 shadow-[0_1px_0_0_rgba(255,255,255,0.03),0_8px_32px_-8px_rgba(0,0,0,0.4)] transition-all duration-300 ease-in-out",
      ...
      {showMobileSearch && (
        <div 
          className="fixed inset-0 bg-black/60 backdrop-blur-md z-[90] lg:hidden animate-in fade-in duration-200" 
          onClick={() => setShowMobileSearch(false)}
        >
          <div 
            className="absolute top-0 left-0 right-0 bg-gradient-to-b from-[#0f1419] to-[#0a0e1a] px-3 py-4 border-b border-white/[0.06] shadow-[0_8px_32px_rgba(0,0,0,0.4)] z-[91]" 
      ...
      <style jsx>{`
        ...
        /* Remove any debug dots or development indicators */
        aside *::before,
        aside *::after {
          content: none !important;
        }
        
        /* Hide any potential debug overlays */
        aside [data-debug],
        aside [data-test-id],
        aside .debug-indicator,
        aside .react-dev-overlay {
