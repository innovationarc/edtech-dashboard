...
export const DashboardProvider = ({ children }: DashboardProviderProps) => {
  ...
  useEffect(() => {
    ...
    document.body.style.backgroundColor = resolvedIsImageBg ? 'transparent' : colors.bg;
  ...
  const handleSetTheme = (newTheme: string) => {
    setTheme(newTheme);
    saveAppearanceToDb({ theme: newTheme });
  };

  const handleSetPrimaryColor = (color: string) => {
    setPrimaryColor(color);
    saveAppearanceToDb({ primaryColor: color });
  };

  const handleSetAccentColor = (color: string) => {
    setAccentColor(color);
    saveAppearanceToDb({ accentColor: color });
  };

  const handleSetFontFamily = (font: string) => {
    setFontFamily(font);
    saveAppearanceToDb({ fontFamily: font });
  };
  ...
  const handleSetCardStyle = (style: string) => {
    setCardStyle(style);
    saveAppearanceToDb({ cardStyle: style });
  };

  const handleSetCardAnimation = (anim: string) => {
    setCardAnimation(anim);
    saveAppearanceToDb({ cardAnimation: anim });
  };
